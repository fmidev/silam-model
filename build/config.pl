...
die(usage()) if (@ARGV < 4); 
my ($filelist, $rootfile, $md_choice, $mpi_choice) = @ARGV;

$mpi_choice eq 'mpi' || $mpi_choice eq 'no_mpi' || die('mpi_choice must be either mpi or no_mpi');

    
my $use_mpi = $mpi_choice eq 'mpi';
print "MPI compilation enabled\n" if $use_mpi;
...
my @external = qw(iflport ifport omp_lib netcdf pnetcdf ifcore grib_api iso_c_binding iso_fortran_env mpi omp_lib_kinds eccodes); 
