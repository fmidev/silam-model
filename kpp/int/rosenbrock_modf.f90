...
!~~~>  Arguments
   ...
   KPP_REAL, intent(inout) :: garbage(N)
...
!~~~>  Compute the error estimation
   ...
   Hnew = H*Fac
   !print *, 't, h, hnew, err, max(ynew), min(ynew), max(y), min(y)', &
   !     & t, h, hnew, err, maxval(ynew), minval(ynew), maxval(y), minval(y)
...
!~~~> Local variables
   ...
   !Told = TIME
   !TIME = T
   ! JV: do not update reaction constants inside integrator
   !CALL Update_SUN()
   !CALL Update_RCONST()
