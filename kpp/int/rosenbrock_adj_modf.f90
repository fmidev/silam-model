...
!~~~>  Statistics on the work performed by the Rosenbrock method
   ...
   KPP_REAL, DIMENSION(:),   allocatable :: chk_H, chk_T
   KPP_REAL, DIMENSION(:,:), allocatable :: chk_Y, chk_K, chk_J
   KPP_REAL, DIMENSION(:,:), allocatable :: chk_dY, chk_d2Y
   KPP_REAL, DIMENSION(:), allocatable :: Ystage
   
   !$OMP THREADPRIVATE(chk_h, chk_t, chk_y, chk_k, chk_j, chk_dy, chk_d2y, ystage)
...
subroutine init_solver_adj(icntrl, ierr)
  ! moved here, no need to call every time
  integer, intent(in), dimension(:) :: icntrl
  integer, intent(out) :: ierr

  integer, parameter :: adjointType = adj_discrete
  integer :: stat
  ...
  IF (AdjointType == Adj_discrete) THEN
    !$OMP PARALLEL DEFAULT(SHARED) PRIVATE(stat)
    ALLOCATE(Ystage(NVAR*ros_S), STAT=stat)
...
contains
 ...
 SUBROUTINE ros_ErrorMsg(Code,IERR)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~   
...
end subroutine init_solver_adj
...
!    Revised by Philipp Miehe and Adrian Sandu, May 2006
!    
!    Modiefied by Julius Vira, FMI for coupling with the SILAM model, Nov 2012
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!    
!~~~>   INPUT ARGUMENTS: 
!    
...
!~~~>  Checkpoints in memory
   INTEGER, save :: stack_ptr = 0 ! last written entry
   !$OMP THREADPRIVATE(stack_ptr)

!~~~>  Local variables     
   KPP_REAL :: FacMin, FacMax, FacRej, FacSafe
...
! ~~~~ Local variables        
   ...
   LOGICAL :: RejectLastH, RejectMoreH, Singular, all_non_neg
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
TimeLoop: DO WHILE ( (Direction > 0).AND.((T-Tend) <= -roundoff) &
       .OR. (Direction < 0).AND.((Tend-T) <= -roundoff) ) 
...
!~~~>    Compute LU decomposition 
   IF (.NOT.SaveLU) THEN
     ...
     CALL jac_sp(Ystage(1),fix,rconst,Ghimj)
     ISTATUS(Njac) = ISTATUS(Njac) + 1
...
!~~~>   Compute Hessian at the beginning of the interval
   ...
   call hessian(ystage(1), fix, rconst, hes0)
!~~~>   Compute the stages
Stage: DO istage = ros_S, 1, -1
      ...
      !~~~> Compute V 
       ...
       call jac_sp(ystage(istart), fix, rconst, jac)
       ISTATUS(Njac) = ISTATUS(Njac) + 1
       DO m = 1,NADJ
#ifdef FULL_ALGEBRA
...
!!$  END SUBROUTINE ros_SimpleCadjInt
  ...
  subroutine sweep_garbage(values, thresholds, all_non_neg)
    ...
    ! collect the negatives since there's nowhere to put them.
    implicit none
    KPP_REAL, intent(inout) :: values(NVAR)
    KPP_REAL, intent(in) :: thresholds(NVAR)
...
! Local variables
   ...
   KPP_REAL, parameter :: big_real = 1e30
...
!~~~> Local variables     
   ...
   Delta = SQRT(Roundoff)*MAX(DeltaMin,ABS(T))
   print *, 'Not implemented (jactimedervi)'
   stop
...
!~~~> Local variables
    ...
    INTEGER :: i, j
#endif   
    print *, 'not implemented (jacTemplate)'
    stop
!!$    Told = TIME
!!$    TIME = T
!!$    CALL Update_SUN()
