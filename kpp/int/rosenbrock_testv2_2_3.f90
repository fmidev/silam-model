...
!
! Modified by JV/FMI, Nov 2012         (modifications for rosenbrock.f90 module in KPP-2.2.1)
! Further modified by RH/FMI, Aug 2017 (modifications for rosenbrock.f90 module in KPP-2.2.3)
!
...
MODULE KPP_ROOT_Integrator
  ...
  USE KPP_ROOT_Global
  use KPP_ROOT_Precision !RH2018 for Silam: added line
  IMPLICIT NONE
  PUBLIC
  SAVE
...
!RH2018 for Silam: The following parameters where initially in subroutine rosenbrock:
...
!RH2018 for Silam: END: The following parameters ...
  ...
  ! single precision:
  KPP_REAL, parameter :: roundoff = 5.96046448E-08 !RH2018 for Silam: addition

  
CONTAINS

!RH2018 for silam: Added an extra subroutine init_solver which selects the Rosenbrock method.
!    It contains additionally the following subroutines that originally were under subroutine
!    Rosenbrock:   Ros2, Ros3, Ros4, Rodas3, Rodas4, Rang3
subroutine init_solver(icntrl, ierr)
  implicit none
  integer, dimension(:), intent(in) :: icntrl
  ...
  CASE DEFAULT
    ...
    !CALL ros_ErrorMsg(-2,Tstart,ZERO,IERR) !RH2018 for Silam: original
    !RH2018 for Silam: No reason to use ros_ErrorMsg here since 
    IERR = -2
    PRINT * , &
...
!RH2018 for Silam: end of added subroutine init_solver
...
SUBROUTINE Rosenbrock(N,Y,Tstart,Tend, &
           AbsTol,RelTol,garbage       & !RH2018 for Silam: added garbage
           RCNTRL,ICNTRL,RSTATUS,ISTATUS,IERR)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  USE KPP_ROOT_Jacobian, ONLY: Jac_SP, LU_IROW, LU_ICOL !RH2018 for Silam: added line
  USE KPP_ROOT_Function, ONLY: Fun                      !RH2018 for Silam: added line
  IMPLICIT NONE
...
!~~~>  Arguments
   ...
   INTEGER, INTENT(OUT)   :: IERR
   !JV addition: collect clipped negative masses
   KPP_REAL, intent(inout) :: garbage(N) !RH2018 for Silam: added garbage.
!!$   !RH2018 for Silam: the following is now moved at the begin of the module
!!$!~~~>  Parameters of the Rosenbrock method, up to 6 stages
!!$   INTEGER ::  ros_S, rosMethod
!!$   INTEGER, PARAMETER :: RS2=1, RS3=2, RS4=3, RD3=4, RD4=5, RG3=6
!!$   KPP_REAL :: ros_A(15), ros_C(15), ros_M(6), ros_E(6), &
...
!~~~>  Local variables
   !KPP_REAL :: Roundoff, FacMin, FacMax, FacRej, FacSafe !RH2018 for Silam: original
   KPP_REAL :: FacMin, FacMax, FacRej, FacSafe            !RH2018 for Silam: new (removed Roundoff)
...
!!$!RH2018 for Silam: The following selection is now moved up to subroutine init_solver
!!$!~~~>   Initialize the particular Rosenbrock method selected
!!$   SELECT CASE (ICNTRL(3))
!!$     CASE (1)
!!$       CALL Ros2
...
!~~~>  Unit roundoff (1+Roundoff>1)
   !Roundoff = WLAMCH('E') !RH2018 for Silam: commented out
...
!~~~>  Local parameters
   KPP_REAL, PARAMETER :: ZERO = 0.0_dp, ONE  = 1.0_dp
   KPP_REAL, PARAMETER :: DeltaMin = 1.0E-5_dp
   logical :: all_non_neg !RH2018 for Silam: Addition.
...
!~~~>   Compute the function at current time
   ! --JV: can call fun directly:
   !CALL FunTemplate(T,Y,Fcn0) !RH2018 for Silam: commented out
   CALL fun(Y, FIX, RCONST, Fcn0) !RH2018 for Silam: addition
...
!~~~>   Compute the Jacobian at current time
   ! --JV: can call jac directly:
   !CALL JacTemplate(T,Y,Jac0) !RH2018 for Silam: commented out
   CALL jac_sp(Y, FIX, RCONST, Jac0) !RH2018 for Silam: addition
...
Stage: DO istage = 1, ros_S
	 ...
	 Ynew(1:N) = Y(1:N)
         ...
         !CALL FunTemplate(Tau,Ynew,Fcn)  !RH2018 for Silam: commented out
         CALL fun(Ynew, FIX, RCONST, Fcn) !RH2018 for Silam: addition
         ISTATUS(Nfun) = ISTATUS(Nfun) + 1
...
!~~~>  Compute the error estimation
   ...
   call sweep_garbage(Ynew, AbsTol, all_non_neg) !RH2018 for Silam: addition
   
!~~~> New step size is bounded by FacMin <= Hnew/H <= FacMax
   !Fac  = MIN(FacMax,MAX(FacMin,FacSafe/Err**(ONE/ros_ELO))) !RH2018 for Silam: Commented and changed to ... 
   if (all_non_neg) then
     Fac  = MIN(FacMax,MAX(FacMin,FacSafe/Err**(ONE/ros_ELO)))
   else
     Fac = FacMin
   end if
   !END RH2018 for Silam: End of changes.
   Hnew = H*Fac

!~~~>  Check the error magnitude and adjust step size
   ISTATUS(Nstp) = ISTATUS(Nstp) + 1
   !IF ( (Err <= ONE).OR.(H <= Hmin) ) THEN  !~~~> Accept step   !RH2018 for Silam: commented out
   IF ( (Err <= ONE .and. all_non_neg).OR.(H <= Hmin) ) THEN  !~~~> Accept step  !RH2018 for Silam: addition
      call sweep_and_add_garbage(Ynew, AbsTol, garbage) !RH2018 for Silam: addition
...
!~~~> Succesful exit
  ...
  !RH2018 for Silam: The following subroutine is added for SILAM use.
  subroutine sweep_and_add_garbage(values, thresholds, garbage)
    implicit none
    KPP_REAL, intent(inout) :: values(N)
    KPP_REAL, intent(in) :: thresholds(N)
    KPP_REAL, intent(out) :: garbage(N)

    integer :: ii

    do ii = 1, N
      if (values(ii) >= ZERO) cycle
      if (values(ii) > -thresholds(ii)) then
        garbage(ii) = garbage(ii) + values(ii)
        !print *, 'Clipping:', ii, values(ii)
        values(ii) = ZERO
      end if
    end do

  end subroutine sweep_and_add_garbage
  ...
  subroutine sweep_garbage(values, thresholds, all_non_neg)
    ...
    KPP_REAL, intent(inout) :: values(N)
    KPP_REAL, intent(in) :: thresholds(N)
    logical, intent(out) :: all_non_neg
    integer :: ii
...
! Local variables
   ...
   KPP_REAL, PARAMETER :: big_real = 1e30 !RH2018 for Silam: addition

   Err = ZERO
   DO i=1,N
     !Ymax = MAX(ABS(Y(i)),ABS(Ynew(i))) !RH2018 for Silam: commented out
     ! --JV: max is not good: if ynew is close to zero, 
     ! the tolerance will allow negative concentration.
     

     !RH2018 for Silam: addition of the following.
     if (abs(Ynew(i)) > big_real .or. abs(Yerr(i)) > big_real) then
       ! In stiff situations with a too long initial timestep, the solution could diverge
       ! leading to overflow at this point. Return large error to force recomputation with
       ! a shorter step.
       ros_errorNorm = big_real
       return
     end if
     !11Aug2017 by R.H. Note that this is used in cbm4 but not in cbm42_strato!
     !if (ynew(i) < -abstol(i)*1e5) then
     !  ! Large negative value
     !  ros_errorNorm = big_real
     !  return
     !end if
     !RH2018 for Silam: end addition.

     IF (VectorTol) THEN
       Scale = AbsTol(i)+RelTol(i)*Ynew(i) !RH2018 for Silam: Ymax -> Ynew(i)
     ELSE
       Scale = AbsTol(1)+RelTol(1)*Ynew(i) !RH2018 for Silam: Ymax -> Ynew(i)
     END IF
...
!~~~> InOut variables
  ...
  !RH2018 for Silam: Subroutine Ros2 was originally here. Moved up, below subroutine init_solver.

  !RH2018 for Silam: Subroutine Ros3 was originally here. Moved up, below subroutine init_solver.

  !RH2018 for Silam: Subroutine Ros4 was originally here. Moved up, below subroutine init_solver.

  !RH2018 for Silam: Subroutine Rodas3 was originally here. Moved up, below subroutine init_solver.

  !RH2018 for Silam: Subroutine Rodas4 was originally here. Moved up, below subroutine init_solver.

  !RH2018 for Silam: Subroutine Rang3 was originally here. Moved up, below subroutine init_solver.
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 ...
 USE KPP_ROOT_Global, ONLY: FIX, RCONST !, TIME !RH2018 for Silam: omitted TIME 
 USE KPP_ROOT_Function, ONLY: Fun
 !USE KPP_ROOT_Rates, ONLY: Update_SUN, Update_RCONST !RH2018 for Silam: commented out.
!~~~> Input variables
   KPP_REAL :: T, Y(NVAR)
!~~~> Output variables
   KPP_REAL :: Ydot(NVAR)
!~~~> Local variables
   ...
   !RH2018 for Silam: Do not update reaction rates for every KPP time step. 
   !Told = TIME
   !TIME = T
   !CALL Update_SUN()
   !CALL Update_RCONST()
   !END RH2018 for Silam:
   CALL Fun( Y, FIX, RCONST, Ydot )
   !TIME = Told !RH2018 for Silam: commented out.
