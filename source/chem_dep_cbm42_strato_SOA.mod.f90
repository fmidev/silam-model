module chem_dep_cbm42_strato_SOA
  !
  ...
  !
  use cbm42_strato_SOA_interface
  use cocktail_basic
  use cbm42_strato_SOA_Parameters !R.H. 10Jan2018 for adjusting the accuracy for different species!
  !$use omp_lib

  implicit none
  private

  public set_chem_rules_cbm42_strato_SOA
  public set_missing
  
  public fu_lifetime
  public check_low_mass_threshold_cbm42_strato_SOA
  public registerSpeciescbm42_strato_SOA
  public inventory_cbm42_strato_SOA
  public transform_cbm42_strato_SOA
  public transform_cbm42_strato_SOA_adj

  public cbm42_strato_SOA_input_needs
  public fu_if_specific_deposition
  public prepare_step_cbm42_strato_SOA
  
  ! inherited from the interface
  public init_chemicals_cbm42_strato_SOA

  !
  ! Private routines of the sulphur DMAT cocktail and chemical rules
  !
  private set_miss_chem_rules_cbm42_strato_SOA

  private fu_lifetime_cbm42_strato_SOA
  private fu_if_specific_deposition_cbm42_strato_SOA

  interface set_missing
    module procedure set_miss_chem_rules_cbm42_strato_SOA
  end interface

  interface fu_lifetime
    module procedure fu_lifetime_cbm42_strato_SOA
  end interface

  interface fu_if_specific_deposition
    module procedure fu_if_specific_deposition_cbm42_strato_SOA
  end interface

  public fu_if_tla_required
  private fu_if_tla_required_cbm42_strato_SOA
  interface fu_if_tla_required
     module procedure fu_if_tla_required_cbm42_strato_SOA
  end interface


  type Tchem_rules_cbm42_strato_SOA
     private
     logical :: defined = .false.
     integer, dimension(20) :: kpp_icntrl
     ! solver relative tolerance
     real(precision_cbm42_strato_SOA), dimension(num_species_cbm42_strato_SOA) :: kpp_rtol
     ! solver absolute tolerance, solver units
     real(precision_cbm42_strato_SOA), dimension(num_species_cbm42_strato_SOA) :: kpp_atol
     ! the rcntrl doesn't work here: it is updated per cell
     !real, dimension(num_species_cbm42_strato_SOA) :: kpp_rcntrl
     ...
     integer :: SSP_scenario !Future SSP scenario for CH4 and CO2 concentrations for 2015->. 
  end type Tchem_rules_cbm42_strato_SOA
  
  public Tchem_rules_cbm42_strato_SOA


  !
  ! The mark of this transformation module:
  !
  integer, parameter, public :: transformation_cbm42_strato_SOA = 5012
  ...
  real(precision_cbm42_strato_SOA), dimension(20), private, save :: kpp_rcntrl
...
CONTAINS
  ...
  subroutine prepare_step_cbm42_strato_SOA(if_report_rates, photoarr)
    implicit none
    logical, optional, intent(in) :: if_report_rates
    real, dimension(:), optional, intent(in) :: photoarr
    character(len=*), parameter :: sub_name = 'prepare_step_cbm42_strato_SOA'
    real :: cAir
    
    call set_const_rates_cbm42_strato_SOA()
    kpp_rcntrl = 0.0
    ...
    if (present(if_report_rates)) then
      ...
      if (if_report_rates) then
        ...
        call set_rates_cbm42_strato_SOA(photoarr, temp=300.0, cAir=cAir, press=101325.0, cH2O=0.01&
                             &, soa_b=0.0, ASOA_aging=0.0, BSOA_aging=0.0, IVOC_aging=0.0, ifMTprods=0.0&
                             &, MCF_factor=1.0&
	                       &)	
        call report_rates_cbm42_strato_SOA()
      end if
    end if
  end subroutine prepare_step_cbm42_strato_SOA


  subroutine inventory_cbm42_strato_SOA(rules, &
    ...
    type(Tchem_rules_cbm42_strato_SOA), intent(in) :: rules
    ...
    call inventory_int(transformation_cbm42_strato_SOA, speciesEmis, &
                     & speciesTransp, speciesShortlived, speciesAerosol,&
                     & nSpeciesEmis, nSpeciesTransp, nSpeciesShortlived, nSpeciesAerosol, &
                     & iClaimedSpecies)

  end subroutine inventory_cbm42_strato_SOA


  !************************************************************************************

  subroutine registerSpeciescbm42_strato_SOA(rules, &
                                    & speciesTransp, speciesShortlived, speciesAerosol,&
  ...
  end subroutine registerSpeciescbm42_strato_SOA


  !*****************************************************************

  subroutine cbm42_strato_SOA_input_needs(rules, metdat)
    !
    ...
    if (.not. rules%defined) then
      call set_error('Undefined cbm42_strato_SOA rules', 'cbm42_strato_SOA_input_needs')
      return
    end if
  ...
  end subroutine cbm42_strato_SOA_input_needs

  !***********************************************************************

  subroutine set_miss_chem_rules_cbm42_strato_SOA(rules)
    implicit none
    type(Tchem_rules_cbm42_strato_SOA), intent(out) :: rules

    rules%defined = .false.

  end subroutine set_miss_chem_rules_cbm42_strato_SOA
  
  !***********************************************************************

  subroutine set_chem_rules_cbm42_strato_SOA(nlSetup, rules)
    !
    ...
    CHARACTER(len=*), PARAMETER :: sub_name = 'set_chem_rules_cbm42_strato_SOA'
    ...
    select case (fu_content(nlSetup, 'cbm_tolerance'))
         case ('FAST')
            call msg("Using fast (aka old v55 tolerances) for cbm42_strato_SOA")
            rules%kpp_atol = 1e-12*mol2molec !old ver5.5
            rules%kpp_rtol = 0.01            !old ver5.5
         case ('LOW')
            call msg("Using low tolerances for cbm42_strato_SOA")
            rules%kpp_atol = 3e-13*mol2molec
            rules%kpp_rtol = 0.003
         case ('MODERATE')
            call msg("Using moderate tolerances for cbm42_strato_SOA")
            rules%kpp_atol = 1e-13*mol2molec
            rules%kpp_rtol = 0.001
         case ('HIGH')
            call msg("Using high tolerances for cbm42_strato_SOA")
            rules%kpp_atol = 2e-14*mol2molec
            rules%kpp_rtol = 0.0003
         case ('STRICT')
            call msg("Using strict tolerances for cbm42_strato_SOA")
         case ('')
           call msg_warning("cbm_tolerance can be FAST, LOW, MODERATE, HIGH or STRICT")
    ...
    !Future SSP scenario that determines the CH4 and CO2 levels for years 2015-> 
    select case (fu_content(nlSetup, 'SSP_scenario'))
         case ('SSP126')
            call msg("Using SSP126 future scenario for cbm5_strato_SOA")
            rules%SSP_scenario = 126
         case ('SSP245')
            call msg("Using SSP245 future scenario for cbm5_strato_SOA")
            rules%SSP_scenario = 245
         case ('SSP370')
            call msg("Using SSP370 future scenario for cbm5_strato_SOA")
            rules%SSP_scenario = 370
         case ('')
           call msg_warning("Future scenario (SSP126, SSP245, SSP370) omitted in config.")
           call msg("Using default CO2 and CH4 mixing rations")
           rules%SSP_scenario = int_missing
         case default
            call set_error("Invalid SSP_scenario (can only be SSP126, SSP245, or SSP370)", sub_name)
            return
    end select

    rules%defined = .true.

  end subroutine set_chem_rules_cbm42_strato_SOA


  !***********************************************************************
  subroutine transform_cbm42_strato_SOA(vSp, photo, rules, metdat, timestep_sec, garbage, &
                               & zenith_cos, lat, h_start, now, print_it)

    implicit none
    real(precision_cbm42_strato_SOA), dimension(:), intent(inout) :: vSp
    real(precision_cbm42_strato_SOA), dimension(:), intent(in) :: photo
    type(Tchem_rules_cbm42_strato_SOA), intent(in) :: rules
    ...
    real :: tempr, sun, sunD, &
         & fPressure, fLandFr, &
         & fMixing_ratio_2_cnc, &
         & cH2O, cH2, cO2, cCH4, cCO2, fYear, cAir, CH4alt
    real :: SOA_b !With SOA
    integer :: indT, indZ, iSubst, iMode, nSteps, iStep, iHourTmp, jTmp, i
    real, dimension(num_species_cbm42_strato_SOA) :: garbage_local
    ...
    real(precision_cbm42_strato_SOA), dimension(20) :: rstatus
    !real, dimension(num_species_cbm42_strato_SOA) :: atol, rtol

    integer :: ierr
    !real, dimension(num_react_cbm42_strato_SOA) :: rc
    !real, dimension(num_fixed_cbm42_strato_SOA) :: fix
    ...
    !  CH4alt = 1.
    !else
    !  CH4alt = 1. - 3.25e4 * (0.35 - fPressure/std_pressure_sl)**10   ! 1 -> 0.1 in stratosphere
    !end if
    ! Apply latitudinal distribution and refine absolute value. R.H. NOTE: THIS IS BUGGY AND OMITTED NOW!
    !CH4alt = CH4alt * (0.65 + 0.52 * (1.-fPressure/std_pressure_sl) * cos(lat * Pi / 180.0))
    !
    ...
    CH4alt = tanh((fPressure/std_pressure_sl)/(0.01+0.09*sin(Pi*lat/180.0)**2))**0.5    
    !
    ...
    if (fYear > 2019.0) then !Old fit which MIGHT be a reasonable extrapolation in the future!?!? 
       cCH4 = (4.09e-5 / 46.1) * (0.83 + 1.5e-3*(fYear-1800) + 0.012 * (fYear-1930) / (1.+exp((1920-fYear)/50.)))
    elseif (fYear > 2006.0) then  !linear fit for years 2006->2017
       cCH4 = (6.668006993*fYear-11603.725)*1e-9
    elseif (fYear > 1980.0) then  !This region between 1980->2006 is difficult to fit, but the following is a reasonable estimate.
       cCH4 = (1491.64+283.0*sin((fYear-1980)*Pi/52.0)**0.42)*1e-9
    elseif (fYear > 1900.0) then  !fit for years 1900->1980.
       cCH4 = exp(6.06038980e-07*fYear**3-3.48031329e-03*fYear**2+6.66681891*fYear-4253.08570)*1e-9  
    elseif (fYear > 1750.0) then  !fit for years 1750->1900.
       cCH4 = exp(4.13844474e-08*fYear**3-2.20634143e-04*fYear**2+0.393026442*fYear-227.357323)*1e-9  
    else !Take the value 694 ppb at 1750 for earlier years. 
       cCH4 = 0.694e-6
    end if
    !Future CMIP6 SSP scenarios for 2015->
    !SSP scenarios are set after the standard VMRs to make sure that VMRs are correctly
    !set for years before 2015 in cases were SSP scanario is set in control file. 
    !Fitting the VMR's form the CESM meteo using the values at July 1st of each year. 
    !NOTE: Fits are only reasonable up to year 2100!
    select case (rules%SSP_scenario)
    case (126)
       if (fYear > 2030.96) then
          cCH4 = 1e-9*(fYear-2531.4470)*(-1.121980e-04*fYear**2+4.805063e-01*fYear-516.70428)
       elseif (fYear > 2015.0) then
          cCH4 = 1e-9*(fYear-1994.5813)*(5.226406e-02*fYear**2-213.9618*fYear+219018.796)
       end if
    case (245)
       if (fYear > 2064.0) then
          cCH4 = 0.07661546e-9*((fYear-2183.4402)*(fYear-2065.0686))+1.900e-6
       elseif (fYear > 2045.15) then
          cCH4 = -0.17979529e-9*((fYear-2141.859)*(fYear-1927.600))
       elseif (fYear > 2015.0) then
          cCH4 = -0.27385343e-9*((fYear-2129.231)*(fYear-1956.383))
       end if
    case (370)
       if (fYear > 2015.0) then
          cCH4 = 18.36646e-9*(fYear-1915.9262)
       end if
    case default
       !Use the standard profiles already set above.
    end select
    !END: Future SSP scenario selection    

    cCH4 = cCH4*CH4alt !Apply the altitude profile
    ...
    if (fYear > 1958.5) then !Using the fit for years 1950 to 2017.
       ...
       cCO2 = min(cCO2,1000.0) !Limit to 1000 ppm
    ...
    case (126)
       if (fYear > 2015.0) then
          !cCO2 = (fYear-1932.166).*(fYear-1863.704)*(1.10735e-06*fYear**2-4.797979e-03*fYear+5.20357166)
          cCO2 = 1.1017750e-06*((fYear-2144.0539)*(fYear-1832.1762)*(fYear-2133.5526)*(fYear-2018.9524))+410.
       end if
    case (245)
       if (fYear > 2015.0) then
          cCO2 = -3.2344977e-04*((fYear-2137.6423)*(fYear-2048.2138)*(fYear-1939.3630))+500.
       end if
    case (370)
       if (fYear > 2015.0) then
          cCO2 = 0.029385*((fYear-2004.005)*(fYear-1926.232))+370.
       end if
    case default
       !Use the standard profiles already set above.
    end select
    ...
    call set_rates_cbm42_strato_SOA(photo, tempr, cAir, fPressure, cH2O&
...
! Check which argumets are needed, use named arguments to avoid mismatches. For cbm4_SOA remove H2 and CO2 species.
    call set_fixed_cbm42_strato_SOA(cnc_h2o=cH2O, cnc_o2=cO2, cnc_m=cAir, cnc_ch4=cCH4, cnc_h2=cH2, cnc_co2=cCO2) !NOTE: Arguments specific to cbm42_strato/strato_SOA!  
    !call set_fixed_cbm42_strato_SOA(cnc_h2o=cH2O, cnc_o2=cO2, cnc_m=cAir, cnc_ch4=cCH4, cnc_h2=cH2) !NOTE: Arguments specific to cbm5_SOA!  
    !call set_fixed_cbm4_SOA(cnc_h2o=cH2O, cnc_o2=cO2, cnc_m=cAir, cnc_ch4=cCH4) !NOTE: Arguments specific to cbm4_SOA! 

    vSp(:) = vSp(:) * mol2molec
!    garbage(1:num_species_cbm42_strato_SOA) = garbage(1:num_species_cbm42_strato_SOA)*mol2molec
    garbage_local = 0.0

    call rosenbrock(num_species_cbm42_strato_SOA, vSp(1:num_species_cbm42_strato_SOA), &
                  & 0.0_precision_cbm42_strato_SOA, real(timestep_sec, precision_cbm42_strato_SOA),&
                  & rules%kpp_atol, rules%kpp_rtol, garbage_local, &
!                  & rules%kpp_atol, rules%kpp_rtol, garbage(1:num_species_cbm42_strato_SOA), &
...
!!$ if (isnan(vsp(i))) call msg('nan in cocktail', omp_get_thread_num())
      if (vSp(i) < 0.) then
        if (vSp(i) > -1e-11) then
          call msg('Concentration between -1e-11 and 0.0 after chemistry'+ &
                 & fu_str(species_cbm42_strato_SOA(i)),vSp(i))
          ...
          call msg_warning('Concentration between -1e-8 and -1e-9 after chemistry, species:' + &
                         & fu_str(species_cbm42_strato_SOA(i)) + '_' + fu_str(vSp(i)), &
                         & 'transform_cbm42_strato_SOA')
          !do jTmp = 1, num_species_cbm42_strato_SOA
          !  call msg('Concentration for:' + fu_str(species_cbm42_strato_SOA(jTmp)), vSp(jTmp))
          !enddo
        else
    ...
    garbage(1:num_species_cbm42_strato_SOA) = garbage(1:num_species_cbm42_strato_SOA) + &
                                        & garbage_local(1:num_species_cbm42_strato_SOA)

    if (ierr < 0)then
      call msg_warning('The integrator had an error:' + fu_str(ierr), 'transf_cbm42_strato_SOA_single_val')
      !do i = 1, num_species_cbm42_strato_SOA
      !  call msg('Concentration for:' + fu_str(species_cbm42_strato_SOA(i)), vSp(i))
      !enddo
      call msg('h_start on entry:', h_start)
      call set_error('The integrator had an error', 'transf_cbm42_strato_SOA_single_val')
      print_it = .true.
      
    endif
    h_start = rstatus(3)
    
  end subroutine transform_cbm42_strato_SOA
  
  !********************************************************************************

  subroutine transform_cbm42_strato_SOA_adj(vSp, vSp_lin, photo, rules, metdat, timestep_sec, garbage, &
                                   & zenith_cos, lat, h_start, now, print_it)

    implicit none
    real(precision_cbm42_strato_SOA), dimension(:), intent(inout) :: vSp
    real(precision_cbm42_strato_SOA), dimension(:), intent(inout) :: vSp_lin ! linearization point
    real(precision_cbm42_strato_SOA), dimension(:), intent(in) :: photo
    type(Tchem_rules_cbm42_strato_SOA), intent(in) :: rules
    real, dimension(:), intent(in) :: metdat
    real, intent(in) :: timestep_sec
    ...
    !call set_error('Adjoint not available', 'transform_cbm42_strato_SOA')
    call rosenbrockAdj(vSp_lin(1:num_species_cbm42_strato_SOA), 1, & 
                     & vSp(1:num_species_cbm42_strato_SOA), &
                     & 0.0_precision_cbm42_strato_SOA, -real(timestep_sec, precision_cbm42_strato_SOA), &
    ...
    if (ierr < 0) then
      call msg('ierr', ierr)
      call set_error('The integrator had an error', 'transf_cbm42_strato_SOA_adj')
      print_it = .true.
    end if
    
  end subroutine transform_cbm42_strato_SOA_adj
  ...
  function fu_lifetime_cbm42_strato_SOA(rules) result(lifetime)
    !
  ...
  end function fu_lifetime_cbm42_strato_SOA

  logical function fu_if_specific_deposition_cbm42_strato_SOA(rules_cbm42_strato_SOA) result(if_specific)
    implicit none
    type(Tchem_rules_cbm42_strato_SOA), intent(in) :: rules_cbm42_strato_SOA
    if_specific = .false.     ! no specific deposition whatsoever
  end function fu_if_specific_deposition_cbm42_strato_SOA
  ...
  subroutine check_low_mass_threshold_cbm42_strato_SOA(rules_cbm42_strato_SOA, speciesTrn, nSpeciesTrn, ifForwardRun, &
    ...
    if(fu_fails(indOLE /= int_missing,'No OLE in CB4 list','check_low_mass_threshold_cbm42_strato_SOA'))return
    if(fu_fails(indPAR /= int_missing,'No PAR in CB4 list','check_low_mass_threshold_cbm42_strato_SOA'))return
    !
    ! Check the relation
    !
    if(low_cnc_threshold(indPAR) > low_cnc_threshold(indOLE))then
      call msg('cbm42_strato_SOA: bad relation between OLE and PAR thresholds. Take OLE (the second):', &
                                            & low_cnc_threshold(indPAR), low_cnc_threshold(indOLE))
      low_cnc_threshold(indPAR) = low_cnc_threshold(indOLE)
      low_mass_threshold(indPAR) = low_mass_threshold(indOLE)
    endif
    
  end subroutine check_low_mass_threshold_cbm42_strato_SOA
  ...
  logical function fu_if_tla_required_cbm42_strato_SOA(rules) result(required)
    implicit none
    type(Tchem_rules_cbm42_strato_SOA), intent(in) :: rules
    
    ! call set_error('TLA not available for cbm42_strato_SOA', 'fu_if_tla_required_cbm42_strato_SOA')
     required = .true.
    
  end function fu_if_tla_required_cbm42_strato_SOA


END MODULE chem_dep_cbm42_strato_SOA
