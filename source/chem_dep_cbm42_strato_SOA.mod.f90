...
CONTAINS
  ...
  subroutine prepare_step_cbm42_strato_SOA(if_report_rates, photoarr)
    ...
    if (present(if_report_rates)) then
      ...
      if (if_report_rates) then
        ...
        !call set_rates_cbm42_strato_SOA(photoarr, temp=300.0, cAir=cAir, press=101325.0, cH2O=0.01) !Without SOA
        call set_rates_cbm42_strato_SOA(photoarr, temp=300.0, cAir=cAir, press=101325.0, cH2O=0.01, &
                               & soa_b=0.0, ASOA_aging=0.0, BSOA_aging=0.0, IVOC_aging=0.0, ifMTprods=0.0) !With SOA
  ...
  subroutine set_chem_rules_cbm42_strato_SOA(nlSetup, rules)
    ...
    rules%kpp_atol(ind_Br2)  = 1e-18*mol2molec
    rules%kpp_atol(ind_Br)   = 1e-17*mol2molec
    rules%kpp_atol(ind_BrCl) = 1e-17*mol2molec
    rules%kpp_atol(ind_Cl2)  = 1e-16*mol2molec
    rules%kpp_atol(ind_OClO) = 2e-17*mol2molec
    ...
    rules%kpp_atol(ind_BrO)  = 1e-16*mol2molec
    rules%kpp_atol(ind_Cl)   = 1e-17*mol2molec
    rules%kpp_atol(ind_O)    = 1e-15*mol2molec
    !rules%kpp_atol(ind_BrNO2)= 1e-16*mol2molec
    rules%kpp_atol(ind_Cl2O2)= 1e-15*mol2molec
  ...
  subroutine transform_cbm42_strato_SOA(vSp, photo, rules, metdat, timestep_sec, garbage, &
    ...
    !call set_rates_cbm42_strato_SOA(photo, tempr, cAir, fPressure, cH2O) !Without SOA
    call set_rates_cbm42_strato_SOA(photo, tempr, cAir, fPressure, cH2O, &
                           & SOA_b, rules%ASOA_aging, rules%BSOA_aging, rules%IVOC_aging, rules%ifMTprods) !With SOA
...
!!$ if (isnan(vsp(i))) call msg('nan in cocktail', omp_get_thread_num())
  ...
  subroutine transform_cbm42_strato_SOA_adj(vSp, vSp_lin, photo, rules, metdat, timestep_sec, garbage, &
    ...
    call set_rates_cbm42_strato_SOA(photo, tempr, cAir, fPressure, cH2O, &
                           ...
                           & IVOC_aging=rules%IVOC_aging, ifMTprods=rules%ifMTprods) !With SOA
