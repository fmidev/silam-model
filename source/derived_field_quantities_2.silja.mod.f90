...
!    call free_work_array(IPV)
  ...
  SUBROUTINE dq_bulk_richardson_number(meteoMarketPtr, met_src, valid_times, idxTimes, abl_param)
    ...
    DO tloop = 1, SIZE(valid_times)
      ...
      u3d => fu_sm_obstime_3d_field(meteoMarketPtr, met_src, u_flag, time, single_time)
      v3d => fu_sm_obstime_3d_field(meteoMarketPtr, met_src, v_flag, time, single_time)
      call grid_dimensions(fu_grid(u3d), nxu, ny)
...
!        CALL dq_store_2d(meteoMarketPtr, id, richardson_number, multi_time_stack_flag )
  ...
  subroutine bulk_richardson_nbr_one_level(u_1, u_2, v_1, v_2, q_1, q_2, theta_1, theta_2, &
    ...
    !
    ! Imported parameters
    real, dimension(:), intent(in) :: u_1, u_2, v_1, v_2, theta_1, theta_2, height_1, height_2
    real, dimension(:), pointer :: q_1, q_2
    ...
    do iy = 1, ny
      do ix = 1, nx
        ...
        if(abs(bulk_richardson_number(i)) >= 10. )then
          ...
          fTmp = 0.0
          iCount = 0
          do iy1 = max(iy-1,1),min(ny,iy+1)
            do ix1 = max(ix-1,1),min(nx,ix+1)
              i1 = ix1 + (iy1-1)* nx
...
!      Cp_ro = specific_heat_dryair*density_air_288K
...
!    call msg('ABL-Pa 14')
  ...
  SUBROUTINE dq_cwcabove_3D(meteoMarketPtr, met_src, valid_times, idxTimes, ifMakeCWC, ifMakePWC, ifMakeLCWC)
    ...
    REAL, DIMENSION(:), POINTER :: cwc_above, pCWC_prev, pwc_above, pPWC_prev, &
    ...
    loop_over_times: DO tloop = 1, SIZE(valid_times)
      ...
      DO lev = number_of_levels, 1, -1 ! top->bottom
        ...
        if (ifMakeLCWC) then
          ...
          !$omp workshare               
          lcwc_above(1:fs) = pLCWC_prev(1:fs) + cwc(1:fs) *  (dag + psrf(1:fs)*dbg)
          !$omp end workshare                                                
          pLCWC_prev => lcwc_above
        endif
...
!             G_sto(iCell) = 0.3 * fu_g_stomatal(LAI(iCell), T2m(iCell), rh2m(iCell), pressure(iCell), cloud(iCell), cosZEN)
  ...
  real  function fu_g_stomatal(LAI, T2m, rh2m, sm, c4_frac, irrigated_area, p, cloud, snowd, cosZEN, if_high_stomatal_conductance)
   ...
   !
    ...
    ! the scaling factor for c4 plants is set to a higher value than for c3 plants
    f_vpd_c4 = 75*vpd**(-.65)
...
!        ET0(iCell) = (dPsw_dT * net_radiation + air_density(iCell) * specific_heat_dryair * &
        ET0(iCell) = (dPsw_dT * net_radiation + density_air_288K * specific_heat_dryair * &
