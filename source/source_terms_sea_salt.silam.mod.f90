MODULE source_terms_sea_salt
  ...
  public add_source_species_sslt_src
...
!  integer, private, parameter :: emisMonahan_fricVel = 4104
  ...
  TYPE silam_sea_salt_source
    ...
    character(len=fnlen) :: dataDir           ! main directory for sea salt metadata
...
!        rulesSeaSalt%emisMethod = emisMonahan_fricVel
    ...
    if(fu_str_u_case(fu_content(nlSetup,'wind_selection')) == 'WIND_10M')then
      !Even if both u and v flags exist, the 10m option currently uses windspeed at 10m (scalar)!
  ...
  subroutine add_input_needs_sea_salt_src(sslt_src, q_met_dynamic, q_met_static, &
    ...
    select case(sslt_src%emisMethod)
      case(emisHybrid_w10m, emisHybrid_spume_w10m)
        !Disabled the following (in order to use simply windspeed at 10m)
        ...
        !Enabled the following for 10m windspeed (scalar) flag since not all meteo provied the components:
...
!                                   & fNbrFlux, fMassFlux, fPartDensity(iSpecies)*fNbrFlux*(Pi/6.)*fMassMeanDiam**3 !RISTO NOTE: volume -> mass
  ...
  subroutine add_source_species_sslt_src(sslt_src, species_list, nSpecies)
    !
  ...
  end subroutine add_source_species_sslt_src
...
!!$    if(error)return
    ...
    select case(sslt_src%emisMethod)
      ...
      case(emisHybrid_w10m, emisHybrid_spume_w10m)
        ...
        !
        !Enabled the following since not all meteo provide the components: 
        ...
        !Disabled the following (currently using directly the 10m windspeed (scalar):
        ...
        if(sslt_src%uWindFlag == u_flag)then
          !Moved the above (disabled) 6 lines here when switching to 10m windspeed without components:
          indU = fu_index(met_buf, sslt_src%uWindFlag)
          indV = fu_index(met_buf, sslt_src%vWindFlag)
          if(error .or. indU < 1 .or. indV < 1)then
            call set_error('Failed to find near-surface wind fields','compute_emission_for_sea_salt')
        ...
        if(error .or. indFricVel < 1)then
          call set_error('Failed to find friction velocity field','compute_emission_for_sea_salt')
          return
        endif
        !
...
!                                 & pHorizInterpMet2DispStruct, ifHorizInterp)
              !Commented the following when switching form 10m wind components --> 10m windspeed:
              ...
              if(sslt_src%uWindFlag == u_10m_flag)then
                height = 10.0
                !Added the following when switching to 10m windspeed (without components): 
                windspeed = fu_get_value(met_buf%p2d(indW10m), nx_meteo, ix, iy, 1., &
                                 & pHorizInterpMet2DispStruct, ifHorizInterp)
              else
              ...
              !windspeed = sqrt(u*u + v*v) !Commented when switching to 10m windspeed (without components).
