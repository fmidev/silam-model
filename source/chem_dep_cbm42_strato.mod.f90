module chem_dep_cbm42_strato
  ...
  !
  use cbm42_strato_interface
  use cocktail_basic
  use cbm42_strato_Parameters !R.H. 10Jan2018 for adjusting the accuracy for different species!
  !$use omp_lib

  implicit none
  private

  public set_chem_rules_cbm42_strato
  public set_missing
  
  public fu_lifetime
  public check_low_mass_threshold_cbm42_strato
  public registerSpeciescbm42_strato
  public inventory_cbm42_strato
  public transform_cbm42_strato
  public transform_cbm42_strato_adj

  public cbm42_strato_input_needs
  ...
  !
  private set_miss_chem_rules_cbm42_strato

  private fu_lifetime_cbm42_strato
  private fu_if_specific_deposition_cbm42_strato

  interface set_missing
    module procedure set_miss_chem_rules_cbm42_strato
  end interface

  interface fu_lifetime
    module procedure fu_lifetime_cbm42_strato
  end interface

  interface fu_if_specific_deposition
    module procedure fu_if_specific_deposition_cbm42_strato
  end interface

  public fu_if_tla_required
  private fu_if_tla_required_cbm42_strato
  interface fu_if_tla_required
     module procedure fu_if_tla_required_cbm42_strato
  end interface


  type Tchem_rules_cbm42_strato
     private
     logical :: defined = .false.
     integer, dimension(20) :: kpp_icntrl
     ! solver relative tolerance
     real(precision_cbm42_strato), dimension(num_species_cbm42_strato) :: kpp_rtol
     ! solver absolute tolerance, solver units
     real(precision_cbm42_strato), dimension(num_species_cbm42_strato) :: kpp_atol
     ! the rcntrl doesn't work here: it is updated per cell
     !real, dimension(num_species_cbm42_strato) :: kpp_rcntrl
  ...
  end type Tchem_rules_cbm42_strato
  
  public Tchem_rules_cbm42_strato


  !
  ! The mark of this transformation module:
  !
  integer, parameter, public :: transformation_cbm42_strato = 5008
  ...
  real(precision_cbm42_strato), dimension(20), private, save :: kpp_rcntrl
...
CONTAINS
  ...
  subroutine prepare_step_cbm42_strato(if_report_rates, photoarr)
    implicit none
    logical, optional, intent(in) :: if_report_rates
    real, dimension(:), optional, intent(in) :: photoarr
    character(len=*), parameter :: sub_name = 'prepare_step_cbm42_strato'
    real :: cAir
    
    call set_const_rates_cbm42_strato()
    kpp_rcntrl = 0.0
    ...
    if (present(if_report_rates)) then
      ...
      if (if_report_rates) then
        ...
        call set_rates_cbm42_strato(photoarr, temp=300.0, cAir=cAir, press=101325.0, cH2O=0.01&
        !                     &, soa_b=0.0, ASOA_aging=0.0, BSOA_aging=0.0, IVOC_aging=0.0, ifMTprods=0.0&
                             &, MCF_factor=1.0&
	                       &)	
        call report_rates_cbm42_strato()
      end if
    end if
  end subroutine prepare_step_cbm42_strato


  subroutine inventory_cbm42_strato(rules, &
    ...
    type(Tchem_rules_cbm42_strato), intent(in) :: rules
    ...
    call inventory_int(transformation_cbm42_strato, speciesEmis, &
                     & speciesTransp, speciesShortlived, speciesAerosol,&
                     & nSpeciesEmis, nSpeciesTransp, nSpeciesShortlived, nSpeciesAerosol, &
                     & iClaimedSpecies)

  end subroutine inventory_cbm42_strato


  !************************************************************************************

  subroutine registerSpeciescbm42_strato(rules, &
                                    & speciesTransp, speciesShortlived, speciesAerosol,&
  ...
  end subroutine registerSpeciescbm42_strato


  !*****************************************************************

  subroutine cbm42_strato_input_needs(rules, metdat)
    !
    ...
    if (.not. rules%defined) then
      call set_error('Undefined cbm42_strato rules', 'cbm42_strato_input_needs')
      return
    end if
  ...
  end subroutine cbm42_strato_input_needs

  !***********************************************************************

  subroutine set_miss_chem_rules_cbm42_strato(rules)
    implicit none
    type(Tchem_rules_cbm42_strato), intent(out) :: rules

    rules%defined = .false.

  end subroutine set_miss_chem_rules_cbm42_strato
  
  !***********************************************************************

  subroutine set_chem_rules_cbm42_strato(nlSetup, rules)
    !
    ...
    type(Tchem_rules_cbm42_strato) :: rules

    ! Local variables
    integer :: i, jTmp, ierr
    real :: fTempr, z, ratio_K_prod, ratio_K_loss, f300_T, fRelatDensity, fRelatPress, fRelatTempr
    CHARACTER(len=*), PARAMETER :: sub_name = 'set_chem_rules_cbm42_strato'
    ...
    select case (fu_content(nlSetup, 'cbm_tolerance'))
         case ('FAST')
            call msg("Using fast (aka old v55 tolerances) for cbm42_strato")
            rules%kpp_atol = 1e-12*mol2molec !old ver5.5
            rules%kpp_rtol = 0.01            !old ver5.5
         case ('LOW')
            call msg("Using low tolerances for cbm42_strato")
            rules%kpp_atol = 3e-13*mol2molec
            rules%kpp_rtol = 0.003
         case ('MODERATE')
            call msg("Using moderate tolerances for cbm42_strato")
            rules%kpp_atol = 1e-13*mol2molec
            rules%kpp_rtol = 0.001
         case ('HIGH')
            call msg("Using high tolerances for cbm42_strato")
            rules%kpp_atol = 2e-14*mol2molec
            rules%kpp_rtol = 0.0003
         case ('STRICT')
            call msg("Using strict tolerances for cbm42_strato")
         case ('')
  ...
  end subroutine set_chem_rules_cbm42_strato


  !***********************************************************************
  subroutine transform_cbm42_strato(vSp, photo, rules, metdat, timestep_sec, garbage, &
                               & zenith_cos, lat, h_start, now, print_it)

    implicit none
    real(precision_cbm42_strato), dimension(:), intent(inout) :: vSp
    real(precision_cbm42_strato), dimension(:), intent(in) :: photo
    type(Tchem_rules_cbm42_strato), intent(in) :: rules
    ...
    real, dimension(num_species_cbm42_strato) :: garbage_local
    ...
    real(precision_cbm42_strato), dimension(20) :: rstatus
    !real, dimension(num_species_cbm42_strato) :: atol, rtol

    integer :: ierr
    !real, dimension(num_react_cbm42_strato) :: rc
    !real, dimension(num_fixed_cbm42_strato) :: fix
    ...
    call set_rates_cbm42_strato(photo, tempr, cAir, fPressure, cH2O&
...
! Check which argumets are needed, use named arguments to avoid mismatches. For cbm4_SOA remove H2 and CO2 species.
    call set_fixed_cbm42_strato(cnc_h2o=cH2O, cnc_o2=cO2, cnc_m=cAir, cnc_ch4=cCH4, cnc_h2=cH2, cnc_co2=cCO2) !NOTE: Arguments specific to cbm42_strato/strato_SOA!  
    !call set_fixed_cbm42_strato(cnc_h2o=cH2O, cnc_o2=cO2, cnc_m=cAir, cnc_ch4=cCH4, cnc_h2=cH2) !NOTE: Arguments specific to cbm5_SOA!  
    !call set_fixed_cbm4_SOA(cnc_h2o=cH2O, cnc_o2=cO2, cnc_m=cAir, cnc_ch4=cCH4) !NOTE: Arguments specific to cbm4_SOA! 

    vSp(:) = vSp(:) * mol2molec
!    garbage(1:num_species_cbm42_strato) = garbage(1:num_species_cbm42_strato)*mol2molec
    garbage_local = 0.0

    call rosenbrock(num_species_cbm42_strato, vSp(1:num_species_cbm42_strato), &
                  & 0.0_precision_cbm42_strato, real(timestep_sec, precision_cbm42_strato),&
                  & rules%kpp_atol, rules%kpp_rtol, garbage_local, &
!                  & rules%kpp_atol, rules%kpp_rtol, garbage(1:num_species_cbm42_strato), &
...
!                                        & fu_str(species_cbm42_strato(i)) + ', count, value =' + &
...
!!$ if (isnan(vsp(i))) call msg('nan in cocktail', omp_get_thread_num())
      if (vSp(i) < 0.) then
        if (vSp(i) > -1e-11) then
          call msg('Concentration between -1e-11 and 0.0 after chemistry'+ &
                 & fu_str(species_cbm42_strato(i)),vSp(i))
          ...
          call msg_warning('Concentration between -1e-8 and -1e-9 after chemistry, species:' + &
                         & fu_str(species_cbm42_strato(i)) + '_' + fu_str(vSp(i)), &
                         & 'transform_cbm42_strato')
          !do jTmp = 1, num_species_cbm42_strato
          !  call msg('Concentration for:' + fu_str(species_cbm42_strato(jTmp)), vSp(jTmp))
          !enddo
        else
    ...
    garbage(1:num_species_cbm42_strato) = garbage(1:num_species_cbm42_strato) + &
                                        & garbage_local(1:num_species_cbm42_strato)

    if (ierr < 0)then
      call msg_warning('The integrator had an error:' + fu_str(ierr), 'transf_cbm42_strato_single_val')
      !do i = 1, num_species_cbm42_strato
      !  call msg('Concentration for:' + fu_str(species_cbm42_strato(i)), vSp(i))
      !enddo
      call msg('h_start on entry:', h_start)
      call set_error('The integrator had an error', 'transf_cbm42_strato_single_val')
      print_it = .true.
      
    endif
    h_start = rstatus(3)
    
  end subroutine transform_cbm42_strato
  
  !********************************************************************************

  subroutine transform_cbm42_strato_adj(vSp, vSp_lin, photo, rules, metdat, timestep_sec, garbage, &
                                   & zenith_cos, lat, h_start, now, print_it)

    implicit none
    real(precision_cbm42_strato), dimension(:), intent(inout) :: vSp
    real(precision_cbm42_strato), dimension(:), intent(inout) :: vSp_lin ! linearization point
    real(precision_cbm42_strato), dimension(:), intent(in) :: photo
    type(Tchem_rules_cbm42_strato), intent(in) :: rules
    real, dimension(:), intent(in) :: metdat
    real, intent(in) :: timestep_sec
    ...
    !call set_error('Adjoint not available', 'transform_cbm42_strato')
    call rosenbrockAdj(vSp_lin(1:num_species_cbm42_strato), 1, & 
                     & vSp(1:num_species_cbm42_strato), &
                     & 0.0_precision_cbm42_strato, -real(timestep_sec, precision_cbm42_strato), &
    ...
    if (ierr < 0) then
      call msg('ierr', ierr)
      call set_error('The integrator had an error', 'transf_cbm42_strato_adj')
      print_it = .true.
    end if
    
  end subroutine transform_cbm42_strato_adj
  ...
  function fu_lifetime_cbm42_strato(rules) result(lifetime)
    !
  ...
  end function fu_lifetime_cbm42_strato

  logical function fu_if_specific_deposition_cbm42_strato(rules_cbm42_strato) result(if_specific)
    implicit none
    type(Tchem_rules_cbm42_strato), intent(in) :: rules_cbm42_strato
    if_specific = .false.     ! no specific deposition whatsoever
  end function fu_if_specific_deposition_cbm42_strato
  ...
  subroutine check_low_mass_threshold_cbm42_strato(rules_cbm42_strato, speciesTrn, nSpeciesTrn, ifForwardRun, &
    ...
    if(fu_fails(indOLE /= int_missing,'No OLE in CB4 list','check_low_mass_threshold_cbm42_strato'))return
    if(fu_fails(indPAR /= int_missing,'No PAR in CB4 list','check_low_mass_threshold_cbm42_strato'))return
    !
    ! Check the relation
    !
    if(low_cnc_threshold(indPAR) > low_cnc_threshold(indOLE))then
      call msg('cbm42_strato: bad relation between OLE and PAR thresholds. Take OLE (the second):', &
                                            & low_cnc_threshold(indPAR), low_cnc_threshold(indOLE))
      low_cnc_threshold(indPAR) = low_cnc_threshold(indOLE)
      low_mass_threshold(indPAR) = low_mass_threshold(indOLE)
    endif
    
  end subroutine check_low_mass_threshold_cbm42_strato
  ...
  logical function fu_if_tla_required_cbm42_strato(rules) result(required)
    implicit none
    type(Tchem_rules_cbm42_strato), intent(in) :: rules
    
    ! call set_error('TLA not available for cbm42_strato', 'fu_if_tla_required_cbm42_strato')
     required = .true.
    
  end function fu_if_tla_required_cbm42_strato


END MODULE chem_dep_cbm42_strato
