...
!  public set_standard_cocktails_fname
  ...
  type(Taerosol_mode), parameter, public :: aerosol_mode_missing = Taerosol_mode( 'MISSING', &
  ...
  ! gas-phase species.
  !
  type(Taerosol_mode), parameter, public :: in_gas_phase = Taerosol_mode( 'GAS', &
                                                      & gas_phase_flag, 0, &
                                                      & real_missing, &
                                                      & real_missing, real_missing, real_missing, &
                                                      & .false., &
  ...
  type(Taerosol_mode), parameter, public :: no_mode = Taerosol_mode( 'NO_MODE', &
                                                      & no_mode_flag, 0, &
                                                      & real_missing, &
                                                      & real_missing, real_missing, real_missing, &
                                                      & .false., &
...
!    endif
  ...
  logical function fu_descriptors_equal(descr1, descr2) result(OK)
    ...
    !! Differnt names -- different cocktails 
    !! Otherwise, AS4 fire sources should be carefully scanned for equal cocktails
    OK = (descr1%cocktail_name == descr2%cocktail_name)
    if (.not. OK) return
...
#ifdef DEBUG
     call msg('')
     call msg("Modes form namelist:")
     do iMode = 1, aerosol%n_modes
       call report(aerosol%modes(iMode))
       call msg('')
     enddo
#endif     
...
!    fu_aerosol_defined = aerosol%n_modes > 0 .and. aerosol%n_modes < 100
  ...
  subroutine report_aerosol_mode(aerosol_mode)
    ...
    call msg('Mode name, distribution type, solubility: "'//trim(aerosol_mode%name) // '", ' // &
           & trim(fu_name(aerosol_mode%distr_type)), aerosol_mode%solubility)
...
!
  ...
  function fu_integrate_volume(d1, d2, mode) result(fract)
    ...
    real :: fract
    real(r8k) :: cumu1, cumu2, mass_median_log, a, b, mean_vol_input, nbr_fract, mfp1, mfp2
...
!        cumu1 = (1.0 + fu_erf((log(d1) - mass_median_log) / (dsqrt_2*log(mfp2)))) * 0.5
!        cumu2 = (1.0 + fu_erf((log(d2) - mass_median_log) / (dsqrt_2*log(mfp2)))) * 0.5
        cumu1 = (1.0 + erf((log(d1) - mass_median_log) / (dsqrt_2*log(mfp2)))) * 0.5
        cumu2 = (1.0 + erf((log(d2) - mass_median_log) / (dsqrt_2*log(mfp2)))) * 0.5
        fract = cumu2 - cumu1
  ...
  function fu_integrate_number(d1, d2, mode) result(fract)
    ...
    real(r8k) :: fract
    real(r8k) :: cumu1, cumu2, a, b, median_log, mfp1, mfp2, fTmp
    ...
    select case(mode%distr_type)
      case (lognormal_flag)
        ...
        mfp2 = mode%fp2
        median_log = log(mfp1)
!        fTmp = (dlog(d1) - median_log)
!        fTmp = (dlog(d1) - median_log) / (dsqrt_2*dlog(mfp2))
!        fTmp = fu_erf((dlog(d1) - median_log) / (dsqrt_2*dlog(mfp2)))
!        fTmp = (dlog(d2) - median_log)
!        fTmp = (dlog(d2) - median_log) / (dsqrt_2*dlog(mfp2))
!        fTmp = fu_erf((dlog(d2) - median_log) / (dsqrt_2*dlog(mfp2)))
!        cumu1 = (1.0 + fu_erf((log(d1) - median_log) / (dsqrt_2*log(mfp2)))) * 0.5
!        cumu2 = (1.0 + fu_erf((log(d2) - median_log) / (dsqrt_2*log(mfp2)))) * 0.5
        cumu1 = (1.0 + erf((log(d1) - median_log) / (dsqrt_2*log(mfp2)))) * 0.5
        cumu2 = (1.0 + erf((log(d2) - median_log) / (dsqrt_2*log(mfp2)))) * 0.5
        fract = cumu2 - cumu1
