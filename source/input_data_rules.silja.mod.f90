MODULE input_data_rules
  ...
  public set_staggered_winds
  public fuIfStaggeredWindsWDR
  ...
  TYPE silja_wdr
    ...
    logical :: ifStaggeredWinds  !! Winds stored on staggered grids. OBS: different data size
...
!    call msg_warning('Unknown data source','fu_mds_ptr')
  ...
  subroutine set_staggered_winds(wdr, ifStaggeredWinds) 
    IMPLICIT NONE
    logical, intent(in) :: ifStaggeredWinds
    TYPE(silja_wdr), INTENT(inout) :: wdr
    wdr%ifStaggeredWinds = ifStaggeredWinds
  END subroutine set_staggered_winds

  ! ***************************************************************

  logical FUNCTION fuIfStaggeredWindsWDR(wdr) 
    IMPLICIT NONE
    TYPE(silja_wdr), INTENT(in) :: wdr
     fuIfStaggeredWindsWDR = wdr%ifStaggeredWinds 
  END  FUNCTION fuIfStaggeredWindsWDR
...
!      print *, 'No analysis time is given - trying reasonable combinations'
      ...
      do i = iStart, max_forecast_steps
         ...
         if (nint(fu_sec(anal_time_tmp)) /= 0) cycle  ! No analysis time with seconds
