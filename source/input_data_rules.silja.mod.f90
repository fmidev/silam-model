...
!            print *, 'Done'
      ...
      if(ifWait)then
        ...
        if(iProceed == max_wait_count)then
          ...
          call msg('Last tried file:' + strFNm)
        else
          do j = 1, 60
            call wait(5.)                         ! waiting
            if(error)return
    ...
    do i=1, size(arTemplate)
      ...
      select case(arTemplFormats(i)%iFormat)
        ...
        case(test_field_value_flag)  ! No files at all
          ...
          fnames(1)%sp = fu_collection(arTemplate(i))
          if(size(fnames) > 1) fnames(2)%sp = ''
          iFormatsTmp(iFilesReceived + 1) = i
          iFilesReceived = iFilesReceived + 1
        case default
  ...
  function fu_input_file_format(chFileString)
    ...
    elseif(index(chFileString,'NETCDF') == 1)then
      ...
      if(index(chFileString,'NETCDF:') == 1)then
          if(index(chFileString,' ') == 0)then
              fu_input_file_format%title = chFileString(8:)
          else
              fu_input_file_format%title = chFileString(8:index(chFileString,' ')-1)
          endif
      endif
