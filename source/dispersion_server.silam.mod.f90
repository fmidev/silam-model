...
!    mapper => MassMapIn%mapper
  ...
  subroutine add_field_to_mass_map_interp(field_3d, indSpeciesOut, fMassFractionDescr, nSpeciesDescr, fMassAdded, &
                                        & fRateScaling, &
    ...
    integer :: nxFrom, nyFrom, nxTo, nyTo, nLevsFrom, nLevsTo, ixTo, iyTo, iLevFrom, iLevTo, i1dFrom, &
             & iSpecies, iCoef, ixStartFrom, iyStartFrom, ixEndFrom, iyEndFrom, &
             & ixStartTo, iyStartTo, ixEndTo, iyEndTo, &
             & iLevStartFrom, iLevEndFrom, iLevStartTo, iLevEndTo
    type(THorizInterpCells) :: ptrCoefHoriz
    type(TVertInterpOneCell) :: ptrCoefVert
...
call check_mass_moment_point(pMassMap%arM(indSpeciesOut(iSpecies), iSourceId, iLevTo, ixTo,iyTo), &
        ...
        do iyTo = 1, nyTo
          do ixTo = 1, nxTo
            do iLevTo = 1, nLevsTo
              do iSpecies = 1, nSpeciesDescr
                fTmp = pDataFrom(iLevTo)%pp(ixTo+nxTo*(iyTo-1)) * fMassFractionDescr(iSpecies) * fRateScaling
                fPtr => pMassMap%arM(indSpeciesOut(iSpecies),iSourceId,iLevTo,ixTo,iyTo)
                fPtr = fPtr + fTmp 
                fMassAdded(indSpeciesOut(iSpecies)) = fMassAdded(indSpeciesOut(iSpecies)) + fTmp 
                pMassMap%ifGridValid(ilevTo,iSourceId) = .true.
        ...
        do while(.not. eof)
          ...
          if(fu_grid(id) == pMap%gridTemplate)then
            ...
            call grid_data_hor_select_new_grid(fu_grid(id), &  ! grid_original
                                             & pMap%gridTemplate, &        ! storage_grid
                                             & area_missing, &       ! storage_area
                                             & .false., &    ! ifAdjust to system_frid
                                             & gridTmp)      ! new grid_new
            if(error)return
            dataMap => fu_work_array(fu_number_of_gridpoints(gridTmp))
            if(error)return
...
!call msg('Sum of GrADS:',sum(dataMap(1:fu_number_of_gridpoints(pMap%gridTemplate))))
                ...
                ! Reproject the data to the map grid. Can be interpolation or averaging/summation
                ! The actual approach is controlled by regridding_method, specific for each quantity.
                ! In some cases this might be costly but this sub is called once per run, so should not
                ! be too horrible.
                !
                ! Data selection is done in two steps: (i) get the new grid and allocate space for ther data
                ! (ii) do the reprojection itself
...
!call msg('Ave of GRADS3d, local unit:',sum(dataMap(1:fu_number_of_gridpoints(pMap%gridTemplate)))/fu_number_of_gridpoints(pMap%gridTemplate))
                ...
                call check_quantity_range(fu_quantity(id), dataMap, null(), ix*iy, ix, &
...
call msg('Ave of GRADS, converted:',sum(dataMap(1:fu_number_of_gridpoints(pMap%gridTemplate)))/fu_number_of_gridpoints(pMap%gridTemplate))
      ...
      case(netcdf_file_flag)  
        ...
        call id_list_from_netcdf_file(uIn, idList, fields_found, .true., .true.)
...
call msg('Ave of NetCDF, converted:',sum(dataMap(1:fu_number_of_gridpoints(pMap%gridTemplate)))/fu_number_of_gridpoints(pMap%gridTemplate))
      ...
      case(test_field_value_flag)   
        ...
        do iLevMap = 1, fu_NbrOfLevels(pMap%vertTemplate)
          call make_test_field(chFName,  id,  dataIn,  timeOfMap,  one_day * 3650., pGrid)
...
!call msg('vmr2cnc dataInOut Ave:', sum(dataInOut(1:fu_number_of_gridpoints(gridOut)))/fu_number_of_gridpoints(gridOut))
  ...
  subroutine get_scaling(quantity, iVerticalTreatment, &
    ...
    ifTimestepIntegrated = .False.
    if (quantity == emission_intensity_flag) ifTimestepIntegrated = .True.
  ...
  subroutine merge_mass_map_2_mass_map(pMassMapLinks, &      ! Structure to merge
                                     & data_buf, writingTime, now, timestepIn, ifFirstTime)
    !
...
!                dataOut(1:fs) = dataIn(1:fs) / abs(fu_sec(fu_valid_time(idIn) - fu_valid_time(idOut)))
  ...
  subroutine merge_vector_data_to_stack(idIn, dataIn, stackOut, targetId, ifRandomise) !, direction)
    ...
    if (.not. (fu_grid(targetId) == fu_grid(idIn))) then
      ! dataInOutgrid is allocated in grid_data_horizontal_select
      ! dataInOutgrid => fu_work_array()
      !  call msg('Will do grid operations, merge_vector_...')
      ! In MPI runs we need to select the interior of fields defined in the extended area
      ! (u, v, etc.).
      ! Data selection is done in two steps: (i) get the new grid and allocate space for ther data
      ! (ii) do the reprojection itself
      !
!      call grid_data_hor_select_new_grid(fu_grid(idIn), &  ! grid_original
!                                       & fu_grid(targetId), &        ! storage_grid
!                                       & area_missing, &       ! storage_area
!                                       & .false., &    ! ifAdjust to system_frid
...
!      if(error)return
      ...
      if (.not. grid_ok) then
        ...
        call set_error('Can''t get grids to match', 'merge_vector_data_to_stack')
        return
      end if
    else
...
!!!    endif
         ...
         case(forecast_flag)  
           ...
           select case(fu_field_kind(idIn))
             ...
             case default
               dataOut(1:fs) = 0.
               call set_valid_time(idOut, fu_valid_time(targetId))
               call set_error('Unsupported input field type, instant target','merge_vector_data_to_stack')
           end select
         ...
         case(accumulated_flag, averaged_flag) !, accumulated_period_valid_flag) ! Target ID
           ...
           select case(fu_field_kind(idIn))
             ...
             case (accumulated_flag) !, accumulated_period_valid_flag)
               ...
               if(found_internal)then 
                 ...
                 if(idIn == targetId)then
                   ...
                   call set_error('Can not find internal field for merging','merge_vector_data_to_stack')
                   call msg('Input ID')
                   call report(idIn)
             ...
             case default ! idIn
               ...
               ! Types like difference_flag, period_averaging_flag
               !
               call set_error('Non-supported averaging type of input field','merge_vector_data_to_stack')
           end select
   ...
   !          call msg("Finalizing averaging for "+ fu_quantity_short_string(fu_quantity(targetId)))
         ...
         case default ! target ID
           dataOut(1:fs) = 0.
           call set_valid_time(idOut, fu_valid_time(targetId))
           call set_error('Non-supported averaging type of output field','merge_vector_data_to_stack')
       end select
    endif ! 
