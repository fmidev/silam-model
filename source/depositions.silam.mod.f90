...
!  public get_Rb
  ...
  public get_Rs_2013
  public fu_get_vd
  public get_vd_species
!    public check_deposition_rules
  public get_settling_velocity_species
  ...
  integer, private, parameter :: scavStandard = 6101 ! Standard 3D scavenging ratio

  integer, private, parameter :: scav2011 = 6112     ! Scavenge accounting
                                                     ! for rain profile,
                                                     ! saturations etc..
  integer, private, parameter :: scav2011fc = 6113   ! Same as above, but with fake cloud profile
  integer, private, parameter :: scavNoScav = 6114     ! No scavenging at all
  integer, private, parameter :: scav2018 = 6115     ! #Limited washout
...
CONTAINS
  ...
  subroutine set_deposition_rules(nlSetup, rulesDeposition)
    ...
    !
    select case(fu_str_u_case(fu_content(nlSetup,'wet_deposition_scheme')))
      ...
      case default
        call set_error(fu_connect_strings('Unknown scavenging type:', &
                                        & fu_content(nlSetup,'particle_scavenging_method')), &
                     & 'set_deposition_rules')
        call msg("Valid values for wet_deposition_scheme are:")
        call msg(" (STANDARD_3D_SCAVENGING|NEW2011_SCAVENGING|NEW2011_SCAVENGING_FAKECLOUD|NO_SCAVENGING)")
        rulesDeposition%defined = silja_false
        return
    end select
...
!        rulesDeposition%DryDepType = DryD_resist_grav_combine
  ...
  subroutine init_standard_deposition(speciesTransport, indDepositionType, nSpeciesTransport, &
    ...
    ! Sets indices for some special species and check if they have needed
    ! parameters set.
    ...
    select case (rulesDeposition%scavengingType)
      ...
      case(scavNoScav)
                ! Nothing is needded
      case(scav2011, scav2011fc, scav2018)
        do iSpecies = 1,rulesDeposition%nGasesDepositing
          ...
          if(fu_true(DepData%ifGasDeposited))then
            if(DepData%Henry_const_298K < 0.)    then
              DepData%Henry_const_298K = 0.
              call msg ("Henry constant is missing for:" + fu_name(speciesTransport(iSpTr)%material))
              call set_error("Henry constant set to zero", "init_standard_deposition")
              return
            endif
...
!        iTmp = fu_merge_integer_to_array(abl_height_m_flag, q_met_dyn)
      ...
      case(scav2011,scav2011fc, scav2018)
        ...
        if (any(rulesDeposition%scavengingType == (/scav2011, scav2018/))) then
           iTmp = fu_merge_integer_to_array(cwcabove_3d_flag, q_met_dyn)
        endif
        if (rulesDeposition%scavengingType == scav2018) then
           iTmp = fu_merge_integer_to_array(pwcabove_3d_flag, q_met_dyn)
        endif
      ...
      case default
        call set_error('Unknown scavenging type','add_deposition_input_needs')
        ...
        if (fRoughfr > 0.) then ! rough fraction exist
           ...
           vdrough = fRoughfr * u_star * &
            ...
            if (.not. (vdrough .ge. 0.)) then
                  call msg("**DEPO** Bad vdrough! vdrough,fz0:", vdrough, fZ0)
  ...
  subroutine verify_deposition(speciesTransport, nspecies, rulesDeposition)
    ...
    do i = 1, nspecies
      if(.not. rulesDeposition%pSpecies(i) == speciesTransport(i))then
        ...
        call set_error('Species are not the same','get_Rs')
        return
      endif
    end do

    if(rulesDeposition%nAerosolsDepositing < 0 .or. rulesDeposition%nGasesDepositing < 0 .or. &
     & rulesDeposition%nAerosolsDepositing + rulesDeposition%nGasesDepositing > nSpecies)then
      ...
      call set_error('Inconsistent number of species','get_Rs')
    endif
...
!                                & pMeteo2DispInterpHoriz, pMeteo2DispInterpVert, &
  ...
  subroutine scavenge_map(mapConc, mapWetDep, arGarbage, tla_step, &
                        & timestep_sec, weight_past, &
                        & rulesDeposition, arLowMassThreshold, &
    ...
    ! Local variables
    real :: scav_coef_std, scav_coef, fTempr_1, timestep_sec_abs, fSO2, fS_rest, fMaxScav, precip_rate
    real :: fTmp, fTmp1, scav_coef_ic, scav_coef_sc
    real :: precip_rate_avg, cloud_cover, fTempr, fPressure, fCloudWC, mu_air, nu_air, rho_air
    real :: is_rain, drop_size, drop_mass, drop_vel, drop_Re
    real :: cell_height, cell_volume
    real :: fA, fB, fC, fD 
    double precision :: tauf_over_RdCd 
    integer :: ix, iy, iLev, iSrc, iSpecies, iDisp, iSpTr, iMeteo, ithread
    integer :: iSrc1, iSp, ispecies1, iSpTr1
    real, dimension(:), pointer :: xSizePtr, ySizePtr
    real, dimension(max_species) :: rSettling
    real, dimension(max_levels) :: cloudwcabove, rain_rate_prof,  deltarain,   cloudwc  
    real :: IsRealCloud
    logical :: not_adjoint
    ...
    if(ifFirst)then
      ...
      allocate(scavAmount(mapConc%nSpecies, mapConc%nSrc, mapConc%n3D, 0:nthreads), stat=ix)
      if(fu_fails(ix == 0, 'Failed to allocate the scavenging amount array','scavenge_map'))return
      scavAmount(1:mapConc%nSpecies, 1:mapConc%nSrc, 1:mapConc%n3D, 0:nthreads) = 0.
      ifFirst = .false.
    endif
    !
...
!             rSettling(iSpecies) = min(1000.0, rSettling(iSpecies) * 2.5e4)  ! sqrt may be too much, try linear
       ...
       !
       ! Start the main cycle over the map.
       !
       do iy = 1, mapConc%ny
        do ix = 1, mapConc%nx

         if(.not. any(mapConc%ifColumnValid(1:mapConc%nSrc,ix,iy)))cycle
         ...
         ! Get needed fields
         !
         fTempr_1 = fu_get_value(fldTempr, &   ! field_ptr_4d
                               & nx_meteo, &     ! nxFrom
                               & ix, iy, 1, & 
                               & weight_past, &
                               & interpCoefMet2DispHoriz, interpCoefMet2DispVert, &
         ...
         do iLev = 1, mapConc%n3D
          do iSrc = 1, mapConc%nSrc
            ...
            scav_coef_std = fu_get_value(fldScavStd, &   ! field_ptr_4d
                                       & nx_meteo, &     ! nxFrom
                                       & ix, iy, iLev, & 
                                       & 0.0, & !weight_past, because precip rate is true at the end of range
                                       & interpCoefMet2DispHoriz, interpCoefMet2DispVert, &
                                       & ifInterpMet2DispHoriz, ifInterpMet2DispVert)
...
!                mapWetDep%arM(iSpTr,iSrc,1,ix,iy) = mapWetDep%arM(iSpTr,iSrc,1,ix,iy) + ScavAmount
         ...
         if(rulesDeposition%ifSulphurSaturation)then
           ...
           if(fTempr_1  > 273.15)then    ! Rain
             fMaxScav = precip_rate * xSizePtr(iDisp) * ySizePtr(iDisp) * timestep_sec_abs * &
      ...
      case(scav2011fc, scav2011, scav2018 )
...
!       call  check_deposition_rules(rulesDeposition, mapConc%species)
        ...
        if (defined(tla_step)) then
          call msg('Have tla step, SCAV201X')
          have_tla = .true.
          TL_store_mass_in_air => fu_get_tla_point(tla_step, rulesDeposition%scavengingType)
          if(fu_fails(mapConc%n3d == 1,'TLA allow only one source in mass map','scavenge_map'))return
        else
        ...
        !$OMP PARALLEL DEFAULT(NONE) &
        !$OMP PRIVATE ( iy, ix, iLev, iSrc, iThread, iSpTr, precipContent, precipContent3d, r, &
        !$OMP         & r1d) &
        !$OMP SHARED (timestep_sec_abs, mapConc, mapWetDep,  arLowMassThreshold, &
        !$OMP         & timestep_sec, weight_past, rulesDeposition, interpCoefMet2DispHoriz, &
        !$OMP         & interpCoefMet2DispVert, ifInterpMet2DispHoriz, ifInterpMet2DispVert,  &
        !$OMP         & scavAmount, TL_store_mass_in_air, tla_step, have_tla)
        ...
        !$OMP MASTER
        if (rulesDeposition%scavengingType == scav2018) then
            call msg('New 2018 scavenging scheme:'+fu_str(rulesDeposition%scavengingType) &
                      !$ & + ', parallel, OMP NUM_THREADS:'+fu_str(omp_get_num_threads()) &
            &)
        elseif (rulesDeposition%scavengingType == scav2011) then
            call msg('New 2011 scavenging scheme:'+fu_str(rulesDeposition%scavengingType) &
                      !$ & + ', parallel, OMP NUM_THREADS:'+fu_str(omp_get_num_threads()) &
            &)
        else
            call msg('New 2011 scavenging scheme with fake cloud water:'+fu_str(rulesDeposition%scavengingType) &
                      !$ & + ', parallel, OMP NUM_THREADS:'+fu_str(omp_get_num_threads()) &
            &)
        endif
        !$OMP ENDMASTER
...
!                call msg("***Making rain, cell "+fu_str(ix)+","+fu_str(iY)+', level= '+fu_str(iLev))
                call  make_rain_in_cell(iX, iY, iLev, mapConc%n3D, r,  weight_past, & 
                                      & interpCoefMet2DispHoriz, interpCoefMet2DispVert, &
                                      & ifInterpMet2DispHoriz, ifInterpMet2DispVert, rulesDeposition%scavengingType)
!                call report_rain(r)
                !
                ! No matter what, mass must be stored if we are going to compute adjoint
                !
                if (have_tla) then
                  TL_store_mass_in_air(:,iLev,ix,iy) = mapConc%arM(:, 1, iLev, ix, iy)
                end if
                !
                ! Speedup is now possible
                if (.not.r%ifRainSfc) exit  !No rain in the column
...
!$OMP MASTER
    ...
    if (r%ifRainSfc)then
      call msg('>>>>>>>>>>>>>>>>>>>>>>>>>> TEST TL SCAVENGING <<<<<<<<<<<<<<< dispersion coord', ix, iy)

      do iLev = mapConc%n3D,1,-1 !Top->bottom
        call  make_rain_in_cell(iX, iY, iLev, mapConc%n3D, r1d%r(iLev),  weight_past, & 
                              & interpCoefMet2DispHoriz, interpCoefMet2DispVert, &
                              & ifInterpMet2DispHoriz, ifInterpMet2DispVert, rulesDeposition%scavengingType)
      end do  ! iLev
      call test_TL_ADJ(mapConc%arM(:,1,:,ix,iy), &  ! sensit mass in precipitation (sSpecies, nSrc, nz)
...
!                call report_rain(r)
              ...
              if (.not. r%ifRainyCell) cycle  ! go to the next column
              !
      ...
      case default
        call msg('Unknown scavenging type',rulesDeposition%scavengingType)
        call set_error('Unknown scavenging type','scavenge_map')
    end select ! type of scavenging

  end subroutine scavenge_map
  ...
  subroutine report_rain(r)
    ...
    WRITE (l%sp, '(A,F10.3,A,F10.3,A,F10.3)') &
...
!        real, dimension(:,:), pointer, intent(in) :: arGarbage
    ...
    real :: equilibrium_aq_SIV, fA, fB, fC, fD ! stuff for sulphur equilibrium
    ...
    do iSrc = 1, nSources
      do iSp = 1, nSpecies
        ...
        if(iSpTr == indSO2) then       !adjust water_capacitance_factor for SO2
          ...
          if (total_SIV_in_scav_zone == 0.) then
            call msg("iSpTr, iSrc",iSpTr,iSrc)
          ...
          if (fB*fB > 1000*abs(fC)  ) then ! No need for quadratic equation
            equilibrium_aq_SIV = - fC/fB
          else
            fD = fB*fB - 4*fC
            equilibrium_aq_SIV =  0.5*(-fB + sqrt(fD))
          endif
          water_capacitance_factor = scav_zone_volume /  &
                  & (total_SIV_in_scav_zone / equilibrium_aq_SIV - total_water_in_scav_zone)

          if (.not. water_capacitance_factor >= 0) then 
!$OMP CRITICAL(WC_SO2)
            call msg("*** Warning! Negative SO2 capacitance factor:",water_capacitance_factor)
            call msg("fA, fB, fC, fD, fB^2, 4*fA*fC",(/fA, fB, fC, fD, fB*fB, 4.*fA*fC/))
...
!$OMP END CRITICAL(WC_SO2)
        ...
        if (precip_rate > 0.000001) then ! > ~0.003mm/h subcloud scavenging on
...
!        if (scav_coef < 1e-6) cycle !Scav. time more than ~10 days
        ...
        scav_coef = (scav_coef*r%max_scav_rate)/(scav_coef+r%max_scav_rate) !1./(1/a+1/b) == ab/a+b
        ...
        if(present(coefs))then
          ...
          if(iSpTr == indSO2)then
            ...
            coefs%dCeq_dMs = -1. / (scav_zone_volume * (2 * fA * coefs%Ceq + fB)) ! d Ceq / d M_SO2, p.37
            ...
            coefs%dCeq_dMa = - coefs%Ceq / (2. * fA * coefs%Ceq  + fB)  ! d Ceq / d Ma - non-SO2 acids, p.37
  ...
  subroutine scavenge_column_2011_adjoint(sens_mass_in_air, &    ! sensitivity in the air   (nSpecies, nSrc, n3d)
    ...
    ! It consists mostly of zeroes but storing and computing even zeroes is quite expensive.
    ...
    !        (iii) non-scavengeable, linearly scavengeable and perfectly scavengeable species are trivial
    ! See notebook 10a, pp.22-35
    ! mas
    ...
    type(silja_rp_2d), dimension(:), optional, pointer :: pTL_in, pAdj_in
...
!    ! Local variables
    ...
    real, dimension(:,:), pointer :: pTL, pAdj
    ...
    do iSp = 1, nSpecies
      ...
      if (iSp == indSO2)then
        ...
        ! Its matrix consists of nAcidityAffectingSpecies squared matrices n3d x n3d, notebook 10a p.32
        !
        if(present(pTL_in))then
          pTL => pTL_in(iSp)%pp
        else
          pTL => fu_work_array_2d(n3d, n3d*nAcidityAffectingSpecies)   ! sulphur is affected by other species
        endif
        pTL(1:n3d, 1:n3d*nAcidityAffectingSpecies) = 0.0
        do iAAS = 1, nAcidityAffectingSpecies  ! non-zero matrices are only for acidity-affecting species
          ...
          !
          ! SO2 is sensitive to the above-cumulated amount of other acidity-affecting species
          !
!if (indAcidityAffectingSp(iAAS) == indSO2)then
          call fill_triangle(pTL, (iAAS-1)*n3d, indAcidityAffectingSp(iAAS)*iSign, n3d, r1d, c1d)
!endif
          !
          ...
          if(indAcidityAffectingSp(iAAS) == indSO2)then
            do iTmp = 1, n3d
              pTL(iTmp,1+iAAS:n3d+iAAS) = pTL(iTmp,1+iAAS:n3d+iAAS) * &   ! dMs / dmSO2, non-local 
        ...
        do iTmp = 1,n3d
          pAdj(1:n3d*nAcidityAffectingSpecies, iTmp) = pTL(iTmp, 1:n3d*nAcidityAffectingSpecies)
        enddo
        ...
        if(present(pTL_in))then
          ...
          pTL => fu_work_array_2d(n3d, n3d)
        endif
        call fill_triangle(pTL, 0, iSp, n3d, r1d, c1d)        ! Basic case: non-SO2 gas
        !
        ! Having TL matrix made, transpose it to adjoint
        !
        if(present(pAdj_in))then
        ...
        do iTmp = 1,n3d
          pAdj(1:n3d,iTmp) = pTL(iTmp,1:n3d)
        enddo
      endif  ! if SO2
      !
      ! Having done the TL and Adjoint matrices, now can make the adjoint scavenging
      ! Note that now we use diagonal TL matrix, whihc is then the sanm as adjoint
      !
      do iLev = 1, n3d
        do iSrc = 1, nSources
          sens_mass_in_air(iSp, iSrc, ilev) = sens_mass_in_air(iSp, iSrc, ilev) + &
                                            & pTL(iLev,iLev) * sens_mass_in_air(iSp, iSrc, ilev)
        end do
...
!!              do iTmp = 1, n3d
!!                fTmp = fTmp + pTL(iSp)%pp(iLev, iTmp+(iAAS-1)*n3d) * &
!!                            & TL_mass_in_air(abs(indAcidityAffectingSp(iAAS)), 1, iTmp, iTime)
!!              end do  ! iTmp 1:n3d
!            end do
!          else              
!            ! non-SO2, squared TL matrix
!            fTmp = fTmp + pAdj(iSp)%pp(iLev,iLev) * TL_mass_in_air(iSp,1,iLev,iTime)
!            call msg('All Triangle full')
!            do iTmp = 1, n3d
!              fTmp = fTmp + pTL(iSp)%pp(iLev,iTmp) * TL_mass_in_air(iSp,1,iTmp,iTime)
!            end do  ! iTmp 1:n3d
!          endif  ! iSp == indSO2
!          TL_mass_in_air(iSp, 1, iLev, min(nTimes,iTime+1)) = TL_mass_in_air(iSp, 1, iLev, iTime) + fTmp
!          TL_mass_in_water(iSp, 1, iLev, min(nTimes,iTime+1)) = TL_mass_in_water(iSp, 1, iLev, iTime) - fTmp
      end do  ! iLev

      if(.not. present(pTL_in))call free_work_array(pTL)
    ...
    contains
      ...
      subroutine fill_triangle(pTL, iShift, iSp, n3d, r1d, c1d)
        !
  ...
  subroutine material_properties(species, r, water_capacitance_factor, efficiency)
    ...
    if ( species%mode == in_gas_phase) then
        ...
        efficiency =  4/(r%drop_Re*fSc) * (2 + 0.6*sqrt(r%drop_Re)*(fSc**0.33333))*r%is_liquid
        ...
        efficiency = 4. / (r%drop_Re*fSc) * &
                                & (2 + 0.6*sqrt(r%drop_Re)*(fSc**0.33333+0.4*sqrt(fSc))*r%is_liquid)&
  ...
  subroutine make_rain_in_cell(iX, iY, iLev, nLevels, r, &
    ...
    real  :: fTmp, fTmp1, precip_rate, u, v
    ...
    if (r%fRainRateSfc < 0.1/3600.) then  ! <0.1 mm/h is not a rain
      r%ifRainSfc = .FALSE.
      r%ifRainyCell = .FALSE.
      return
    endif
    ...
    r%fTemperature  = fu_get_value(fldTempr, &   ! field_ptr_4d
                                 ...
                                 & weight_past, &
                                 & pHorizInterpStruct, pVertInterpStruct, ifHorizInterp, ifVertInterp)
    u = fu_get_value(fldUmet, &   ! field_ptr_4d
                   & nx_meteo, &     ! nxFrom
                   & ix, iy, iLev, & 
                   & weight_past, &
                   & pHorizInterpStruct, pVertInterpStruct, ifHorizInterp, ifVertInterp)
    v = fu_get_value(fldVmet, &   ! field_ptr_4d
                   & nx_meteo, &     ! nxFrom
                   & ix, iy, iLev, & 
                   & weight_past, &
                   & pHorizInterpStruct, pVertInterpStruct, ifHorizInterp, ifVertInterp)

    !! meteo_cell_size is wrong scaling.. Fixed scaling works better (port form v5_7)
    !r%max_scav_rate = 0.3 * sqrt(u*u+v*v+1.)/meteo_cell_size !!! 1m/s minimum cell turnover speed
    !To avoid max_scav_rate to depend on meteo cell-size use the following !This is from Andreas 
    r%max_scav_rate = 0.3 * sqrt(u*u+v*v+1.)/10000.0 !!! 1m/s minimum cell turnover speed

    ! Cloud water content
    !
    if (scavType == scav2018 .or. scavType == scav2011) then 
      ...
      if (scavType == scav2018) then 
          r%pwcColumn =  fu_get_value(fldPwcabove, nx_meteo, ix, iy, 1 , 0.0, &
                              & pHorizInterpStruct, pVertInterpStruct, ifHorizInterp, .false.)
      else
    ...
    if (r%pwcColumn > 1e-4) then ! 0.1 mm cloud water ...
      ...
      cwcAboveBottom =  fu_get_value(fldCwcabove, nx_meteo, ix, iy, iLev , 0.0, &
                                   & pHorizInterpStruct, pVertInterpStruct, ifHorizInterp, ifVertInterp)
      if (scavType == scav2018) then
        pwcAboveBottom =  fu_get_value(fldPwcabove, nx_meteo, ix, iy, iLev , 0.0, &
                                     & pHorizInterpStruct, pVertInterpStruct, ifHorizInterp, ifVertInterp)
      else
      ...
      if (iLev < nLevels) then
        cwcAboveTop   =  fu_get_value(fldCwcabove, nx_meteo, ix, iy, iLev + 1, 0.0, &
                                    & pHorizInterpStruct, pVertInterpStruct, ifHorizInterp, ifVertInterp)
        if (scavType == scav2018) then
          pwcAboveTop   =  fu_get_value(fldPwcabove, nx_meteo, ix, iy, iLev + 1, 0.0, &
                                      & pHorizInterpStruct, pVertInterpStruct, ifHorizInterp, ifVertInterp)
        else  ! trick for the upper layer
      ...
      if (pwcAboveBottom < 1e-5 * r%pwcColumn) then
        r%ifRainyCell = .FALSE.
        return
      endif 
      ...
      if (cwcAboveBottom < 1e-5 * r%cwcColumn) then
        r%ifRainyCell = .FALSE.
        return
      endif 
      if (iLev < nLevels) then 
    ...
    if (cwcAboveBottom < 0.05*r%pwcColumn) then ! could be done earlier...
    ...
    if(r%fTemperature > 273)then ! rain
      r%is_liquid = 1.
  ...
  subroutine scavenge_lagr(src_masses, dest_masses, &
    ...
    ! Local variables
    real :: scav_coef_std, scav_coef, scavAmount, fTempr_1
    integer :: iSpecies
...
!                                      & rulesDeposition%indGasesDepositing(iSpecies))%material))cycle
         ...
         ScavAmount = src_masses(rulesDeposition%indGasesDepositing(iSpecies),1) * scav_coef/(1. + scav_coef)
         src_masses(rulesDeposition%indGasesDepositing(iSpecies),1) = &
                                & src_masses(rulesDeposition%indGasesDepositing(iSpecies),1) - ScavAmount
         dest_masses(rulesDeposition%indGasesDepositing(iSpecies),1) = &
                                & dest_masses(rulesDeposition%indGasesDepositing(iSpecies),1) + ScavAmount
       end do  !  gases
       ...
       do iSpecies = 1, rulesDeposition%nAerosolsDepositing
         ...
         ScavAmount = src_masses(rulesDeposition%indAerosolDepositing(iSpecies),1) * scav_coef/(1. + scav_coef)
         src_masses(rulesDeposition%indAerosolDepositing(iSpecies),1) = &
                            & src_masses(rulesDeposition%indAerosolDepositing(iSpecies),1) - ScavAmount
         dest_masses(rulesDeposition%indAerosolDepositing(iSpecies),1) = &
                            & dest_masses(rulesDeposition%indAerosolDepositing(iSpecies),1) + ScavAmount
       end do  !  aerosols
...
!	(Dimensionless) deposition velocity 
  ...
  subroutine test_TL_ADJ(mass_in_air, &  ! sensit mass in precipitation (sSpecies, nSrc, nz)
    ...
    type(Tscav_coefs_1d) :: c1d  ! internal coefficients stored from forward model
    type(silja_rp_2d), dimension(:), pointer :: pTL, pAdj
    real :: fTmp
...
!    ! (re)store intial masses
    ...
    call get_work_arrays_set(nSpecies, pTL)
...
!            fTmp = fTmp + pTL(iSp)%pp(iLev,iLev) * TL_mass_in_air(iSp,1,iLev,iTime)
    ...
    do iSp = 1, nSpecies
      ...
      call msg('TL matrix:' + fu_str(species(iSp)))
      do iLev = 1, n3d
        if(iSp == indSO2)then
          call msg(fu_str(iLev) + ':', (/pTL(iSp)%pp(iLev,1:n3d*nAcidityAffectingSpecies)/))
        else
          call msg(fu_str(iLev) + ':', (/pTL(iSp)%pp(iLev,1:n3d)/))
        endif
      end do
