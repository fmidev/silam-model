...
!  public modify_lonlat_faster
  ...
  subroutine transform_latlon(lon_from, lat_from, arRotation, direction, lon_new, lat_new)
   ...
   ! ***************************************************************
    ...
    ! Local declarations:
    REAL (kind=8) :: lon_rad, lat_rad, alpha, beta, gamma, ca, cb, cg, sa, sb, sg, pole_lat, pole_lon
...
!    ELSE IF (pole == pole_geographical) THEN
      ...
      pole_lat = fu_southpole_lat(pole_new) * ddeg_to_rad
      pole_lon = fu_southpole_lon(pole_new) * ddeg_to_rad
