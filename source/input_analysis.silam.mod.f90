...
call report(IDlist(iID))
  ...
  subroutine get_content_from_netcdf_file(FName, fformat, q_list, InputContent)
    ...
    !
    call id_list_from_netcdf_file(iUnit, idList, nVars, .true., .false.)
    if (error) return
...
!    InputContent%NbrFields = 0
  ...
  subroutine put_test_field_to_content(chFieldDescr, q_list, InputContent)
    ...
    !
    ! If the quantity is strange or already in the InputContent, skip the rest
    !
    if(quantity == int_missing .or. fu_quantity_in_list(quantity,q_list)) return
