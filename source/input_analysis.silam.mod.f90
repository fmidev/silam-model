...
public fu_staggered_winds !INFO. True if U and V are on a staggered grid
...
type Tinput_content ! Full content of the GRIB file(s) plus results of analysis
  ...
  logical :: ifStaggeredWinds = .False.
...
type (Tinput_content), parameter, public :: input_content_missing = &
   & Tinput_content( 0,0,0,0,0,0,0,0, .FALSE., &
...
logical function fu_staggered_winds(GribContent)
!
! Information only. Tells the number of fields
!
  implicit none
  ...
  if(.not.(GribContent%defined == silja_true)) then
    call set_error('Sorry, empty content','fu_nbr_of_fields')
    fu_staggered_winds = .FALSE.
    return
  end if

  fu_staggered_winds = GribContent%ifStaggeredWinds

end function fu_staggered_winds
...
!  endif
  ...
  if(IC%NbrGrids > 1) then
    ...
    !!! Check if we have winds staggered
    hasStagU = .False.
    hasStagV = .False.
    do i=1,IC%NbrGrids
          ...
          do j=1,IC%grids(i)%NFlds_grd
            if(fu_quantity(IC%ids(IC%grids(i)%flds(j))) == u_flag)then
              if (fu_grids_correspond(gridTmp, IC%grids(i)%grid)) then
                hasStagU = .true.
                call msg("Staggered U wind")
                exit
              endif
            end if
          end do
          gridTmp = fu_staggered_grid("stagY", IC%grids(IC%SystemGridPtr)%grid, .False., .True.)
          do j=1,IC%grids(i)%NFlds_grd
            if(fu_quantity(IC%ids(IC%grids(i)%flds(j))) == v_flag)then
              if (fu_grids_correspond(gridTmp, IC%grids(i)%grid)) then
                hasStagV = .true.
                call msg("Staggered V wind")
                exit
              endif
            end if
          end do
    enddo
    IC%ifStaggeredWinds = (hasStagU .and. hasStagV)
...
!!!!      end do
    ...
    IC%ifStaggeredWinds = .False.
    call msg("No staggered winds..")
