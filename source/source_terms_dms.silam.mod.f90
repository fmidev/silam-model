MODULE source_terms_dms
  ...
  public add_source_species_dms_src
...
CONTAINS
  ...
  subroutine add_input_needs_dmssrc(dmssrc, q_met_dynamic, q_met_static, &
    ...
    !Use 10m windspeed since not all meteo provide the components:
    size_new = fu_merge_integer_to_array(windspeed_10m_flag, q_met_dynamic)
  ...
  subroutine add_source_species_dms_src(dmssrc, species_list, nSpecies)
    !
    ...
    character(len=*), parameter :: subname = 'add_source_species_dms_src'
  ...
  end subroutine add_source_species_dms_src
  ...
  subroutine compute_emission_for_dms(dmssrc, &
    ...
    integer :: ind_sst, ind_w10m !,ind_u10m, ind_v10m
    ...
    !if (fu_fails(ind_u10m > 0, 'Cannot find u10m', subname)) return
    !ind_v10m = fu_index(met_buf, v_10m_flag)
    !if (fu_fails(ind_w10m > 0, 'Cannot find v10m', subname)) return
    !Use 10m windspeed since not all meteo provide the components:
    ind_w10m = fu_index(met_buf, windspeed_10m_flag)
    if (fu_fails(ind_w10m > 0, 'Cannot find 10m windspeed', subname)) return
    ...
    do iy = 1, emisMap%ny
      do ix = 1, emisMap%nx
        ...
        !Use 10m windspeed since not all meteo provide the components:
        windspeed = fu_get_value(met_buf%p2d(ind_w10m), nx_meteo, ix, iy, weight_past, &
                          & pHorizInterpMet2DispStruct, ifHorizInterp, ifForceWeightPast_=.false.)
