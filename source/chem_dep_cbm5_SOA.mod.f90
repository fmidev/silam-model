...
CONTAINS
  ...
  !***********************************************************************
  subroutine transform_cbm5_SOA(vSp, tla_cell, photo, rules, metdat, timestep_sec, garbage, &
    ...
    if (associated(tla_cell)) then  !! Store TLA part
        tla_cell(1:num_species_cbm5_SOA)  = vSp(1:num_species_cbm5_SOA)
        tla_cell(num_species_cbm5_SOA + 1) = h_start
    endif
...
!!$ if (isnan(vsp(i))) call msg('nan in cocktail', omp_get_thread_num())
  ...
  subroutine transform_cbm5_SOA_adj(vSp, tla_point, photo, rules, metdat, timestep_sec, garbage, &
    ...
    ! Local variables
    real, dimension(num_species_cbm5_SOA) :: vSp_lin ! linearization point
    real :: tempr, sun, sunD, &
         & fPressure, &
         & fMixing_ratio_2_cnc, &
    ...
    vSp_lin(:) = tla_point(1:num_species_cbm5_SOA) * mol2molec
    kpp_rcntrl(3) = max(tla_point(num_species_cbm5_SOA + 1), 0.)
    print_it = .false.
    !----------------------------------	----------------------------------------------
    !
    ! Get temperature from the meteo buffer
  ...
  integer function fu_tla_size_cbm5_SOA(rules) result(n)
    implicit none
    type(Tchem_rules_cbm5_SOA), intent(in) :: rules
    
    n = num_species_cbm5_SOA + 1 !!! FIXME TODO
    
  end function fu_tla_size_cbm5_SOA
