...
!!#define FORCING_TOP
  ...
  public make_wet_deposition_map
...
!    !type(Tchem_rules_POP) :: rulesPOP
    ...
    logical :: ifLowMassThreshUseEmission  = .true.
...
!                                     & chemRules%rulesPassive)
  ...
  subroutine add_transformation_input_needs(chemRules, &
    ...
    nMeteoInputs = chemRules%nTransformations + chemRules%nAerosolDynamics
...
!!$                                      & q_dynamicTmp, q_staticTmp)
    ...
    do iType = 1, chemRules%nAerosolDynamics
      iMetInput = iType + chemRules%nTransformations
...
!          required = fu_if_tla_required(chemRules%rulesAerDynVBS)
  ...
  subroutine transform_maps(mapTransport, mapShortLived, mapAerosol, mapDryDep, mapWetDep, &
    ...
    if (ind_dz < 1) then
      call set_error('Failed to find the dz field', 'transform_maps')
      return
    end if
    
    ind_air_mass = fu_index(disp_buf, disp_cell_airmass_flag)
    if (ind_air_mass < 1) then
      call set_error('Failed to find the ind_air_mass field', 'transform_maps')
      return
    end if
    ...
    if (have_cb4 .and. .not. allocated(cb4_h_start_array)) then
      ...
      allocate(cb4_h_start_array(mapTransport%n3d, mapTransport%nx, mapTransport%ny), &
             & stat=status)
      if(fu_fails(status == 0, 'Allocate failed', 'transform_maps'))return
    ...
    if (seconds < 0 .and. have_cb4 .and. .not. have_tla) then
      call set_error('Have cb4, timestep < 0 but no linearization trajectory', 'transform_maps')
      return
    end if
    ...
    !$OMP & cell_volume, zenith_cos, lat, lon, dz_past, dz_future, dz, i1d, garb_array, cncTrn, &
    !$OMP & photorates, aodext, aodscat, cncAer, cncSL, ithread)
         ...
         & cncTrn(iSpecies,isrc))
            ...
            if (seconds < 0) then
              do iTransf = 1, chemRules%nAerosolDynamics
                ...
                case(aerosol_dynamics_basic)
                  call set_error('aerosol_dynamics_basic is not available for adjoint','transform_maps')
...
!
                case(aerosol_dynamics_simple)
                  call set_error('aerosol_dynamics_simple is not available for adjoint','transform_maps')
                ...
                case(aerosol_dynamics_Mid_Atmosph)
                  call set_error('aerosol_dynamics_Mid_Atmosph is not available for adjoint','transform_maps')
                ...
                case(aerosol_dynamics_VBS)
                  call set_error('aerosol_dynamics_VBS is not available for adjoint','transform_maps')
                ...
                case default
                  ...
                  call set_error('Unknown aerosol dynamics','transform_maps')
                  cycle
                end select
...
if(ix == 1 .and. iy == 1) call msg_warning('Forcing top O3 to ~0.1ppm','transform_maps')
...
!                                            & mapTransport%arM(:,isrc,i3d,ix,iy), &
                ...
                case default
                  ...
                  call set_error('Unknown transformation','transform_maps')
                  cycle
              end select
              if (error) cycle
            end do      ! iTransf
...
!$OMP END CRITICAL(aerdyn)
            ...
            if(error .or. print_it)then
              ...
              if(error)call set_error('Trouble with chemistry start report','transform_maps')
              ...
              if(error)call set_error('Trouble with chemistry end_report','transform_maps')
              call msg("")
              call msg("")
              call msg("")
              print_it = .false.
...
!return
  ...
  subroutine make_wet_deposition_map(mapTransport, mapWetDeposition, garbage, &
                                   & met_buf, disp_buf, tla_step, &
                                   & pHorizInterpStruct, pVertInterpStruct, &
                                   & ifHorizInterp, ifVertInterp, weight_past,&
                                   & chemRules, seconds, now)
    implicit none

    ! Imported parameters
    type(Tmass_map), intent(inout) :: mapTransport, mapWetDeposition
    real, dimension(:,:), intent(inout) :: garbage
    ...
    !
    ! In fact, just pass the request further disclosing only deposition rules and low mass threshold
    !
    call scavenge_map(mapTransport, mapWetDeposition, garbage, tla_step, &
                    & seconds, weight_past, &
                    & chemRules%rulesDeposition, chemRules%low_mass_trsh, &
                    & pHorizInterpStruct, pVertInterpStruct, &
                    & ifHorizInterp, ifVertInterp)

  end subroutine make_wet_deposition_map
...
!    call unset_error('set_chemistry_rules')
    ...
    ! 
    !
    select case(fu_str_u_case(fu_content(nlStdSetup,'reference_4_low_mass_threshold')))
      case('EMISSION')
        rulesChemistry%ifLowMassThreshUseEmission = .true.
        call msg("fLowMassThreshUseEmission = .true.")
      case('DEFAULT')
        rulesChemistry%ifLowMassThreshUseEmission = .false.
        call msg("fLowMassThreshUseEmission = .false.")
      case('')
        call msg_warning('empty reference_4_low_mass_threshold, must be EMISSION or DEFAULT, use EMISSION', &
                       & 'set_chemistry_rules')
        rulesChemistry%ifLowMassThreshUseEmission = .true.
      case default
        rulesChemistry%ifLowMassThreshUseEmission = .true.
        call msg_warning('reference_4_low_mass_threshold must be EMISSION or DEFAULT, not:>>' + &
                       & fu_content(nlStdSetup,'reference_4_low_mass_threshold') + '<<, use EMISSION', &
                       & 'set_chemistry_rules')
    end select
...
!!$      & .or. fu_index(transformation_cbm4, chemRules%iTransformTypes) > 0 ) then
    if (fu_index(transformation_cbm42_strato, rulesChemistry%iTransformTypes) > 0 &
      ...
      & .or. fu_index(transformation_cbm4_SOA, rulesChemistry%iTransformTypes) > 0 ) then
  ...
  subroutine set_low_mass_threshold(chemRules, &
      ...
      select case(chemrules%iTransformTypes(i))
        ...
        case(transformation_cbm42_strato_SOA)
          call check_low_mass_threshold_cbm42_strato_SOA(chemrules%rulesCBM42_strato_SOA, transport_species, &
                                             & nSpecies_transport, ifForwardRun, &
                                             & chemrules%low_cnc_trsh, chemrules%low_mass_trsh)
        case(transformation_passive, transformation_pollen, &
    ...
    subroutine calculate_thresholds(pThresholdsCnc, pThresholdsMass)
      ...
      if(ifFromEmission)then
        ...
        if(chemrules%ifLowMassThreshUseEmission)then
