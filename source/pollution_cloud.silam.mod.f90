...
! Modules used:
  ...
  public fu_pollution_cloud_quantity ! if this is a particle cloud-related or buffer-field quantity
...
!    call free_work_array(nModes)
  ...
  function fu_pollution_cloud_quantity(iQ) result(ifInCloud)
    !
    ! Returns true if the quantity is attributed to cloud of particles and 
    ! false if it is a field-related parameter. Since an error can cost a lot,
    ! a strict checking is used.
    ! OBS: so far, the dry/wet depositions are tight to cloud of particles, while
    ! they are essentially the field date. Later this will be changed.
    !
    implicit none
    ...
    select case(iQ)
      ...
      case default
        call set_error('Unknown quantity:' + fu_quantity_string(iQ), 'fu_pollution_cloud_quantity')
        ifInCloud = silja_undefined

    end select

  end function fu_pollution_cloud_quantity
  ...
  logical function fu_if_variable_available(Cloud, pDispersionMarket, quantity, species, nlStdSetup, &
    ...
    select case(quantity)
      case(emission_intensity_flag)                          ! Only if species is emitted
