...
!    call set_missing(rules_adj%iniBoundaryRules)
  ...
  subroutine forward_3d(model)
    ...
    call observeAll(model%obs_ptr, model%cloud,&
                  & model%meteo_ptr, model%rules%chemicalRules, &
...
!!$    end if
  ...
  subroutine adj_test_obs(model)
    ...
    ! include timing
    !call observeAll(model%obs_ptr, model%cloud, model%meteo_ptr, model%rules%chemicalRules, &
...
!!$    end do
  ...
  contains
    ...
    real function fu_inner(x, y) result(inner)
      implicit none
