module cbm4_SOA_interface
  ...
  character(len=substNmLen), dimension(num_species), parameter, private :: &
       & subst_names = (/ &
                      'AVB0  ','AVB1e0','AVB1e1','AVB1e2','AVB1e3','AVB1e4', &
& 'AVB1e5','AVB1e6','BVB0  ','BVB1e0','BVB1e1','BVB1e2', &
& 'BVB1e3','O1D   ','PAN   ','CRO   ','H2O2  ','TOL   ', &
& 'N2O5  ','XYL   ','HONO  ','PNA   ','TO2   ','XO2N  ', &
& 'HNO3  ','CRES  ','ROR   ','MGLY  ','CO    ','OPEN  ', &
& 'XO2   ','ETH   ','C5H8_2','PAR   ','HCHO  ','OLE   ', &
& 'C5H8  ','ALD2  ','O3    ','NO3   ','OH    ','NO2   ', &
& 'HO2   ','C2O3  ','NO    ','O     ' &
                      & /)
...
contains
  ...
  subroutine set_rates_cbm4_SOA(photoarr, temp, cAir, press, cH2O, &
    ...
    RCONST(83) = (3.60E-11*ifMTprods)
    RCONST(84) = (ARRZ(1.5d-11,-449.0d0)*ifMTprods)
    RCONST(85) = (ARRZ(1.2d-15,821.0d0)*ifMTprods)
    RCONST(86) = (ARRZ(3.7d-12,-175.0d0)*ifMTprods)
    RCONST(87) = (3.60E-11*(1.0-ifMTprods))
    RCONST(88) = (ARRZ(1.5d-11,-449.0d0)*(1.0-ifMTprods))
    RCONST(89) = (ARRZ(1.2d-15,821.0d0)*(1.0-ifMTprods))
    RCONST(90) = (ARRZ(3.7d-12,-175.0d0)*(1.0-ifMTprods))
    RCONST(91) = (ARRZ(1.8d-12,-340.0d0)*SOA_b)
    RCONST(92) = (ARRZ(1.7d-11,-116.0d0)*SOA_b)
    RCONST(93) = (3.60E-11*SOA_b)
    RCONST(94) = (ARRZ(2.7d-11,-390.0d0)*SOA_b)
    RCONST(95) = (ARRZ(1.03d-14,1995.0d0)*SOA_b)
    RCONST(96) = (ARRZ(3.15d-12,450.0d0)*SOA_b)
    RCONST(97) = (3.60E-11*SOA_b)
    RCONST(98) = (ARRZ(1.5d-11,-449.0d0)*SOA_b)
    RCONST(99) = (ARRZ(1.2d-15,821.0d0)*SOA_b)
    RCONST(100) = (ARRZ(3.7d-12,-175.0d0)*SOA_b)
    RCONST(101) = (ARRZ(1.8d-12,-340.0d0)*(1.0-SOA_b))
    RCONST(102) = (ARRZ(1.7d-11,-116.0d0)*(1.0-SOA_b))
    RCONST(103) = (3.60E-11*(1.0-SOA_b))
    RCONST(104) = (ARRZ(2.7d-11,-390.0d0)*(1.0-SOA_b))
    RCONST(105) = (ARRZ(1.03d-14,1995.0d0)*(1.0-SOA_b))
    RCONST(106) = (ARRZ(3.15d-12,450.0d0)*(1.0-SOA_b))
    RCONST(107) = (3.60E-11*(1.0-SOA_b))
    RCONST(108) = (ARRZ(1.5d-11,-449.0d0)*(1.0-SOA_b))
    RCONST(109) = (ARRZ(1.2d-15,821.0d0)*(1.0-SOA_b))
    RCONST(110) = (ARRZ(3.7d-12,-175.0d0)*(1.0-SOA_b))
    RCONST(111) = (ASOA_aging)
    RCONST(112) = (ASOA_aging)
    RCONST(113) = (ASOA_aging)
    RCONST(114) = (ASOA_aging)
    RCONST(115) = (IVOC_aging)
    RCONST(116) = (IVOC_aging)
    RCONST(117) = (IVOC_aging)
    RCONST(118) = (BSOA_aging)
    RCONST(119) = (BSOA_aging)
    RCONST(120) = (BSOA_aging)
    RCONST(121) = (BSOA_aging)
