module photolysis
  ...
  ! - each table includes its standard atmosphere (50 or 67 levels, height, pressure and ozone density)
  ! - the LUT dimensions are reaction, zenith angle, total O3 column relative to standard and albedo.
  ! - for albedo one may use either single albedo datafile (albedo = 0.3), or multialbedo file
  !   (currently up to 7 values)
...
!!!!#endif
  ...
  subroutine effective_albedo_v2(ssa, g, tau_above_bott, alb_sfc, cosza_in, num_levs, att_prof, alb_eff)
    ...
    real(8) , parameter :: maxtau = 300. ! maxtau = 1000. causes numerical errors
