MODULE diagnostic_variables
  ...
  private df_cumul_daily_tempr

  private df_update_realtime_met_fields
  private df_daily_mean_tempr
    ...
    select case (method)
      ...
      case ( 'test_wind')
        ...
        if (iTmp > 0) then ! Requires a type of wind, may be, more parameters
          ...
          call set_error('test_wind requires at least 1 parameter: wind type index','set_diagnostic_rules')
          return
        endif
      ...
      case default
        ...
        call msg_warning('Strange or no continuity_equation defined in standard setup,' + &
                       & 'will use differential form v2','set_diagnostic_rules')
        rules%continuity_equation = incompressible_v2
    end select
    ...
    select case (method)
      ...
      case default
        ...
        call msg_warning('Strange or no wind_diagnostics defined in standard setup,' + &
                       & 'will use winds opentop','set_diagnostic_rules')
        rules%wind_method = opentop
    end select
...
!    type(silja_field), pointer :: field
    ...
    !
    CALL make_derived_meteo_fields(meteoMarketPtr, meteo_dyn_shopping_list, wdr, ifNewMeteoData)
    if(error)return
    ...
    if(error)return
    weight_past = meteo_buf_ptr%weight_past
...
!        endif
      ...
      case(day_temperature_acc_flag, day_temperature_2m_acc_flag)
        !
        ! Make daily cumulative temperature, 3D and 2m
        !
        ifHorizInterp = .not. meteo_grid == dispersion_grid
        ifVertInterp = .not. fu_cmp_verts_eq(meteo_vertical, dispersion_vertical)
        if(ifHorizInterp) &
        ...
        call df_cumul_daily_tempr(desiredQ, met_src_missing, &
                                & met_buf, &
                                & obstimes, &
                                & pHorizInterpStruct, ifHorizInterp, &
                                & pVertInterpStruct, ifVertInterp, &
                                & dispMarketPtr, dispersion_grid, dispersion_vertical, &
                                & rules%ifAllowColdstartDayTemperature)
...
!         call msg("*************************CHECKUP OVER******************")
  ...
  subroutine df_cumul_daily_tempr(desiredQ, met_src, &
                                & met_buf, &
                                & obstimes, &
                                & pHorizInterpStruct, ifHorizInterp, &
                                & pVertInterpStruct, ifVertInterp_, &
                                & dispMarketPtr, gridTo, verticalTo, ifColdstartOK)
    !
    ! Computes the cumulative daily temperature from instant one
    ! At the beginning of each day it is supposed to be zero, at 24:00 it
    ! reaches its maximum value of daily temperature integral. This ambiguity is resolved
    ! by skipping the 00:00 value - it is zero all the time. The quantity therefore
    ! has a meaning of cumulative value by the moment it is made for
    !
    ! Careful with the (un)initialised fields: mean daily temperature cannot be zero even
    ! at the start of the run. If not initialised, a surrogate has to be invented.
    !
    implicit none

    ! Imported parameters
    integer, intent(in) :: desiredQ
    TYPE(Tfield_buffer), POINTER :: met_buf
    type(meteo_data_source), intent(in) :: met_src
    type(silja_time), dimension(:), intent(in) :: obstimes
    type(THorizInterpStruct), pointer :: pHorizInterpStruct
    type(TVertInterpStruct), pointer :: pVertInterpStruct
    logical, intent(in) :: ifHorizInterp, ifVertInterp_
    type(mini_market_of_stacks), intent(inout) :: dispMarketPtr
    type(silja_grid), intent(in) :: gridTo
    type(silam_vertical), intent(in) :: verticalTo
    logical, intent(in) :: ifColdstartOK

    ! Local variables
    INTEGER :: iQ, ix, iy, iTo, iMeteo, iTime, fsTo, iLev, nLevs, nxTo, nyTo, iCoef, iTmp, indMean
    real, DIMENSION(:), POINTER :: pTemprPast, pTemprFuture, pTCum, pTCum_past
    TYPE(Tfield_buffer), POINTER :: met_bufPtr
    type(silja_field_id) :: idTmp
    type(silja_field_id), pointer :: idTemprPast, idTemprFuture
    integer, dimension(2) :: quantities = (/int_missing, int_missing/), &
                           & q_mean = (/int_missing, int_missing/), &
                           & q_in =     (/temperature_flag,          temperature_2m_flag/), &
                           & q_acc_in = (/day_temperature_acc_flag,  day_temperature_2m_acc_flag/), &
                           & q_mean_in =(/day_mean_temperature_flag, day_mean_temperature_2m_flag/),&
                           & indInQ = (/int_missing, int_missing/)
    logical :: if3D, ifVertInterp, ifPastMidnight
    type(silja_level) :: levelTo, levelTmp
    type(silja_time) :: start_of_day
    real :: seconds
    real, dimension(:), pointer :: pData
    type(silja_field), pointer :: fldMeanT
    character(len=20), parameter :: chSubNm = 'df_cumul_daily_tempr'

    met_bufPtr => met_buf
    call grid_dimensions(gridTo, nxTo, nyTo)
    fsTo = nxTo * nyTo
    !
    ! Can make 2m and 3D cumulative temperature - or both
    !
    iQ = 1
    do iy = 1, 2
      if(q_acc_in(iy) == desiredQ)then
!      if(fu_quantity_in_quantities(q_acc_in(iy), qTmp))then
!      idTmp = fu_set_field_id_simple(met_src_missing, &
!                                   & q_acc_in(iy), obstimes(1), level_missing)
!      if(associated(fu_get_field_from_mm_general(dispMarketPtr, idTmp, .false.)))then
        quantities(iQ) = q_acc_in(iy)                                ! what to diagnose
        indInQ(iQ) = fu_index(met_buf%buffer_quantities, q_in(iy))   ! input for it
        q_mean(iQ) = q_mean_in(iy)                            ! corresponding daily mean temperature
        iQ = iQ + 1
      endif
    enddo
    if((quantities(1) == int_missing .neqv. indInQ(1) == int_missing) .or. &
     & (quantities(2) == int_missing .neqv. indInQ(2) == int_missing))then
      call msg('something is wrong with quantities. requested quantities:', quantities)
      call msg('something is wrong with quantities. Input quantities:', indInQ)
      call set_error('Inconsistent input and request',chSubNm)
      return
    endif
    !
    ! The grand cycle over the requested quantities
    !
    do iQ = 1, 2
      if(quantities(iQ) == int_missing)exit  ! all done

      if3D = fu_multi_level_quantity(quantities(iQ))
      if(if3D)then  ! want 3-d temperature
        nLevs = fu_NbrOfLevels(verticalTo)
        ifVertInterp = ifVertInterp_
      else          ! want 2m temperature
        nLevs = 1
        ifVertInterp = .false.
        pTemprPast => met_buf%p2d(indInQ(iQ))%past%ptr
        pTemprFuture => met_buf%p2d(indInQ(iQ))%future%ptr
        idTemprPast => met_buf%p2d(indInQ(iQ))%past%idPtr
        idTemprFuture => met_buf%p2d(indInQ(iQ))%future%idPtr
        levelTo = fu_level(idTemprPast)     ! near-surface level is taken from meteo
      endif
      !
      ! Cycle over the requested times. Note that this below work is to propagate the
      ! accumulation from time_past to time_future of the meteo buffer, no other options exist.
      ! If past value turned out to be not defined, we are at the start of the run and have to do
      ! something funny to get more-or-less inline with the case.
      !
      do iTime = 1, size(obstimes)
        if(.not.defined(obstimes(iTime)))exit
        start_of_day = fu_start_of_day_utc(obstimes(iTime))
        !
        ! Cycle over levels. 
        !
        do iLev = 1, nLevs
          if(if3D)then
            pTemprPast => met_buf%p4d(indInQ(iQ))%past%p2d(iLev)%ptr
            pTemprFuture => met_buf%p4d(indInQ(iQ))%future%p2d(iLev)%ptr
            idTemprPast => met_buf%p4d(indInQ(iQ))%past%p2d(iLev)%idPtr
            idTemprFuture => met_buf%p4d(indInQ(iQ))%future%p2d(iLev)%idPtr
            levelTo = fu_level(verticalTo, iLev)                           ! in verticalTo
          endif
          !
          ! Accumulation length at the end of the day is always one day. Just make sure...
          !
          if(obstimes(iTime) == start_of_day)then
            seconds = 24. * 3600.  ! the whole previous day!
          else
            seconds = fu_sec(obstimes(iTime) - start_of_day)
          endif
          !
          ! Get the placeholder for the data
          !
          idTmp = fu_set_field_id(fu_met_src(idTemprPast),&
                                & quantities(iQ), &
                                & start_of_day, &  !analysis_time,&
                                & obstimes(iTime) - start_of_day, & !forecast_length, &
                                & gridTo, &
                                & levelTo, &
                                & fu_set_interval_sec(seconds), & !obstimes(iTime) - start_of_day, & !length_of_accumulation, optional
          ...
          if(fu_fails(.not.error,'Failed field data pointer',chSubNm))return
          !
          ! What time are we updating?
          !
          if(obstimes(iTime) == fu_valid_time(idTemprPast))then
            !
            ! Need to diagnose past at the start of the run without temperature initialization. 
            ! Depending on what is available from mean temperature field, have to make a surrogate: 
            ! either from the mean temperature or extrapolate the past temperature till the start 
            ! of the day, start of the previous day if necessary.
            !
            ! Do we have mean temperature and is it initialised? If not, fldMeanT is null
            !
            
            if (if3D) then
              levelTmp = levelTo
            else
              levelTmp = level_missing ! Dirty hack: 2m field can come as 2m or as sfc field
            endif                                      ! until NetCDF_io is capable of handling different verticals
            idTmp = fu_set_field_id_simple(fu_met_src(idTemprPast), &
                                         & q_mean(iQ), time_missing, levelTmp)  !obstimes(iTime), levelTo)
            fldMeanT => fu_get_field_from_mm_general(dispMarketPtr, idTmp, .false.)
            if(error)return
            
            !
            ! Finally, kake the surrogate field from the mean field or from available temperatures
            !
            if(associated(fldMeanT))then
              !
              ! Multiply mean T with day length - that will be the cumulative one...
              !
              pData => fu_grid_data(fldMeanT)
              pTCum(1:fsTo) = pData(1:fsTo) * seconds
            else
              ! No valid mean T, have to use available temperatures
              !
              call msg_warning("Failed to find Daymean temperature! Tried ID:" )
              call report(idTmp)
              if (.not. ifColdstartOK) then
                call msg("Coldstart is not allowed!")
                call msg("To enable coldstart  put  'allow_coldstart_day_temperature = yes' to standard_setup!")
                call set_error("Coldstart is not allowed!" ,chSubNm)
                return
              endif
                ...
                do iy = 1, nyTo
                  do ix = 1, nxTo
                    ...
                    if(ifVertInterp)then
                      pTCum(iTo) = fu_get_value(met_buf%p4d(indInQ(iQ)), nx_meteo, ix, iy, iLev, &
                                              & 0.5, &                                ! weight_past, &
                                              & pHorizInterpStruct, pVertInterpStruct, &
                                              & ifHorizInterp, ifVertInterp) * seconds
                    else
                      pTCum(iTo) = 0.
                      ...
                      enddo
                      pTCum(iTo) = pTCum(iTo) * seconds * 0.5
                    endif  ! if vertical interpolation
                  end do  ! ix
                end do  ! iy
              else
                pTCum(1:fsTo) = (pTemprPast(1:fsTo) + pTemprFuture(1:fsTo)) * 0.5 * seconds
              endif  ! if horizontal interpolation
            endif  ! valid mean tempearture
            
          elseif(obstimes(iTime) == fu_valid_time(idTemprFuture))then
            !
            ! Normal case: past is available, make the future.
            ! Integrating from past to future is a half-sum of the temperatures times seconds
            ! But do not forget to ignore previous-day sum if starting new day
            !
            ifPastMidnight = fu_valid_time(idTemprPast) == start_of_day
            if(.not. ifPastMidnight)then
              call set_valid_time(idTmp, fu_valid_time(idTemprPast))
              call set_accumulation_length(idTmp, fu_valid_time(idTemprPast) - &
                                                & fu_start_of_day_utc(fu_valid_time(idTemprPast))) !fu_set_interval_sec(seconds))
              call find_field_data_storage_2d(dispMarketPtr, idTmp, multi_time_stack_flag, pTCum_past)
              if(fu_fails(.not.error,'Failed past-time field data pointer',chSubNm))return
            endif
            if (ifHorizInterp) then
              !
              ! Get the increment and add to the past-time field
              !
                  ...
                  if(ifVertInterp)then
                    ...
                    do iCoef = 1, pHorizInterpStruct%nCoefs
                      ...
                      pTCum(iTo) = pTCum(iTo) + pHorizInterpStruct%weight(iCoef,ix,iy) * &
                                              & (pTemprPast(iTmp) + pTemprFuture(iTmp))
                    enddo
                    if(ifPastMidnight)then
                      pTCum(iTo) = pTCum(iTo) * seconds * 0.5
                    else
                      pTCum(iTo) = pTCum_past(iTo) + pTCum(iTo) * seconds * 0.5
                    endif
                  endif
                end do
              end do
              ...
              if(ifPastMidnight)then
                pTCum(1:fsTo) = (pTemprPast(1:fsTo) + pTemprFuture(1:fsTo)) * seconds * 0.5
              else
                pTCum(1:fsTo) = pTCum_past(1:fsTo) + (pTemprPast(1:fsTo) + pTemprFuture(1:fsTo)) * seconds * 0.5
              endif
            endif
            
          else
            !
            ! Cannot be the case. Something went wrong
            !
            call set_error('Requested time=' + fu_str(obstimes(iTime)) + &
                                         & ', is in the past from the past meteo time=' + &
                                         & fu_str(fu_valid_time(idTemprPast)), chSubNm)
            return
          endif  ! options for the obstime vs meteo times
        end do  ! levels 
      end do  ! requested times
    end do  ! requested quantities

  end subroutine df_cumul_daily_tempr
...
! call report(fu_id(field))
        ...
        case(day_mean_temperature_flag, day_mean_temperature_2m_flag)
            ...
            call df_daily_mean_tempr(buf_ptr, valid_time_border_1, valid_time_border_2, &
                                   & shopQ == day_mean_temperature_flag)
...
!  call msg("")
 ...
 subroutine df_update_cellfluxcorr_rt(disp_buf, valid_time_border_1, valid_time_border_2, diag_rules, now) 
  ...
  subroutine df_daily_mean_tempr(buf, valid_time_border_1, valid_time_border_2, if3D)
    !
    ! Creates the daily-cumulative temperature, [K sec] and mean [K] out of instant one.
    ! Note that daily cumulative temperature at 00:00 has the zero value if "now" is after it
    ! (start of the next day) or it is mean_temperature * 86400 if "now" is before it (end of 
    ! the present day). Therefore, the daily cumulative must be used with extreme care with 
    ! clear and unequivocal understanding what the 00-value is at the specific moment. 
    ! Note that this duality is the only way to ensure the reasonable interpoaltion in time for
    ! the output, which is a screen-shot of "now".
    ! At "now"==00:00 cumulative temperature is zeroed - and the mean daily temperature is 
    ! created (until that mean-daily temperature is equal to the value of the previous day).
    ! Therefore, the mean and cumulative temperatures together always provide full information.
    !
    implicit none
    ...
    integer :: iT_mean, iT_cum, iLev, nLevs, nx, ny, fs, q_mean, n_out_of_range, npatched, nFailed
    real :: seconds
    logical :: ifInitialised
    character(len=19), parameter :: chSubNm = 'df_daily_mean_tempr'
    !
    ...
    ! Preparatory steps
    !
    if(if3D)then  ! want 3-d temperature but how many levels ??
      q_mean = day_mean_temperature_flag
      iT_cum  = fu_index(buf%buffer_quantities, day_temperature_acc_flag)
      iT_mean = fu_index(buf%buffer_quantities, day_mean_temperature_flag)
      pIdMean => buf%p4d(iT_mean)%present%p2d(1)%IDptr
      do iLev = 1, size(buf%p4d(iT_mean)%present%p2d)
        if(.not. defined(buf%p4d(iT_mean)%present%p2d(iLev)%idPtr))then
          nLevs = iLev - 1
          exit
        endif
      end do
    else          ! want 2m temperature
      q_mean = day_mean_temperature_2m_flag
    ...
    time_past = fu_valid_time(buf%p2d(iT_cum)%past%IDptr)
    time_future = fu_valid_time(buf%p2d(iT_cum)%future%IDptr)

    if((fu_valid_time(pIdMean) <= time_past) .and. &
     & (fu_valid_time(pIdMean) + fu_validity_length(pIdMean) >= time_future))then
      !
      ! All seems fine: the mean temperature validity interval covers the neeed range
      ! Do not really understand why to call the sub.
      !
      call msg_warning('The sub is called but nothing to do', chSubNm)
    else
      !
      ! Work needed: the mean field does not cover the period between meteo_past and meteo_future
      !
      ...
      do iLev = 1, nLevs
        ...
        !
        if(if3D)then  ! want 3-d temperature
          pIdCum_past => buf%p4d(iT_cum)%past%p2d(iLev)%IDptr
          pIdCum_future => buf%p4d(iT_cum)%future%p2d(iLev)%IDptr
          pIdMean => buf%p4d(iT_mean)%present%p2d(iLev)%IDptr   ! single-time field, only present
          cum_T_past => buf%p4d(iT_cum)%past%p2d(iLev)%ptr
          cum_T_future => buf%p4d(iT_cum)%future%p2d(iLev)%ptr
          mean_T => buf%p4d(iT_mean)%present%p2d(iLev)%ptr   ! single-time field, only present
        else          ! want 2m temperature
          pIdCum_past => buf%p2d(iT_cum)%past%IDptr
          pIdCum_future => buf%p2d(iT_cum)%future%IDptr
          pIdMean => buf%p2d(iT_mean)%present%IDptr   ! single-time field, only present
        ...
        call grid_dimensions(fu_grid(pIdMean), nx, ny)
        fs = nx * ny
        if(error)return
        !
        ! - normal case: the past-meteo-time is midnight, divide cumulative temperature with its
        !     accumulation length
        ! - at the run start, anything is possible but cumulatives should be now set into
        !     something useful, just check whether the past is indeed at midnight
        !
        if(time_past == start_of_day)then
          !
          ! Past is at the midnight, must have accumulated temperature for the whole previous day
          !
          seconds = fu_sec(fu_accumulation_length(pIdCum_past))
          if(seconds < 1.0)then
            call report(pIdCum_past)
            call set_error('Strange accumulation length of this past cumulative temperature',chSubNm)
            return
          endif
          mean_T(1:fs) = cum_T_past(1:fs) / seconds
        else
          !
          ! Past is not at midnight. Must be a start of the run, with or without initialization. 
          !
          ifInitialised = .false.
          if(defined(pIdMean))then
            !
            ! A chance for the mean temperature to be initialized. Then the ID is funny but defined
            !
            if(fu_valid_time(pIdMean) >= start_of_day .and. &
             & fu_valid_time(pIdMean) + fu_validity_length(pIdMean) <= start_of_day + one_day)then
              call check_quantity_range(q_mean, mean_T, null(), fs, nx, .true., .true., & !ifRequireValidity, ifSilent
                                      & n_out_of_range, npatched, nFailed)
              ifInitialised = (nFailed + n_out_of_range == 0.)
            endif  ! meanT ID valid
          endif  ! defioned meanT ID
          !
          ! Then the best 
          ! approximation of daily mean is future-time cumulative divided with its accumulation time.
          !
          if(.not. ifInitialised)then
!call msg('')
!call msg('Day start, range_check outcome n_out_of_range, npatched, nFailed:' + fu_str(start_of_day)) !, &
                                                                  !& (/n_out_of_range, npatched, nFailed/))
!call msg('Logicals:', (/fu_valid_time(pIdMean) >= start_of_day, &
!                      & fu_valid_time(pIdMean) + fu_validity_length(pIdMean) <= start_of_day + one_day, &
!                      & ifInitialised/))
!call report(pIdMean)
!call msg('NOT INITIALISED MEAN TEMPERATURE, T(1,1), T(2,1)=',mean_T(1), mean_T(2))
            seconds = fu_sec(fu_accumulation_length(pIdCum_future))
            if(seconds < 1.0)then
              call report(pIdCum_future)
              call set_error('Strange accumulation length of this future cumulative temperature',chSubNm)
              return
            endif
            mean_T(1:fs) = cum_T_future(1:fs) / seconds
!call msg('New  T(1,1), T(2,1)=',mean_T(1), mean_T(2))
          endif
        endif   ! time_past == start_of_day
        !
        ! No matter what, the mean field must be valid the whole current day
        !
        call set_field_kind(pIdMean, averaged_flag)
        call set_analysis_time(pIdMean, start_of_day - one_day)
        call set_valid_time(pIdMean, start_of_day)
        call set_validity_length(pIdMean, one_day)
        call set_accumulation_length(pIdMean, one_day)
        call set_level(pIdMean, fu_level(pIdCum_past))  ! after init_singletime_fields, surface_level is here
  
!call check_quantity_range(day_mean_temperature_2m_flag, mean_T, mean_T, fs_meteo, nx_meteo, n_out_of_range, npatched)
!
!call msg('grid-averaged cumulative and mean-daily temperature for:' + fu_str(midpoint), &
!       & sum(cum_T(1:fs_meteo))/fs_meteo, sum(mean_T(1:fs_meteo))/fs_meteo)

      end do  ! iLev
    endif  ! if anything should be done

    if(valid_time_border_1 < fu_valid_time(pIdMean)) valid_time_border_1 = fu_valid_time(pIdMean)
    if(valid_time_border_2 > fu_valid_time(pIdMean) + fu_validity_length(pIdMean)) &
                  & valid_time_border_2 = fu_valid_time(pIdMean) + fu_validity_length(pIdMean)

  end subroutine df_daily_mean_tempr
