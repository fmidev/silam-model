...
!                              & perturb_emis = 441701, &
                              ...
                              & perturb_meteo_time = 441704, &
                              & perturb_emis_xy = 441705
...
CONTAINS
  ...
  subroutine set_perturbations(perturbations, species_emission, species_transport, &
    ...
    integer :: ind_pert, num_contr_species, pert_size, stat, num_pert
    logical :: need_cov
    ...
    if (fu_have_meteo_time(rules%perturbVariable)) then
      num_pert = num_pert + 1
      perturbations(num_pert)%perturb_type = perturb_meteo_time
    end if
    ...
    do ind_pert = 1, size(perturbations)
      ...
      case (perturb_emis_xy)
        ...
        pert_size = fu_number_of_gridpoints(analysis_grid)*num_contr_species
        perturbations(ind_pert)%pert_size = pert_size
        ...
        perturbations(ind_pert)%correlation_time = rules%volc_volflux_corr_time
...
!!$        perturbations(ind_pert)%defined = .true.        
      ...
      case (perturb_emis_volc)
        ...
        case (perturb_meteo_time)
          if(fu_true(rules%perturb_meteo%defined))then
            call msg('Setting perturbation: meteo time shift')
          else
            call set_error('Undefined meteo perturbation rules',sub_name)
          endif
          allocate(perturbations(ind_pert)%scale(2), perturbations(ind_pert)%offset(2), stat=stat)
          if (fu_fails(stat == 0, 'Allocate meteo perturbation failed', sub_name)) return
          perturbations(ind_pert)%pert_size = 1  ! we need one shift, the rest is just abuse of space
          perturbations(ind_pert)%scale(1) = fu_sec(rules%perturb_meteo%time_shift_stdev)
          perturbations(ind_pert)%scale(2) = fu_sec(rules%perturb_meteo%max_time_shift)
          perturbations(ind_pert)%offset(1:2) = 0.0
          perturbations(ind_pert)%distribution = rules%perturb_meteo%time_shift_distribution
          perturbations(ind_pert)%defined = .true.
  ...
  subroutine apply_perturbations(cloud, disp_buffer, perturbations, timestep, wdr, OutDef)
    ...
    do ind_pert = 1, size(perturbations)
      select case(perturbations(ind_pert)%perturb_type)
        ...
        case(perturb_meteo_time)
          call msg('... meteo-time shift')
          call perturb_meteo_time_shift(wdr, OutDef, timestep, perturbations(ind_pert))
...
!!$    call msg('Mean:', sum(sample, 2) / sample_size)
  ...
  subroutine perturb_emission_xy(proc, pert, timestep)
    ...
    real, dimension(:), pointer :: values_pert, proc_data

    character(len=*), parameter :: sub_name = 'perturb_emis_xy'

    real :: alpha, corr_time, aveval
    integer :: nn, stat
    ...
    if (pert%first_time) then
      ...
      call msg('timestep, correlation time', fu_sec(timestep), pert%correlation_time)
    ...
    call msg('average storage', sum(pert%storage(1:nn))/nn)
  ...
  subroutine perturb_meteo_time_shift(wdr, OutDef, timestep, perturbation)
    !
    ! Actually, just decides on the shift of the meteo time. A non-trivial problem however
    ! is that Output definition and weather data rules have already been set. Need to change.
    !
    implicit none
    ...
    ! Get the time shift following the definitions.
    ! Note that we might require the shift to be constrained
    !
    time_shift(1) = real_missing
    if(perturbation%scale(2) .eps. real_missing)then
      call get_perturbation(perturbation, time_shift)
    else
      do while (time_shift(1) > perturbation%scale(2))
        call get_perturbation(perturbation, time_shift)
      end do  
    endif ! if shift constrained
    !
    ! Note that the shift cannot be non-integer number of model time steps
    !
    time_delta = fu_set_interval_sec(time_shift(1))
    time_delta = timestep * nint(time_delta / timestep)
    !
    ! Now, propagate this shift to the main model: meteo data rules
    !
    call set_meteo_time_shift(wdr, time_delta)
    if(error)return
    !
    ! ... and tell the same to io_server to reset output stacks
    !
    call align_OutDef_initial_time_with_shift(OutDef, wdr)
    
  end subroutine perturb_meteo_time_shift
