MODULE source_terms_area
  ...
  interface add_source_species
    module procedure add_source_species_a_src
  end interface
...
!    type(silja_shopping_list), pointer :: pShopLst
    ...
    real,  dimension(:), allocatable :: cell_area !! array to multiply fluxes with to get rates. Only for timeVertResolved sources
  ...
  type(field_4d_data_ptr), private, pointer, save :: fldKz
...
!                                                         & sub_name)
 ...
 !         if(error)return
          ...
          call id_list_from_netcdf_file(a_src%uField(iItem), idList, nIDs, .true., .false.)
        case default
...
!      endif
    ...
    if(ifTimeVertResolved)then
      ...
      !
      allocate(a_src%pFldEmis(a_src%nDescriptors), stat=iTmp)
      if(fu_fails(iTmp==0,'Failed to allocate emission field_3d',sub_name))return
      pData => fu_work_array(fu_number_of_gridpoints(a_src%grid))
      if(error)return

!      call set_missing(a_src%pShopLst) 
      do iDescr = 1, a_src%nDescriptors
        !
        ! Allocate the 3d field pointers
        allocate(a_src%pFldEmis(iDescr)%fp, stat=iTmp)
        if(fu_fails(iTmp==0,'Failed emission field_3d%fp allocation',sub_name))return
        call set_3d_field_empty(a_src%pFldEmis(iDescr)%fp)
        !
        ! Species or cocktail?
        !
        if(a_src%ifSpecies(iDescr))then
          speciesArPtr => fu_species(a_src%cocktail_descr_lst(iDescr))
          speciesTmp = speciesArPtr(1)
          chTmp = ''
        else
          speciesTmp = species_missing
          chTmp = fu_name(a_src%cocktail_descr_lst(iDescr))
        endif
        !
        ! And then add levels
        !
        do iLev = 1, fu_NbrOfLevels(a_src%vertLevs)
          idTmp = fu_set_field_id(met_src_missing,&
                                ...
                                & a_src%times(1), &          !analysis_time,&
                                & zero_interval, &            !forecast_length, &
                                & a_src%grid, &
                                & fu_level(a_src%vertLevs,iLev), &
          ...
          call create_field_in_field_3d(a_src%pFldEmis(iDescr)%fp, iLev, idTmp, pData)
          if(error)return

        enddo ! iLev

        call set_3dField_params_from_fieldId(a_src%pFldEmis(iDescr)%fp, idTmp)
        if(error)return
        call organize_fields_vertically(a_src%pFldEmis(iDescr)%fp)
        if(error)return

      enddo  ! Descriptors

      a_src%nCells = fu_number_of_gridpoints(a_src%grid)

      if (a_src%ifFluxes) then
        allocate(a_src%cell_area(a_src%nCells))
        do iTmp = 1,a_src%nCells
          a_src%cell_area(iTmp) = fu_cell_size(a_src%grid, iTmp)
        enddo
      endif
...
!        call msg('Closing the binary file:' + a_src%chFIeldFNms(iItem))
  ...
  subroutine add_inventory_a_src(a_src, species_list, nSpecies)
    !
    ...
    !
    ! The source may not yet have the complete list of own species. Then it has to be created
    ! from descriptors, otherwise just picked from the source itself
    !
    itmp=nSpecies
    do iDesc = 1, a_src%nDescriptors
      call get_inventory(a_src%cocktail_descr_lst(iDesc), speciesPtr, n)
      if(error)return
      call addSpecies(species_list, nSpecies, speciesPtr, n)
      if(error)return
    end do

  end subroutine add_inventory_a_src
  ...
  subroutine getTimeSlots_of_area_source(a_src, time, iSlot1, iSlot2)
    ...
    iSlot2=-1
    !
    ! Use times array: params array is copied here anyway
    !
    if(time < a_src%times(1) .or. time > a_src%times(size(a_src%times)))then
      call set_error('Given time is outside the source activity period',&
                   & 'getTimeSlots_of_point_source')
      return
    endif
    !
    ! Search
    !
    do iSlot2 = 1, size(a_src%times)
      if(time < a_src%times(iSlot2)) then
        iSlot1 = iSlot2 -1
        return
      endif
    end do
    ...
    ! Last time moment ?
    !
    if (time == a_src%times(size(a_src%times))) then
      iSlot1 = size(a_src%times)
      iSlot2 = iSlot1
      return 
    end if
    !
    ...
    !
    call set_error('Failed to find the proper time slots','getTimeSlots_of_area_source')
  ...
  subroutine add_source_species_a_src(a_src, speciesLst, nSpecies)
    ...
    ! Local variables
    integer :: iParam, iDescr, nSpeciesDescr
...
!call msg('3')
    ...
    nullify(species)
    nSpecies = 0
    call add_inventory_a_src(a_src, species, nSpecies)
...
if (ifRandomise)then
  ...
  subroutine project_a_src_second_grd(as, grid, vert_disp, vert_proj, iAccuracy, tmpCellDispGrd_val, &
    ...
    real :: fxTmp, fyTmp
    integer :: iNewCell, nNewCells, nxOut, nyOut, nSmall, iCell, i,j,k, nSmallSkipped, iDescr, iThread, nSmallTmp, iTry
    ...
    if(as%grid == grid)then
      ...
      if(as%ifFieldGiven) then 
          !$OMP CRITICAL(mk_h_interp_struct)
          as%pHorizIS => fu_horiz_interp_struct(as%grid, grid, summation, ifRandomise, iOutside=setZero)
          !$OMP END CRITICAL(mk_h_interp_struct)
      end if
    endif
    ...
    if(fu_cmp_verts_eq(as%vertLevs, vert_proj))then
      ...
      if(as%ifFieldGiven)then
          ...
          as%pVertIS => fu_vertical_interp_struct(&
                                                ...
                                                & grid, summation, & !!Dispersion grid!!!
                                                & one_hour*3., &
    ...
    if(iAccuracy < 1 .or. iAccuracy > 10)then
      call msg('Accuracy switch must be from 1 to 10, not:',iAccuracy)
      call set_error('Accuracy switch must be from 1 to 10','project_a_src_second_grd')
      return
    endif
    ...
    if(as%grid == grid)then
      ...
      if(fu_fails(i == 0, 'Failed dispersion-vertical level fractions allocation','project_a_src_second_grd'))return
...
!!      nSmall = max(11, nSmall) ! Still not too crude interpolation...
    ...
    !            call msg_warning('Point is outside the x-limits.Skipping','project_a_src_second_grd')
                nSmallSkipped = nSmallSkipped + 1
              elseif(yNew <= 0.501 .or. yNew >= nyOut+0.499)then
    !            call msg('Point is outside the y-limits.iCell;y=',iCell,yNew)
    !            call msg_warning('Point is outside the y-limits.Skipping','project_a_src_second_grd')
                nSmallSkipped = nSmallSkipped + 1
    ...
    if(fu_fails(iCell==0,'Failed allocation of dispersion cells','project_a_src_second_grd'))return
    ...
    if(fu_fails(i==0,'Failed to allocate dispersion-vertical level fractions', &
                                                                    & 'project_a_src_second_grd'))return
    as%levFractDispVert(:) = 0.0
    as%fzDisp(:) = 0.0
...
!    nullify(as%cell_fY)
  ...
  subroutine prepare_src_vert_params_a_src(a_src, vert_disp, vert_proj)
    ...
    if(a_src%ifFieldGiven)return ! vertical structure is enough in case of field
    ...
    do indTime = 1, size(a_src%params)
      ...
      if(a_src%ifMultiLevelFixed)then
        ...
        !
        nullify(a_src%params(indTime)%levFractDispVert, a_src%params(indTime)%fzDisp)
      else
        !
...
!!        end do   ! nz
  ...
  subroutine prepare_inject_a_src(met_buf)
    !
    ! The subroutine prepares the private module pointers to the fields possibly requested 
    ! for injecting the area sources. As this is called only once before all area sources
  ...
  subroutine inject_emission_area_source_field(a_src, &
    ...
    ! Local variables
    integer :: iSlot, iLev, nLevs, iDescr, iFile, fs, nSlots, uTmp, &
             & iSpecies, nSpecies, iSlotStart, iSlotEnd, iSpeciesEmis
    real :: fMassTmp, fCellTotal, fWeightPast, fLevFraction, duration_sec
    real, dimension(:), pointer :: fMassTimeCommon, pData
    ...
    type(silja_field_id) :: idFromFile
    ...
    if(.not. (iSlotStart == iSlotEnd-1 .and. &
     & fu_valid_time(idRequest) == a_src%times(iSlotStart+1)))then ! get field from file   
        a_src%fDescrTotals(1:a_src%nDescriptors) = 0.0
        nLevs = fu_NbrOfLevels(a_src%vertLevs)
        fs = fu_number_of_gridpoints(a_src%grid)
...
!              fMassTimeCommon(1:fs) = 0.0   not needed, overwritten anyway
              ...
              call get_input_field(chTmp, &
                                 ...
                                 & grid_missing, &      ! storage grid - we know that it is as%grid /= dispersion_grid
                                 & .false., setZero, &  ! ifAcceptSameMonth, iOutside, &
                                 & idFromFile, &        ! what actually has been read
                                 & 5, &                 ! iAccuracy, &
                                 & wdr_missing, &
                                 & real_missing, &        ! fFillValue_, &
...
!                close(uTmp)
        ...
        if (a_src%ifFluxes) then !!Convert fluxes to rates
          do iLev = 1, nLevs
            do iDescr = 1, a_src%nDescriptors
              ...
              idRequest => fu_id(pField)
              call set_quantity(idRequest, emission_flux_flag)
              pData(1:fs) = pData(1:fs)*a_src%cell_area(1:fs)
            enddo
          enddo
        endif
      ...
      do iDescr = 1, a_src%nDescriptors
        ...
        !
        call add_field_to_mass_map_interp(a_src%pFldEmis(iDescr)%fp, &
                                        ...
                                        & fu_true(a_src%ifHorizInterp), fu_true(a_src%ifVertInterp), &
                                        & a_src%pHorizIS, a_src%pVertIS, &
                                        & a_src%id_nbr, &
                                        & mapEmis, mapCoordX, mapCoordY, mapCoordZ)
