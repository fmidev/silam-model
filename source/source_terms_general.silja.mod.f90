...
!  public create_src_containing_vert
  ...
  TYPE silam_source
    INTEGER :: n_area=0, n_bio_voc=0, n_bomb=0, n_fire=0, n_point=0, &
             & n_pollen=0, n_sea_salt=0, n_wb_dust=0, n_dms=0
  ...
  subroutine count_srcs_in_all_data_files(em_source, &                  ! the main source
    ...
    do iSrc = 1, nSourceTerms
      ...
      if(index(linetmp,' INVENTORY ') + index(linetmp,' POLLEN ') + index(linetmp,' BIOGENIC_VOC ') +&
       ...
       & index(linetmp,' WILD_LAND_FIRES ') + index(linetmp,' DMS ') > 0)then
...
!call msg(fnames(iFileGlobal))
  ...
  subroutine count_srcs_one_data_file(file_unit, &
    ...
    ! Local variables
    integer :: iTmp, nValLinesTmp, stat, size_from_nc, size_from_nl, nctag
    integer, pointer :: iSrcCounter
    logical :: eof, ifArea, ifFound, ifStart
    ...
    do while(.not.eof)
      ...
      if (index(line, 'AREA_SOURCE_') == 1)then
        iSrcCounter => em_source%n_area != nArea + 1
        ifArea = .true.
        if(ifStart)then
        ...
        ifStart = .true.
        srcInfoTmp(nSrcRead+1)%iSrcType = area_source
        nValLinesTmp = 0

      elseif (index(line,'BOMB_SOURCE_') == 1)then
        iSrcCounter => em_source%n_bomb
        ifArea = .false.
        if(ifStart)then
          call set_error('Start new source without ending previous:'+line, &
                       & 'count_srcs_one_data_file')
        ...
        ifStart = .true.
        srcInfoTmp(nSrcRead+1)%iSrcType = bomb_source

      elseif (index(line,'FIRE_SOURCE_') == 1)then
        iSrcCounter => em_source%n_fire
        ifArea = .false.
        if(ifStart)then
          call set_error('Start new source without ending previous:'+line, &
                       & 'count_srcs_one_data_file')
        ...
        ifStart = .true.
        srcInfoTmp(nSrcRead+1)%iSrcType = fire_source

      elseif (index(line,'POINT_SOURCE_') == 1) THEN
        iSrcCounter => em_source%n_point
        ifArea = .false.
        if(ifStart)then
          call set_error('Start new source without ending previous:'+line, &
                       & 'count_srcs_one_data_file')
        ...
        ifStart = .true.
        srcInfoTmp(nSrcRead+1)%iSrcType = point_source
        

      elseif (index(line,'POLLEN_SOURCE_') == 1)then
        iSrcCounter => em_source%n_pollen
        ifArea = .false.
        if(ifStart)then
          call set_error('Start new source without ending previous:'+line, &
                       & 'count_srcs_one_data_file')
        ...
        ifStart = .true.
        srcInfoTmp(nSrcRead+1)%iSrcType = pollen_source

      elseif (index(line,'BIOGENIC_VOC_SOURCE_') == 1)then
        iSrcCounter => em_source%n_bio_voc
        ifArea = .false.
        if(ifStart)then
          call set_error('Start new source without ending previous:'+line, &
                       & 'count_srcs_one_data_file')
        ...
        ifStart = .true.
        srcInfoTmp(nSrcRead+1)%iSrcType = bio_voc_source

      elseif (index(line,'SEA_SALT_SOURCE_') == 1)then
        iSrcCounter => em_source%n_sea_salt
        ifArea = .false.
        if(ifStart)then
          call set_error('Start new source without ending previous:'+line, &
                       & 'count_srcs_one_data_file')
        ...
        ifStart = .true.
        srcInfoTmp(nSrcRead+1)%iSrcType = sea_salt_source

      elseif (index(line,'WIND_BLOWN_DUST_SOURCE_') == 1)then
        iSrcCounter => em_source%n_wb_dust
        ifArea = .false.
        if(ifStart)then
          call set_error('Start new source without ending previous:'+line, &
                       & 'count_srcs_one_data_file')
        ...
        ifStart = .true.
        srcInfoTmp(nSrcRead+1)%iSrcType = wind_blown_dust_source

      elseif (index(line,'DMS_SOURCE_') == 1)then
        iSrcCounter => em_source%n_dms
        ifArea = .false.
        if(ifStart)then
          call set_error('Start new source without ending previous:'+line, &
                       & 'count_srcs_one_data_file')
        ...
        ifStart = .true.
        srcInfoTmp(nSrcRead+1)%iSrcType = dms_source

      elseif (index(line,'source_name') > 0)then
        if(ifStart)then
          iTmp = index(line,'=')
...
!call msg('Source:' + srcInfoTmp(nSrcRead+1)%chSrcNm)
      ...
      elseif (index(line, 'END_') > 0)then
        ...
        if(ifStart)then
          ...
          if(srcInfoTmp(nSrcRead+1)%iIdNbr < 0)then ! old source ID, have to add the newly read source info
            ...
            srcInfoTmp(nSrcRead)%iSrcNbr = iSrcCounter
  ...
  subroutine allocate_source_space(source)
    ...
    if(source%n_dms > 0) then                           !============= Wind-BLOWN DUST
      call msg('Allocating wind-blown dust sources:',source%n_dms)
...
!        call msg('Reading the:' + chLabel + '- source from file:' + trim(fname))
      ...
      if(len_trim(fu_content(nlsrc, 'netcdf_dir')) > 0)then
        ...
        chDataDir = fu_process_filepath(fu_content(nlsrc, 'netcdf_dir')) + dir_slash
      else
        if(len_trim(fu_content(nlsrc, 'data_dir')) > 0)then
          chDataDir = fu_process_filepath(fu_content(nlsrc, 'data_dir')) + dir_slash
        else
          chDataDir = fu_process_filepath(fu_dirname(file_name)) + dir_slash
        endif
      endif
...
!      call reset_namelist(nlSrc)
  ...
  recursive subroutine set_next_gen_src_from_nl(source, nlSrc, chSrcFNm, chDataDir_, iSrcIndex, &
    ...
    if(spContent%sp /= '')then
      ...
      if(iStatus == 0)then
        call msg('Reading the source from file:' + spContent%sp)
        
        nlSrcLocal => fu_read_namelist(iLocal, .false., fu_connect_strings('END_',chLabel))
...
call msg('Dumping Px')
  ...
  FUNCTION fu_latest_end_time(src)
    ...
    !
    if(src%n_area > 0)then
      fu_latest_end_time = fu_end_time(src%a_ptr(1)%a_src)
    else
      if(src%n_bomb > 0)then
        fu_latest_end_time = fu_time(fu_start_position(src%b_ptr(1)%b_src))
      else
        if(src%n_point > 0)then
          fu_latest_end_time = fu_end_time(src%p_ptr(1)%p_src)
        else
          if(src%n_fire > 0)then
            fu_latest_end_time = fu_end_time(src%fire_ptr(1)%fire_src)
          else
...
!call msg('Amounts from the inside 2,' + fu_str(nSpecies), amounts(1:nSpecies))
  ...
  integer function fu_n_disp_grd_cells_src_inv(src) result(nbr)
    ...
    !
    if(src%n_fire + src%n_area + src%n_point + src%n_bomb > 0)then
      do i = 1, src%n_fire
      ...
      !  Add point sources
      !
      nbr = nbr + src%n_point + src%n_bomb
      !
  ...
  integer function fu_source_id_nbr_of_source(src, &
                        & i_point, i_area, i_bomb, i_fire, i_sea_salt, i_pollen, i_bio_voc, i_wb_dust, i_dms)
    !
    ...
    ! Imported parameters with intent IN
    type(silam_source), intent(in) :: src
    integer, intent(in) :: i_point, i_area, i_bomb, i_fire, i_sea_salt, i_pollen, i_bio_voc, i_wb_dust, i_dms
    ...
    if(i_point > 0 .and. i_area > 0 .and. i_bomb > 0 &
     & .and. i_sea_salt > 0 .and. i_pollen + i_wb_dust > 0)then
      call set_error('Strange indices given','fu_source_nbr_of_source')
      return
    endif
    if(i_area > 0)then
