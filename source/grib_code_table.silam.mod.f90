...
!  USE names_of_quantities
  ...
  public read_GRIB_name_table
  public get_silam_params_for_grib_1
  public get_silam_params_for_grib_2
  public get_silam_params_for_grib  !attempt to replace codes with  grib-agnostic approach
  public get_GRIB_codes

  !!!  Clumsy mechanism to initialize definitions path
  public grib_name_table_enabled
...
CONTAINS
  ...
  subroutine read_GRIB_name_table(chFNm)
    ...
    silamDefinitionPath = fu_extend_grads_hat(fu_content(nlTmp,'local_definition_path'), chFNm)
    if (definitionPath /= '') then
       ...
       CALL GET_ENVIRONMENT_VARIABLE(ENVV, definitionPath, STATUS=iStat)
       if (iStat == 0) then !!Got it

         newDefinitionPath = trim(silamDefinitionPath)//":"//trim(definitionPath)
       elseif (iStat == 1) then
         !! Variable not set prepend custom path to the eccodes default one
         newDefinitionPath = trim(silamDefinitionPath)//":/MEMFS/definitions"
         call msg("")
       else
          call set_error("GET_ENVIRONMENT_VARIABLE returned "//trim(fu_str(iStat))//" for "//ENVV, sub_name)
          return
       endif
       call set_environment_variable(ENVV,newDefinitionPath,iStat)
       if (iStat == 0) then 
           call msg("Restting "//ENVV//" from '"//trim(definitionPath)//"' to '"//trim(newDefinitionPath)//"'" )
           CALL GET_ENVIRONMENT_VARIABLE(ENVV, definitionPath, STATUS=iStat)
           call msg("Set value: "//trim(definitionPath))
       else
           call msg("Failed to set environment, status", iStat)
           call msg(ENVV//'='//trim(newDefinitionPath))
           call set_error("Failed to set environment", sub_name)
           return
       endif
    endif
...
!      call check_entry_decode_grib_2(grib_code_table_main%pCte(iEntry), &
  ...
  subroutine get_silam_params_for_grib(cfName, shortName, silam_quantity, silam_species_string, silam_level, factor)
    !
    ! Returns the SILAM quantity for the given eccodes shortname and cfName
    ! lookup from the nametable
    !
    implicit none
    ...
    do iTmp = 1, grib_name_table%nEntries
      if (grib_name_table%shortName(iTmp) == "*") then
         found =  (grib_name_table%cfName(iTmp) == cfName)
      elseif (grib_name_table%cfName(iTmp) == "*") then
         found = (grib_name_table%shortName(iTmp) == shortName)
      else
        found = (grib_name_table%shortName(iTmp) == shortName) &
              &.and. (grib_name_table%cfName(iTmp) == cfName)
      endif
