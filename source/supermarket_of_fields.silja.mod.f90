...
!  public merge_data_to_supermarket
  ...
  PUBLIC fu_sm_obstime_windfield
...
!      endif
      ...
      if(fform%iFormat == test_field_value_flag)then
        ...
        do iFNm = 1, size(fnames)
          ...
          CALL store_input_to_supermarket(miniMarketPtr, &
                                      ...
                                      & wdr,& 
                                      & iUpdateType, &
                                      & single_time_stack_flag, &  ! ifUpdateAllowed
                                      & iAccuracy, &
                                      & ifNewDataHere, &
                                      & storage_grid = storage_grid, &
                                      & ifVerbose_ = .True., &
  ...
  SUBROUTINE fill_meteo_market(miniMarketPtr, wdr, shopping_list, direction, iAccuracy, ifGotNewData)
    !
    ...
    integer, intent(in) :: direction, iAccuracy
    logical, intent(out) :: ifGotNewData
    ...
    select case(direction)
      ...
      case(backwards)
        fSign = 1.0
      case default
       call set_error('Strange direction:' + fu_str(direction),'fill_meteo_market')
       return
    endselect
      
    !
    ! Check if some or all of required data is already in the supermarket.
    ! However, if the ptrStackToFill is given, no checking is done - we will
    ! simply try to fill it and see what happens
    !
...
!    call msg("end shopping list for store_input_to_supermarket")
    ...
    if( present(stack_indices_))then
      call msg('Storing to supermarket '//trim(miniMarketPtr%name)//' with stack_indices:' + chFName)
      ...
      if(stack_type == single_time_stack_flag) then
        call msg('Storing to supermarket '//trim(miniMarketPtr%name)//' singletime:' + chFName)
      ...
      if(stack_type == multi_time_stack_flag) then
        call msg('Storing to supermarket '//trim(miniMarketPtr%name)//' multitime:' + chFName)
    ...
    case (netcdf_file_flag)
      ...
      call id_list_from_netcdf_file(input_unit, idList, fields_found, .true., .false.)
                    ...
                    if (.not. all( work_array(1:fs_grid) >=0)) then
                         do jTmp=1,fs_grid
                            if (.not.  work_array(jTmp) >=0) then
                              call msg("Negatibe vaalue at index:", jTmp, fs_grid)
                              exit
                            endif
                         enddo 
    ...
    !
     case(test_field_value_flag)        
       ...
       call make_test_field(chFName, &  ! chFName contain the field name and value
                          & id, &
                          & work_array, &
                          & fu_start_time(shopping_list), &
                          ! length of validity:
                          & fu_end_time(shopping_list) - fu_start_time(shopping_list), & 
                          & storage_grid)
       eof = .true. ! Exit at the next cycle
       call msg('The following test field has been created')
       call report(id)
       call msg('')
    ...
    case(grib_file_flag)
      ...
      do iID = 1, nIDs  !Select messages and create storage for them
        ...
        IF (fu_field_id_in_list(idList(iID), shopping_list, indexVar, data_time_features=TweakTime)) THEN ! Accept the field
          ...
          call  inputID2MarketID(idList(iID), id, &
            & stack_type, storage_grid, TweakTime,  ifAdjustLocal, shopping_list, indexVar, time_to_force)
         ! Tfld_ptr
         ! MarketFldPtrs
      ...
      if (defined(storage_grid) .and. (.not. ifMapNeeded)) then
        ...
        do itmp = 1, nOrder  !Get data
           ...
           call check_quantity_range(quant, marketFldPtrs(iTmp)%ptr, null(), nx*ny, nx, &
  ...
  subroutine inputID2MarketID(idIn, id, stack_type, storage_grid, TweakTime, ifAdjust,shpLst, indexVar, time_to_force)
      ...
      logical, intent(in) :: ifAdjust  ! HUOM! ifAdjust named ifMeteo gid
                                      ! Means arakawa allowed
      ...
      if (defined(storage_grid)) then
        call grid_data_hor_select_new_grid(fu_grid(id), &  ! grid_original
                                         ...
                                         & ifAdjust, &    ! ifAdjust to system_grid
                                         & gridTmp)      ! new grid_new
        call set_grid(id, gridTmp)
      else
...
!call report(idOut)
    ...
    case (test_field_value_flag)
      ...
      intervalTmp = fu_validity_length(idRequested)
      if (.not. defined(intervalTmp)) intervalTmp = very_long_interval

      call make_test_field(chFName, &  ! chFName contain the field name and value
                               & idOut, &    ! idRequested
                         & work_array, &
                         & timeTmp, &
                         & intervalTmp, & !one_day * 3650., &  ! length of validity
                         & pGrid)
call msg('The following test field has been created')
call report(idOut)
call msg('')
...
!                                               & fMissingValue, & ! the grid-data
    ...
    if(found)then
      if(ifWA)then 
          call grid_data_hor_select_new_grid(fu_grid(idOut), &  ! grid_original
                                           & Grid_in, &        ! storage_grid
                                           & area_missing, &       ! storage_area
          ...
          call  remap_field( fu_grid(idOut), &
           ...
           call grid_dimensions(gridTmp, nx, ny)
           call check_quantity_range(quant, data_requested, null(), nx*ny, nx, &
...
!        call msg('Opening GrADS file:' + chFName)
  ...
  subroutine get_id_list_from_input_file(input_unit, fileFormat, idList, number_of_ids)
    ...
    select case(fileFormat)
      ...
      case(netcdf_file_flag)
        call id_list_from_netcdf_file(input_unit, idList, number_of_ids, .false., .false.) !ifFirstTime, ifFirstLev)
...
!!!$      CALL print_supermarket_contents()
  ...
  FUNCTION fu_sm_obstime_windfield(miniMarketPtr, quantity, met_src, level, time, direction) result(field)

    ! Description:
    ! Finds nwpm 2D-windfields from memory. Data is returned as a
    ! pointer to a silja_windfield.
    !
    ! No vertical interpolation is done, so the
    ! wind has to be available on the given level.
    !
    ! No time interpolation is done, so data is returned to the
    ! closest existing sm observation time
    ! Direction may have values, backwards, forwards,
    ...
    TYPE(silja_level), INTENT(in)  :: level
    TYPE(silja_time), INTENT(in) :: time
    INTEGER, INTENT(in) :: quantity, direction ! in time
    ...
    !
    IF (.NOT. miniMarketPtr%stack_multiTime_initialized) THEN
      CALL set_error('minimarket not init','fu_sm_obstime_windfield')
      RETURN
    END IF
    ...
    id = fu_set_windfield_id_simple(quantity, met_src, obstime, level)
    IF (error) RETURN
    ...
    ! 2. Check the hit list.
    !
    CALL find_wind_from_hit_list(miniMarketPtr, id, field, found_in_hit_list)

    IF (found_in_hit_list) THEN
      hit_count = hit_count + 1
      cpu_usage = cpu_usage + fu_total_cpu_time_sec() - before
      windfields_delivered = windfields_delivered + 1
      RETURN
    ELSE
      hit_miss = hit_miss + 1
    END IF
    ...
    ! ------------------------------------------------------
    !
    ! 3. Check the stack for desired field.
    !
    CALL find_wind_from_stack(stack, id, field, found_in_memory)
    ...
    IF (found_in_memory) THEN
      CALL put_wind_to_hit_list(miniMarketPtr,field)
      windfields_delivered = windfields_delivered + 1
    ELSE
      CALL set_error('wind not found in supermarket',&
                   & 'fu_sm_obstime_windfield')
      CALL report(id)
      NULLIFY(field)
    END IF
  ...
  END FUNCTION fu_sm_obstime_windfield
  ...
  subroutine find_field_data_storage_2d(miniMarketPtr, IDin, stack_type, pData, pID)
    ...
    IF(fu_fails(defined(IDin),'undefined id given','find_field_data_storage_2d'))RETURN

    ! Find the stack we need to address
    !
      s = fu_met_src_storage_index(miniMarketPtr, stack_type, fu_met_src(IDin))
      if(fu_fails(s >= 1 .and. s <= size(miniMarketPtr%stacks_multiTime,1), &
                & 'Bad met_src, can not find proper stack','find_field_data_storage_2d'))return
      select case(stack_type)
        ...
        case default
          call msg('Strange stack type:', stack_type)
          call set_error('Strange stack type:', 'find_field_data_storage_2d')
      end select
    ...
    if(fu_fails(.not.error,'Failed to create the place for this field','find_field_data_storage_2d'))then
      call report(IDin)
      return
    endif
...
!        call msg('Store 4')
    CALL put_field_to_stack(id,&
                              & grid_data,&
                              & stackptrLocal, &
                              & .false., & ! Agreement with the system_grid is not forced
                              & iUpdateTypeLocal, &
                              & ifRandomise, &
...
!                endif
  ...
  SUBROUTINE check_sm_for_data_in_list(miniMarketPtr, list,&
    ...
    TYPE(silja_shopping_list), intent(out), target :: list_of_missing_stuff
    ...
    LOGICAL :: level_check
    TYPE(silja_shopping_list), pointer :: listMissPtr
    ...
    listMissPtr => list_of_missing_stuff
    ...
    CALL fix_shopping_time_boundaries(listMissPtr, req_earliest, req_latest)
    IF (error) RETURN
...
!    IF (.NOT.ANY(met_srcs == met_src)) RETURN
    ...
    bottom = fu_floor_level(list_of_missing_stuff)
    top = fu_ceiling_level(list_of_missing_stuff)
    IF (error) RETURN

    level_check = (defined(bottom).and.(defined(top)))
    ...
    IF (miss_count > 0) THEN
      CALL fix_shopping_time_boundaries(listMissPtr,&
