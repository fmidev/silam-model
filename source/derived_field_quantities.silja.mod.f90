...
!                              & fu_realtime_quantity (fu_quantity(requested_dyn_list,i)),&
  ...
  SUBROUTINE make_derived_meteo_fields(meteoMarketPtr, list, wdr, ifNewMeteoData)
    !
...
!        different meaning used in R_down_meteo_flag
      ...
      ! Downward radiation can be computed from net radiationa and albedo
      ! use the climatological albedo here to indicate the meteo value
      ! and teh cumulative quantities so the rest could work as it used to
      ...
      IF (boolTmp1 .or. boolTmp2) THEN
        ...
        CALL dq_cwcabove_3D(meteoMarketPtr, met_src, valid_times, boolTmp1, boolTmp2)
        IF (error) THEN
