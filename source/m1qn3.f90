...
c-----------------------------------------------------------------------
c
c     M1QN3, Version 3.2.c, December 2008
c
...
c
  ...
  900 format (/" M1QN3 (Version 3.2.c, December 2008): entry point"/
...
c
 9999 continue
     ...
     &             dz(ialpha),dz(iybar),dz(isbar),reverse,indic,izs,rzs,
     &             dzs)
...
c
     ...
     &                   sbar,reverse,indic,izs,rzs,dzs)
c----
c
...
100   niter=niter+1
  ...
  910 format (" m1qn3: iter ",i0,", simul ",i0,
     &        ", f=",1pd15.8,", h'(0)=",d12.5)
c
c     --- call the simulator every iteration with "indic = 1"
c
c     if (.not.reverse .and. impres.lt.0) then
c         if (mod(niter,-impres).eq.0) then
c             indic=1
c             call simul (indic,n,x,f,g,izs,rzs,dzs)
c         endif
c     endif
c
...
400       continue
          if (impres.ge.5) then
              ...
              dk1=dsqrt(ps)
              if (niter.gt.1 .and. dk > 0) write (io,930) dk1/dk
...
c
c        en sortie indic =
c        1           mlis3 a termine (Wolfe realise, arret demande par
c                    simulateur, ...)
