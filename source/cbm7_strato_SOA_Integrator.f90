...
! 
! File                 : cbm7_strato_SOA_Integrator.f90
...
MODULE cbm7_strato_SOA_Integrator

  USE cbm7_strato_SOA_Parameters, ONLY: NVAR, LU_NONZERO
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  USE cbm7_strato_SOA_Jacobian, ONLY: Jac_SP, LU_IROW, LU_ICOL
  USE cbm7_strato_SOA_Function, ONLY: Fun
...
!~~~>  Arguments
   ...
   REAL(kind=sp), INTENT(IN)    :: AbsTol(N),RelTol(N)
   ...
   REAL(kind=sp), intent(inout) :: garbage(N)
...
!~~~>   Parameters
   ...
   REAL(kind=sp), PARAMETER :: DeltaMin = 1.0E-5_dp
...
!~~~> Input: integration parameters
   ...
   REAL(kind=sp), INTENT(IN) :: Hstart, Hmin, Hmax
   INTEGER, INTENT(IN) :: Max_no_steps
   REAL(kind=sp), INTENT(IN) :: Roundoff, FacMin, FacMax, FacRej, FacSafe
!~~~> Output: Error indicator
   INTEGER, INTENT(OUT) :: IERR
! ~~~~ Local variables
   REAL(kind=sp) :: Ynew(N), Fcn0(N), Fcn(N)
   REAL(kind=sp) :: K(N*ros_S), dFdT(N)
#ifdef FULL_ALGEBRA    
   REAL(kind=sp) :: Jac0(N,N), Ghimj(N,N)
#else
   REAL(kind=sp) :: Jac0(LU_NONZERO), Ghimj(LU_NONZERO)
#endif
   REAL(kind=sp) :: H, Hnew, HC, HG, Fac, Tau
   REAL(kind=sp) :: Err, Yerr(N)
...
!~~~> Succesful exit
  ...
  subroutine sweep_and_add_garbage(values, thresholds, garbage)
    implicit none
    REAL(kind=sp), intent(inout) :: values(N)
    REAL(kind=sp), intent(in) :: thresholds(N)
    REAL(kind=sp), intent(out) :: garbage(N)
...
! Input arguments
   REAL(kind=sp), INTENT(IN) :: Y(N), Ynew(N), &
...
!~~~> Input arguments
   REAL(kind=sp), INTENT(IN) :: T, Roundoff, Y(N), Fcn0(N)
!~~~> Output arguments
   REAL(kind=sp), INTENT(OUT) :: dFdT(N)
!~~~> Local variables
   REAL(kind=sp) :: Delta
   REAL(kind=sp), PARAMETER :: ONE = 1.0_dp, DeltaMin = 1.0E-6_dp
...
!~~~> Input arguments
#ifdef FULL_ALGEBRA    
   REAL(kind=sp), INTENT(IN) ::  Jac0(N,N)
#else
...
!~~~> Output arguments
#ifdef FULL_ALGEBRA    
   REAL(kind=sp), INTENT(OUT) :: Ghimj(N,N)
#else
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 USE cbm7_strato_SOA_Parameters, ONLY: NVAR, LU_NONZERO
 USE cbm7_strato_SOA_Global, ONLY: FIX, RCONST!, TIME
 USE cbm7_strato_SOA_Function, ONLY: Fun
 !USE cbm7_strato_SOA_Rates, ONLY: Update_SUN, Update_RCONST
!~~~> Input variables
   REAL(kind=sp) :: T, Y(NVAR)
!~~~> Output variables
   REAL(kind=sp) :: Ydot(NVAR)
!~~~> Local variables
   REAL(kind=sp) :: Told
...
END MODULE cbm7_strato_SOA_Integrator
