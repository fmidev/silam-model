module dispersion_supplementary
  ...
  type general_dispersion_rules
     ...
     logical :: make_da_debug_output =.false.
...
contains 
  ...
  subroutine run_dispersion(cloud, em_source, wdr, simrules, &
    ...
    type(silja_time), intent(inout) :: now
    ...
    integer :: nParticlesToReset, iTmp, jTmp, num_steps, ind_step, step_count, number_of_times, direction
!    logical :: first_step,  almost_done, ifGotNewMeteoData, have_tla_traj, have_perturbs, ifCalculate,&
...
!         & bc_lim1, bc_lim2, bc_lim1_prev, bc_lim2_prev
  ...
  !  call msg ("Oops..")
    ...
    num_steps = fu_nbr_of_timesteps(simrules)
    ...
    loop_over_time: DO WHILE(fu_between_times(now, &
                                            & simrules%startTime, &
    ...
    if(simRules%ifRunDispersion)then
      ...
      if (simrules%ifRunDispersion .and. fu_interval_positive(simrules%timestep) &
        ...
        if(any(simrules%chemicalRules%iTransformTypes(:) == transformation_cbm42_strato) .or. &
          ...
          call check_species(cloud,'beginning of step', fu_low_mass_threshold(simrules%chemicalRules), &
                          & 'OLE_gas', 'PAR_gas')
        end if
      end if
    endif
...
!          almost_done = (simrules%startTime + simrules%periodToCompute < now + simrules%timestep/2.)
!      else
!          almost_done = (simrules%startTime + simrules%periodToCompute > now + simrules%timestep/2.)
!      endif
!      almost_done = almost_done .and. simrules%if_finalize
!      if (almost_done) call msg("Almost done! Finalizing time loop....")
      
      if(error)then
        call msg_warning('Problem at the beginning of time step','silam_v4')
        exit loop_over_time
      endif
      !
      ...
      if(simRules%ifWriteProgressFile)then
        call write_progress_file(simRules%chProgressFNmTemplate, &
                               & fu_compute_progress_time(simrules%startTime, &
                                                    & simrules%startTime + simrules%periodToCompute, &
                                                    & now))
        if(error)call unset_error('silam_v5')
      endif
      ...
      if (have_perturbs) then
        call apply_perturbations(cloud, disp_buf_ptr, perturbations, simrules%timestep, wdr, OutDef)
        if (error) return
      end if
      ...
      if(now == simrules%startTime .or. &    ! start of the run, read no matter what
       & (.not. fu_between_times(reftime , &
        ...
        ifGotNewMeteoData = .false.
      endif    ! if now is outside the meteodata availability
      ...
      ! Boundary market is not yet handled here due to its highly specific features. 
      ! But eventually it should be included as well.
      !
!      if (.not. almost_done) then
         ...
         call make_all_diagnostic_fields(meteoMarketPtr, meteo_ptr, &         ! meteo market & buffer
                                    ...
                                    & fu_output_dyn_shopping_list(OutDef, now), &  ! full list of output dynamic fields
                                    & disp_stat_shopping_list, &     ! dispersion static shopping list
                                    & fu_output_st_shopping_list(OutDef, now), &   ! fuopenll list of output static fields
                                    & wdr, &                          ! weather data rules
                                    & simRules%diagnosticRules, &     ! diagnostic rules
                                    & ifGotNewMeteoData, &            ! if new meteo data avaialble
                                    & now, simRules%timestep, weight_past, & ! now, step & weight of past-meteo time
                                    & simRules%ifRunDispersion)       ! if dispersion structures exist
...
!            call stop_count(chCounterNm = command_string)
            ...
            CALL fillBoundaryStruct(simRules%IniBoundaryRules, BCMarketPtr, &
                                  & now, fu_boundaryStructures(cloud), meteo_ptr, weight_past, first_step)
            IF (error) EXIT loop_over_time
...
!call check_supermarket_fields_range(outputMarketPtr)
      ...
      if (simrules%make_da_debug_output) call da_debug_output()

!      if(error .or. almost_done) exit loop_over_time 
      if(error) exit loop_over_time 
...
!          call msg('Reporting emission mass after dump')
        ...
        if (obs_ptrs%hasObservations .and. .not. simrules%if_invert_substeps) then
          ...
          ! removed asap.
          !
          call observeAll(obs_ptrs, cloud, meteo_ptr, simRules%chemicalRules, &
