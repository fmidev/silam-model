...
!       & .not. (fu_y(p_src%position) .eps. fu_y(posTmp)))then
  ...
  subroutine getTimeSlots_of_point_source(p_src, time, iSlot1, iSlot2)
    ...
    ! Stupidity check
    !
    if(time < p_src%params(1)%time .or. time > p_src%params(size(p_src%params))%time)then
    ...
    ! Search
    !
    do iSlot2 = 1, size(p_src%params)
      if(time < p_src%params(iSlot2)%time) then
        iSlot1 = iSlot2 -1
        return
      endif
    end do
    ...
    ! Last time moment ?
    !
    if (time == p_src%params(size(p_src%params))%time) then
      iSlot1 = size(p_src%params)
      iSlot2 = iSlot1
      return 
    end if
    !
    ...
    !
    call set_error('Failed to find the proper time slots','getTimeSlots_of_point_source')
    iSlot1=-1
    iSlot2=-1
  ...
  subroutine prepare_src_vert_params_p_src(p_src, vertMeteo, vertDisp, vertProj)
    ...
    if(p_src%vertDispType == multiLevelFixed)then
      ...
      do indTime = 1, size(p_src%params)
        nullify(p_src%params(indTime)%levFractDispVert, p_src%params(indTime)%fzDisp)
      end do
      ...
      if(p_src%ifEmissionLagrangian)then
        ...
        if(fu_fails(i==0,'Failed to allocate dispersion-vertical lagrangian level fractions', &
                       & 'prepare_src_vert_params_p_src'))return
...
!call msg('in')
    ...
    !
    do iSlot = 1, size(p_src%params)
      if(p_src%params(iSlot)%time <= timeStart) iSlotStart = iSlot
      if(p_src%params(iSlot)%time >= timeEnd)then
        iSlotEnd = iSlot
        exit
      endif
    end do

!call msg('Start and end slots:',iSlotStart, iSlotEnd)
