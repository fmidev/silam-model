  ...
  subroutine fill_p_src_from_namelist(nlSrc, p_src, chPointSrcFileVersion)
    ...
    select case(chPointSrcFileVersion)
      ...
      case default
        ...
        call set_error('Wrong point source file version',sub_name)
        return
    end select
...
!call msg("p_src%nDescriptors",p_src%nDescriptors)
    ...
    do iTmp = 1, nItems
      ...
      if(error)then
        call set_error('Failed source:' + p_src%src_nm + '_' + p_src%sector_nm,sub_name)
        return
      endif
    end do   ! time slots
...
!                                                 & 92500.,p_src%params(1)%time)
    ...
    if( p_src%tz_name/= '')then 
      ...
      elseif (fu_str_u_case(p_src%tz_name) == 'SOLAR')then
        ...
        if (p_src%tz_index < 1) then
          call set_error("Failed to find Timezone index for '"//trim(p_src%tz_name)//"'", sub_name)
          return
        endif
      endif
    else 
...
!          call msg('Monthly variation indices are read. sum=',real_value=sum(a_src%indMonth(1:24)))
      ...
      case default
        call set_error('Only POINT_SOURCE_4 and POINT_SOURCE_5 are supported',sub_name)
        return
    end select
...
!end do
    ...
    nullify(species)
    nSpecies = 0
    call add_source_species_p_src(p_src, species, nSpecies)
