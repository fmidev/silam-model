MODULE ini_boundary_conditions
  ...
  type Tini_boundary_rules
    ...
    integer, dimension(6) :: bTypes
    ! a list of files, each with a list of boundaries to apply
    type(Tboundary_file), dimension(:), pointer :: bFiles =>null()
    logical :: ifBoundary = .false.
...
!    private
  ...
  type TboundaryBuffer
    ...
    type(silja_rp_1d), dimension(2) :: PoleCapThickness, PoleCapAirmass
  ...
  subroutine set_ini_boundary_rules_from_nl(nlIni, nlStandardSetup, rulesIniBoundary, start_time)
    ...
    if(rulesIniBoundary%defined == silja_true)then
      call set_error('Cannot redefine the boundary conditions','set_ini_boundary_rules_from_nl')
      return
    endif
    ...
    if(.not. associated(nlIni))then
      call msg('')
      call msg('')
      call set_error('No initial_and_boundary_conditions list','set_ini_boundary_rules_from_nl')
      return
    endif
    if(.not.defined(nlIni))then
      call set_error('Undefined namelist given','set_ini_boundary_rules_from_nl')
      return
    endif
    ...
    if(iTmp < 1)then
      ...
      if(fu_fails(iStat == 0, 'Failed to allocate initial quantity list', &
                            & 'set_ini_boundary_rules_from_nl'))return
      do iStat = 1, iTmp
        rulesIniBoundary%qInitial(iStat) = fu_get_silam_quantity(fu_content(ptrItems(iStat)))
        if(error)then
          call set_error('Strange quantity name:' + fu_content(ptrItems(iStat)), &
                       & 'set_ini_boundary_rules_from_nl')
          return
        endif
      end do
      !
      ...
      if(iTmp < 1)then
        call set_error('Initialization requested but no files given','set_ini_boundary_rules_from_nl')
        return
      else
        allocate(rulesIniBoundary%chIniFile(iTmp),stat=iStat)
        if(fu_fails(iStat == 0, 'Failed to allocate initial file names', &
                              & 'set_ini_boundary_rules_from_nl'))return
        do iStat = 1, iTmp
    ...
    ! - zero (the only possible for lagrangian advection)
    ! - external concentrations via field
    ! - external concentrations given at a few points
    ! - MPI interface, transparent from both sides. This one always has priority
    !
    ...
    if(chTmp == '' .or. chTmp == 'ZERO')then
      ...
      if(trim(chTmp) == 'DIRICHLET')then
        rulesIniBoundary%bTypes(1:6) = dirichlet_boundary_type
      else
        call set_error('Unknown boundary type:' + chTmp,'set_ini_boundary_rules_from_nl')
        return
      endif
    ...
    endif  ! Initial definition of the boundary types
    !
    ! If the boundaries around the main domain happened to be defined, look for headers
    !
    if(rulesIniBoundary%ifBoundary)then
      ...
      if(iTmp < 1)then
        nullify(rulesIniBoundary%bFiles)    ! e.g. MPI run or all global
      else
        allocate(rulesIniBoundary%bFiles(iTmp),stat=iStat)
        if(fu_fails(iStat == 0, 'Failed to allocate boundary files structure', &
                              & 'set_ini_boundary_rules_from_nl'))return
        !
    ...
    !
    ! If any of the boudanries appeared to be the MPI boundary, everything is overwritten.
    ! Beware: the header file is NOT needed for MPI boundaries
    !
    do iStat = 1, 4
      if(adv_mpi_neighbours(iStat) /= int_missing)then
        rulesIniBoundary%bTypes(iStat) = smpi_comm_boundary_type
        rulesIniBoundary%ifBoundary = .true.
      endif
    end do
    !
  ...
  subroutine set_missing_b_rules(b_rules)
    ...
    nullify(b_rules%chIniFile, b_rules%qInitial, b_rules%bFiles)
  ...
  subroutine set_initial_conditions(mapMass, mapPx, mapPy, mapPz, mapDD, mapWD, &
    ...
    ! Local variables
    type(silja_shopping_list) :: shopLst
    integer :: iFile, iQ, iQMetStack, iQDispStack, iQmassMap, nFields_updated
    type(Tsilam_namelist), pointer :: nlPtr
    integer, dimension(:), pointer :: arQMetStack, arQDispStack, arQmassMap
    ...
    arQMetStack => fu_work_int_array()
    arQDispStack => fu_work_int_array()
    arQmassMap => fu_work_int_array()
    if(error)return
    ...
    do iQ = 1, size(rulesIniBoundary%qInitial)
      select case(rulesIniBoundary%qInitial(iQ))
        case(concentration_flag, drydep_flag, wetdep_flag, mass_in_air_flag, &
           & advection_moment_X_flag, advection_moment_Y_flag, advection_moment_Z_flag)
          !
          ! Mass map quantities
          !
          iQmassMap = iQmassMap + 1
          arQmassMap(iQmassMap) = rulesIniBoundary%qInitial(iQ)
          call msg('Ordering mass-map initialization of:' + fu_quantity_string(arQmassMap(iQmassMap)))
        case default
          !
          ! Meteorological or dispersion quantities
          !
          if(fu_SILAM_dispersion_quantity(rulesIniBoundary%qInitial(iQ)))then
            iQDispStack = iQDispStack + 1
            arQDispStack(iQDispStack) = rulesIniBoundary%qInitial(iQ)
            call msg('Ordering dispersion-stack initialization of:' + &
                   & fu_quantity_string(arQDispStack(iQDispStack)), arQDispStack(iQDispStack))
          else
            iQMetStack = iQMetStack + 1
            arQMetStack(iQMetStack) = rulesIniBoundary%qInitial(iQ)
            call msg('Ordering meteo-stack initialization of:' + &
                   & fu_quantity_string(arQMetStack(iQMetStack)), arQMetStack(iQMetStack))
          endif
      end select
    end do
    arQMetStack(iQMetStack+1) = int_missing
    arQDispStack(iQDispStack+1) = int_missing
    arQmassMap(iQmassMap+1) = int_missing

    if(iQMetStack + iQDispStack + iQmassMap > 0)then
      !
      ...
      !----------------------------------------------------------------------
      !
      ! Step 1: initialize meteo stack, if anything is requested
      !
      if(iQMetStack > 0)then
        !
        ! Something is found to be initialized in the meteo_stack. Get it!
        !
        call msg('Meteo stack')
        shopLst = fu_set_shopping_list(met_src_missing, &
                                     & arQMetStack, &
                                     & now-one_minute, & ! First time boundary
                                     & now+one_minute, &
                                     & level_missing, level_missing)
        if(error)return

        !Initial values might have very strange timestamps: make them valid for only for now
        call fill_minimarket_from_namelist(meteoMarketPtr, nlPtr, 'initial_file', &
                                         & shopLst, now, instant_map, overwrite_field_anytime, &
                                         & wdr_missing,&
                                         & meteo_gridPtr, &   ! in meteo market this is mandatory
                                         & 5, .true., & ! iAccuracy, ifAdjustGrid
                                         & ifNewDataHere)
!                                         & shopLst, time_missing, single_time_stack_flag)
        if(error)return

      endif ! if some quantities for intialization are in meteo_stack

      if(iQDispStack > 0)then
        !
        ! Something is found to be initialized in the dispersion_stack. Get it!
        !
        call msg('Dispersion stack')
        
        call msg_warning('Using dispersion_grid as a storage grid','set_initial_conditions')
        
        shopLst = fu_set_shopping_list(met_src_missing, &
                                     & arQDispStack, &
                                     & now-one_second, & ! First time boundary
                                     & now+one_second, &
                                     & level_missing, level_missing)
        if(error)return
...
!                                         & shopLst, time_missing, single_time_stack_flag)
    ...
    call free_work_array(arQMetStack)
    call free_work_array(arQDispStack)
    call free_work_array(arQmassMap)
    ...
    subroutine check_poles(pMap, pBoundary, indYEdge)
      ...
      do iz = 1, pMap%n3d
        fArea = 0.
        dz = fu_layer_thickness_m(fu_level(pMap%vertTemplate,iz))
        do ix = 1, pMap%nx
        ...
        pBoundary%polarCapMassMom(1:pMap%nSpecies,1:pMap%nSrc,1,iz) = &
                    & pBoundary%polarCapMassMom(1:pMap%nSpecies,1:pMap%nSrc,1,iz) / &
                    & (fArea * dz) * &
                    & pBoundary%polarCapThickness(iz) * pBoundary%polarCapArea
  ...
  subroutine init_boundary_structures(ptrRulesIniBoundary, &
                                    & wdr, &
    ...
    ! Imported parameters
    type(Tini_boundary_rules), intent(inout), target :: ptrRulesIniBoundary
...
!    bcrPtr => ptrRulesIniBoundary
    ...
    !
    ! If global grid, create polar boundary structures.
    ! Create poles only if there is no further subdomain
    !
!     call msg("init_boundary_structures got", ptrRulesIniBoundary%bTypes(1:6)) 
    if (ptrRulesIniBoundary%bTypes(northern_boundary) /= smpi_comm_boundary_type) then
       if (fu_ifPolarCapPossible(dispersion_grid, northern_boundary)) then
         call msg('Creating north pole')
         ptrRulesIniBoundary%ifBoundary = .true.
         ptrRulesIniBoundary%bTypes(northern_boundary) = polar_boundary_type
       end if
    endif

    if (ptrRulesIniBoundary%bTypes(southern_boundary) /= smpi_comm_boundary_type) then
       if (fu_ifPolarCapPossible(dispersion_grid, southern_boundary)) then
         call msg('Creating south pole')
         ptrRulesIniBoundary%ifBoundary = .true.
         ptrRulesIniBoundary%bTypes(southern_boundary) = polar_boundary_type
       end if
    endif

    ! Anything useful this far?
    !
    if (.not. ptrRulesIniBoundary%ifBoundary) then
      call msg('Applying zero boundaries')
      pBBuffer%iBoundaryType(1:top_boundary) = zero_boundary_type
      pBBuffer%iBoundaryType(bottom_boundary) = surface_boundary_type
!!      return  !!!! No return here!! Boundary structure 
    endif
    !
    !-----------------------------------------------------------------------------------------------
    !
    ! Find Dirichlet boundary conditions and, if any, initialize the corresponding structures
    !
    iffound = .false.
    do iTmp = 1, 6
      if(ptrRulesIniBoundary%bTypes(iTmp)==0) ptrRulesIniBoundary%bTypes(iTmp)=zero_boundary_type
      if(ptrRulesIniBoundary%bTypes(iTmp)==dirichlet_boundary_type)then
        iffound = .true.
        exit
      endif
    enddo
    call msg("Btypes:", ptrRulesIniBoundary%bTypes(1:6))

    if(iffound)then
      if(fu_fails(associated(ptrRulesIniBoundary%bFiles),'Dirichlet boundary requested but no boundary files', &
      ...
      !
      do iHeader = 1, size(ptrRulesIniBoundary%bFiles)
      
        call set_missing(ptrRulesIniBoundary%bFiles(iHeader)%shopLst) 
        call set_missing(ptrRulesIniBoundary%bFiles(iHeader)%vertical, .true.)
        ptrRulesIniBoundary%bFiles(iHeader)%unit_binary(:) = int_missing
        ptrRulesIniBoundary%bFiles(iHeader)%nTSpecies = 0
        !
        ! Read the header file
        !
        iFile = fu_next_free_unit()
        open(file=ptrRulesIniBoundary%bFiles(iHeader)%bHeaderFNm, &
        & unit=iFile, action='read', status='old', iostat=iStat)
        if(iStat /= 0)then
          call set_error('Failed to open boundary map file:' + &
                       & ptrRulesIniBoundary%bFiles(iHeader)%bHeaderFNm,'init_boundary_structures')
          return
        endif
        ...
        ptrRulesIniBoundary%bFiles(iHeader)%bNames = fu_content(nlPtr, 'boundary_names')
        ptrRulesIniBoundary%bFiles(iHeader)%ifToBoundary(:) = .false.    
        do iTmp = 1, 6
          included = index(ptrRulesIniBoundary%bFiles(iHeader)%bNames, trim(ptrRulesIniBoundary%bNames(iTmp)))>0
          if(included .and. ptrRulesIniBoundary%btypes(itmp) == dirichlet_boundary_type) then
            ptrRulesIniBoundary%bFiles(iHeader)%ifToBoundary(iTmp) = .true.  
          endif  
        enddo

        if(fu_content(nlPtr, 'ifClimatology') == 'NO')then
          ptrRulesIniBoundary%bFiles(iHeader)%ifStatic = .false.
          ptrRulesIniBoundary%bFiles(iHeader)%fieldType = dynamic_map
        else
          if(index(fu_content(nlPtr, 'climatologyTimestep'),'MONTHLY') == 1)then
            ptrRulesIniBoundary%bFiles(iHeader)%fieldType = monthly_climatology
            ptrRulesIniBoundary%bFiles(iHeader)%ifStatic = .false.
          elseif(index(fu_content(nlPtr, 'climatologyTimestep'),'STATIC') == 1)then
            ptrRulesIniBoundary%bFiles(iHeader)%fieldType = static_climatology
            ptrRulesIniBoundary%bFiles(iHeader)%ifStatic = .true.
          else
        ...
        ptrRulesIniBoundary%bFiles(iHeader)%bFileFormat = &
       ...
       ! Get the content of input data-files 
        ...
        allocate(ptrRulesIniBoundary%bFiles(iHeader)%bFnameTemplate(nFiles), stat = iStat)
        ...
        do iFile = 1, nFiles
          ...
          chtmp  =  fu_extend_grads_hat( chtmp, &
                                    & ptrRulesIniBoundary%bFiles(iHeader)%bHeaderFNm )

          call decode_template_string(chtmp, &
                                    & ptrRulesIniBoundary%bFiles(iHeader)%bFnameTemplate(iFile))
        ...
        do iFile = 1, nFiles
          ...
          if(ptrRulesIniBoundary%bFiles(iHeader)%bFnameTemplate(iFile) == template_missing) exit
          if(fu_if_input_file(ptrRulesIniBoundary%bFiles(iHeader)%bFileFormat))then
            call FNm_from_single_template(ptrRulesIniBoundary%bFiles(iHeader)%bFnameTemplate(iFile), &
                                        & ptrRulesIniBoundary%start_time, &
                                        & fnames, &
                                        & ifAdd = .false., & 
                                        & ifStrict = .false., &
            ...
            if(error)return
            fnames(1)%sp = fu_collection(ptrRulesIniBoundary%bFiles(iHeader)%bFnameTemplate(iFile))
...
!call report(fu_mode(lstId(iId)))
          ...
          call add_shopping_variable(ptrRulesIniBoundary%bFiles(iHeader)%shoplst, idTmp, 2)
          if(.not. fu_field_id_in_list(idTmp, &
                                     & ptrRulesIniBoundary%bFiles(iHeader)%shoplst, &
                                     & indexVar))then
...
!                         & fu_set_mode(fixed_diameter_flag, tModeVal, tModeVal, tModeVal))
          ...
          shpLstPtr => ptrRulesIniBoundary%bFiles(iHeader)%shoplst
          call add_maplink_to_shopVar(fu_shopping_var(shpLstPtr, &
                                                    & indexVar), idTmp, b2tFactor)

          ifFound = .false.
          do iTmp = 1, ptrRulesIniBoundary%bFiles(iHeader)%nTspecies
            if(ptrRulesIniBoundary%bFiles(iHeader)%tSpecies(iTmp) == iTSpecies)then
              iffound = .true.
              exit
            endif
          enddo
          if(.not. iffound)then
            ptrRulesIniBoundary%bFiles(iHeader)%nTspecies = &
                         & ptrRulesIniBoundary%bFiles(iHeader)%nTSpecies + 1
            ptrRulesIniBoundary%bFiles(iHeader)&
                         & %tSpecies(ptrRulesIniBoundary%bFiles(iHeader)%nTSpecies) = iTSpecies
            ptrRulesIniBoundary%bFiles(iHeader)&
                         & %fieldInStack(ptrRulesIniBoundary%bFiles(iHeader)%nTSpecies) = idTmp  
          endif
        ...
        do iId = 1, fu_nbr_of_fields(inputContent)
          ...
          if(.not. defined(ptrRulesIniBoundary%bFiles(iHeader)%vertical))then
            call set_vertical(fu_level(lstId(iId)), ptrRulesIniBoundary%bFiles(iHeader)%vertical)
          elseif(.not. fu_level_belongs_to_vertical(fu_level(lstId(iId)), &
                                                  & ptrRulesIniBoundary%bFiles(iHeader)%vertical))then
            call add_level(ptrRulesIniBoundary%bFiles(iHeader)%vertical, fu_level(lstId(iId)))
          endif
        enddo
        call arrange_levels_in_vertical(ptrRulesIniBoundary%bFiles(iHeader)%vertical)      
        maxlevs = max(fu_NbrOfLevels(ptrRulesIniBoundary%bFiles(iHeader)%vertical),maxlevs)
        call msg('Vertical in boundary file')
        call report(ptrRulesIniBoundary%bFiles(iHeader)%vertical)
        call msg('')
      ...
      !
      do iTmp = 1,6
        grid(iTmp) = fu_boundary_grid(dispersion_grid, ptrRulesIniBoundary%bNames(iTmp))
      enddo
      ...
      do iTmp = 1, 6
        ...
        nSingletimeFs = 0
        nMultitimeFs = 0
        do iFile = 1, size(ptrRulesIniBoundary%bFiles)
          if(index(ptrRulesIniBoundary%bFiles(iFile)%bNames, ptrRulesIniBoundary%bNames(iTmp))>0)then
            if(ptrRulesIniBoundary%bFiles(iFile)%ifStatic)then
              do iBSpecies = 1, size(ptrRulesIniBoundary%bFiles(iFile)%bFnameTemplate)
                nSingletimeFs = nSingletimeFs + 1
                fnm_singletime_tmpl(nSingletimeFs) = &
                                      & ptrRulesIniBoundary%bFiles(iFile)%bFnameTemplate(iBSpecies)
                fFmt_singletime(nSingletimeFs) = ptrRulesIniBoundary%bFiles(iFile)%bFileFormat
              enddo
            else   
              do iBSpecies = 1, size(ptrRulesIniBoundary%bFiles(iFile)%bFnameTemplate) 
                ...
                fnm_multitime_tmpl(nMultitimeFs) = &
                                      & ptrRulesIniBoundary%bFiles(iFile)%bFnameTemplate(iBSpecies)
                fFmt_multitime(nMultitimeFs) = ptrRulesIniBoundary%bFiles(iFile)%bFileFormat
              enddo
            endif
          endif
        enddo
        ...
        boundStructArray(iTmp)%ptr => fu_makeBoundaryStruct(iTmp, &
                                                         & ptrRulesIniBoundary%bNames(iTmp), &
                                                         & ptrRulesIniBoundary%bTypes(iTmp), & 
                                                         & grid(iTmp),  &
                                                         & vertical(iTmp), &
                                                         & fnm_multitime_tmpl, &
                                                         & fnm_singletime_tmpl, &
      ...
      do iHeader = 1, size(ptrRulesIniBoundary%bFiles)
        do iTmp = 1, 6
          ...
          !
          ptrRulesIniBoundary%bFiles(iHeader)%interpCoefBndInp2DispVert(iTmp)%ptr => &
                   & fu_vertical_interp_struct(ptrRulesIniBoundary%bFiles(iHeader)%vertical, &
                                             & vertical(iTmp), &
                                             & grid(iTmp), &
          ...
          !
          ptrRulesIniBoundary%bFiles(iHeader)%interpCoefMet2BndInp(iTmp)%ptr => &
                   & fu_vertical_interp_struct(meteo_vertical, &
                                             & ptrRulesIniBoundary%bFiles(iHeader)%vertical, &
                                             & grid(iTmp), &
                                             & linear, & ! a bit crude but fast. Log-linear can be tried
                                             & one_hour*3., &  ! recommended update interval
      ...
      !
      do iTmp = 1, 6
        boundStructArray(iTmp)%ptr => fu_makeBoundaryStruct(iTmp, ptrRulesIniBoundary%bNames(iTmp), &
                                                          & ptrRulesIniBoundary%bTypes(iTmp))
        if (error) return
      end do
    ...
    !
    call init_boundary_buffer(ptrRulesIniBoundary, boundStructArray, pBBuffer, mapDisp%nSrc, iTransportSpeciesInvolved)
...
!          nullify(struct%polarCapThickness)
        ...
        case(polar_boundary_type)
          ...
          allocate(struct%polarCapMassMom(nSpeciesTransp, mapDisp%nSrc, 2, nz_dispersion), &
                 & struct%polarCapThickness(nz_dispersion),&
          ...
          do i = 1,nz_dispersion
            dz = fu_layer_thickness_m(fu_level(dispersion_vertical, i))
!            struct%polarCapVolume(i) = dz * pi * rCap**2
            struct%polarCapThickness(i) = dz
!call msg('iz, volume',iTmp, struct%polarCapVolume(iTmp))
          end do
         ...
         !
          ...
          struct%bc_wdr = fu_set_wdr(1, &      ! nr of met srces
                                   & (/fu_set_met_src(bIndex, &       ! Index of the met_src
                                                    ...
                                                    & ptrRulesIniBoundary%bNames(bIndex))/), &
                                   & fnm_multitime_tmpl, &
                                   & fnm_singletime_tmpl, &
                                   & fFmt_multitime, &
                                   & fFmt_singletime, &
                                   ...
                                   & fu_start_time(wdr), &
                                   & ptrRulesIniBoundary%bInterval, fu_period_to_compute(wdr), &
         ...
         ! Allocate past and future field structure, fill with 0-s
          ...
          do iS = 1, nSpeciesTransp
            ...
            do iZ = 1, fu_NbrOfLevels(bvertical)
              ...
              call create_field_in_field_3d(struct%Fld3dPtr(1,iS)%fp, iZ,id,work_array)
              if(error)return
              call create_field_in_field_3d(struct%Fld3dPtr(2,iS)%fp, iZ,id,work_array)
              if(error)return
...
!          nullify(struct%polarCapThickness)
    ...
    subroutine init_boundary_buffer(ptrRulesIniBoundary, &
                                  & boundStructArray, &
                                  & pBBuffer, &
      ...
      type(Tini_boundary_rules), intent(inout), target :: ptrRulesIniBoundary
      type(TboundaryStructPtr), dimension(:), pointer :: boundStructArray
      type(TBoundaryBuffer), intent(out) :: pBBuffer
      integer, intent(in) :: nSrcTrn
      integer, dimension(:), intent(inout) :: iTransportSpeciesInvolved

      ! Local parameters
      integer :: iB, nPolar, nDirichlet, nPlaneDirichlet, iSpecies, nPoles

      !
      ! Understand the boundary structure 
      !
      pBBuffer%iBoundaryType(1:6) = ptrRulesIniBoundary%bTypes(1:6)
      nPolar = count(pBBuffer%iBoundaryType(northern_boundary : southern_boundary) == &
                                                                              & polar_boundary_type)
      nDirichlet = count(pBBuffer%iBoundaryType(northern_boundary : western_boundary) == &
                                                                              & dirichlet_boundary_type)
      nPlaneDirichlet = count(pBBuffer%iBoundaryType(top_boundary : bottom_boundary) == &
                                                                              & dirichlet_boundary_type)
      !
      ...
      !
      iSpecies = 0
      if(nDirichlet + nPlaneDirichlet + nPolar > 0)then
        ...
        allocate(pBBuffer%iTransportSpecies(iSpecies,6), pBBuffer%iBoundarySpecies(nSpeciesTransp,6), &
               & pBBuffer%wind_to_pole(nx_dispersion,2,nz_dispersion), pBBuffer%outflowFactor(2,nz_dispersion),  &
               & stat=iB)
        if(fu_fails(iB==0,'Failed species mapping & wind storage allocation','init_boundary_buffer'))return
        pBBuffer%iTransportSpecies(:,:) = int_missing
        pBBuffer%iBoundarySpecies(:,:) = int_missing
        pBBuffer%wind_to_pole(1:nx_dispersion,1:2,:) = 0.
        pBBuffer%outflowFactor(:,:) = 1.
        !
        ! ... and now make the mapping itself
        !
        do iB = northern_boundary, bottom_boundary
          pBBuffer%nBoundarySpecies(iB) = 0
          if(boundStructArray(iB)%ptr%boundaryType == dirichlet_boundary_type) then
            do iSpecies = 1, nSpeciesTransp
              if(iTransportSpeciesInvolved(iSpecies) == 1)then
                pBBuffer%nBoundarySpecies(iB) = pBBuffer%nBoundarySpecies(iB) + 1
                pBBuffer%iTransportSpecies(pBBuffer%nBoundarySpecies(iB),iB) = iSpecies
                pBBuffer%iBoundarySpecies(iSpecies,iB) = pBBuffer%nBoundarySpecies(iB)
              endif
            end do
          elseif(boundStructArray(iB)%ptr%boundaryType == polar_boundary_type) then
            pBBuffer%nBoundarySpecies(iB) = nSpeciesTransp
            do iSpecies = 1, nSpeciesTransp
              pBBuffer%iTransportSpecies(iSpecies,iB) = iSpecies
              pBBuffer%iBoundarySpecies(iSpecies,iB) = iSpecies
            end do
          endif
        end do
      else
!        nullify(pBBuffer%iTransportSpecies)
!        nullify(pBBuffer%iBoundarySpecies)
        pBBuffer%nBoundarySpecies(1:6) = 0
      ...
      pBBuffer%nSrcTrn = 1
      nPoles = 0
      select case(pBBuffer%iBoundaryType(northern_boundary))                                          ! NORTH
        case(polar_boundary_type)
          pBBuffer%bNorth => boundStructArray(northern_boundary)%ptr%polarCapMassMom
!          pBBuffer%pBoundaryData(northern_boundary)%ptr => pBBuffer%bNorth
!          pBBuffer%pNorthSouth(northern_boundary)%ptr => boundStructArray(northern_boundary)%ptr%polarCapMassMom
          pBBuffer%PoleCapThickness(northern_boundary)%pp => boundStructArray(northern_boundary)%ptr%polarCapThickness
          pBBuffer%PoleCapAirmass(northern_boundary)%pp => boundStructArray(northern_boundary)%ptr%polarCapAirmass
          pBBuffer%fPoleCapArea(northern_boundary) = boundStructArray(northern_boundary)%ptr%polarCapArea
          pBBuffer%indStep(northern_boundary) = 0
          pBBuffer%nSrcTrn(northern_boundary) = nSrcTrn
          nPoles = 1                       ! enough to addresss northern_boundary

        case(dirichlet_boundary_type)
          allocate(pBBuffer%bNorth(pBBuffer%nBoundarySpecies(northern_boundary), &
                                 & 1, nx_dispersion, nz_dispersion), stat=iB)
          if(fu_fails(iB==0,'Failed northern buffer allocation','init_boundary_buffer'))return
          pBBuffer%indStep(northern_boundary) = 1
        case default
          nullify(pBBuffer%bNorth)
          pBBuffer%indStep(northern_boundary) = int_missing
      end select

      select case(pBBuffer%iBoundaryType(southern_boundary))                                         ! SOUTH
        case(polar_boundary_type)
          pBBuffer%bSouth => boundStructArray(southern_boundary)%ptr%polarCapMassMom
!          pBBuffer%pBoundaryData(southern_boundary)%ptr => pBBuffer%bSouth
!          pBBuffer%pNorthSouth(southern_boundary)%ptr => boundStructArray(southern_boundary)%ptr%polarCapMassMom
          pBBuffer%PoleCapThickness(southern_boundary)%pp => boundStructArray(southern_boundary)%ptr%polarCapThickness
          pBBuffer%PoleCapAirmass(southern_boundary)%pp => boundStructArray(southern_boundary)%ptr%polarCapAirmass
          pBBuffer%fPoleCapArea(southern_boundary) = boundStructArray(southern_boundary)%ptr%polarCapArea
          pBBuffer%indStep(southern_boundary) = 0
          pBBuffer%nSrcTrn(southern_boundary) = nSrcTrn
          nPoles = 2                       ! mandatory to reach southern_boundary

        case(dirichlet_boundary_type)
          allocate(pBBuffer%bSouth(pBBuffer%nBoundarySpecies(southern_boundary), &
                                 & 1, nx_dispersion, nz_dispersion), stat=iB)
          if(fu_fails(iB==0,'Failed southern buffer allocation','init_boundary_buffer'))return
          pBBuffer%indStep(southern_boundary) = 1
        case default
          nullify(pBBuffer%bSouth)
          pBBuffer%indStep(southern_boundary) = int_missing
      end select
      !
      ...
      !
      if(nPoles > 0)then
        iSPecies = max(pBBuffer%nBoundarySpecies(northern_boundary), &
                     & pBBuffer%nBoundarySpecies(southern_boundary))
        allocate( pBBuffer%poleDryDep(nPoles, iSpecies, nSrcTrn), stat = iB)
        if(fu_fails(iB==0,'Failed allocation of pole dry dep','init_boundary_buffer'))return
        pBBuffer%poleDryDep = 0.
      endif

      if(pBBuffer%iBoundaryType(eastern_boundary) == dirichlet_boundary_type)then                    ! EAST
        allocate(pBBuffer%bEast(pBBuffer%nBoundarySpecies(eastern_boundary), &
                              & 1, ny_dispersion, nz_dispersion), stat=iB)
        if(fu_fails(iB==0,'Failed eastern buffer allocation','init_boundary_buffer'))return
        pBBuffer%indStep(eastern_boundary) = 1
      else
!        nullify(pBBuffer%bEast)
        pBBuffer%indStep(eastern_boundary) = int_missing
      end if

      if(pBBuffer%iBoundaryType(western_boundary) == dirichlet_boundary_type)then                    ! WEST
        allocate(pBBuffer%bWest(pBBuffer%nBoundarySpecies(western_boundary), &
                              & 1, ny_dispersion, nz_dispersion), stat=iB)
        if(fu_fails(iB==0,'Failed western buffer allocation','init_boundary_buffer'))return
        pBBuffer%indStep(western_boundary) = 1
      else
!        nullify(pBBuffer%bWest)
        pBBuffer%indStep(western_boundary) = int_missing
      end if

      if(pBBuffer%iBoundaryType(top_boundary) == dirichlet_boundary_type)then                        ! TOP
        allocate(pBBuffer%bTop(pBBuffer%nBoundarySpecies(top_boundary), &
                             & 1, nx_dispersion, ny_dispersion), stat=iB)
        if(fu_fails(iB==0,'Failed top buffer allocation','init_boundary_buffer'))return
        pBBuffer%indStep(top_boundary) = 1
      else
!        nullify(pBBuffer%bTop)
        pBBuffer%indStep(top_boundary) = int_missing
      end if

      if(pBBuffer%iBoundaryType(bottom_boundary) == dirichlet_boundary_type)then                     ! BOTTOM
        allocate(pBBuffer%bBottom(pBBuffer%nBoundarySpecies(bottom_boundary), &
                                & 1, nx_dispersion, ny_dispersion), stat=iB)
        if(fu_fails(iB==0,'Failed bottom buffer allocation','init_boundary_buffer'))return
        pBBuffer%indStep(bottom_boundary) = 1
      else
!        nullify(pBBuffer%bBottom)
        pBBuffer%indStep(bottom_boundary) = int_missing
      end if

      pBBuffer%pBoundaryData(northern_boundary)%ptr => pBBuffer%bNorth
      pBBuffer%pBoundaryData(southern_boundary)%ptr => pBBuffer%bSouth
      pBBuffer%pBoundaryData(eastern_boundary)%ptr => pBBuffer%bEast
      pBBuffer%pBoundaryData(western_boundary)%ptr => pBBuffer%bWest
      pBBuffer%pBoundaryData(top_boundary)%ptr => pBBuffer%bTop
      pBBuffer%pBoundaryData(bottom_boundary)%ptr => pBBuffer%bBottom
...
!call report(fu_valid_time(boundStruct%Fld3dPtr(1+switch, iSpecies)%fp))
          ...
          if(inputQuantity /= concentration_flag)then
            ...
            if(inputQuantity == volume_mixing_ratio_flag)then
              ...
              ! Convert
              !
              call make_cnc_from_vmr(met_buf, weight_past_meteo, &
...
!    !
!    ! Get dz - we shall multiply all boundaries with it to get concentrations per-area
!    ! for all lateral conditions. It will simplify the life of advection routine
!    !
!    if(arBoundaries(northern_boundary)%ptr%boundaryType == polar_boundary_type .or. &
!     & arBoundaries(southern_boundary)%ptr%boundaryType == polar_boundary_type))
!      ind_dz = fu_index(disp_buf, cell_size_z_flag)
!      if(fu_fails(ind_dz /= int_missing, 'No dz in disp. buffer', 'boundary_conditions_now'))return
!    else
!      ind_dz = int_missing
!    endif
...
!                     & 1:bBuffer%nSrcTrn(northern_boundary), 1, 1:nz_dispersion) = &
!                             & bnd%polarCapConc(1:bBuffer%nBoundarySpecies(northern_boundary),&
!                                              & 1:bBuffer%nSrcTrn(northern_boundary),1:nz_dispersion)
        bBuffer%bNorth => bnd%polarCapMassMom
!        do iz = 1, nz_dispersion
!          bBuffer%PoleCapThickness(northern_boundary)%pp(iz) = sum(disp_buf%p4d(ind_dz)%)
!        end do
...
!                     & 1:bBuffer%nSrcTrn(southern_boundary),1,1:nz_dispersion) = &
!                             & bnd%polarCapConc(1:bBuffer%nBoundarySpecies(southern_boundary), &
!                                              & 1:bBuffer%nSrcTrn(southern_boundary),1:nz_dispersion)
...
!                  i1d = ix + (iy - 1) * nx_dispersion
                  buf_data(iSpecies,1,iCell,iz) = &
                                 & (weight_past_boundary(iSpeciesTrn) * val_past(iCell) + &
                                  & (1. - weight_past_boundary(iSpeciesTrn)) * val_future(iCell)) !* &
!                                 & (weight_past * z_cell_size_past(i1d) + &
!                                  & (1. - weight_past) * z_cell_size_future(i1d))
#ifdef DEBUG
  ...
  subroutine force_mass_map_cell_mmr(mapConc, indSpecies, pBoundaryBuffer, disp_buf, &
    ...
    type(TMass_map), pointer, optional :: mapPx_conc, mapPy_conc, mapPz_conc
    ...
    call msg('Resetting mass map with '+fu_str(fTmp)+ ' mol/kg cnc')
    ...
    do iz = 1,nz
      ...
      if(pBoundaryBuffer%iBoundaryType(northern_boundary) == polar_boundary_type)then
            ...
            fMass =     sum(disp_buf%p4d(m_ind)%past%p2d(iz)%ptr(i1d:i1d+nx_dispersion-1))
            fMass = fMass * pBoundaryBuffer%fPoleCapArea(northern_boundary)/ &
                                 & (pYCellSize(i1d)*pXCellSize(i1d)*nx_dispersion)  
       ...
       if(pBoundaryBuffer%iBoundaryType(southern_boundary) == polar_boundary_type)then
            ...
            fMass = fMass * pBoundaryBuffer%fPoleCapArea(southern_boundary)/ & 
                                 &( pYCellSize(1)*pXCellSize(1)*nx_dispersion)  
    ...
    enddo
!   pBoundaryBuffer%bNorth(:,:,:,:) = 0
    if(present(mapPx_conc)) mapPx_conc%arm(:,:,:,:,:) = 0.
    if(present(mapPy_conc)) mapPy_conc%arm(:,:,:,:,:) = 0.
    if(present(mapPz_conc)) mapPz_conc%arm(:,:,:,:,:) = 0.
