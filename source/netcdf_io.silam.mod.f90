...
!        call put_chatt_nc(nf, nf%nvars(nf%n_vars)%varId, nf%nvars(nf%n_vars)%chVarNm, "standard_name", fu_quantity_standard_name(outvar_quantity))
    ...
    call smpi_get_maxsubdomain_size(nx, ny) !!!! Make sure no buffer contains more than MAX_INT32 bytes
    !Some MPI implementations  use 32bit tintegers for counts PNETCDF counts bytes
    iTmp8 = nx*ny !Maximum fieldsize over MPI domains
...
#ifdef WITH_PNETCDF
      !!! NoFields2d
      nf%nf90mpiBUFfdlds = min(fu_get_default_mpi_buf_size(), MAX_INT32 / iTmp8 / 4)

      !! Might make sense to output more often
      nf%nf90mpiBUFfdlds = min(buffer_timesteps*NoFields2d , nf%nf90mpiBUFfdlds)
       ...
       case(nf90_char)
         if(attlen > len(chAtt))then
          ...
          call msg('Global attribute ('//trim(fu_str(iTmp))//'): ' // trim(attname) // ' :: too long, sorry')
           ...
           if (ichar(chAtt(1:1)) == 0) chAtt = '';!!! For some reason empty attributes
                                           !! result in  '\000' coming from nf90_get_att 
           if(ifMsgs)then
          ...
          if (istat /= 0)then 
            ...
            case(nf90_char)
              ...
              if (ichar(chAtt(1:1)) == 0) chAtt = '';!!! For some reason empty attributes
                                           !! result in  '\000' coming from nf90_get_att 
              if(ifMsgs)then
                call msg('Attribute: "'//trim(attname)//'"::"'//trim(chAtt)//'"')
              endif
            ...
            elseif(nf%nVars(iVar)%long_name /= '')then  !CDO kills standard_name
              ...
              select case(fu_str_l_case(adjustl(nf%nVars(iVar)%long_name)))
                 ...
                 case('standard height of hybrid layer midpoint')
                   ...
                   nf%nDims(iDim)%a_half_Var =  aHalfVar
                   nf%nDims(iDim)%b_half_Var =  bHalfVar
                 !Risto: 2020-03-26: Added extra cases for newer CESM data:
                 !NOTE: Using HEIGHT_LYR_FROM_SURF even if should be something below surface!!!!
                 case('coordinate soil levels')
                   nf%nDims(iDim)%chType = 'HEIGHT_LYR_FROM_SURF'
                   !nf%nDims(iDim)%chType = 'DEPTH_LYR_FROM_SURF'
                 case('coordinate lake levels')
                   nf%nDims(iDim)%chType = 'HEIGHT_LYR_FROM_SURF'
                   !nf%nDims(iDim)%chType = 'DEPTH_LYR_FROM_SURF'
                 case default
...
!!!       if (fu_fails(modulo(nf%nf90mpifieldCnt, nf%nf90mpiNfieldsPerTimestep + 1) == 0, "Incomplete timestep was written", sub_name)) return
  ...
  !
    ...
    !Write vars string
    call FSEEK_md(iCtlUnit, iOff,  0)  ! FSEEK needs use IFPORT for Intel, see md odule
    if(error)then
      call set_error('Failed to search the nvars offset','write_ctl_file_4_netcdf')
    else
