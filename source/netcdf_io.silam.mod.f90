...
!  use grib_api_io
  ...
  TYPE netcdf_grid
    ...
    type(silja_grid), pointer :: sGrid =>null()
  ...
  TYPE netcdf_file
    PRIVATE
!    type(netcdf_grid), dimension(5) :: nGrids
    type(netcdf_grid), dimension(:), pointer :: nGrids => null()

    CHARACTER (LEN=fnlen) :: fname='', chTemplate='', title='', chTStart='',tmpfname=''
    type(meteo_data_source) :: met_src = met_src_missing
    type(grads_template) :: grTemplate = template_missing
  ...
  subroutine create_nc(fname, ounit, ncver, ifMPIIO)
   ...
   elseif (ncver == 4) then
      if (ifMPIIO) then
         ...
         iStat = nf90_create(fname, IOR(NF90_NETCDF4, NF90_MPIIO), ounit, & 
...
#ifdef WITH_NF90_MPIIO 
      if (ifCollective .and. nf%ncver==4 .and. nf%ifmpiio) then
        iStat = nf90_var_par_access(nf%unit_bin, varid, NF90_COLLECTIVE)
        if (iStat /= 0) call set_error("nf90_var_par_access: "//nf90_strerror(iStat), sub_name)
      endif
#endif
    endif
    if (iStat /= 0)then 
 ...
 !
  ...
  integer function open_netcdf_file_o(fname, myOutGrid, vertical, timeValid, &
   ...
   ! First global attributes:
    ...
    allocate(nf%nGrids(1), stat=istat)
    if (istat /= 0)then 
    ...
    allocate(nf%nGrids(1)%sGrid, stat=istat)
    if (istat /= 0)then 
         call msg('Failed to allocate NetCDF file structure No:', iFile)
         call set_error('failed to allocate nf%ndims(nf%n_Dims), nf%nvars(nf%n_Vars)',sub_name)
         return
    ...
    nf%levType = fu_leveltype(vertical)
...
!    integer, dimension(4) :: dimIds
    ...
    character (len=nf90_max_name), dimension(:), pointer :: lstVarName, lstDimName, &
                                       &lstDimVar,  gVarNmNf, gVarNm,  refVarNm
    character (len=clen), dimension(:), pointer :: lstAxis, lstSilamQ, lstVType,  &
                                                   & lstDimType, lstSubst
    character, dimension(:), pointer :: arrChTmp
    real, dimension(:), pointer :: fAtt, lstlevValue, lstMode, lstWavelen, lstFactor, lstOffset, vals
    real(kind=8), dimension(:), pointer :: f8arrPtr
    real, dimension(:,:), pointer :: lons2Dptr, lats2Dptr
    integer, dimension(:), pointer :: gVarId, refVarId
    ...
    real ::  fTmp, x1, y1, x2, y2, dx, dy, wavelength, mean_diameter, bottom, thickness
    real(r8k) :: jDate
   ...
   character (len=*), parameter :: sub_name='open_netcdf_file_i'
    ...
    nullify(lstDimName,lstAxis,lstDimVar,lstDimType,lstVarName,lstSilamQ,lstVType,lstlevValue,lstSubst,&
            &lstMode,lstWavelen,lstFactor,lstOffset,gVarNmNf,gVarNm,gVarId,refVarNm,refVarId )
    ...
    if(ifFound)then !Proceed with the nametable
      ...
      if(nDims > 0)then
        ...
        do iTmp = 1, nDims
           ...
           read(unit=spContent%sp, iostat=iStat, fmt=*) lstDimName(iTmp), lstAxis(iTmp), lstDimVar(iTmp), lstDimType(iTmp)
        enddo
      else
        nullify(lstDimName)
        nullify(lstAxis)
      ...
      if (nVals > 0)then
        do iTmp = 1, nVals
          spContent%sp = fu_content(ptrItems(iTmp))
          jTmp = index(spContent%sp, " ")
          do iDim = 1, nf%n_dims
            if (fu_str_u_case(adjustl(nf%nDims(iDim)%dimName)) == fu_str_u_case(spContent%sp(1:jTmp-1)))then
             call split_string(spContent%sp(jTmp+1:), ' ', fAtt, iVar)
      ...
      if (nVals > 0)then
        do iTmp = 1, nVals
          spContent%sp = fu_content(ptrItems(iTmp))
          read(unit=spContent%sp, iostat=iStat, fmt=*) chTmp, fTmp
          do iDim = 1, nf%n_dims
      ...
      if (nVals > 0)then
        do iTmp = 1, nVals
          spContent%sp = fu_content(ptrItems(iTmp))
          read(unit=spContent%sp, iostat=iStat, fmt=*) chTmp, chTmp2
          do iDim = 1, nf%n_dims
            if (fu_str_u_case(adjustl(nf%nDims(iDim)%dimName)) == fu_str_u_case(adjustl(chTmp)))then
      ...
      if(nVals > 0)then
        ...
        do iTmp = 1, nVals
          spContent%sp = fu_content(ptrItems(iTmp))
          read(unit=spContent%sp, iostat=iStat, fmt=*) gVarNmNf(iTmp), gVarNm(iTmp), refVarNm(iTmp)
      ...
      if(nVals > 0)then
        ...
        do iTmp = 1, nVals
          spContent%sp = fu_content(ptrItems(iTmp))
          read(unit=spContent%sp, iostat=iStat, fmt=*) lstVarName(iTmp), lstSilamQ(iTmp), &
        ...
        if(fu_str_l_case(adjustl(nf%nDims(iTmp)%axis)) == 't')then
          ...
          if(nf%nVars(nf%nDims(iTmp)%varId)%xType == nf90_char)then 
            ...
            if(iT > 0)then
              ...
              if(index(chTmp, 'simulation') > 0)then  ! Nonstandard time unit from WRF - hours since simulation start
                if(nf%chTStart == '')then
                  call set_error('Simulation start time missing', sub_name)
                  return
                else
                  nf%nTime%tDimStart = nf%nTime%analysis_time
                endif
              else
          ...
          elseif( chTmp == 'HYBRID_LYR')then
            ...
            do iLev = 1, nf%nDims(iTmp)%dimLen
              lstLevs(ilev) = fu_set_layer_between_two(layer_btw_2_hybrid, ilev, ilev-1, &
                        & nf%nDims(iTmp)%a_half(iLev+1), nf%nDims(iTmp)%b_half(iLev+1), &
                        & nf%nDims(iTmp)%a_half(iLev), nf%nDims(iTmp)%b_half(iLev))
            enddo  
            lstLevs(nf%nDims(iTmp)%dimlen+1) = level_missing
            call set_vertical(lstLevs, nf%nDims(iTmp)%sVert)
...
!            call msg(fu_connect_strings('Nr of dims for: ', nf%nDims(iTmp)%varName), nf%nVars(nf%nDims(iTmp)%varid)%n_dims)
   ...
   ! Set silam verticals and grid pointers for variables
    allocate(nf%ngrids(max_nbr_of_grids_in_file))
...
!!                                  nf%nGrids(iTmp)%sPole_lat and nf%nGrids(iTmp)%sPole_lon are wrong here!
            ...
            do iVar = 1, size(gVarId)
              ...
              if(any(nf%nvars(gVarId(iVar))%dimIds == nf%nGrids(iTmp)%lonDimId) .and. &
               & any(nf%nvars(gVarId(iVar))%dimIds == nf%nGrids(iTmp)%latDimId))then
                ...
                call read_2d_gridvar(nf, gVarId(iVar), gVarNm(iVar), refVarId(iVar), refVarNm(iVar), vals)
                if(gVarNm(iVar) == "dx") vals(1:nf%nGrids(iTmp)%nx*nf%nGrids(iTmp)%ny) = vals(1:nf%nGrids(iTmp)%nx*nf%nGrids(iTmp)%ny)*dx
                if(gVarNm(iVar) == "dy") vals(1:nf%nGrids(iTmp)%nx*nf%nGrids(iTmp)%ny) = vals(1:nf%nGrids(iTmp)%nx*nf%nGrids(iTmp)%ny)*dy
    ...
    if(associated(lstDimName))deallocate(lstDimName)
    if(associated(lstAxis))   deallocate(lstAxis)
    if(associated(lstDimVar)) deallocate(lstDimVar)
    if(associated(lstDimType))deallocate(lstDimType)

    if(associated(lstVarName)) deallocate(lstVarName) 
    if(associated(lstSilamQ)) deallocate(lstSilamQ)
    if(associated(lstVType))  deallocate(lstVType)
    if(associated(lstlevValue))deallocate(lstlevValue)
    if(associated(lstSubst)) deallocate(lstSubst)
    if(associated(lstMode)) deallocate(lstMode)
    if(associated(lstWavelen))deallocate(lstWavelen)
    if(associated(lstFactor))deallocate(lstFactor)
    if(associated(lstOffset))deallocate(lstOffset)

    if(associated(gVarNmNf))deallocate(gVarNmNf)
    if(associated(gVarNm))deallocate(gVarNm)
    if(associated(gVarId))deallocate(gVarId)
    if(associated(refVarNm))deallocate(refVarNm)
    if(associated(refVarId))deallocate(refVarId)
...
! ===============================================================================
    subroutine read_2d_gridvar(nf, varId, varNm, refVarId, refVarNm, vals)
      ...
      subroutine parse_times(time_start, increments, num_times, incr_unit, calendar_type, times_parsed)
        ...
        case('standard','gregorian','proleptic_gregorian','proleptic','noleap')
          !
          ! Normal calendar
          !
        ...
        case( '365_day', 'all_leap', '366_day', '365_days')
          !
...
!!! Dirty hack for concentrations packed as integers
  ...
  subroutine close_netcdf_file(inf)
    ...
    if(associated(nf%nGrids))then
      do i = 1, size(nf%nGrids)
        if(associated(nf%nGrids(i)%sGrid))then
          deallocate(nf%nGrids(i)%sGrid)
        endif
      enddo
      deallocate(nf%nGrids)
    endif
    ...
    nf%grTemplate = template_missing
    nf%unit_bin=-1
    nf%unit_ctl=-1
    ...
    nf%n_vars=0
    nf%n_levs=0
    nf%levType = int_missing       ! Type of 3D levels in the file
  ...
  !
    ...
    if(associated(nf%nGrids))then
      if(associated(nf%nGrids(1)%sGrid))then
...
!***************************************************************************
  ...
  subroutine id_list_from_netcdf_file(iUnit, idList, iCount, ifFirstTime, ifFirstLev)

    implicit none
    integer, intent(in) :: iUnit
    integer, intent(out) :: iCount
    logical, intent(in) :: ifFirsttime, ifFirstLev
  
    type(silja_field_id),dimension(:), pointer :: idList
    type(netcdf_file),pointer :: nf
    type(silja_interval) :: length_of_accumulation, fc_length
    integer :: iVar, iLev, iTime, field_kind, nFields, ntimes, nlevs
    ...
    do iVar = 1, nf%n_vars
      ...
      do iLev = 1, nlevs
        do iTime = 1, ntimes
         iCount = iCount + 1
         call id_for_nc_index(iUnit, idList(iCount), iVar, iTime, iLev)
...
!***************************************************************************
  ...
  subroutine expand_output_list(Lst, nNewItems)
    !
    ! Expands the list of output variables
    !
    implicit none
