...
!end do
    ...
    nullify(species)
    nSpecies = 0
    call add_inventory_volc_src(v_src, species, nSpecies)
    if(error)return
...
!call msg('in')
  ...
  subroutine add_inventory_volc_src(v_src, species_list, nSpecies)
    !
    ...
    do iDesc = 1, v_src%nDescriptors
      call get_inventory(v_src%cocktail_descr_lst(iDesc), speciesPtr, n)  ! Getting the pointers!!
      if(error)return
      call addSpecies(species_list, nSpecies, speciesPtr, n)
      if(error)return
    end do
  end subroutine add_inventory_volc_src
