MODULE source_terms_wind_blown_dust
  ...
  public add_source_species_wb_dust_src
...
! call msg('wind, z/z0, gust forcing factor:' + fu_str(fWind), fZ_z0, fu_gust_integral/fWind**fPwr)
  ...
  subroutine add_source_species_wb_dust_src(wb_dust_src, species_list, nSpecies)
    !
  ...
  end subroutine add_source_species_wb_dust_src
...
!      return
  ...
  subroutine compute_emission_for_wb_dust(wb_dust_src, &
    ...
    real, dimension(:), pointer :: ptrXSzDisp, ptrYSzDisp, pSoilVolumeWater, &
                                 ...
                                 & pErodibleFraction, pU10m, pV10m, pW10m, pU_star, pLAI, &
...
!    if(fu_fails(fu_index(met_buf, soil_moisture_content_flag, pSoilVolumeWater) /= int_missing, &  ! soil water content, m3/m3
    ...
    !Rather use directly the 10 m windspeed since not all meteo provided the components:
    if(fu_fails(fu_index(met_buf, windspeed_10m_flag, pW10m) /= int_missing, &                  ! windspeed at 10m, m/s
                                & 'Failed 10m windspeed','compute_emission_for_wb_dust'))return
...
iArStatistics(4) = iArStatistics(4) + 1
        ...
        select case(wb_dust_src%emisMethod)
          case(simple_dust_flag)
            ...
            fWindSpeed = pW10m(iMeteo) !Use directly the 10m windspeed
