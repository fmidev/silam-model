...
CONTAINS
  ...
  subroutine fill_wb_dust_src_from_namelist(nlSetup, srcWBDust, expected_species, src_data_dir)
    ...
    if(error)return
!    if(nFiles < 1)then
!      call set_error('No supplementary files are given','fill_wb_dust_src_from_namelist')
!      return
!    endif
    !
    ! Files are not checked for existence when expanding system variables and hats: the string
    ! contains the file format
    !
    do iTmp = 1, nFiles
      call add_namelist_item(srcWBDust%nlInputFiles, 'supplementary_file', &
    ...
    !
    call add_namelist_item(srcWBDust%nlInputFiles, 'source_area_mask', &
  ...
  subroutine add_input_needs_wb_dust_src(wb_dust_src, q_met_dynamic, q_met_st, &
    ...
    select case(wb_dust_src%emisMethod)
      ...
      case default
        call set_error('Unknown emission computation algorythm', 'input_needs_wb_dust_source')
        return
    end select
...
!      return
    ...
    if(i /= 0)then
      call set_error('Failed to allocate dispersion-vertical level fractions','project_wbdust_src_second_grd')
      return
    endif
  ...
  subroutine compute_emission_for_wb_dust(wb_dust_src, &
    ...
    real, parameter :: fSandDensity = 2600, &
                     ...
                     & sqrt_density_air = sqrt(density_air)
