...
!!!!!!!!!!!!!#define XXX ABCDR   Uncoment to enable buggy time difference
 ...
 integer(8), parameter, private :: TM_YEAR_BASE = 1900  !Dull legacy of unix struct_tm
  ...
  ! current offset, to be updated every diagnostic
  integer, public, dimension(:), allocatable :: TZ_offset ! localtime(i) = UTC_time + TZ_offset(i)
  integer, public, dimension(:), allocatable :: TZ_special_date_now !0 for nothing, DOW
...
!  integer, private, parameter :: mean_timezone_method = 66601
  ...
  !!! Number of specal dates for timezones (nvals)
  integer, private, dimension(:), allocatable :: special_date_dates, special_date_timezone, special_date_type
...
!    END IF
  ...
  FUNCTION fu_io_string_to_time (string) result(time)
    ...
    ELSE
      ...
      IF (io_status /= 0) THEN
        CALL set_error('Something strange in given string:' + string, 'fu_io_string_to_time')
        RETURN
      END IF
      ...
      time = fu_set_time_utc(components(1),&
                           ...
                           & sec)
      IF (error) THEN
        CALL set_error('Cannot make time out of:' + string, 'fu_io_string_to_time')
      END IF
    END IF
...
!******************************************************************************
 ...
 function timezone_index_by_name(myTZName) result (i)

   CHARACTER (LEN=*), INTENT(in) :: myTZName
   integer :: i

   do i = 1, number_of_time_zones
     if (trim(adjustl(TZ_name(i))) == trim(adjustl(myTZname))) exit
   end do
   if (i>number_of_time_zones) then
     i = int_missing
     call msg_warning("Can't find index for timezone '"//trim(myTZName)//"'", &
 ...
 subroutine init_timezones(nlStdSetup) ! initializes timezones form a file
    ...
    character(len=*), parameter :: sub_name = 'init_timezones'
   ...
   !             timezone_method = mean_timezone_method
        ...
        case default
                call set_error("Strange timezone method" + &
                        &fu_content(nlStdSetup,'timezone_method'), sub_name )
    end select


    ! No timezone file -- only UTC zone 
    fname = fu_content(nlStdSetup,'timezone_list_fnm') 
    if (fname /= '') then

            ! Read the timezone table file. 
            iTmp = fu_next_free_unit()
            open(file=fname, unit=iTmp, action='read', status='old', iostat=iStat)
            if(iStat /= 0)then
              call set_error('Failed to open timezone list: '//trim(fname), sub_name)
              return
            endif
            ...
            if(.not. associated(nmTblNlGrp))then
              call msg_warning('TIMEZONE INDEX namelist group is not associated',sub_name)
            endif

            if (fu_nbr_of_namelists(nmTblNlGrp) /= 1) then
                call set_error("Only one timezone index is allowed",sub_name)
                return
            endif
            ...
            if(nVals < 1)then
                 call set_error('No zones in namelist',sub_name)
                 return
            endif
      ...
      allocate(TZ_offset_summer(number_of_time_zones), &
              ...
              & TZ_offset(number_of_time_zones), &
              & TZ_special_date_now(number_of_time_zones),stat = iStat)
      if(iStat /= 0)then
        call set_error('Failed to allocate TZ arrays :', sub_name)
        return
      endif
      TZ_special_date_now(:) = 0 !! No special dates by default
     ...
     if (nVals == 0) then
       ...
       do iZone = 1, nVals
            ...
            if (iZone /= iInd) then
              ...
              call set_error('Wrong timezone index :', sub_name)
              return
            endif
...
!            call msg( "")
    ...
    fname=fu_content(nlStdSetup,'holiday_list_fnm')
    if (fname /= '') then
      call init_holidays(fname)
    endif
 ...
 subroutine init_holidays(fname) ! initializes holidays from a file
     !
     ! File format: YYYY-mm-dd   TZ/Name  ForceDOW
     ! special_date = 2017-01-06            Europe/Zurich 7

    implicit none
    character(len=*) :: fname
    type (silja_time) :: time
    character(len=fnlen) :: timestr
    character(len=1024) :: chTZName, chDate, chContent
    integer :: iTmp, iStat, nVals, YYYY,mm,dd, iType, jTmp, kTmp
    type(Tsilam_namelist), pointer :: nlPtr
    type(Tsilam_nl_item_ptr), dimension(:), pointer :: ptrItems
    integer, dimension(:), allocatable :: DatesTmp, iDatesTmp
    character(len=*), parameter :: sub_name = 'init_holidays'
            ...
            nlPtr => fu_read_namelist(iTmp, .false.)
            close(iTmp)
            IF (error) RETURN

            ! Process timezone list
            if(.not. associated(nlPtr))then
              call msg_warning('Holidays namelist  is not associated',sub_name)
            endif

            nullify(ptrItems)
            call get_items(nlPtr, 'special_date', ptrItems, nVals)
            if(nVals < 1)then
                 call report(nlPtr)
                 call set_error('No special_date in namelist',sub_name)
                 return
            endif

            allocate(special_date_dates(nVals), special_date_timezone(nVals), special_date_type(nVals),&
               & DatesTmp(nVals), iDatesTmp(nVals), stat=iStat)
            if(iStat /= 0)then
              call set_error('Failed to allocate TZ arrays :', sub_name)
              return
            endif
            ...
            do iTmp = 1, nVals
                chContent = fu_content(ptrItems(iTmp))
                read(unit=chContent,fmt='((A10))',iostat=iStat) chDate
                !!! YYYY-mm-dd
                if ((chDate(5:5) == '-') .and. (chDate(8:8) == '-')  ) then
                  read(unit=chDate,fmt='((I4),(X),(I2),(X),(I2))',iostat=iStat) YYYY,mm,dd 
                else
                  call set_error("Failed to parse special_date from '"//trim(chContent)//"'", sub_name)
                  return
                endif
                time = fu_set_time_utc(YYYY,mm,dd, 0, 0, 0.)
                DatesTmp(iTmp) = nint(silja_time_to_real8(time) / seconds_in_day)
                iDatesTmp(iTmp) = iTmp
            enddo

            call  argMergeSort_int(DatesTmp, iDatesTmp)


            do iTmp = 1, nVals ! Loop over ordered dataset
                kTmp = iDatesTmp(iTmp) !! Source index
                chContent = fu_content(ptrItems(kTmp))
                chContent = adjustl(chContent(11:))
                jTmp=index(chContent,' ')
                read(unit=chContent(1:jTmp-1),fmt='((A))',iostat=iStat) chTZName
                read(unit=chContent(jTmp+1:),fmt='(I10)',iostat=iStat) iType
                special_date_dates(iTmp) = DatesTmp(kTmp) !Date parsed already
                special_date_timezone(iTmp) = timezone_index_by_name(chTZName)
                special_date_type(iTmp) = iType
            enddo


            !do iTmp = 1, nVals ! Loop over the items in original order
            !    jTmp = special_date_timezone(iTmp)
            !    write (*,*) fu_str(epoch + one_day*special_date_dates(iTmp)), trim(TZ_name(jTmp)), special_date_dates(iTmp), special_date_type(iTmp)
            !enddo
            if (any(special_date_timezone(:)<0)) then
              call set_error("Failed to find some timezones '"//trim(chContent)//"'", sub_name)
              return
            endif

            deallocate(DatesTmp, iDatesTmp)
!           

 endsubroutine init_holidays! initializes holidays from a file
 ...
 subroutine update_timezone_offsets(time)
    ...
    real :: s
    real(r8k) :: rTimeUtc
    integer :: iZone, iDayUtc, iTmp, iTz, localday
    logical :: Winter
...
!                call msg("No timezone update method: winter/summer time")
    ...
    if (allocated(special_date_dates)) then
      TZ_special_date_now(1:number_of_time_zones) = 0  !reset them
      rTimeUtc = silja_time_to_real8(time) 
      iDayUtc = floor(rTimeUtc / seconds_in_day)

      ! loop over  dates
      do iTmp = 1,size(special_date_dates)
        if (special_date_dates(iTmp)  < iDayUtc - 1) continue
        if (special_date_dates(iTmp)  > iDayUtc + 1) exit !! We are far in the future
        iTz = special_date_timezone(iTmp) !!Timezone for this soecial date
        localday = floor((rTimeUtc + TZ_offset(iTz))/seconds_in_day)
        if (special_date_dates(iTmp) == localday) &
             & TZ_special_date_now(iTz) = special_date_type(iTmp)
