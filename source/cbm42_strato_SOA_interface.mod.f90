module cbm42_strato_SOA_interface
  ...
  character(len=substNmLen), dimension(num_species), parameter, private :: &
       & subst_names = (/ &
                      'BrNO2  ','CCl3F  ','CCl2F2 ','CCl4   ','NTR    ','AVB0   ', &
& 'AVB1e0 ','AVB1e1 ','AVB1e2 ','AVB1e3 ','AVB1e4 ','AVB1e5 ', &
& 'AVB1e6 ','BVB0   ','BVB1e0 ','BVB1e1 ','BVB1e2 ','BVB1e3 ', &
& 'Br2    ','CH3CCl3','MEOH   ','ETOH   ','CRO    ','BrONO2 ', &
& 'BrCl   ','PAN    ','CH3Br  ','TOL    ','Cl2O2  ','N2O5   ', &
& 'XYL    ','N      ','N2O    ','HONO   ','TO2    ','ClNO2  ', &
& 'HOBr   ','PNA    ','CH3Cl  ','ROR4   ','MGLY   ','H2O2   ', &
& 'CRES   ','ETH    ','HNO3   ','CO     ','OPEN   ','Cl2    ', &
& 'XO2N   ','ClONO2 ','HOCl   ','HBr    ','HCl    ','OLE4   ', &
& 'C5H8_2 ','XO2    ','PAR4   ','C2O3   ','ISPD   ','C5H8   ', &
& 'ALD2   ','H      ','OClO   ','NO3    ','Br     ','OH     ', &
& 'O3     ','HO2    ','Cl     ','O      ','O1D    ','NO     ', &
& 'NO2    ','ClO    ','BrO    ','HCHO   ' &
                      & /)
...
contains
   ...
   subroutine set_rates_cbm42_strato_SOA(photoarr, temp, cAir, press, cH2O&
    ...
    RCONST(209) = (3.60E-11*ifMTprods)
    RCONST(210) = (ARRZ(1.5d-11,-449.0d0)*ifMTprods)
    RCONST(211) = (ARRZ(1.2d-15,821.0d0)*ifMTprods)
    RCONST(212) = (ARRZ(3.7d-12,-175.0d0)*ifMTprods)
    RCONST(213) = (3.60E-11*(1.0-ifMTprods))
    RCONST(214) = (ARRZ(1.5d-11,-449.0d0)*(1.0-ifMTprods))
    RCONST(215) = (ARRZ(1.2d-15,821.0d0)*(1.0-ifMTprods))
    RCONST(216) = (ARRZ(3.7d-12,-175.0d0)*(1.0-ifMTprods))
    RCONST(217) = (ARRZ(1.8d-12,-340.0d0)*SOA_b)
    RCONST(218) = (1.51e-11*SOA_b)
    RCONST(219) = (3.60E-11*SOA_b)
    RCONST(220) = (ARRZ(2.7d-11,-390.0d0)*SOA_b)
    RCONST(221) = (ARRZ(1.05d-14,2000.0d0)*SOA_b)
    RCONST(222) = (ARRZ(2.95d-12,450.0d0)*SOA_b)
    RCONST(223) = (3.60E-11*SOA_b)
    RCONST(224) = (ARRZ(1.5d-11,-449.0d0)*SOA_b)
    RCONST(225) = (ARRZ(1.2d-15,821.0d0)*SOA_b)
    RCONST(226) = (ARRZ(3.7d-12,-175.0d0)*SOA_b)
    RCONST(227) = (ARRZ(1.8d-12,-340.0d0)*(1.0-SOA_b))
    RCONST(228) = (1.51e-11*(1.0-SOA_b))
    RCONST(229) = (3.60E-11*(1.0-SOA_b))
    RCONST(230) = (ARRZ(2.7d-11,-390.0d0)*(1.0-SOA_b))
    RCONST(231) = (ARRZ(1.05d-14,2000.0d0)*(1.0-SOA_b))
    RCONST(232) = (ARRZ(2.95d-12,450.0d0)*(1.0-SOA_b))
    RCONST(233) = (3.60E-11*(1.0-SOA_b))
    RCONST(234) = (ARRZ(1.5d-11,-449.0d0)*(1.0-SOA_b))
    RCONST(235) = (ARRZ(1.2d-15,821.0d0)*(1.0-SOA_b))
    RCONST(236) = (ARRZ(3.7d-12,-175.0d0)*(1.0-SOA_b))
    RCONST(237) = (ASOA_aging)
    RCONST(238) = (ASOA_aging)
    RCONST(239) = (ASOA_aging)
    RCONST(240) = (ASOA_aging)
    RCONST(241) = (IVOC_aging)
    RCONST(242) = (IVOC_aging)
    RCONST(243) = (IVOC_aging)
    RCONST(244) = (BSOA_aging)
    RCONST(245) = (BSOA_aging)
    RCONST(246) = (BSOA_aging)
    RCONST(247) = (BSOA_aging)
