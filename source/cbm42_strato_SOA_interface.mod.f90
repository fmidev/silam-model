module cbm42_strato_SOA_interface
  use cbm42_strato_SOA_Parameters
  use cbm42_strato_SOA_Integrator, only : rosenbrock, init_solver
  use cbm42_strato_SOA_Global
  use cbm42_strato_SOA_Precision
  use cbm42_strato_SOA_adj_Integrator, only : rosenbrockAdj, init_solver_adj
  use cocktail_basic
  use photolysis
  !$use omp_lib
  implicit none
  ...
  public rosenbrock
  public set_rates_cbm42_strato_SOA
  public set_const_rates_cbm42_strato_SOA
  public set_fixed_cbm42_strato_SOA
  public register_species_int
  public inventory_int
  public init_solver
  public report_rates_cbm42_strato_SOA
  ...
  integer, parameter, public :: num_species_cbm42_strato_SOA = num_species ! just for laziness
  integer, parameter, public :: num_react_cbm42_strato_SOA = nreact
  integer, parameter, public :: num_fixed_cbm42_strato_SOA = nfix
  integer, parameter, public :: precision_cbm42_strato_SOA = sp ! from Precision module
  character(len=substNmLen), dimension(num_species), parameter, private :: &
       & subst_names = (/ &
                      'Br2    ','BrNO2  ','CCl3F  ','CCl2F2 ','CCl4   ','AVB0   ', &
& 'AVB1e0 ','AVB1e1 ','AVB1e2 ','AVB1e3 ','AVB1e4 ','AVB1e5 ', &
& 'AVB1e6 ','BVB0   ','BVB1e0 ','BVB1e1 ','BVB1e2 ','BVB1e3 ', &
& 'CH3CCl3','CRO    ','BrONO2 ','BrCl   ','PAN    ','TOL    ', &
& 'CH3Cl  ','CH3Br  ','Cl2O2  ','N2O5   ','XYL    ','N      ', &
& 'N2O    ','HONO   ','XO2N   ','TO2    ','ClNO2  ','HOBr   ', &
& 'PNA    ','ROR    ','MGLY   ','H2O2   ','CRES   ','HNO3   ', &
& 'CO     ','Cl2    ','ETH    ','OPEN   ','XO2    ','ClONO2 ', &
& 'HOCl   ','PAR    ','C5H8_2 ','HBr    ','HCl    ','C2O3   ', &
& 'C5H8   ','OLE    ','ALD2   ','H      ','OClO   ','NO3    ', &
& 'Br     ','O      ','OH     ','O3     ','HO2    ','O1D    ', &
& 'Cl     ','ClO    ','BrO    ','NO     ','NO2    ','HCHO   ' &
                      & /)
  type(silam_species), dimension(num_species), public, target, save :: species_cbm42_strato_SOA
...
contains
  ...
  !subroutine set_rates_cbm42_strato_SOA(photoarr, temp, cAir, press, cH2O)
  subroutine set_rates_cbm42_strato_SOA(photoarr, temp, cAir, press, cH2O, &
    ...
    RCONST(196) = (3.60E-11*ifMTprods)
    RCONST(197) = (ARRZ(1.5d-11,-449.0d0)*ifMTprods)
    RCONST(198) = (ARRZ(1.2d-15,821.0d0)*ifMTprods)
    RCONST(199) = (ARRZ(3.7d-12,-175.0d0)*ifMTprods)
    RCONST(200) = (3.60E-11*(1.0-ifMTprods))
    RCONST(201) = (ARRZ(1.5d-11,-449.0d0)*(1.0-ifMTprods))
    RCONST(202) = (ARRZ(1.2d-15,821.0d0)*(1.0-ifMTprods))
    RCONST(203) = (ARRZ(3.7d-12,-175.0d0)*(1.0-ifMTprods))
    RCONST(204) = (ARRZ(1.8d-12,-340.0d0)*SOA_b)
    RCONST(205) = (ARRZ(1.7d-11,-116.0d0)*SOA_b)
    RCONST(206) = (3.60E-11*SOA_b)
    RCONST(207) = (ARRZ(2.7d-11,-390.0d0)*SOA_b)
    RCONST(208) = (ARRZ(1.03d-14,1995.0d0)*SOA_b)
    RCONST(209) = (ARRZ(3.15d-12,450.0d0)*SOA_b)
    RCONST(210) = (3.60E-11*SOA_b)
    RCONST(211) = (ARRZ(1.5d-11,-449.0d0)*SOA_b)
    RCONST(212) = (ARRZ(1.2d-15,821.0d0)*SOA_b)
    RCONST(213) = (ARRZ(3.7d-12,-175.0d0)*SOA_b)
    RCONST(214) = (ARRZ(1.8d-12,-340.0d0)*(1.0-SOA_b))
    RCONST(215) = (ARRZ(1.7d-11,-116.0d0)*(1.0-SOA_b))
    RCONST(216) = (3.60E-11*(1.0-SOA_b))
    RCONST(217) = (ARRZ(2.7d-11,-390.0d0)*(1.0-SOA_b))
    RCONST(218) = (ARRZ(1.03d-14,1995.0d0)*(1.0-SOA_b))
    RCONST(219) = (ARRZ(3.15d-12,450.0d0)*(1.0-SOA_b))
    RCONST(220) = (3.60E-11*(1.0-SOA_b))
    RCONST(221) = (ARRZ(1.5d-11,-449.0d0)*(1.0-SOA_b))
    RCONST(222) = (ARRZ(1.2d-15,821.0d0)*(1.0-SOA_b))
    RCONST(223) = (ARRZ(3.7d-12,-175.0d0)*(1.0-SOA_b))
    RCONST(224) = (ASOA_aging)
    RCONST(225) = (ASOA_aging)
    RCONST(226) = (ASOA_aging)
    RCONST(227) = (ASOA_aging)
    RCONST(228) = (IVOC_aging)
    RCONST(229) = (IVOC_aging)
    RCONST(230) = (IVOC_aging)
    RCONST(231) = (BSOA_aging)
    RCONST(232) = (BSOA_aging)
    RCONST(233) = (BSOA_aging)
    RCONST(234) = (BSOA_aging)
...
!!$    end function photo_var
  ...
  end subroutine set_rates_cbm42_strato_SOA

  subroutine report_rates_cbm42_strato_SOA()
    implicit none
  ...
  end subroutine report_rates_cbm42_strato_SOA

  subroutine set_const_rates_cbm42_strato_SOA()
    implicit none
        RCONST(11) = 2.14e-10
    RCONST(19) = 1e-22
    RCONST(22) = 4.41e-40
    ...
    RCONST(134) = 4e-12
    RCONST(161) = 1.5e-10
  end subroutine set_const_rates_cbm42_strato_SOA

  !subroutine set_fixed_cbm42_strato_SOA(cnc_m, cnc_H2O,cnc_H2,cnc_O2,cnc_CH4,cnc_CO2,cnc_M)
  subroutine set_fixed_cbm42_strato_SOA(cnc_H2O,cnc_H2,cnc_O2,cnc_CH4,cnc_CO2,cnc_M)
    implicit none
...
fix(6) = cnc_M
  ...
  end subroutine set_fixed_cbm42_strato_SOA

  subroutine init_chemicals_cbm42_strato_SOA()
    implicit none

    integer :: i

    do i = 1, num_species_cbm42_strato_SOA
      call set_species(species_cbm42_strato_SOA(i), fu_get_material_ptr(subst_names(i)), in_gas_phase)
      if (error) return
    end do

  end subroutine init_chemicals_cbm42_strato_SOA
  ...
  subroutine inventory_int(flag_transf, &
    ...
    call addSpecies(speciesTransp, nSpeciesTransp, species_cbm42_strato_SOA, num_species, .true.)
    if(error)return
    ...
    do iEmis = 1, nSpeciesEmis
      if(fu_index(speciesEmis(iEmis), species_cbm42_strato_SOA, num_species) > 0)then
        if(iClaimedSpecies(iEmis) < 0)then
          call msg('cbm42_strato_SOA owns:' + fu_substance_name(speciesEmis(iEmis)))
  ...
  !                            & nspeciesTransp, nspeciesShortlived, nspeciesAerosol)                     !Without SOA
    ...
    do i = 1, num_species
      if (.not. fu_index(species_cbm42_strato_SOA(i), speciesTransp) == i) then
        call msg('Looking for:')
        call report(species_cbm42_strato_SOA(i))
        call msg('Index in cbm42_strato_SOA, transport:', i, fu_index(species_cbm42_strato_SOA(i), speciesTransp))
        call set_error('Incompatible ordering of transport species', &
                     & 'register_species_int')
        return
      end if
      if (fu_name(fu_material(species_cbm42_strato_SOA(i)))=='NO') iNO = i
      if (fu_name(fu_material(species_cbm42_strato_SOA(i)))=='XO2') iXO2 = i
      if (fu_name(fu_material(species_cbm42_strato_SOA(i)))=='HO2') iHO2 = i
    end do  ! species
...
end module cbm42_strato_SOA_interface
