...
! 
! File                 : cbm42_strato_SOA_adj_Integrator.f90
! Time                 : Fri Apr 23 13:17:44 2021
! Working directory    : /fmi/scratch/project_2001411/risto/silam_v5_7/kpp/cbm42_strato_SOA
! Equation file        : cbm42_strato_SOA_adj.kpp
! Output root filename : cbm42_strato_SOA_adj
! 
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
MODULE cbm42_strato_SOA_adj_Integrator

   USE cbm42_strato_SOA_Precision
   USE cbm42_strato_SOA_Parameters
   USE cbm42_strato_SOA_Global
   USE cbm42_strato_SOA_LinearAlgebra
   !USE cbm42_strato_SOA_adj_Rates
   USE cbm42_strato_SOA_Function
   USE cbm42_strato_SOA_Jacobian
   USE cbm42_strato_SOA_Hessian
   !<USE cbm42_strato_SOA_adj_Util
...
!~~~>  Statistics on the work performed by the Rosenbrock method
   ...
   REAL(kind=sp) :: ros_A(15), ros_C(15), ros_M(6), ros_E(6), &
        ros_Alpha(6), ros_Gamma(6), ros_ELO
   LOGICAL :: ros_NewF(6)
   CHARACTER(LEN=12) :: ros_Name
   INTEGER, PARAMETER :: bufsize = 200000
   REAL(kind=sp), DIMENSION(:),   allocatable :: chk_H, chk_T
   REAL(kind=sp), DIMENSION(:,:), allocatable :: chk_Y, chk_K, chk_J
   REAL(kind=sp), DIMENSION(:,:), allocatable :: chk_dY, chk_d2Y
   REAL(kind=sp), DIMENSION(:), allocatable :: Ystage
  ...
  REAL(kind=sp), parameter :: roundoff = 5.96046448E-08

CONTAINS ! Routines in the module cbm42_strato_SOA_adj_Integrator
...
!~~~>  Arguments   
   REAL(kind=sp), INTENT(INOUT) :: Y(NVAR)
   INTEGER, INTENT(IN)          :: NADJ
   REAL(kind=sp), INTENT(INOUT) :: Lambda(NVAR,NADJ)
   REAL(kind=sp), INTENT(IN)    :: Tstart,Tend
   REAL(kind=sp), INTENT(IN)    :: AbsTol(NVAR),RelTol(NVAR)
   REAL(kind=sp), INTENT(IN)    :: AbsTol_adj(NVAR,NADJ), RelTol_adj(NVAR,NADJ)
   INTEGER, INTENT(IN)          :: ICNTRL(20)
   REAL(kind=sp), INTENT(IN)    :: RCNTRL(20)
   INTEGER, INTENT(INOUT)       :: ISTATUS(20)
   REAL(kind=sp), INTENT(INOUT) :: RSTATUS(20)
   INTEGER, INTENT(OUT)         :: IERR
...
!~~~>  Local variables     
   REAL(kind=sp) :: FacMin, FacMax, FacRej, FacSafe
   REAL(kind=sp) :: Hmin, Hmax, Hstart
   REAL(kind=sp) :: Texit
   INTEGER :: i, UplimTol, Max_no_steps
   INTEGER :: AdjointType, CadjMethod 
   LOGICAL :: Autonomous, VectorTol
!~~~>   Parameters
   REAL(kind=sp), PARAMETER :: ZERO = 0.0d0, ONE  = 1.0d0
   REAL(kind=sp), PARAMETER :: DeltaMin = 1.0d-5
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   REAL(kind=sp) :: T, H, Ystage(NVAR*S), K(NVAR*S)
   INTEGER       :: P(NVAR)
#ifdef FULL_ALGEBRA
   REAL(kind=sp) :: E(NVAR,NVAR)
#else       
   REAL(kind=sp) :: E(LU_NONZERO)
#endif
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   REAL(kind=sp) :: T, H, Y(NVAR), dY(NVAR), d2Y(NVAR)
...
!~~~> Output: Error indicator
   INTEGER, INTENT(OUT) :: IERR
! ~~~~ Local variables        
   REAL(kind=sp) :: Ynew(NVAR), Fcn0(NVAR), Fcn(NVAR) 
   REAL(kind=sp) :: K(NVAR*ros_S), dFdT(NVAR)
#ifdef FULL_ALGEBRA    
   REAL(kind=sp) :: Jac0(NVAR,NVAR),  Ghimj(NVAR,NVAR)
#else
   REAL(kind=sp) :: Jac0(LU_NONZERO), Ghimj(LU_NONZERO)
#endif
   REAL(kind=sp) :: H, Hnew, HC, HG, Fac, Tau 
   REAL(kind=sp) :: Err, Yerr(NVAR)
...
!~~~> Output: Error indicator
   INTEGER, INTENT(OUT) :: IERR
! ~~~~ Local variables        
   REAL(kind=sp) :: Ystage(NVAR*ros_S), K(NVAR*ros_S)
   REAL(kind=sp) :: U(NVAR*ros_S,NADJ), V(NVAR*ros_S,NADJ)
#ifdef FULL_ALGEBRA
   REAL(kind=sp), DIMENSION(NVAR,NVAR)  :: Jac, dJdT, Ghimj
#else
   REAL(kind=sp), DIMENSION(LU_NONZERO) :: Jac, dJdT, Ghimj
#endif
   REAL(kind=sp) :: Hes0(NHESS)
   REAL(kind=sp) :: Tmp(NVAR), Tmp2(NVAR)
   REAL(kind=sp) :: H, HC, HA, Tau 
...
!!$!~~~> Input: the initial condition at Tstart; Output: the solution at T   
!!$   INTEGER, INTENT(IN) :: NADJ
!!$   REAL(kind=sp), INTENT(INOUT) :: Y(NVAR,NADJ)
!!$!~~~> Input: integration interval   
!!$   REAL(kind=sp), INTENT(IN) :: Tstart,Tend      
...
!!$   INTEGER, INTENT(OUT) :: IERR
!!$! ~~~~ Local variables        
!!$   REAL(kind=sp) :: Y0(NVAR)
!!$   REAL(kind=sp) :: Ynew(NVAR,NADJ), Fcn0(NVAR,NADJ), Fcn(NVAR,NADJ) 
!!$   REAL(kind=sp) :: K(NVAR*ros_S,NADJ), dFdT(NVAR,NADJ)
!!$#ifdef FULL_ALGEBRA
!!$   REAL(kind=sp), DIMENSION(NVAR,NVAR)  :: Jac0, Ghimj, Jac, dJdT
!!$#else
!!$   REAL(kind=sp), DIMENSION(LU_NONZERO) :: Jac0, Ghimj, Jac, dJdT
!!$#endif   
!!$   REAL(kind=sp) :: H, Hnew, HC, HG, Fac, Tau 
!!$   REAL(kind=sp) :: Err, Yerr(NVAR,NADJ)
...
!!$  END SUBROUTINE ros_SimpleCadjInt
  ...
  subroutine sweep_garbage(values, thresholds, all_non_neg)
    ...
    implicit none
    REAL(kind=sp), intent(inout) :: values(NVAR)
    REAL(kind=sp), intent(in) :: thresholds(NVAR)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  REAL(kind=sp) FUNCTION ros_ErrorNorm ( Y, Ynew, Yerr, &
...
! Input arguments
   REAL(kind=sp), INTENT(IN) :: Y(NVAR), Ynew(NVAR), &
          Yerr(NVAR), AbsTol(NVAR), RelTol(NVAR)
   LOGICAL, INTENT(IN) ::  VectorTol
! Local variables
   REAL(kind=sp) :: Err, Scale, Ymax
   INTEGER  :: i
   REAL(kind=sp), PARAMETER :: ZERO = 0.0_dp
   REAL(kind=sp), parameter :: big_real = 1e30
...
!~~~> Input arguments   
   REAL(kind=sp), INTENT(IN) :: T, Roundoff, Y(NVAR), Fcn0(NVAR) 
!~~~> Output arguments   
   REAL(kind=sp), INTENT(OUT) :: dFdT(NVAR)   
!~~~> Local variables     
   REAL(kind=sp) :: Delta  
   REAL(kind=sp), PARAMETER :: ONE = 1.0d0, DeltaMin = 1.0d-6
...
!~~~> Arguments   
   REAL(kind=sp), INTENT(IN) :: T, Roundoff, Y(NVAR)
#ifdef FULL_ALGEBRA    
   REAL(kind=sp), INTENT(IN)  :: Jac0(NVAR,NVAR) 
   REAL(kind=sp), INTENT(OUT) :: dJdT(NVAR,NVAR)   
#else
   REAL(kind=sp), INTENT(IN)  :: Jac0(LU_NONZERO) 
   REAL(kind=sp), INTENT(OUT) :: dJdT(LU_NONZERO)   
#endif   
!~~~> Local variables     
   REAL(kind=sp) :: Delta  
...
!~~~> Input arguments
   ...
   REAL(kind=sp), INTENT(IN) ::  Jac0(LU_NONZERO)
#endif   
   REAL(kind=sp), INTENT(IN) ::  gam
   INTEGER, INTENT(IN) ::  Direction
!~~~> Output arguments
#ifdef FULL_ALGEBRA    
   REAL(kind=sp), INTENT(OUT) :: Ghimj(NVAR,NVAR)
#else
   REAL(kind=sp), INTENT(OUT) :: Ghimj(LU_NONZERO)
#endif   
   LOGICAL, INTENT(OUT) ::  Singular
   INTEGER, INTENT(OUT) ::  Pivot(NVAR)
!~~~> Inout arguments
   REAL(kind=sp), INTENT(INOUT) :: H   ! step size is decreased when LU fails
!~~~> Local variables
...
!~~~> Inout variables
#ifdef FULL_ALGEBRA    
   REAL(kind=sp), INTENT(INOUT) :: A(NVAR,NVAR)
#else   
   REAL(kind=sp), INTENT(INOUT) :: A(LU_NONZERO)
#endif
!~~~> Output variables
...
#ifdef FULL_ALGEBRA    
   REAL(kind=sp), INTENT(IN) :: A(NVAR,NVAR)
#else   
   REAL(kind=sp), INTENT(IN) :: A(LU_NONZERO)
#endif
   INTEGER, INTENT(IN) :: Pivot(NVAR)
!~~~> InOut variables
   REAL(kind=sp), INTENT(INOUT) :: b(NVAR)
...
!~~~> Output variables     
   REAL(kind=sp), INTENT(OUT) :: Y(NVAR)
!~~~> Local variables     
   INTEGER     :: i
   REAL(kind=sp), PARAMETER  :: ONE = 1.0d0
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~   
   IMPLICIT NONE
!~~~> Input variables 
   REAL(kind=sp), INTENT(IN) :: a, b, T, Ya(NVAR), Yb(NVAR)
   REAL(kind=sp), INTENT(IN) :: Ja(NVAR), Jb(NVAR)
!~~~> Output variables     
   REAL(kind=sp), INTENT(OUT) :: Y(NVAR)
!~~~> Local variables     
   REAL(kind=sp) :: Tau, amb(3), C(NVAR,4)
...
!~~~> Input variables 
   REAL(kind=sp), INTENT(IN) :: a, b, T, Ya(NVAR), Yb(NVAR)
   REAL(kind=sp), INTENT(IN) :: Ja(NVAR), Jb(NVAR), Ha(NVAR), Hb(NVAR)
!~~~> Output variables     
   REAL(kind=sp), INTENT(OUT) :: Y(NVAR)
!~~~> Local variables     
   REAL(kind=sp) :: Tau, amb(5), C(NVAR,6)
   REAL(kind=sp), PARAMETER :: ZERO = 0.0d0, HALF = 0.5d0
   INTEGER :: i, j
...
!!$ 
!!$!~~~> Input variables     
!!$   REAL(kind=sp), INTENT(IN)  :: T, Y(NVAR)
!!$!~~~> Output variables     
!!$   REAL(kind=sp), INTENT(OUT) :: Ydot(NVAR)
!!$!~~~> Local variables
!!$   REAL(kind=sp) :: Told     
!!$
!!$   Told = TIME
...
!~~~> Input variables
    REAL(kind=sp) :: T, Y(NVAR)
!~~~> Output variables
#ifdef FULL_ALGEBRA    
    REAL(kind=sp) :: JV(LU_NONZERO), Jcb(NVAR,NVAR)
#else
    REAL(kind=sp) :: Jcb(LU_NONZERO)
#endif   
!~~~> Local variables
    REAL(kind=sp) :: Told
#ifdef FULL_ALGEBRA    
...
!!$    REAL(kind=sp), INTENT(IN)  :: T, Y(NVAR)
!!$!~~~> Output variables     
!!$    REAL(kind=sp), INTENT(OUT) :: Hes(NHESS)
!!$!~~~> Local variables
!!$    REAL(kind=sp) :: Told     
!!$
!!$    Told = TIME
...
END MODULE cbm42_strato_SOA_adj_Integrator
