...
!    end do
  ...
  subroutine set_da_rules(nlptr, nlStandardSetup, darules, startTime, periodToCompute)
    ...
    if (rules%method == flag_4dvar .or. rules%method == flag_h_matrix) then
      rules%da_begin = startTime
      rules%da_window = periodToCompute
      if (fu_fails(fu_interval_positive(rules%da_window), 'Bad assimilation window', sub_name)) return

    else if (rules%method == flag_3dvar .or. rules%method == flag_4dvar_seq &
           & .or. rules%method == flag_enkf .or. rules%method == flag_enks) then

      nl_content = fu_content(nlptr, 'assimilation_window')
      if (fu_fails(nl_content /= '', 'Missing assimilation_window', sub_name)) return
      rules%da_window = fu_set_named_interval(nl_content)
      if (error) return
      ...
      if (nl_content == '') then
        call msg('No assimilation_interval - will use assimilation_window')
        rules%assim_interval = rules%da_window
      else
        rules%assim_interval = fu_set_named_interval(nl_content)
        if (error) return
      end if

      nl_content = fu_content(nlptr, 'smoother_step')
      if (nl_content == '') then
        if (rules%method == flag_enks) then
      ...
      if (nl_content /= '') then
        ...
        rules%first_analysis_time = startTime
      end if
    ...
    nl_content = fu_content(nlptr, 'output_level')
    select case(nl_content)
    case ('all')
      rules%output_level = 2
    case ('first_last')
      rules%output_level = 1
    case ('none')
      rules%output_level = 0
    case ('')
      rules%output_level = 1
    case default
