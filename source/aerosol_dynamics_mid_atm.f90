...
call msg('Sufficient particles. H2SO4 total, so4, naero(0.1um):', (/h2so4_tot, masses(iH2SO4_loc,iModeFine), naero(iModeFine)/))
    ...
    !
    !RH: TESTcnc2vmrChange
    !pw = h2o_tot * rulesRates%cnc2vmr * patm  ! water pressure (cnc to m.r. to atm)
...
!$OMP END CRITICAL(debug_ter)
    ...
    h2o_aer = ns*((1.-ws-wn)/ws)*(mwh2so4/mwh2o)
...
call msg('v,ws,wn,mwh2so4,mwhno3,mwh2o:',(/v,ws,wn,mwh2so4,mwhno3,mwh2o/))
      ...
      masses(iH2O_loc,iModeFine) = ns*((1.-ws-wn)/ws)*(mwh2so4/mwh2o)
#ifdef DEBUG_MORE
if(print_it)then
write(unit=strTmp,fmt='(1x,1000(E9.3,2x))')masses
...
call msg('New STS: masses    :' + strTmp)
      ...
      do iBin = 1, nBins
        ...
        masses(iH2O_loc,iBin) = masses(iH2O_loc,iBin) + delta*sad(iBin)*((1.-ws-wn)/ws)*(mwh2so4/mwh2o)
      end do
...
!    ! partitioning of HNO3 between liquid and gas phase 
!    if (pn0 > 0.) then
!      hno3g = pn/pn0*hno3_tot  ! conc in gas phase, mole/m3
!!      xhno3p = amax1(0., (pn0-pn)/pn0*hno3_tot) ! conc in liquid phase, mole/m3
!    else
!      hno3g = 0. !small         ! m.r. in gas phase
!!      xhno3p = 0. !small         ! m.r. in liquid phase
!    endif
!    hno3c = xhno3p * hnm        ! # density in liquid phase
!    hno3g = xhno3g*hnm        ! # density in gas phase
!      write(*,*) hno3r,hno3c,hno3g,xhno3p,xhno3g,pn,pn0,p,t
...
!!$       !RISTO Cl&Br: 3Aug2017 by R.H. We put HCl, HOCl, HBr in Fine/Coarse mode with ratios given by the sad.
!!$       if (hclg > hcl_tot) then
!!$          hclg = hcl_tot
!!$       elseif (hclg < 0.0) then
!!$          hclg = 0.0
!!$       end if
!!$       masses(iHCl_loc,iModeFine)   = (hcl_tot-hclg)*sad(iModeFine)/sum(sad(1:nBins))  !Amount of HCl in aerosols
!!$       masses(iHCl_loc,iModeCoarse) = (hcl_tot-hclg)*sad(iModeCoarse)/sum(sad(1:nBins)) 
!!$       if (hoclg > hocl_tot) then
!!$          hoclg = hocl_tot
!!$       elseif (hoclg < 0.0) then
!!$          hoclg = 0.0
!!$       end if
!!$       masses(iHOCl_loc,iModeFine)   = (hocl_tot-hoclg)*sad(iModeFine)/sum(sad(1:nBins))  !Amount of HOCl in aerosols
!!$       masses(iHOCl_loc,iModeCoarse) = (hocl_tot-hoclg)*sad(iModeCoarse)/sum(sad(1:nBins)) 
!!$       if (hbrg > hbr_tot) then
!!$          hbrg = hbr_tot
!!$       elseif (hbrg < 0.0) then
!!$          hbrg = 0.0
!!$       end if
!!$       masses(iHBr_loc,iModeFine)   = (hbr_tot-hbrg)*sad(iModeFine)/sum(sad(1:nBins))  !Amount of HBr in aerosols
!!$       masses(iHBr_loc,iModeCoarse) = (hbr_tot-hbrg)*sad(iModeCoarse)/sum(sad(1:nBins)) 

    endif ! partition of hcl, hbr and hocl: if inside the model applicability range and there are aerosol particles present. 
    
!!$    !RISTO TEST 7Sep2017
!!$    call msg('HCl, HOCl, HBr test in TER:')
!!$    write(*,*) ( ws+wn.gt.0.7 .or. t.gt.235. )
!!$    call msg('masses:', (/masses(iH2SO4_loc,1), masses(iHNO3_loc,1), masses(iH2O_loc,1), &
!!$                        & masses(iHCl_loc,1),   masses(iHOCl_loc,1), masses(iHBr_loc,1) /) )
!!$    call msg('masses:', (/masses(iH2SO4_loc,2), masses(iHNO3_loc,2), masses(iH2O_loc,2), &
!!$                        & masses(iHCl_loc,2),   masses(iHOCl_loc,2), masses(iHBr_loc,2) /) )
!!$    call msg('sads: ',sad(1:nBins) )
!!$    call msg('hhcl,hhbr,hhocl: ',(/hhcl,hhbr,hhocl/) )
!!$    call msg('mcl,mbr,mocl: ',(/mcl,mbr,mocl/) )
!!$    call msg('mwhcl,mwhbr,mwhocl: ',(/mwhcl,mwhbr,mwhocl/) )
!!$    !call msg('pcl,pbr,pocl: ',(/pcl,pbr,pocl/) )
!!$    call msg('pcl0,pbr0,pocl0: ',(/pcl0,pbr0,pocl0/) )
!!$    call msg('hcl_aer,hbr_aer,hocl_aer: ',(/hcl_aer,hbr_aer,hocl_aer/) )
!!$    call msg('hclg,hbrg,hoclg: ',(/hclg,hbrg,hoclg/) )
!!$    call msg('hcl_tot,hbr_tot,hocl_tot: ',(/hcl_tot,hbr_tot,hocl_tot/) )
!!$    write(*,*) 'mass HBr Fine:', masses(iHBr_loc,1)
!!$    write(*,*) rulesRates%cnc2vmr*patm,rulesRates%cnc2vmr, patm
!!$    write(*,*) gas_constant_uni/std_pressure_sl,T,(gas_constant_uni/std_pressure_sl)*T
!!$    write(*,*) pbr, pbr0, hbr_tot*rulesRates%cnc2vmr*patm
!!$    write(*,*) pbr/(patm*rulesRates%cnc2vmr),pbr0/(patm*rulesRates%cnc2vmr),hbr_tot
!!$    !call msg(': ',(/ /) )
!!$    !END RISTO TEST 7Sep2017
