...
!    type(silam_pollution_cloud), pointer :: cloud_adj
    ...
    do while (now < end_time)
      if (model%rules%darules%output_level > 0) call update_enkf_outdir(model, simrules_3d%darules, now)
      call smpi_global_barrier()
      if (error) return
  ...
  contains
    ...
    subroutine assimilate_master_only(now, ens, fatal_error)
      ...
      if (simrules_3d%darules%output_level > 0) then
...
!    type(silam_pollution_cloud), pointer :: cloud_adj
    ...
    model_integr%rules%periodToCompute = step_integr - model%rules%timestep
...
!!$                                   & (/nz_dispersion,nx_dispersion,ny_dispersion/))
  ...
  contains
    ...
    subroutine start_cycle(time_start, time_end)
      ...
      call set_observations(model_integr%rules%darules, fu_species_transport(cloud), &
