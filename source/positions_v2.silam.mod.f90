...
!  PUBLIC fu_silja_4d_interpolation ! New. Uses 2d-routine. Fast
  ...
  ! Statistical stuff
  PUBLIC masspoint_xy
  PUBLIC fu_masspoint_z
  PUBLIC standard_deviation_xy
  PUBLIC fu_standard_deviation_z
  PUBLIC fu_standard_deviation
...
!        CALL set_error('dispersion_grid is still undefined','set_position')
  ...
  function fu_set_pos_in_geo_global_grid(x,y,pressure,time) result(pos)
    ...
    ! Local variables
    REAL :: geo_corner_lon_E,geo_corner_lat_N, southpole_lon_E, southpole_lat_N, & 
          & geo_dx_deg, geo_dy_deg
    LOGICAL :: corner_in_geographical_latlon, if_south_pole
    INTEGER :: number_of_points_x, number_of_points_y
    ...
    CALL lonlat_grid_parameters(geo_global_grid,&
                       & geo_corner_lon_E, geo_corner_lat_N, corner_in_geographical_latlon, &
                       & number_of_points_x, number_of_points_y, &
                       & southpole_lon_E, southpole_lat_N, &
                       & geo_dx_deg, geo_dy_deg)

    pos%x = (x - geo_corner_lon_E) / geo_dx_deg + 1.
    pos%y = (y - geo_corner_lat_N) / geo_dy_deg + 1.
...
!    IF(grid == dispersion_grid)THEN
    IF(grid == meteo_grid)THEN
      x = fu_x(pos)
      y = fu_y(pos)
      out_of_grid = x<0.5 .or. y<0.5 .or. x>nx_meteo+0.5 .or. y>ny_meteo+0.5
      RETURN
    END IF
...
!      CALL report(grid)
  ...
  REAL FUNCTION  fu_geographical_lat(pos)
    
    ! Description:
    ! Returns the geographical latitude of a position. Northern
    ! latitudes (N) are positive. Use this mainly for IO, it is
    ! ineffective.
    !
    ! Language: ANSI Fortran 90
    !
    ...
    CALL modify_lonlat(fu_lat_native_from_grid(pos%x, pos%y, meteo_grid), &
                     & fu_lon_native_from_grid(pos%x, pos%y, meteo_grid), &
                     & meteo_pole, pole_geographical, lat, lon)
!    CALL modify_lonlat(fu_lat_native_from_grid(pos%x, pos%y, dispersion_grid), &
!                     & fu_lon_native_from_grid(pos%x, pos%y, dispersion_grid), &
!                     & dispersion_pole, pole_geographical, lat, lon)

    fu_geographical_lat = lat

  END FUNCTION fu_geographical_lat
  
  
  ! ***************************************************************
  
  REAL FUNCTION fu_geographical_lon(pos)
    
    ! Description:
    ! Returns the geographical longitude of a position. Eastern
    ! latitudes (E) are positive. Use this mainly for IO, it is
    ! ineffective.
    !
    ! Language: ANSI Fortran 90
    !
...
!                     & dispersion_pole, pole_geographical, lat, lon)
    ...
    fu_geographical_lon = lon

  END FUNCTION fu_geographical_lon
...
!    IF(error)RETURN
  ...
  SUBROUTINE masspoint_xy(number_of_particles, position_of_particles, &
      & masspoint_x, masspoint_y, R_x, R_y)
    
    ! Calculates center of particle clould. Point is shown as (lat, lon)- 
    ! degrees and distance (m) from point (0.0E,0.0N)  
    !
    ! Language: ANSI Fortran 90
    !
    ! Author: Pilvi Siljamo, FMI
    ! Adjusted to silam_grid_position type by M.Sofiev
    ! 
    IMPLICIT NONE
    !
    ! Imported parameters with intent(in):
    INTEGER, INTENT(in) :: number_of_particles
    TYPE(silam_grid_position), DIMENSION(:), INTENT(in) :: position_of_particles

    ! Imported parameters with intent(out)
    REAL, INTENT(out) :: masspoint_x, masspoint_y ! lat, lon 
    REAL, INTENT(out) ::  R_x, R_y ! m
    ...
    DO i=1, number_of_particles
      pos =  position_of_particles(i)
      IF (defined(pos)) THEN   
        lat = fu_geographical_lat(pos)
        lon = fu_geographical_lon(pos)
!        CALL get_lonlat(pos, pole_geographical, lon, lat)
        y = fu_dy_deg_to_m(lat)
        x = fu_dx_deg_to_m(lon, lat)
        R_y = R_y + y
        R_x = R_x + x
      ELSE
        CALL msg_warning('I do not calculate masspoints.Particle out of grid',&
            & 'masspoint_xy') 
        RETURN
      END IF
    END DO

    R_y = R_y/number_of_particles
    R_x = R_x/number_of_particles

    masspoint_y = fu_dy_m_to_deg(R_y)
    masspoint_x = fu_dx_m_to_deg(R_x, masspoint_y)

    !    PRINT*,' masspoint_y (lat/deg): ', masspoint_y
    !    PRINT*,' masspoint_x (lon/deg): ', masspoint_x

  END SUBROUTINE masspoint_xy

  !**********************************************************

  REAL FUNCTION fu_masspoint_z(number_of_particles, position_of_particles)
    
    ! Calculates vertical center of particle clould (m).
    !
    ! Language: ANSI Fortran 90
    !
    ! Author: Pilvi Siljamo, FMI
    ...
    DO i=1, number_of_particles
      IF (defined(position_of_particles(i))) THEN 
        height = fu_height(position_of_particles(i))  
        R_z = R_z+height
      ELSE
        CALL msg_warning('I do not calculate masspoint.Particle out of grid',&
            & 'masspoint_z')
        RETURN
      END IF
    END DO

    fu_masspoint_z = R_z/number_of_particles
    !    PRINT*,'masspoint_z (m): ', fu_masspoint_z

  END FUNCTION fu_masspoint_z

  !**********************************************************

  SUBROUTINE standard_deviation_xy(number_of_particles, position_of_particles, sd_x, sd_y)
    
    ! Calculates standard deviation for east-west- (x) and south-north- (y)
    ! axel 
    !
    ! Language: ANSI Fortran 90
    !
    ...
    REAL :: x, y, x2, y2 ! m
    TYPE(silam_grid_position):: pos
    REAL :: masspoint_x, masspoint_y ! (lat, lon)
    REAL:: R_x, R_y ! m
 
    CALL masspoint_xy(number_of_particles, position_of_particles, &
        & masspoint_x, masspoint_y, R_x, R_y)
    PRINT*,' masspoint_y (lat/deg): ', masspoint_y
    PRINT*,' masspoint_x (lon/deg): ', masspoint_x
    ...
    DO i=1, number_of_particles
      ...
      IF ( defined(pos)) THEN
        ...
        lon = fu_geographical_lon(pos)
!        CALL get_lonlat(pos, pole_geographical, lon, lat)
        y = (fu_dy_deg_to_m(lat))**2
        x = (fu_dx_deg_to_m(lon, lat))**2
        x2 = x2+x
        y2 = y2+y
      ELSE
        CALL msg_warning(&
            &'I do not calculate standard deviations.Particle out of grid',&
            & 'standard_deviation_xy')
        RETURN
      END IF
    END DO

    sd_x = SQRT((x2/number_of_particles) -  R_x**2)
    sd_y = SQRT((y2/number_of_particles) -  R_y**2)   

    PRINT*,' sd_x (east-west/km): ', sd_x/1000.
    PRINT*,' sd_y (south-north/km): ', sd_y/1000.

  END SUBROUTINE standard_deviation_xy


  !******************************************************************

  REAL FUNCTION fu_standard_deviation_z(number_of_particles, position_of_particles)

    ! Description:
    !
    ! Calculates standard deviation for vertcal- (z) axel 
    !
    ! Language: ANSI Fortran 90
    !
    ! Author: Pilvi Siljamo, FMI
    ...
    DO i=1, number_of_particles
      ...
      ELSE
        CALL msg_warning(&
            &'I do not calculate z deviation .Particle out of grid',&
            & 'fu_standard_deviation_z')
        RETURN
      END IF
    END DO

    R_z = fu_masspoint_z(number_of_particles,&
        & position_of_particles)
    PRINT*,' masspoint_z (m): ', R_z
    fu_standard_deviation_z = SQRT((z2/number_of_particles) -  R_z**2)
    PRINT*,'sd_z (m): ', fu_standard_deviation_z 

   
  END FUNCTION fu_standard_deviation_z


  !******************************************************************

  REAL FUNCTION fu_standard_deviation(number_of_particles,&
                                    & position_of_particles, z_component_too)

    ! Calculates horizontal (2D) or 3D (x,y,z) standard deviation
    !
    ! Language: ANSI Fortran 90
    !
    ! Author: Pilvi Siljamo, FMI
    ...
    INTEGER, INTENT(in) :: number_of_particles
    TYPE(silam_grid_position), DIMENSION(:), INTENT(in) :: position_of_particles
    LOGICAL, INTENT(in) :: z_component_too  
    ...
    D = 0.
    ! Calculates x- and y- masspoint
    CALL  masspoint_xy(number_of_particles, position_of_particles, &
        & masspoint_x, masspoint_y, R_x, R_y)

    IF (z_component_too) THEN
...
!          CALL get_lonlat(pos, pole_geographical, lon, lat)
          y = fu_dy_deg_to_m(lat)
          x = fu_dx_deg_to_m(lon, lat)
          z = fu_height(position_of_particles(i))
        ...
        ELSE
          CALL msg_warning(&
              &'I do not calculate 3D standard deviation. Particle out of grid',&
              & 'fu_standard_deviation')
        RETURN
      END IF
      END DO
      fu_standard_deviation = SQRT(D/ number_of_particles)
      PRINT*,'sd_xyz (3-D cloud/km): ', fu_standard_deviation/1000.
...
!          CALL get_lonlat(pos, pole_geographical, lon, lat)
      ...
      END DO
      fu_standard_deviation = SQRT(D/ number_of_particles)
      call msg_test('sd_xy (horizontal cloud/km): ', real_value = fu_standard_deviation/1000.)
    END IF

  END FUNCTION fu_standard_deviation
