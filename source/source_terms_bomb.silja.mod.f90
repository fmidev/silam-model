...
!    if(error)return
  ...
  subroutine initialize_vertical_fraction(b_src, vertical)
    ...
    ! Stupidity check
    !
    if (.not. defined(vertical)) call set_error('Undefined vertical given','initialize_vertical_fraction')
    if (error) return
    !
    ...
    !
    if (fu_bottom_of_layer_value(fu_level(vertical,1)) /= 0.0) call set_error('Lowest bound not 0.0 meters???','initialize_vertical_fraction')
    if (error) return
    !
    ...
    do iLev = 1,nlevels
       ...
       if (b_src%height_frac(iLev) < 0.0) call set_error('About to inject negative mass to a level','initialize_vertical_fraction')
       if (error) return
    end do
    if (b_src%height_frac(nlevels) > 0.0) then
       ...
       if (forced_act > 0.01) then
          call msg_warning('Some of the activity is above SILAM top and is forced to the highest SILAM level','initialize_vertical_fraction')
    ...
    !
    if ((sum(b_src%height_frac) > 1.001) .or. (sum(b_src%height_frac) < 0.999)) call set_error('Fraction total not 1','initialize_vertical_fraction')
    if (error) return
  ...
  subroutine fill_b_src_from_namelist(nlSrc, b_src)
    ...
    if (fLat == real_missing) call set_error('Failed to read source','fill_b_src_from_namelist')
    fLon = fu_content_real(nlSrc,'source_longitude')
    if (fLon == real_missing) call set_error('Failed to read source','fill_b_src_from_namelist')
    IF (error) RETURN
    ...
    if(fu_fails(.not.((b_src%total_yield .eps. real_missing) .or. b_src%total_yield < 0.), &
                                    & 'Failed to read yield', 'fill_b_src_from_namelist'))return
    ...
    if(fu_fails(.not.(b_src%blast_height_m .eps. real_missing),'Failed to read source', &
                                      & 'fill_b_src_from_namelist'))return
    !
    ...
    if (fu_fails(b_src%bomb_type == 'URANIUM' .or. b_src%bomb_type == 'PLUTONIUM', &
                                      & 'Unclassified bomb type','fill_b_src_from_namelist'))return
    !
    ...
    if(fu_fails(b_src%dist_type == 'KDFOC3' .or. b_src%dist_type == 'BAKER', &
                        & 'Unclassified distribution type given','fill_b_src_from_namelist'))return
    !
    ! Fission fraction
    ...
    if(fu_str_u_case(spContent%sp) == 'FULL_RANGE')then
      ...
      if(fu_fails(iTmp==0,'Failed to read selected size range from string:' + spContent%sp,'fill_b_src_from_namelist'))return
      if(fu_fails(b_src%smallest_aerosol < b_src%largest_aerosol,'Strange aerosol size range, from:' + &
                & fu_str(b_src%smallest_aerosol) + ', to:' + fu_str(b_src%largest_aerosol), 'fill_b_src_from_namelist'))return
    ...
    b_src%bins_between = fu_content_real(nlSrc,'aerosol_bins')
    if (b_src%bins_between < 2 .or. b_src%bins_between > 100) call set_error('Unreasonable number of bins requested by user','fill_b_src_from_namelist')
    !
