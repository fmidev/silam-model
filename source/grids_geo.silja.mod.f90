...
!    module procedure fu_set_any_grid
  ...
  type silam_any_grid_param
    ...
    !
    real, dimension(:), pointer :: xC,yC   !,xL,xR,yL,yR,xU,xD,yU,yD ! centres, borders
    real, dimension(:), pointer :: dx, dy, sin_map_rot, cos_map_rot ! sizes, rotation angle
    type(silja_logical) :: defined != silja_false
  end type silam_any_grid_param

  type silam_ag_param_ptr
!    private
    type(silam_any_grid_param), pointer :: ptr
  end type silam_ag_param_ptr

  integer, public, parameter :: max_nbr_any_grids = 11 
 
  type(silam_ag_param_ptr), dimension(max_nbr_any_grids), public, target, save :: pAnyGrdParam 
  ! ref count == int_missing => pointer not allocated!
...
!  INTEGER, PARAMETER, PRIVATE :: no_shift = 99
  ...
  TYPE(silja_grid),PUBLIC,POINTER,SAVE :: meteo_gridPtr
  INTEGER,PUBLIC,SAVE :: nx_meteo = int_missing, ny_meteo = int_missing, &
                       & fs_meteo = int_missing
  ...
  function fu_set_grid_from_namelist(nlSetup)result(grid)
    ...
    elseif(fu_str_u_case(fu_content(nlSetup,'grid_type')) == 'LON_LAT')then
      ...
      if(.not.defined(grid%lonlat%pole)) then
         call report(nlSetup)
         call set_error('Cannot find the grid pole','fu_set_grid_from_namelist')
         return
      endif
      ...
      !
      grid%lonlat%sw_corner_modlon = fu_scale_longitude(fu_content_real(nlSetup,'lon_start'))
      grid%lonlat%sw_corner_modlat = fu_scale_latitude(fu_content_real(nlSetup,'lat_start'))

      if((grid%lonlat%sw_corner_modlon .eps. real_missing) .or. &
       & (grid%lonlat%sw_corner_modlat .eps. real_missing)) then
         call report(nlSetup)
         call set_error('Cannot find the grid corner','fu_set_grid_from_namelist')
         return
      endif
      IF (error) RETURN
      ...
      !
      iTmp = fu_content_int(nlSetup,'nx')
      fTmp = fu_content_real(nlSetup,'dx')

      if(iTmp /= int_missing)then
        !
        ! number of cells is defined
        !
        grid%lonlat%nx = iTmp ! EW
        grid%lonlat%ny = fu_content_int(nlSetup,'ny') ! EW

        if(.not.((fTmp.eps.real_missing) .or. (fTmp<0.) .or. (fTmp.eps.0.)))then
          !
          ! Cell size is defined
          !
          grid%lonlat%dx_deg = fTmp
          grid%lonlat%dy_deg = fu_content_real(nlSetup,'dy')
        else
          !
          ! Cell size is undefined - get it from end point and cell size
          !
          fTmp = fu_content_real(nlSetup,'lon_end')
          if((fTmp.eps.0.) .or. &
           & (fTmp.eps.grid%lonlat%sw_corner_modlon) .or. &
           & (fTmp.eps.real_missing))then
             call report(nlSetup)
             call set_error('Both dx and lon_end are absent','fu_set_grid_from_namelist')
             return
          endif
          grid%lonlat%dx_deg = (fTmp - grid%lonlat%sw_corner_modlon) / (grid%lonlat%nx-1)
          grid%lonlat%dy_deg = (fu_content_real(nlSetup,'lat_end') - &
                              & grid%lonlat%sw_corner_modlat) / (grid%lonlat%ny-1)
        endif

      else
        !
        ! Number of cells is undefined - get it from cell size and end point
        !
        if((fTmp.eps.real_missing) .or. (fTmp<0.) .or. (fTmp.eps.0.))then
          call report(nlSetup)
          call set_error('Both nx and dx are absent','fu_set_grid_from_namelist')
          return
        endif
        ...
        fTmp = fu_scale_longitude(fu_content_real(nlSetup,'lon_end'))
        if((fTmp .eps. grid%lonlat%sw_corner_modlon) .or. &
         & (fTmp .eps. real_missing))then
           call report(nlSetup)
           call set_error('Both nx and lon_end are absent','fu_set_grid_from_namelist')
           return
        endif

!        if(fTmp < grid%lonlat%sw_corner_modlon)then
!          call msg_warning('Start longitude bigger than the end one. Swapping...')
!          fTmp2 = fTmp
!          fTmp = grid%lonlat%sw_corner_modlon
!          grid%lonlat%sw_corner_modlon = fTmp2
!        endif

        fTmp2 = fTmp - grid%lonlat%sw_corner_modlon
        if(fTmp2 < 0.0)fTmp2 = fTmp2 + 360.0
        grid%lonlat%nx = int(fTmp2 / grid%lonlat%dx_deg + 0.5) + 1

        fTmp = fu_scale_latitude(fu_content_real(nlSetup,'lat_end'))
        if(fTmp < grid%lonlat%sw_corner_modlat)then
          call msg_warning('Start latitude bigger than the end one. Swapping...')
          fTmp2 = fTmp
          fTmp = grid%lonlat%sw_corner_modlat
          grid%lonlat%sw_corner_modlat = fTmp2
        endif
        grid%lonlat%ny = int((fTmp - grid%lonlat%sw_corner_modlat) / grid%lonlat%dy_deg + 0.5) + 1
        !
        ! Because of numerics, our end point may run over the pole. Check it
        !
        if(grid%lonlat%sw_corner_modlat + grid%lonlat%dy_deg * (grid%lonlat%ny - 1)> 90.)then
          call msg_warning('Grid end runs over the northern pole latitude','fu_set_grid_from_namelist')
          if(grid%lonlat%sw_corner_modlat + grid%lonlat%dy_deg * (grid%lonlat%ny - 1) .eps. 90.)then
            call msg('Correcting the grid dy: numerics')
            grid%lonlat%dy_deg = (90. - grid%lonlat%sw_corner_modlat) / real(grid%lonlat%ny - 1)
          else
            call report(grid)
            call report(nlSetup)
            call set_error('Grid end runs over the northern pole latitude','fu_set_grid_from_namelist')
          endif
        endif
        
        if(grid%lonlat%nx <= 0 .or. grid%lonlat%nx <= 0)then
          call report(nlSetup)
          call set_error('Non-positive number of cells','fu_set_grid_from_namelist')
          return
        endif
      ...
      !     May be, this range is even not achievable - grid [0:360], for example.
      !
      if(grid%lonlat%dx_deg * (grid%lonlat%nx-1) > 360. + eps)then
        call set_error('More than 360 degrees in the grid','fu_set_grid_from_namelist')
        return
      end if
      if(grid%lonlat%sw_corner_modlon > 180.)then
!        grid%lonlat%sw_corner_modlon + grid%lonlat%dx_deg * (grid%lonlat%nx-1) > 180.+ eps)then
        grid%lonlat%sw_corner_modlon = grid%lonlat%sw_corner_modlon - 360.
      elseif(grid%lonlat%sw_corner_modlon + grid%lonlat%dx_deg * (grid%lonlat%nx-1) < -180.)then
        grid%lonlat%sw_corner_modlon = grid%lonlat%sw_corner_modlon + 360.
      end if
    ...
    elseif(fu_str_l_case(fu_content(nlSetup,'grid_type')) == 'anygrid')then
      ...
      iTmp = fu_content_int(nlSetup,'nx')
      if(iTmp <= 0)then
        call set_error('strange number of x gridcells','fu_set_grid_from_namelist')
        return
      endif
      grid%ag%nx = iTmp
      iTmp = fu_content_int(nlSetup,'ny')
      if(iTmp <= 0)then
        call set_error('strange number of y gridcells','fu_set_grid_from_namelist')
        return
      endif
      grid%ag%ny = iTmp
      grdFNm = fu_content(nlSetup,'grid_file')
      if(trim(grdFNm) == '')then
        call set_error('problem with filename','fu_set_grid_from_namelist')
        return
      endif
  ...
  subroutine report_grid_as_namelist(grid, iUnit, fName)
    ...
    select case(grid%gridtype)
      ...
      case(anygrid)
        ...
        write(iUnit,'(A,F15.7)')'lon_start = ', pAnyGrdParam(grid%ag%indParam)%ptr%xc(1)
        write(iUnit,'(A,F15.7)')'lat_start = ', pAnyGrdParam(grid%ag%indParam)%ptr%yc(1)
        write(iUnit,'(A,I5)')'nx = ', grid%ag%nx
        write(iUnit,'(A,I5)')'ny = ', grid%ag%ny
        write(iUnit,'(A,F15.7)')'dx_m = ', pAnyGrdParam(grid%ag%indParam)%ptr%dx(1)
        write(iUnit,'(A,F15.7)')'dy_m = ', pAnyGrdParam(grid%ag%indParam)%ptr%dy(1)
...
!    end if
  ...
  integer function fu_ag_param_index(nc, lon, lat, ifNew) result(index)
    ...
    do i=1, max_nbr_any_grids-1
      if (anyGrdParamRefCount(i) == int_missing) then
        ! never allocated
        allocate(pAnyGrdParam(i)%ptr)
        pAnyGrdParam(i)%ptr%defined = silja_false
        anyGrdParamRefCount(i) = 0
      endif

      if(.not. pAnyGrdParam(i)%ptr%defined == silja_true)then
        ! no longer used
        ...
        pAnyGrdParam(i)%ptr%defined = silja_true
        index = i
        ifNew = .true.
        return
      else
        if(.not.(associated(pAnyGrdParam(i)%ptr%xc) .and. associated(pAnyGrdParam(i)%ptr%yc)))cycle
        if(size(pAnyGrdParam(i)%ptr%xc) /= nc)cycle
        ifNew = .false.   
        do j = 1, nc
          if(.not.((pAnyGrdParam(i)%ptr%xc(j) .eps. lon(j)).and.(pAnyGrdParam(i)%ptr%yc(j) .eps. lat(j))))then
            ifNew = .true.
            exit 
          endif
        enddo
  ...
  subroutine release_grid(grid)
    ...
    if (anyGrdParamRefCount(ind_ag) == 0) then
      ...
      if (associated(agptr%xc)) deallocate(agptr%xc)
      if (associated(agptr%yc)) deallocate(agptr%yc)
      if (associated(agptr%dx)) deallocate(agptr%dx)
      if (associated(agptr%dy)) deallocate(agptr%dy)
      if (associated(agptr%sin_map_rot)) deallocate(agptr%sin_map_rot)
      if (associated(agptr%cos_map_rot)) deallocate(agptr%cos_map_rot)
  ...
  FUNCTION fu_set_any_grid(name, nx, ny, lon, lat) result(grid)
    ...
    if(ifData)then
      ...
      if(error)return
      if(ifnew)then
          nullify(pAnyGrdParam(grid%ag%indParam)%ptr%xc)
          nullify(pAnyGrdParam(grid%ag%indParam)%ptr%yc)
          nullify(pAnyGrdParam(grid%ag%indParam)%ptr%dx)
          nullify(pAnyGrdParam(grid%ag%indParam)%ptr%dy)
          nullify(pAnyGrdParam(grid%ag%indParam)%ptr%sin_map_rot)
          nullify(pAnyGrdParam(grid%ag%indParam)%ptr%cos_map_rot)
          allocate(pAnyGrdParam(grid%ag%indParam)%ptr%xc(nx*ny))
          pAnyGrdParam(grid%ag%indParam)%ptr%xc(1:nx*ny) = lon(1:nx*ny)
          allocate(pAnyGrdParam(grid%ag%indParam)%ptr%yc(nx*ny))
          pAnyGrdParam(grid%ag%indParam)%ptr%yc(1:nx*ny) = lat(1:nx*ny)
          pAnyGrdParam(grid%ag%indParam)%ptr%defined = silja_true
      endif
    else
!      call msg_warning('Setting anygrid without lons or lats given')      
  ...
  subroutine setAgLonlatFlds(grid, lon, lat, ifnew)
    ...
    if(ifnew)then
      ...
      nullify(pAnyGrdParam(grid%ag%indParam)%ptr%sin_map_rot)
      nullify(pAnyGrdParam(grid%ag%indParam)%ptr%cos_map_rot)
      allocate(pAnyGrdParam(grid%ag%indParam)%ptr%xc(nc))
      pAnyGrdParam(grid%ag%indParam)%ptr%xc(1:nc) = lon(1:nc)
      allocate(pAnyGrdParam(grid%ag%indParam)%ptr%yc(nc))
      pAnyGrdParam(grid%ag%indParam)%ptr%yc(1:nc) = lat(1:nc)
      pAnyGrdParam(grid%ag%indParam)%ptr%defined = silja_true
    endif
  ...
  subroutine setAnygridParam(grid, gVarNm, values)
    ...
    TYPE(silja_grid) :: grid
    CHARACTER (LEN=*) :: gVarNm
    real, dimension(:), pointer, optional :: values
    integer :: nc, i
    ...
    select case(gVarNm)
      case("lon")
        do i = 1, nc
          ! Maybe 0 to 360
          if(values(i)>180.)values(i)= values(i)-360.
          ! if still abnormal, error
          if(values(i)>180. .or. values(i)<-180.)then
            call set_error('strange lon values given', 'fu_set_any_grid')
            return
          endif
        enddo
        if(associated(pAnyGrdParam(grid%ag%indParam)%ptr%xc))deallocate(pAnyGrdParam(grid%ag%indParam)%ptr%xc)
        allocate(pAnyGrdParam(grid%ag%indParam)%ptr%xc(nc))
        if(present(values))then
          pAnyGrdParam(grid%ag%indParam)%ptr%xc(1:nc) = values(1:nc)
        else
          call set_error('no lon values given','setAnygridParam')
        endif
      case("lat")
        do i = 1, nc
          if(values(i)>90. .or. values(i)<-90.)then
            call set_error('strange lat values given', 'fu_set_any_grid')
             return
          endif
        enddo
        if(associated(pAnyGrdParam(grid%ag%indParam)%ptr%yc))deallocate(pAnyGrdParam(grid%ag%indParam)%ptr%yc)
        allocate(pAnyGrdParam(grid%ag%indParam)%ptr%yc(nc))
        if(present(values))then
          pAnyGrdParam(grid%ag%indParam)%ptr%yc(1:nc) = values(1:nc)
        else
          call set_error('no lat values given','setAnygridParam')
        endif
      case("dx")
        if(associated(pAnyGrdParam(grid%ag%indParam)%ptr%dx))deallocate(pAnyGrdParam(grid%ag%indParam)%ptr%dx)
        allocate(pAnyGrdParam(grid%ag%indParam)%ptr%dx(nc))
        if(present(values))then
          pAnyGrdParam(grid%ag%indParam)%ptr%dx(1:nc) = values(1:nc)
        else
          call set_error('Doesnt work yet 1','setAnygridParam')
        endif
      case("dy")
        if(associated(pAnyGrdParam(grid%ag%indParam)%ptr%dy))deallocate(pAnyGrdParam(grid%ag%indParam)%ptr%dy)
        allocate(pAnyGrdParam(grid%ag%indParam)%ptr%dy(nc))
        if(present(values))then
          pAnyGrdParam(grid%ag%indParam)%ptr%dy(1:nc) = values(1:nc)
        else
          call set_error('Doesnt work yet 2','setAnygridParam')
        endif
      case("sin_map_rot")
        if(associated(pAnyGrdParam(grid%ag%indParam)%ptr%sin_map_rot))deallocate(pAnyGrdParam(grid%ag%indParam)%ptr%sin_map_rot)
        allocate(pAnyGrdParam(grid%ag%indParam)%ptr%sin_map_rot(nc))
        if(present(values))then
          pAnyGrdParam(grid%ag%indParam)%ptr%sin_map_rot(1:nc) = values(1:nc)
        else
          call set_error('Doesnt work yet 3','setAnygridParam')
        endif
      case("cos_map_rot")
        if(associated(pAnyGrdParam(grid%ag%indParam)%ptr%cos_map_rot))deallocate(pAnyGrdParam(grid%ag%indParam)%ptr%cos_map_rot)
        allocate(pAnyGrdParam(grid%ag%indParam)%ptr%cos_map_rot(nc))
        if(present(values))then
          pAnyGrdParam(grid%ag%indParam)%ptr%cos_map_rot(1:nc) = values(1:nc)
        else
          call set_error('Doesnt work yet 4','setAnygridParam')
        endif
      case default
        call set_error(fu_connect_strings('Unknown parameter name', gVarNm),'setAnygridParam')
    end select
    ...
    if (bName == 'T' .or. bName == 'B')then
      ...
      case(lonlat)
       ...
       case(anygrid)
         ...
         gr_param_ptr => pAnyGrdParam(grid%ag%indParam)%ptr
  ...
  function fu_area_from_grid(grid) result(area)
    ...
    select case (grid%gridtype)
      ...
      case(anygrid)
        ...
        ymax = maxval(pAnyGrdParam(grid%ag%indParam)%ptr%yc)
        ymin = minval(pAnyGrdParam(grid%ag%indParam)%ptr%yc)
        ...
        if(.not. ifPole)then
          ...
          if(.not. ifCross180)then  ! simple
            xmax = maxval(pAnyGrdParam(grid%ag%indParam)%ptr%xc)
            xmin = minval(pAnyGrdParam(grid%ag%indParam)%ptr%xc)
          elseif(.not. ifCross0)then ! from smallest positive to largest negative
            ! cycle the boundaries
            xmin = 180.
            xmax = -180.
            do i = 1, grid%ag%nx
              if(pAnyGrdParam(grid%ag%indParam)%ptr%xC(i) > 0) &
                & xmin = min(xmin, pAnyGrdParam(grid%ag%indParam)%ptr%xC(i))
              if(pAnyGrdParam(grid%ag%indParam)%ptr%xC(grid%ag%nx*(grid%ag%ny-1)+i) > 0) &
                & xmin = min(xmin, pAnyGrdParam(grid%ag%indParam)%ptr%xC(grid%ag%nx*(grid%ag%ny-1)+i))     
              if(pAnyGrdParam(grid%ag%indParam)%ptr%xC(i) < 0) &
                & xmax = max(xmax, pAnyGrdParam(grid%ag%indParam)%ptr%xC(i))
              if(pAnyGrdParam(grid%ag%indParam)%ptr%xC(grid%ag%nx*(grid%ag%ny-1)+i) < 0) &
                & xmax = max(xmax, pAnyGrdParam(grid%ag%indParam)%ptr%xC(grid%ag%nx*(grid%ag%ny-1)+i)) 
            enddo
            do i = 1, grid%ag%ny
              if(pAnyGrdParam(grid%ag%indParam)%ptr%xC(grid%ag%nx*i) > 0) &
                & xmin = min(xmin, pAnyGrdParam(grid%ag%indParam)%ptr%xC(grid%ag%nx*i))
              if(pAnyGrdParam(grid%ag%indParam)%ptr%xC(grid%ag%nx*(i-1)+1) > 0) &
                & xmin = min(xmin, pAnyGrdParam(grid%ag%indParam)%ptr%xC(grid%ag%nx*(i-1)+1))     
              if(pAnyGrdParam(grid%ag%indParam)%ptr%xC(grid%ag%nx*i) < 0) &
                & xmax = max(xmax, pAnyGrdParam(grid%ag%indParam)%ptr%xC(grid%ag%nx*i))
              if(pAnyGrdParam(grid%ag%indParam)%ptr%xC(grid%ag%nx*(i-1)+1) < 0) &
                & xmax = max(xmax, pAnyGrdParam(grid%ag%indParam)%ptr%xC(grid%ag%nx*(i-1)+1)) 
            enddo
  ...
  SUBROUTINE print_grid_report(grid)
    ...
    speak = .not. smpi_is_mpi_version() .or. smpi_global_rank == 0

    SELECT CASE (grid%gridtype)

      CASE (lonlat) 

        call msg(fu_connect_strings('  ------- Grid report of lonlat-grid--', grid%name))


      IF (grid%lonlat%pole == pole_geographical) THEN
        if (speak) then
          WRITE(*, fmt = '(A)') '  Grid in geographical lon-lat'
          WRITE(*, fmt = '(A, 2(F15.7,1x), 2(I12,1x))') &
        ...
        WRITE(run_log_funit, fmt = '(A)') '  Grid in geographical lon-lat'
        WRITE(run_log_funit, fmt = '(A, 2(F15.7,1x), 2(I12,1x))') &
            & '  SW-corner (lon,lat), number of points x,y: ', &
            & grid%lonlat%sw_corner_modlon,&
            & grid%lonlat%sw_corner_modlat, &
            & grid%lonlat%nx, grid%lonlat%ny
      ...
      ELSE
        ...
        WRITE(run_log_funit, fmt = '(A, 2(F15.7,1x))') &
            & '  Grid south pole lon(E) and lat (N):  ',&
            & fu_southpole_lon(grid%lonlat%pole),&
            & fu_southpole_lat(grid%lonlat%pole)
      END IF
      ...
      if (speak) then
        WRITE(*, fmt = '(A, 2(F15.7,1x))')'  Grid_distance x and y degrees: ',&
             & grid%lonlat%dx_deg, grid%lonlat%dy_deg
      end if
      WRITE(run_log_funit, fmt = '(A, 2(F15.7,1x))')'  Grid_distance x and y degrees: ',&
                                 & grid%lonlat%dx_deg, grid%lonlat%dy_deg


      case(anygrid)

      call msg(fu_connect_strings('  ------- Grid report of anygrid--', grid%name))
      if (speak) then
        WRITE(*, fmt = '(A)') '  Grid in geographical lon-lat'
        WRITE(*, fmt = '(A, 2(F15.7,1x), 2(I12,1x))') &
             & '  SW-corner (lon,lat), number of points x,y: ', &
             & pAnyGrdParam(grid%ag%indParam)%ptr%xc(1) ,&
             & pAnyGrdParam(grid%ag%indParam)%ptr%yc(1), &
      ...
      if (speak) then
        WRITE(*, fmt = '(A, 2(F15.7,1x))')'  Grid_distance x and y degrees for first gridcell: ',&
             & fu_dx_cell_deg(grid, 1, 1), &
             & fu_dx_cell_deg(grid, 1, 1)
      end if
      WRITE(run_log_funit, fmt = '(A, 2(F15.7,1x))')'  Grid_distance x and y degrees for first gridcell: ',&
                                 & fu_dx_cell_deg(grid, 1, 1), &
                                 & fu_dx_cell_deg(grid, 1, 1)
      ...
      CASE default
        call msg( '  Grid report: unknown grid, type=',grid%gridtype)

    END SELECT

    call msg(' ------------ End-of-grid-report ------------')
...
!  ! ***************************************************************
  ...
  REAL FUNCTION fu_lon_native_from_grid(x, y, grid)
    ...
    case (anygrid)   ! return the geographical coordinate
      fu_lon_native_from_grid = fu_2d_interpolation(pAnyGrdParam(grid%ag%indParam)%ptr%xc, &
  ...
  case (anygrid) ! return the geographical coordinate
     fu_lat_native_from_grid = fu_2d_interpolation(pAnyGrdParam(grid%ag%indParam)%ptr%yc, &
  ...
  REAL FUNCTION fu_lon_geographical_from_grid(x, y, grid)
    ...
    case (anygrid)      
      fu_lon_geographical_from_grid =  fu_2d_interpolation( &
                                                     & pAnyGrdParam(grid%ag%indParam)%ptr%xc, &
                                                     & x, y, grid%ag%nx, grid%ag%ny, linear, &
                                                     & notAllowed) 
    CASE DEFAULT
  ...
  REAL FUNCTION fu_lat_geographical_from_grid(x, y, grid)
    ...
    case (anygrid)      
      fu_lat_geographical_from_grid =  fu_2d_interpolation(pAnyGrdParam(grid%ag%indParam)%ptr%yc, &
                                                               & x, y, grid%ag%nx, grid%ag%ny, linear, notAllowed)   
    CASE DEFAULT
      CALL set_error('Sorry, only lonlat grids','fu_lon_geographical_from_grid')
    END SELECT
  ...
  function fu_geolats_fld(grid)
    ...
    CASE (lonlat)
      ...
      enddo
    case (anygrid)
      fu_geolats_fld => pAnyGrdParam(grid%ag%indParam)%ptr%yc
    CASE DEFAULT
      CALL set_error('Sorry, only lonlat grids','fu_lon_geographical_from_grid')
    END SELECT
  ...
  function fu_geolons_fld(grid)
    ...
    CASE (lonlat)
      ...
      enddo
    case (anygrid)
      fu_geolons_fld => pAnyGrdParam(grid%ag%indParam)%ptr%xc
    CASE DEFAULT
      CALL set_error('Sorry, only lonlat grids','fu_lon_geographical_from_grid')
    END SELECT
  ...
  function fu_dx_fld_m(grid)
    ...
    CASE (lonlat)
      ...
      enddo
    case (anygrid)
      fu_dx_fld_m => pAnyGrdParam(grid%ag%indParam)%ptr%dx
    CASE DEFAULT
      CALL set_error('Sorry, only lonlat grids','fu_lon_geographical_from_grid')
    END SELECT
  ...
  !***************************************************************
   ...
   function fu_dy_fld_m(grid)
    ...
    CASE (lonlat)
      ...
      enddo
    case (anygrid)
      fu_dy_fld_m => pAnyGrdParam(grid%ag%indParam)%ptr%dy
    CASE DEFAULT
      CALL set_error('Sorry, only lonlat grids','fu_lon_geographical_from_grid')
    END SELECT
  ...
  function fu_sin_map_rot_fld(grid)
    ...
    case (anygrid)
      fu_sin_map_rot_fld => pAnyGrdParam(grid%ag%indParam)%ptr%sin_map_rot
    CASE DEFAULT
  ...
  !***************************************************************
   ...
   function fu_cos_map_rot_fld(grid)
    ...
    case (anygrid)
      fu_cos_map_rot_fld => pAnyGrdParam(grid%ag%indParam)%ptr%cos_map_rot
    CASE DEFAULT
  ...
  real function fu_cell_size_midpoint(grid)
    ...
    select case (fu_gridtype(grid))
      ...
      case(anygrid)
        fu_cell_size_midpoint = pAnyGrdParam(grid%ag%indParam)%ptr%dx(nint(0.5*grid%ag%ny-1)*grid%ag%nx+nint(0.5*grid%ag%nx)) * &
                              & pAnyGrdParam(grid%ag%indParam)%ptr%dy(nint(0.5*grid%ag%ny-1)*grid%ag%nx+nint(0.5*grid%ag%nx))
    ...
    select case (fu_gridtype(grid))
      case(lonlat)
        ...
        fu_cell_size_n_th_cell = fu_dx_deg_to_m(grid%lonlat%dx_deg, &
                               ...
                               & fu_dy_deg_to_m(grid%lonlat%dy_deg)
      case(anygrid)
        fu_cell_size_n_th_cell = pAnyGrdParam(grid%ag%indParam)%ptr%dx(iCell) &
             & * pAnyGrdParam(grid%ag%indParam)%ptr%dy(iCell)
    ...
    select case (fu_gridtype(grid))
      case(lonlat)
        ...
        fu_cell_size_x_y_cell = fu_dx_deg_to_m(grid%lonlat%dx_deg, &
                              ...
                              & fu_dy_deg_to_m(grid%lonlat%dy_deg)
      case(anygrid)
        fu_cell_size_x_y_cell = pAnyGrdParam(grid%ag%indParam)%ptr%dx(grid%ag%nx*(iy-1)+ix) * &
                              & pAnyGrdParam(grid%ag%indParam)%ptr%dy(grid%ag%nx*(iy-1)+ix)
    ...
    select case (fu_gridtype(grid))
      case(lonlat)
        ...
        fu_dx_x_y_cell_m = max(0., fu_dx_deg_to_m(grid%lonlat%dx_deg, &
                                             ...
                                             & grid%lonlat%sw_corner_modlat) )
      case(anygrid)
        fu_dx_x_y_cell_m = pAnyGrdParam(grid%ag%indParam)%ptr%dx(grid%ag%nx*(iy-1)+ix) 
  ...
  real function fu_dy_x_y_cell_m(grid, ix, iy)
    ...
    select case (fu_gridtype(grid))
      ...
      case(anygrid)
        fu_dy_x_y_cell_m = pAnyGrdParam(grid%ag%indParam)%ptr%dy(grid%ag%nx*(iy-1)+ix)
 ...
 !****************************************************************
  ...
  real function fu_dx_x_y_cell_deg(grid, ix, iy)
    ...
    select case (fu_gridtype(grid))
      ...
      case(anygrid)
        fu_dx_x_y_cell_deg = fu_dx_m_to_deg(pAnyGrdParam(grid%ag%indParam)%ptr%dx(grid%ag%nx*(iy-1)+ix), &
                                    &  pAnyGrdParam(grid%ag%indParam)%ptr%yC(grid%ag%nx*(iy-1)+ix))
  ...
  real function fu_dy_x_y_cell_deg(grid, ix, iy)
    ...
    select case (fu_gridtype(grid))
      ...
      case(anygrid)
        fu_dy_x_y_cell_deg = fu_dy_m_to_deg(pAnyGrdParam(grid%ag%indParam)%ptr%dy(grid%ag%nx*(iy-1)+ix))
  ...
  logical function fu_ifLonGlobal(grid)
    ...
    select case (fu_gridtype(grid))
      ...
      ! have to scan all latitudes
        ...
        fu_ifLonGlobal = .true.
        do iy = 1, ny
          dist_max = fu_gc_distance(pAnyGrdParam(grid%ag%indParam)%ptr%xC(nx*iy),   &
                                  & pAnyGrdParam(grid%ag%indParam)%ptr%xC(nx*iy-1), &
                                  & pAnyGrdParam(grid%ag%indParam)%ptr%yC(nx*iy),   &
                                  & pAnyGrdParam(grid%ag%indParam)%ptr%yC(nx*iy-1)) + &
                   & fu_gc_distance(pAnyGrdParam(grid%ag%indParam)%ptr%xC(nx*(iy-1)+1), &
                                  & pAnyGrdParam(grid%ag%indParam)%ptr%xC(nx*(iy-1)+2), &
                                  & pAnyGrdParam(grid%ag%indParam)%ptr%yC(nx*(iy-1)+1), &
                                  & pAnyGrdParam(grid%ag%indParam)%ptr%yC(nx*(iy-1)+2))
         
          dist = fu_gc_distance(pAnyGrdParam(grid%ag%indParam)%ptr%xC(nx*iy), &
                              & pAnyGrdParam(grid%ag%indParam)%ptr%xC(nx*(iy-1)+1), &
                              & pAnyGrdParam(grid%ag%indParam)%ptr%yC(nx*iy), &
                              & pAnyGrdParam(grid%ag%indParam)%ptr%yC(nx*(iy-1)+1))
    ...
    select case (fu_gridtype(grid))
      ...
      case(anygrid)
        ...
        if (iBoundary == northern_boundary) then
           dy = sum(pAnyGrdParam(grid%ag%indParam)%ptr%dy(nx*(ny-1)+1:nx*ny))/nx
           xC => pAnyGrdParam(grid%ag%indParam)%ptr%xC(nx*(ny-1)+1:nx*ny)
           yC => pAnyGrdParam(grid%ag%indParam)%ptr%yC(nx*(ny-1)+1:nx*ny)
        else
           dy = sum(pAnyGrdParam(grid%ag%indParam)%ptr%dy(1:nx))/nx
           xC => pAnyGrdParam(grid%ag%indParam)%ptr%xC(1:nx)
           yC => pAnyGrdParam(grid%ag%indParam)%ptr%yC(1:nx)
        endif
  ...
  logical function fu_stdSilamGrid(grid)
    ...
    select case (fu_gridtype(grid))
      case(lonlat)
        ...
        !
!        call report(grid)
!        call msg('if_left, if_right, >=-180, <=-180',(/grid%lonlat%dx_deg * grid%lonlat%nx, &
!               & 360.0 - 0.1 * grid%lonlat%dx_deg, &
!               & grid%lonlat%sw_corner_modlon + 0.501*grid%lonlat%dx_deg, &
!               & grid%lonlat%sw_corner_modlon - grid%lonlat%dx_deg/))
...
!call report(grid_original)
    ...
    !
#ifdef OLD_REPROJECTION
    CALL grid_transformation(gridTmp,&
                           & arTmp, &  !grid_data,&
                           & grid_new,&
                           & selected_grid_data, &
                           & method, &
...
contains
  ...
  SUBROUTINE grid_transformation(grid_orig, &
                               & grid_data_orig, &
                               & grid_new, &
                               & grid_data_new, &
                               & method, &
                               ...
                               & iOutside)
    !
    ! Performs area transformation from grid to another.
    ! Contrary to the name, the sub does not transform grids
    !
    ! Available methods:
    ! - interpolation: nearest-point, linear, bicubic (whatever is treated in fu_2d_interpolation)
    ! - averaging (good for fractions)
    ! - summation (good for fluxes)
    !
    IMPLICIT NONE

    ! Imported parameters with intent(in):
    TYPE(silja_grid), INTENT(in) :: grid_orig
    TYPE(silja_grid), INTENT(in) :: grid_new
    INTEGER, INTENT(in) :: method, iAccuracy
    REAL, DIMENSION(:), intent(in), target:: grid_data_orig
    real, intent(in) :: missing_value
    logical, intent(in) :: ifRandomise
    integer, intent(in), optional :: iOutside

    ! Imported parameters with intent(out):
    REAL, DIMENSION(:), intent(out) :: grid_data_new

    ! Local declarations:
    real, dimension(3,3) :: R, R2
    real :: x_orig, y_orig, x_new, y_new, lon_orig, lon_new, lat_orig, lat_new, lon_world, lat_world, &
          & fSmall_1, fSmall_2, fMassSml
    integer :: ix, iy, nx_orig, ny_orig, nx_new, ny_new, iOutLocal, iG, iSml, jSml, nSmall
    REAL, DIMENSION(:), pointer :: grid_data_orig_ptr
...
!real*4, dimension(worksize) :: arTmp
    ...
    if(present(iOutside))then
      iOutLocal = iOutside          !notAllowed, nearestPoint, setZero
    else
      iOutLocal = notAllowed
    endif
    !
    ! Check the sizes of given grid-data vectors.
    !
    IF (.NOT.defined(grid_orig)) THEN
      CALL set_error('original grid not defined','grid_transformation')
      RETURN
    END IF

    IF (.NOT.defined(grid_new)) THEN
      CALL set_error('new grid not defined','grid_transformation')
      RETURN
    END IF

    IF (SIZE(grid_data_orig) < fu_number_of_gridpoints(grid_orig)) THEN
      call msg('Size of original data vector:',SIZE(grid_data_orig))
      call msg('Number of gridpoints in the original grid', fu_number_of_gridpoints(grid_orig))
      CALL set_error('too small vector for original grid data','grid_transformation')
      RETURN
    END IF

    IF (SIZE(grid_data_new) < fu_number_of_gridpoints(grid_new)) THEN
      call msg('Size of original data vector:',SIZE(grid_data_new))
      call msg('Number of gridpoints in the original grid', fu_number_of_gridpoints(grid_new))
      CALL set_error('too small vector for new grid data','grid_transformation')
      RETURN
    END IF


    call grid_dimensions(grid_orig, nx_orig, ny_orig)
    call grid_dimensions(grid_new, nx_new, ny_new)
    grid_data_orig_ptr => grid_data_orig

    !
    ! If the grids are substantially differrent, we have to handle the reprojection
    !
    select case(method)

     case(average, summation)
      !
      ! Fluxes, fractions, etc. Procedure is straightforward but tedious:
      ! one has to split the source field to small subfields and then put them one-by-one
      ! to proper places of the destination grid. Accuracy and missing value are the only 
      ! means to speed-up. We also assume that if all corners of a grid cell are outside the 
      ! destination grid, it is fully outside.
      !
      if(method == average)then
        iCounter => fu_work_int_array()
        iCounter = 0
        if(error)return
      endif

      grid_data_new = 0.0  ! Prepare the place.

      !
      ! Select the number of small cells, into which the original grid cell will be broken into
      !
      nSmall = min(101,max(11, &
                     & int(sqrt(2000. * fu_cell_size(grid_orig) / fu_cell_size(grid_new))/2.)*2+1))
      nSmall = min(nSmall, int(sqrt(5.e5/nx_orig*ny_orig))*2+1) ! nSmall*nSmall*nCells < 2*10**6
      ...
      !
      ! Actual work starts. It depends on the grid types
      !
      SELECT CASE (grid_orig%gridtype)
        CASE (lonlat)

          SELECT CASE (grid_new%gridtype)
            CASE (lonlat)

              if(grid_orig%lonlat%pole == grid_new%lonlat%pole)then  !------------- SAME POLE
                do iy = 1, ny_orig
                 do ix = 1, nx_orig
                  if(grid_data_orig(ix+(iy-1)*nx_orig) .eps. missing_value)cycle

                  ! Check corners of the cell: do we need to bother?
                  !
                  lon_orig = grid_orig%lonlat%sw_corner_modlon + (ix-1.5) * grid_orig%lonlat%dx_deg
                  lat_orig = grid_orig%lonlat%sw_corner_modlat + (iy-1.5) * grid_orig%lonlat%dy_deg
                  x_new = 1+ (lon_orig - grid_new%lonlat%sw_corner_modlon) / grid_new%lonlat%dx_deg
                  y_new = 1+ (lat_orig - grid_new%lonlat%sw_corner_modlat) / grid_new%lonlat%dy_deg
                  if(x_new<0.5 .or. x_new>nx_new+0.5 .or. y_new<0.5 .or. y_new>ny_new+0.5)then
                 
                   lat_orig = grid_orig%lonlat%sw_corner_modlat + (iy-0.5) * grid_orig%lonlat%dy_deg
                   x_new = 1+ (lon_orig - grid_new%lonlat%sw_corner_modlon) / grid_new%lonlat%dx_deg
                   y_new = 1+ (lat_orig - grid_new%lonlat%sw_corner_modlat) / grid_new%lonlat%dy_deg
                   if(x_new<0.5 .or. x_new>nx_new+0.5 .or. y_new<0.5 .or. y_new>ny_new+0.5)then
                   
                     lon_orig = grid_orig%lonlat%sw_corner_modlon + (ix-0.5) * grid_orig%lonlat%dx_deg
                     x_new = 1+ (lon_orig - grid_new%lonlat%sw_corner_modlon) / grid_new%lonlat%dx_deg
                     y_new = 1+ (lat_orig - grid_new%lonlat%sw_corner_modlat) / grid_new%lonlat%dy_deg
                     if(x_new<0.5 .or. x_new>nx_new+0.5 .or. y_new<0.5 .or. y_new>ny_new+0.5)then
                       ...
                       x_new = 1+ (lon_orig - grid_new%lonlat%sw_corner_modlon) / grid_new%lonlat%dx_deg
                       y_new = 1+ (lat_orig - grid_new%lonlat%sw_corner_modlat) / grid_new%lonlat%dy_deg
                       if(x_new<0.5 .or. x_new>nx_new+0.5 .or. y_new<0.5 .or. y_new>ny_new+0.5)cycle
                     endif
                   endif
                  endif
                 
                  !OK, have to do the work
                  !
                  fMassSml = grid_data_orig(ix+(iy-1)*nx_orig) * fSmall_2
                  do jSml = 1, nSmall
                   do iSml = 1, nSmall
if(ifRANDOMISE)then
                    lon_orig = grid_orig%lonlat%sw_corner_modlon + &
                         & (ix-1.5+(iSml-0.5)*fSmall_1+fu_random_number_center_rng(rng, 0.,fSmall_1)) * &
                         & grid_orig%lonlat%dx_deg
                    lat_orig = grid_orig%lonlat%sw_corner_modlat + &
                         & (iy-1.5+(jSml-0.5)*fSmall_1+fu_random_number_center_rng(rng, 0.,fSmall_1)) * &
                         & grid_orig%lonlat%dy_deg
else
                    lon_orig = grid_orig%lonlat%sw_corner_modlon + &
                         & (ix-1.5+(iSml-0.5)*fSmall_1) * grid_orig%lonlat%dx_deg
                    lat_orig = grid_orig%lonlat%sw_corner_modlat + &
                         & (iy-1.5+(jSml-0.5)*fSmall_1) * grid_orig%lonlat%dy_deg
endif
                    x_new = 1+ (lon_orig - grid_new%lonlat%sw_corner_modlon) / grid_new%lonlat%dx_deg
                    y_new = 1+ (lat_orig - grid_new%lonlat%sw_corner_modlat) / grid_new%lonlat%dy_deg
                    if(x_new>=0.5 .and. x_new<=nx_new+0.5 .and. y_new>=0.5 .and. y_new<=ny_new+0.5)then
                      grid_data_new(int(x_new+0.5)+(int(y_new+0.5)-1)*nx_new) = &
                                    & grid_data_new(int(x_new+0.5)+(int(y_new+0.5)-1)*nx_new) + fMassSml
                      if(method==average) iCounter(int(x_new+0.5)+(int(y_new+0.5)-1)*nx_new) = &
                                                & iCounter(int(x_new+0.5)+(int(y_new+0.5)-1)*nx_new) + 1
                    endif
                   end do
                  end do
                 end do
              ...
              elseif(grid_orig%lonlat%pole == pole_geographical)then    ! original grid has geogrphical pole

                call get_rotation(fu_southpole_lat(grid_new%lonlat%pole), &
                                & fu_southpole_lon(grid_new%lonlat%pole), R)
                ...
                do iy = 1, ny_orig
                 ...
                 lon_orig = grid_orig%lonlat%sw_corner_modlon + (ix-1.5) * grid_orig%lonlat%dx_deg
                 lat_orig = grid_orig%lonlat%sw_corner_modlat + (iy-1.5) * grid_orig%lonlat%dy_deg
                 call transform_latlon(lon_orig, lat_orig, R, world_to_rotated, lon_new, lat_new)
                 x_new = 1+ (lon_new - grid_new%lonlat%sw_corner_modlon) / grid_new%lonlat%dx_deg
                 y_new = 1+ (lat_new - grid_new%lonlat%sw_corner_modlat) / grid_new%lonlat%dy_deg
                 if(x_new<0.5 .or. x_new>nx_new+0.5 .or. y_new<0.5 .or. y_new>ny_new+0.5)then
...
if(ifRANDOMISE)then
                    ...
                    y_new = 1+ (lat_new - grid_new%lonlat%sw_corner_modlat) / grid_new%lonlat%dy_deg
                    if(error)then
                      call msg('World lon & lat:',lon_orig, lat_orig)
                      call set_error('World to rotated reprojection failed','grid_transformation')
                    endif
                    if(x_new>=0.5 .and. x_new<=nx_new+0.5 .and. y_new>=0.5 .and. y_new<=ny_new+0.5)then
                      grid_data_new(int(x_new+0.5)+(int(y_new+0.5)-1)*nx_new) = &
                                    & grid_data_new(int(x_new+0.5)+(int(y_new+0.5)-1)*nx_new) + fMassSml
              ...
              elseif(grid_new%lonlat%pole == pole_geographical)then    ! New grid has geographical pole
              
               call get_rotation(fu_southpole_lat(grid_orig%lonlat%pole), &
                                & fu_southpole_lon(grid_orig%lonlat%pole), R)
               do iy = 1, ny_orig
                do ix = 1, nx_orig
                  if(grid_data_orig(ix+(iy-1)*nx_orig) .eps. missing_value)cycle
                 !
                 ...
                 lon_orig = grid_orig%lonlat%sw_corner_modlon + (ix-1.5) * grid_orig%lonlat%dx_deg
                 lat_orig = grid_orig%lonlat%sw_corner_modlat + (iy-1.5) * grid_orig%lonlat%dy_deg
                 call transform_latlon(lon_orig, lat_orig, R, rotated_to_world, lon_new, lat_new)
                 x_new = 1+ (lon_new - grid_new%lonlat%sw_corner_modlon) / grid_new%lonlat%dx_deg
                 y_new = 1+ (lat_new - grid_new%lonlat%sw_corner_modlat) / grid_new%lonlat%dy_deg
                 if(x_new<0.5 .or. x_new>nx_new+0.5 .or. y_new<0.5 .or. y_new>ny_new+0.5)then
...
if(ifRANDOMISE)then
                    ...
                    y_new = 1+ (lat_new - grid_new%lonlat%sw_corner_modlat) / grid_new%lonlat%dy_deg
                    if(error)then
                      call msg('Rotated lon & lat:',lon_orig, lat_orig)
                      call set_error('Rotated to world reprojection failed','grid_transformation')
                    endif
                    if(x_new>=0.5 .and. x_new<=nx_new+0.5 .and. y_new>=0.5 .and. y_new<=ny_new+0.5)then
                      grid_data_new(int(x_new+0.5)+(int(y_new+0.5)-1)*nx_new) = &
                                    & grid_data_new(int(x_new+0.5)+(int(y_new+0.5)-1)*nx_new) + fMassSml
              ...
              else                                                           ! both poles are rotated
               call get_rotation(fu_southpole_lat(grid_orig%lonlat%pole), &
                               & fu_southpole_lon(grid_orig%lonlat%pole), R)
               call get_rotation(fu_southpole_lat(grid_new%lonlat%pole), &
                               & fu_southpole_lon(grid_new%lonlat%pole), R2)
               ...
               do iy = 1, ny_orig
                do ix = 1, nx_orig
                 ...
                 lon_orig = grid_orig%lonlat%sw_corner_modlon + (ix-1.5) * grid_orig%lonlat%dx_deg
                 lat_orig = grid_orig%lonlat%sw_corner_modlat + (iy-1.5) * grid_orig%lonlat%dy_deg
                 call transform_latlon(lon_orig, lat_orig, R, rotated_to_world, lon_world, lat_world)
                 call transform_latlon(lon_world, lat_world, R2, world_to_rotated, lon_new, lat_new)
                 x_new = 1+ (lon_new - grid_new%lonlat%sw_corner_modlon) / grid_new%lonlat%dx_deg
                 y_new = 1+ (lat_new - grid_new%lonlat%sw_corner_modlat) / grid_new%lonlat%dy_deg
                 if(x_new<0.5 .or. x_new>nx_new+0.5 .or. y_new<0.5 .or. y_new>ny_new+0.5)then
...
if(ifRANDOMISE)then
                    ...
                    if(error)then
                      call msg('Rotated lon & lat:',lon_orig, lat_orig)
                      call set_error('Rotated to rotated reprojection failed','grid_transformation')
                    endif
                    if(x_new>=0.5 .and. x_new<=nx_new+0.5 .and. y_new>=0.5 .and. y_new<=ny_new+0.5)then
                      grid_data_new(int(x_new+0.5)+(int(y_new+0.5)-1)*nx_new) = &
                                    & grid_data_new(int(x_new+0.5)+(int(y_new+0.5)-1)*nx_new) + fMassSml
              ...
              endif  ! poles of the grids

            case(anygrid)                                        ! grid original is lonlat, grid_new is anygrid

             if(grid_orig%lonlat%pole == pole_geographical)then
              ...
              do iy = 1, ny_orig
               do ix = 1, nx_orig
                 ...
                 ! Check corners of the cell: do we need to bother?
                 !
                 lon_world = grid_orig%lonlat%sw_corner_modlon + (ix-1.5) * grid_orig%lonlat%dx_deg
                 lat_world = grid_orig%lonlat%sw_corner_modlat + (iy-1.5) * grid_orig%lonlat%dy_deg
                 call project_point_to_grid_lonlat(lon_world, lat_world, grid_new, x_new, y_new)
                 if(x_new<0.5 .or. x_new>nx_new+0.5 .or. y_new<0.5 .or. y_new>ny_new+0.5)then
                 
                   lat_world = grid_orig%lonlat%sw_corner_modlat + (iy-0.5) * grid_orig%lonlat%dy_deg
                   call project_point_to_grid_lonlat(lon_world, lat_world, grid_new, x_new, y_new)
                   if(x_new<0.5 .or. x_new>nx_new+0.5 .or. y_new<0.5 .or. y_new>ny_new+0.5)then
                   
                     lon_world = grid_orig%lonlat%sw_corner_modlon + (ix-0.5) * grid_orig%lonlat%dx_deg
                     call project_point_to_grid_lonlat(lon_world, lat_world, grid_new, x_new, y_new)
                     if(x_new<0.5 .or. x_new>nx_new+0.5 .or. y_new<0.5 .or. y_new>ny_new+0.5)then
...
if(ifRANDOMISE)then
                    lon_orig = grid_orig%lonlat%sw_corner_modlon + &
                         & (ix-1.5+(iSml-1.5)*fSmall_1+fu_random_number_center_rng(rng, 0.,fSmall_1)) * &
                         & grid_orig%lonlat%dx_deg
                    lat_orig = grid_orig%lonlat%sw_corner_modlat + &
                         & (iy-1.5+(jSml-1.5)*fSmall_1+fu_random_number_center_rng(rng, 0.,fSmall_1)) * &
                         & grid_orig%lonlat%dy_deg
else
                    lon_orig = grid_orig%lonlat%sw_corner_modlon + &
                         & (ix-1.5+(iSml-1.5)*fSmall_1) * grid_orig%lonlat%dx_deg
                    lat_orig = grid_orig%lonlat%sw_corner_modlat + &
                         & (iy-1.5+(jSml-1.5)*fSmall_1) * grid_orig%lonlat%dy_deg
endif
                    call project_point_to_grid_lonlat(lon_orig, lat_orig, grid_new, x_new, y_new)
                    if(error)then
                      call msg('Rotated lon & lat:',lon_orig, lat_orig)
                      call set_error('Rotated to rotated reprojection failed','grid_transformation')
                    endif
              ...
              else   ! original grid is lonlat rotated
...
if(ifRANDOMISE)then
              ...
              endif   ! pole of the priginal grid

            case default
              CALL set_error('new grid of unknown type','grid_transformation')
              RETURN
          end select  ! type of the new grid

        case(anygrid)  ! Old grid
        
          SELECT CASE (grid_new%gridtype)

            case(lonlat)     ! new grid is lonlat

             if(grid_new%lonlat%pole == pole_geographical)then
              ...
              do iy = 1, ny_orig
               do ix = 1, nx_orig
                 ...
                 !
                 iG = (iy-1)*nx_orig+ix
                 if(grid_data_orig(iG) .eps. missing_value)cycle

                 lon_orig = pAnyGrdParam(grid_orig%ag%indParam)%ptr%xC(iG) - &
                          & pAnyGrdParam(grid_orig%ag%indParam)%ptr%dx(iG) * 0.5 * &
                                      & pAnyGrdParam(grid_orig%ag%indParam)%ptr%cos_map_rot(iG) - &
                          & pAnyGrdParam(grid_orig%ag%indParam)%ptr%dy(iG) * 0.5 * &
                                      & pAnyGrdParam(grid_orig%ag%indParam)%ptr%sin_map_rot(iG)
                 lat_orig = pAnyGrdParam(grid_orig%ag%indParam)%ptr%yC(iG) - &
                          & pAnyGrdParam(grid_orig%ag%indParam)%ptr%dx(iG) * 0.5 * &
                                      & pAnyGrdParam(grid_orig%ag%indParam)%ptr%sin_map_rot(iG) - &
                          & pAnyGrdParam(grid_orig%ag%indParam)%ptr%dy(iG) * 0.5 * &
                                      & pAnyGrdParam(grid_orig%ag%indParam)%ptr%cos_map_rot(iG)
...
if(ifRANDOMISE)then
                    lon_orig = pAnyGrdParam(grid_orig%ag%indParam)%ptr%xC(iG) - &
                             & pAnyGrdParam(grid_orig%ag%indParam)%ptr%dx(iG) * &
                              & (0.5 + (iSml-0.5)*fSmall_1 + fu_random_number_center_rng(rng, 0.,fSmall_1)) * &
                                      & pAnyGrdParam(grid_orig%ag%indParam)%ptr%cos_map_rot(iG) - &
                             & pAnyGrdParam(grid_orig%ag%indParam)%ptr%dy(iG) * &
                              & (0.5 + (jSml-0.5)*fSmall_1 + fu_random_number_center_rng(rng, 0.,fSmall_1)) * &
                                      & pAnyGrdParam(grid_orig%ag%indParam)%ptr%sin_map_rot(iG)
                    lat_orig = pAnyGrdParam(grid_orig%ag%indParam)%ptr%yC(iG) - &
                             & pAnyGrdParam(grid_orig%ag%indParam)%ptr%dx(iG) * &
                              & (0.5 + (iSml-0.5)*fSmall_1 + fu_random_number_center_rng(rng, 0.,fSmall_1)) * &
                    ...
                    lon_orig = pAnyGrdParam(grid_orig%ag%indParam)%ptr%xC(iG) - &
                             & pAnyGrdParam(grid_orig%ag%indParam)%ptr%dx(iG) * &
                              & (0.5 + (iSml-0.5)*fSmall_1) * &
                                      & pAnyGrdParam(grid_orig%ag%indParam)%ptr%cos_map_rot(iG) - &
                             & pAnyGrdParam(grid_orig%ag%indParam)%ptr%dy(iG) * &
                              & (0.5 + (jSml-0.5)*fSmall_1) * &
                                      & pAnyGrdParam(grid_orig%ag%indParam)%ptr%sin_map_rot(iG)
                    lat_orig = pAnyGrdParam(grid_orig%ag%indParam)%ptr%yC(iG) - &
                             & pAnyGrdParam(grid_orig%ag%indParam)%ptr%dx(iG) * &
                              & (0.5 + (iSml-0.5)*fSmall_1) * &
             ...
             else   ! new grid is lonlat rotated, old one is anygrid
...
if(ifRANDOMISE)then
            ...
            case(anygrid)  ! both grids are anygrid
              ...
              do iy = 1, ny_orig
               do ix = 1, nx_orig
                 ...
                 lat_orig = pAnyGrdParam(grid_orig%ag%indParam)%ptr%yC(iG) - &
                          ...
                          & pAnyGrdParam(grid_orig%ag%indParam)%ptr%dy(iG) * 0.5 * &
                                      & pAnyGrdParam(grid_orig%ag%indParam)%ptr%cos_map_rot(iG)
                 call project_point_to_grid_lonlat(lon_orig, lat_orig, grid_orig, x_new, y_new)
                 if(x_new<0.5 .or. x_new>nx_new+0.5 .or. y_new<0.5 .or. y_new>ny_new+0.5)then
...
if(ifRANDOMISE)then
            ...
            case default
              CALL set_error('new grid of unknown type','grid_transformation')
              RETURN
          end select  ! type of the original grid

        case default
          CALL set_error('original grid of unknown type','grid_transformation')
          RETURN
      ...
      if(method == average)then
        do iy = 1, ny_new
          do ix = 1, nx_new
            if(iCounter(ix+(iy-1)*nx_new) > 0) &
                        & grid_data_new(ix+(iy-1)*nx_new) = grid_data_new(ix+(iy-1)*nx_new) / &
                        & iCounter(ix+(iy-1)*nx_new)
          end do
        end do
        call free_work_array(iCounter)
      endif

!open(50, file='alluvial_sediment.grads', form='unformatted', access='direct', &
!   & recl=nx_new*ny_new)
!arTmp(1:nx_new*ny_new) = grid_data_new(:)
!write(50,rec=1)arTmp(1:nx_new*ny_new)
!close(50)
!call report(grid_new)
     ...
     case(int_missing)
       !
       ! Not-interpolatable quantities, such as Pasquill class of land-use type
       ! Can be accepted only if no regridding is expected. These cases should have
       ! been treated above. Not happened, hense error.
       !
       call set_error('Attempt to regrid non-regriddable field','grid_transformation')
       return
     
     case default
      !
      ! All sorts of interpolation: nearest-point, linear, bi-cubic, etc.
      ! All what is implemented in fu_2d_interpolation
      !
      SELECT CASE (grid_orig%gridtype)
        CASE (lonlat)
          ...
          SELECT CASE (grid_new%gridtype)
            CASE (lonlat)
              ...
              if(grid_orig%lonlat%pole == grid_new%lonlat%pole)then
                do iy = 1, ny_new
                 do ix = 1, nx_new

                   lon_new = grid_new%lonlat%sw_corner_modlon + (ix-1) * grid_new%lonlat%dx_deg
                   lat_new = grid_new%lonlat%sw_corner_modlat + (iy-1) * grid_new%lonlat%dy_deg
                   x_orig = 1+ (lon_new - grid_orig%lonlat%sw_corner_modlon) / grid_orig%lonlat%dx_deg
                   y_orig = 1+ (lat_new - grid_orig%lonlat%sw_corner_modlat) / grid_orig%lonlat%dy_deg
                   grid_data_new(ix+(iy-1)*nx_new) = fu_2d_interpolation(grid_data_orig_ptr, x_orig, y_orig, &
                                                                       & nx_orig, ny_orig, method, iOutLocal)
                   if(error)then
                     call msg('Input lon & lat:',lon_new, lat_new)
                     call set_error('same-pole to same-pole reprojection failed','grid_transformation')
                   endif
                 enddo
                enddo
    
              elseif(grid_orig%lonlat%pole == pole_geographical)then

                call get_rotation(fu_southpole_lat(grid_new%lonlat%pole), fu_southpole_lon(grid_new%lonlat%pole), R)
                do iy = 1, ny_new
                 do ix = 1, nx_new
                   lon_new = grid_new%lonlat%sw_corner_modlon + (ix-1) * grid_new%lonlat%dx_deg
                   lat_new = grid_new%lonlat%sw_corner_modlat + (iy-1) * grid_new%lonlat%dy_deg
                   call transform_latlon(lon_new, lat_new, R, rotated_to_world , lon_orig, lat_orig)
                   if(error)return
                   x_orig = 1+ (lon_orig - grid_orig%lonlat%sw_corner_modlon) / grid_orig%lonlat%dx_deg
                   y_orig = 1+ (lat_orig - grid_orig%lonlat%sw_corner_modlat) / grid_orig%lonlat%dy_deg
                   grid_data_new(ix+(iy-1)*nx_new) = fu_2d_interpolation(grid_data_orig_ptr, &
                                                                       & x_orig, y_orig, &
                                                                       & nx_orig, ny_orig, method, iOutLocal)
                   if(error)then
                     call msg('Rotated lon & lat:',lon_new, lat_new)
                     call set_error('Rotated to world reprojection failed','grid_transformation')
                   endif
                 enddo
                enddo

              elseif(grid_new%lonlat%pole == pole_geographical)then
                call get_rotation(fu_southpole_lat(grid_orig%lonlat%pole), &
                                & fu_southpole_lon(grid_orig%lonlat%pole), R)
                do iy = 1, ny_new
                 do ix = 1, nx_new
                   lon_new = grid_new%lonlat%sw_corner_modlon + (ix-1) * grid_new%lonlat%dx_deg
                   lat_new = grid_new%lonlat%sw_corner_modlat + (iy-1) * grid_new%lonlat%dy_deg
                   call transform_latlon(lon_new, lat_new, R, world_to_rotated, lon_orig, lat_orig)
                   if(error)return
                   x_orig = 1+ (lon_orig - grid_orig%lonlat%sw_corner_modlon) / grid_orig%lonlat%dx_deg
                   y_orig = 1+ (lat_orig - grid_orig%lonlat%sw_corner_modlat) / grid_orig%lonlat%dy_deg
                   grid_data_new(ix+(iy-1)*nx_new) = fu_2d_interpolation(grid_data_orig_ptr, &
                                                                       & x_orig, y_orig, nx_orig, &
                                                                       & ny_orig, method, iOutLocal)
                   if(error)then
                     call msg('Rotated lon & lat:',lon_new, lat_new)
                     call set_error('World to rotated reprojection failed','grid_transformation')
                   endif
                ...
                call get_rotation(fu_southpole_lat(grid_new%lonlat%pole), &
                                & fu_southpole_lon(grid_new%lonlat%pole), R)
                call get_rotation(fu_southpole_lat(grid_orig%lonlat%pole), &
                                & fu_southpole_lon(grid_orig%lonlat%pole), R2)

                do iy = 1, ny_new
                 do ix = 1, nx_new
                   lon_new = grid_new%lonlat%sw_corner_modlon + (ix-1) * grid_new%lonlat%dx_deg
                   lat_new = grid_new%lonlat%sw_corner_modlat + (iy-1) * grid_new%lonlat%dy_deg
                   call transform_latlon(lon_new, lat_new, R, rotated_to_world , lon_world, lat_world)
                   if(error)return
                   call transform_latlon(lon_world, lat_world, R2, world_to_rotated, lon_orig, lat_orig)
                   if(error)return
                   x_orig = 1+ (lon_orig - grid_orig%lonlat%sw_corner_modlon) / grid_orig%lonlat%dx_deg
                   y_orig = 1+ (lat_orig - grid_orig%lonlat%sw_corner_modlat) / grid_orig%lonlat%dy_deg
                   grid_data_new(ix+(iy-1)*nx_new) = fu_2d_interpolation(grid_data_orig_ptr, &
            ...
            case(anygrid)
              if(grid_orig%lonlat%pole == pole_geographical)then
                do iy = 1, ny_new
                 do ix = 1, nx_new
                   lon_new = pAnyGrdParam(grid_new%ag%indParam)%ptr%xC((iy-1)*nx_new+ix)
                   lat_new = pAnyGrdParam(grid_new%ag%indParam)%ptr%yC((iy-1)*nx_new+ix)
                   x_orig = 1+ (lon_new - grid_orig%lonlat%sw_corner_modlon) / grid_orig%lonlat%dx_deg
                   y_orig = 1+ (lat_new - grid_orig%lonlat%sw_corner_modlat) / grid_orig%lonlat%dy_deg
                   grid_data_new(ix+(iy-1)*nx_new) = fu_2d_interpolation(grid_data_orig_ptr, &
                                                                       & x_orig, y_orig, &
                   ...
                   if(error)then
                     call msg('Input lon & lat:',lon_new, lat_new)
                     call set_error('World to anygrid reprojection failed','grid_transformation')
                   endif
                 enddo
                enddo
                ...
                do iy = 1, ny_new
                 do ix = 1, nx_new
                   ...
                   if(error)then
                     call msg('Input lon & lat:',lon_new, lat_new)
                     call set_error('Rotated to anygrid reprojection failed','grid_transformation')
                   endif
                 enddo
                enddo
              endif
          ...
          SELECT CASE (grid_new%gridtype)
            case(lonlat)
              ...
              if(grid_new%lonlat%pole == pole_geographical)then
                do iy = 1, ny_new
                 do ix = 1, nx_new
                   lon_world = grid_new%lonlat%sw_corner_modlon + (ix-1) * grid_new%lonlat%dx_deg
                   lat_world = grid_new%lonlat%sw_corner_modlat + (iy-1) * grid_new%lonlat%dy_deg
                   call project_point_to_grid_lonlat(lon_world, lat_world, grid_orig, x_orig, y_orig)
                   grid_data_new(ix+(iy-1)*nx_new) = fu_2d_interpolation(grid_data_orig_ptr, &
                                                                       & x_orig, y_orig, &
                                                                       & nx_orig, ny_orig, method, iOutLocal)
                 enddo
            ...
            case(anygrid)
              ...
              do iy = 1, ny_new
               do ix = 1, nx_new
                 lon_world = pAnyGrdParam(grid_new%ag%indParam)%ptr%xC((iy-1)*nx_new+ix)
                 lat_world = pAnyGrdParam(grid_new%ag%indParam)%ptr%yC((iy-1)*nx_new+ix)
                 call project_point_to_grid_lonlat(lon_world, lat_world, grid_orig, x_orig, y_orig)
                 if(error)return
                 grid_data_new(ix+(iy-1)*nx_new) = fu_2d_interpolation(grid_data_orig_ptr, &
                                                     & x_orig, y_orig, nx_orig, ny_orig, method, iOutLocal)
               enddo
              enddo
            case default
              CALL set_error('new grid of unknown type','grid_transformation')
    ...
    end select  ! method of reprojection

    do ix = 1, nx_new * ny_new
      if(grid_data_new(ix) .eps. real_missing) grid_data_new(ix) = missing_value
    end do

  END SUBROUTINE grid_transformation
  ...
  FUNCTION fu_anygrid_containing_area(gr_orig, area) result(grid_new)
    ...
    do iy = nint(ymin), nint(ymax)
      do ix = nint(xmin), nint(xmax)
        ...
        lons(i) = pAnyGrdParam(gr_orig%ag%indParam)%ptr%xC((iy-1)*gr_orig%ag%nx+ix)
        lats(i) = pAnyGrdParam(gr_orig%ag%indParam)%ptr%xC((iy-1)*gr_orig%ag%nx+ix)
        dx(i) = pAnyGrdParam(gr_orig%ag%indParam)%ptr%xC((iy-1)*gr_orig%ag%nx+ix)
        dy(i) = pAnyGrdParam(gr_orig%ag%indParam)%ptr%xC((iy-1)*gr_orig%ag%nx+ix)
...
!    endif
    ...
    SELECT CASE(fu_gridtype(grid_to_cut))
      ...
      case(anygrid)
        ...
        ptr_grid_pars => pAnyGrdParam(grid_to_cut%ag%indParam)%ptr
...
!        case(anygrid)
  ...
  subroutine cut_grid_size_params(grid_to_cut, ix_start, iy_start, ix_end, iy_end)
    ...
    case (lonlat)
      ...
      grid_to_cut%lonlat%nx = ix_end - ix_start + 1
      grid_to_cut%lonlat%ny = iy_end - iy_start + 1
    ...
    case(anygrid)
      ...
      dx_tmp => fu_work_array()
      dy_tmp => fu_work_array()
      
      iCut = 1
      do j = iy_start, iy_end
        do i = ix_start, ix_end  
          lon_tmp(iCut) = ptr_grid_pars%xC(nx*(j-1)+i)
          lat_tmp(iCut) = ptr_grid_pars%yC(nx*(j-1)+i)
          dx_tmp(iCut) = ptr_grid_pars%dx(nx*(j-1)+i)
          dy_tmp(iCut) = ptr_grid_pars%dy(nx*(j-1)+i)
          iCut = icut + 1
        enddo
      enddo

      grid_to_cut%ag%nx = ix_end - ix_start + 1
      grid_to_cut%ag%ny = iy_end - iy_start + 1
      
      call setAgLonlatFlds(grid_to_cut, lon_tmp, lat_tmp, ifnew)
      !call setAnygridParam(grid_to_cut, 'lon', lon_tmp)
      !call setAnygridParam(grid_to_cut, 'lat', lat_tmp)
      if(ifnew)then
      call setAnygridParam(grid_to_cut, 'dx', dx_tmp)
      call setAnygridParam(grid_to_cut, 'dy', dy_tmp)

        if(associated(ptr_grid_pars%sin_map_rot))then
          iCut = 1
          do j = iy_start, iy_end
            do i = ix_start, ix_end 
              dx_tmp(iCut) = ptr_grid_pars%sin_map_rot(nx*(j-1)+i)
              iCut = icut + 1
            enddo
          enddo
          call setAnygridParam(grid_to_cut, 'sin_map_rot', dx_tmp)
        endif
        if(associated(ptr_grid_pars%cos_map_rot))then
          iCut = 1
          do j = iy_start, iy_end
            do i = ix_start, ix_end 
              dx_tmp(iCut) = ptr_grid_pars%cos_map_rot(nx*(j-1)+i)
              iCut = icut + 1
            enddo
          enddo
          call setAnygridParam(grid_to_cut, 'cos_map_rot', dx_tmp)
        endif
      endif 
       
      call free_work_array(lon_tmp)
      call free_work_array(lat_tmp)
      call free_work_array(dx_tmp)
      call free_work_array(dy_tmp)

    case default
      call set_error('Cannot update unknown grid type','cut_grid_size')
      return
    end select
 ...
 !   call report(gridOut)
    ...
    if (gridIni%gridtype == lonlat .and. gridOut%gridtype == lonlat) then
     ...
     else ! Get true  lon and lat
       if (gridIni%gridtype == anygrid) then 
            fLon = fu_2d_interpolation(pAnyGrdParam(gridIni%ag%indParam)%ptr%xC, &
                                             & x,y,gridIni%ag%nx,gridIni%ag%ny,method, notAllowed)
            fLat = fu_2d_interpolation(pAnyGrdParam(gridIni%ag%indParam)%ptr%yC, &
                                             & x,y,gridIni%ag%nx,gridIni%ag%ny,method, notAllowed)
  ...
  subroutine project_point_to_grid_lonlat(fLon, fLat, gridOut, xNew, yNew)
    ...
    select case(gridOut%gridtype)
      ...
      case (anygrid)
         ...
         lons(1:gridOut%ag%nx,1:gridOut%ag%ny) => pAnyGrdParam(gridOut%ag%indParam)%ptr%xC(1:gridOut%ag%nx*gridOut%ag%ny)
         lats(1:gridOut%ag%nx,1:gridOut%ag%ny) => pAnyGrdParam(gridOut%ag%indParam)%ptr%yC(1:gridOut%ag%nx*gridOut%ag%ny)
         ixC = gridOut%ag%nx / 2 !About the middle
         iyC = gridOut%ag%ny / 2
         !Find nearest point
         d = abs(lons(ixC, iyC)-fLon)  + abs(lats(ixC, iyC)-fLat)
         do ixD = 1, max(gridOut%ag%nx,gridOut%ag%ny)
            done=.true.
            do iyC = iyC, gridOut%ag%ny-1  ! Up
               dnext = abs(lons(ixC, iyC+1)-fLon)  + abs(lats(ixC, iyC+1)-fLat)
               if (dnext>d) exit
               d = dNext
               done=.false.
            enddo
            ...
            enddo
            do ixC = ixC, gridOut%ag%nx-1  ! Right
               dnext = abs(lons(ixC+1, iyC)-fLon)  + abs(lats(ixC+1, iyC)-fLat)
               if (dnext>d) exit
               d = dNext
               done=.false.
            enddo
            ...
            if (done) exit
         enddo
         if (ixD == max(gridOut%ag%nx,gridOut%ag%ny)) then
            call set_error("Could not project a point to anygrid", "project_point_to_grid_lonlat")
         endif


         !Find next nearest in each direction from the nearest one
         if (ixC == gridOut%ag%nx) then
             ixD=-1
         elseif (ixC == 1) then
            ixD=1
         elseif ((lons(ixC+1, iyC)-fLon)*(lons(ixC, iyC)-fLon) + (lats(ixC+1, iyC)-fLat)*(lats(ixC, iyC)-fLat) > 0) then
            ixD=-1
         else
            ixD=1
         endif 

         if (iyC == gridOut%ag%ny) then
            iyD=-1
         elseif (iyC == 1) then
            iyD=1
         elseif ((lons(ixC, iyC+1)-fLon)*(lons(ixC, iyC)-fLon) + (lats(ixC, iyC+1)-fLat)*(lats(ixC, iyC)-fLat) > 0) then
            iyD=-1
         else
            iyD=1
         endif 

         ! fraction of dx vector taken by a projection of (flon, flat)
         !Scalar product normalized by deltax**2
         dlon=lons(ixC, iyC) - lons(ixC + ixD, iyC)
         dlat=lats(ixC, iyC) - lats(ixC + ixD, iyC)
         xNew = ixD * ( (lons(ixC, iyC)-fLon)*dlon + (lats(ixC, iyC)-fLat)*dlat) /(dlat*dlat+dlon*dlon) + ixC
         !Scalar product normalized by deltax**2
         dlon=lons(ixC, iyC) - lons(ixC, iyC + iyD)
         dlat=lats(ixC, iyC) - lats(ixC, iyC + iyD)
         yNew = iyD * ( (lons(ixC, iyC)-fLon)*dlon + (lats(ixC, iyC)-fLat)*dlat) /(dlat*dlat+dlon*dlon) + iyC

      case default
        call report (gridOut)
        call set_error('Non-supported grid type','project_point_to_grid_lonlat')
...
!!$          call get_rotation(lat, lon, rotation_3d) ! gridFrom -> gridTo
      ...
      case(anygrid) !interpStructPtr%gridTo%gridtype
        ...
        anyGridParamTo => pAnyGrdParam(interpStructPtr%gridTo%ag%indParam)%ptr
        allocate(interpStructPtr%rotation(2, 2, nxTo, nyTo), stat=status)
        if (status /= 0) then
          call set_error('Allocating rotation map failed', 'fu_horiz_interp_struct')
          return
        ...
        do iy = 1, nyTo
          do ix = 1, nxTo
            ...
            if(.not. (associated(anyGridParamTo%cos_map_rot) .and. &
                    & associated(anyGridParamTo%sin_map_rot)))then
    ...
    case(anygrid)! interpStructPtr%gridFrom%gridtype
      anyGridParamFrom => pAnyGrdParam(interpStructPtr%gridFrom%ag%indParam)%ptr
      select case(interpStructPtr%gridTo%gridtype)
      case(lonlat) 
        ...
        do iy = 1, nyTo
          do ix = 1, nxTo
            ...
            if(.not. (associated(anyGridParamFrom%cos_map_rot) .and. &
                    & associated(anyGridParamFrom%sin_map_rot)))then
              call set_error('cannot turn winds without knowing map rotation','fu_horiz_interp_struct')
              return
            endif
   ...
   call msg("ixfromMin, ixfromMax", interpStructPtr%ixStartTo, interpStructPtr%ixEndTo)
    ...
    SELECT CASE (grid1%gridtype)
      ...
      case(anygrid)
        ...
        do i = 1, grid1%ag%nx*grid1%ag%ny
          if(.not.( (pAnyGrdParam(grid1%ag%indParam)%ptr%xC(i) &
                   & .eps. pAnyGrdParam(grid2%ag%indParam)%ptr%xC(i)) .and. &
                  & (pAnyGrdParam(grid1%ag%indParam)%ptr%yC(i) &
                   & .eps. pAnyGrdParam(grid2%ag%indParam)%ptr%yC(i)) ))then
            eq = .false.
            exit
          endif
        enddo
  ...
  subroutine SubArea_Chk(grid_large, grid_small, ix_shift, iy_shift, ifSubArea)
    ...
    if(grid_large%gridtype == grid_small%gridtype)then
      ...
      case(anygrid)
        ...
        do ix = 1, nx_large
          do iy = 1, ny_large
            if((pAnyGrdParam(grid_large%ag%indParam)%ptr%xC(nx_large*(iy-1)+ix) .eps. &
              & pAnyGrdParam(grid_small%ag%indParam)%ptr%xC(1)) .and. &
            &  (pAnyGrdParam(grid_large%ag%indParam)%ptr%yC(nx_large*(iy-1)+ix) .eps. &
              & pAnyGrdParam(grid_small%ag%indParam)%ptr%yC(1)))then
              ix_shift = ix-1
              iy_shift = iy-1
              exit
            endif
        ...
        if(ix_shift>=0 .and. (ix_shift+nx_small)<=nx_large .and. (iy_shift+ny_small)<=ny_large)then
          ...
          do ix = 1, nx_small
            do iy = 1, ny_small
              if(.not. ((pAnyGrdParam(grid_small%ag%indParam)%ptr%xC(nx_small*(iy-1)+ix) .eps. &
                       & pAnyGrdParam(grid_large%ag%indParam)%ptr%xC(nx_large*(iy+iy_shift-1)+ix+ix_shift)) &
                       & .and. &
                      & (pAnyGrdParam(grid_small%ag%indParam)%ptr%yC(nx_small*(iy-1)+ix) .eps. &
                       & pAnyGrdParam(grid_large%ag%indParam)%ptr%yC(nx_large*(iy+iy_shift-1)+ix+ix_shift))))then
                ifSubArea = .false.
                return
              endif
            enddo
...
!
    SELECT CASE (grd1%gridtype)
      ...
      case(anygrid)
        ...
        do ix = 1, nx1
          do iy = 1, ny1
            if((pAnyGrdParam(grd1%ag%indParam)%ptr%xC(nx1*(iy-1)+ix) .eps. &
              & pAnyGrdParam(grd2%ag%indParam)%ptr%xC(1)) .and. &
            &  (pAnyGrdParam(grd1%ag%indParam)%ptr%yC(nx1*(iy-1)+ix) .eps. &
              & pAnyGrdParam(grd2%ag%indParam)%ptr%yC(1)))then
              ix_shift = ix-1
              iy_shift = iy-1
              exit
            endif
        ...
        if(ix_shift>=0 .and. iy_shift>=0 .and. (ix_shift+nx2)<=nx1 .and. (iy_shift+ny2)<=ny1)then
          ...
          do ix = 1, nx2
            do iy = 1, ny2
              if(.not. ((pAnyGrdParam(grd2%ag%indParam)%ptr%xC(nx2*(iy-1)+ix) .eps. &
                       & pAnyGrdParam(grd1%ag%indParam)%ptr%xC(nx1*(iy+iy_shift-1)+ix+ix_shift)) .and. &
                      & (pAnyGrdParam(grd2%ag%indParam)%ptr%yC(nx2*(iy-1)+ix) .eps. &
                       & pAnyGrdParam(grd1%ag%indParam)%ptr%yC(nx1*(iy+iy_shift-1)+ix+ix_shift))))then
                fu_grids_correspond = .false.
                return
              endif
            enddo
  ...
  logical function fu_grids_arakawa_correspond(grd1, grd2)   
    ...
    case(anygrid)
      ...
      !
      if(abs(pAnyGrdParam(grd1%ag%indParam)%ptr%xC(1) - &
           & pAnyGrdParam(grd2%ag%indParam)%ptr%xC(1)) > &
       & fu_dx_m_to_deg(pAnyGrdParam(grd1%ag%indParam)%ptr%dx(1), &
                      & pAnyGrdParam(grd1%ag%indParam)%ptr%yC(1)))return
      if(abs(pAnyGrdParam(grd1%ag%indParam)%ptr%yC(1) - &
           & pAnyGrdParam(grd2%ag%indParam)%ptr%yC(1)) > &
       & fu_dy_m_to_deg(pAnyGrdParam(grd1%ag%indParam)%ptr%dy(1)))return

      if(abs(pAnyGrdParam(grd1%ag%indParam)%ptr%xC(grd1%ag%nx) - &
           & pAnyGrdParam(grd2%ag%indParam)%ptr%xC(grd2%ag%nx)) > &
       & fu_dx_m_to_deg(pAnyGrdParam(grd1%ag%indParam)%ptr%dx(grd1%ag%nx), &
                      & pAnyGrdParam(grd1%ag%indParam)%ptr%yC(grd1%ag%nx)))return
      if(abs(pAnyGrdParam(grd1%ag%indParam)%ptr%yC(grd1%ag%nx) - &
           & pAnyGrdParam(grd2%ag%indParam)%ptr%yC(grd2%ag%nx)) > &
       & fu_dy_m_to_deg(pAnyGrdParam(grd1%ag%indParam)%ptr%dy(grd1%ag%nx)))return

      if(abs(pAnyGrdParam(grd1%ag%indParam)%ptr%xC(grd1%ag%nx*grd1%ag%ny) - &
           & pAnyGrdParam(grd2%ag%indParam)%ptr%xC(grd2%ag%nx*grd2%ag%ny)) > &
       & fu_dx_m_to_deg(pAnyGrdParam(grd1%ag%indParam)%ptr%dx(grd1%ag%nx*grd1%ag%ny), &
                      & pAnyGrdParam(grd1%ag%indParam)%ptr%yC(grd1%ag%nx*grd1%ag%ny)))return
      if(abs(pAnyGrdParam(grd1%ag%indParam)%ptr%yC(grd1%ag%nx*grd1%ag%ny) - &
           & pAnyGrdParam(grd2%ag%indParam)%ptr%yC(grd2%ag%nx*grd2%ag%ny)) > &
       & fu_dy_m_to_deg(pAnyGrdParam(grd1%ag%indParam)%ptr%dy(grd1%ag%nx*grd1%ag%ny)))return

      if(abs(pAnyGrdParam(grd1%ag%indParam)%ptr%xC(grd1%ag%nx*(grd1%ag%ny-1)+1) - &
           & pAnyGrdParam(grd2%ag%indParam)%ptr%xC(grd2%ag%nx*(grd2%ag%ny-1)+1)) > &
       & fu_dx_m_to_deg(pAnyGrdParam(grd1%ag%indParam)%ptr%dx(grd1%ag%nx*(grd1%ag%ny-1)+1), &
                      & pAnyGrdParam(grd1%ag%indParam)%ptr%yC(grd1%ag%nx*(grd1%ag%ny-1)+1)))return
      if(abs(pAnyGrdParam(grd1%ag%indParam)%ptr%yC(grd1%ag%nx*(grd1%ag%ny-1)+1) - &
           & pAnyGrdParam(grd2%ag%indParam)%ptr%yC(grd2%ag%nx*(grd2%ag%ny-1)+1)) > &
       & fu_dy_m_to_deg(pAnyGrdParam(grd1%ag%indParam)%ptr%dy(grd1%ag%nx*(grd1%ag%ny-1)+1)))return
    ...
    case(anygrid)
      ...
      ! Check the 4 corners, if distance less than dx or dy
      !
      if(abs(pAnyGrdParam(grid1%ag%indParam)%ptr%xC(1) - &
           & pAnyGrdParam(grid2%ag%indParam)%ptr%xC(1)) > &
       & fu_dx_m_to_deg(pAnyGrdParam(grid1%ag%indParam)%ptr%dx(1), &
                      & pAnyGrdParam(grid1%ag%indParam)%ptr%yC(1)))return
      if(abs(pAnyGrdParam(grid1%ag%indParam)%ptr%yC(1) - &
           & pAnyGrdParam(grid2%ag%indParam)%ptr%yC(1)) > &
       & fu_dy_m_to_deg(pAnyGrdParam(grid1%ag%indParam)%ptr%dy(1)))return

      if(abs(pAnyGrdParam(grid1%ag%indParam)%ptr%xC(grid1%ag%nx) - &
           & pAnyGrdParam(grid2%ag%indParam)%ptr%xC(grid2%ag%nx)) > &
       & fu_dx_m_to_deg(pAnyGrdParam(grid1%ag%indParam)%ptr%dx(grid1%ag%nx), &
                      & pAnyGrdParam(grid1%ag%indParam)%ptr%yC(grid1%ag%nx)))return
      if(abs(pAnyGrdParam(grid1%ag%indParam)%ptr%yC(grid1%ag%nx) - &
           & pAnyGrdParam(grid2%ag%indParam)%ptr%yC(grid2%ag%nx)) > &
       & fu_dy_m_to_deg(pAnyGrdParam(grid1%ag%indParam)%ptr%dy(grid1%ag%nx)))return

      if(abs(pAnyGrdParam(grid1%ag%indParam)%ptr%xC(grid1%ag%nx*grid1%ag%ny) - &
           & pAnyGrdParam(grid2%ag%indParam)%ptr%xC(grid2%ag%nx*grid2%ag%ny)) > &
       & fu_dx_m_to_deg(pAnyGrdParam(grid1%ag%indParam)%ptr%dx(grid1%ag%nx*grid1%ag%ny), &
                      & pAnyGrdParam(grid1%ag%indParam)%ptr%yC(grid1%ag%nx*grid1%ag%ny)))return
      if(abs(pAnyGrdParam(grid1%ag%indParam)%ptr%yC(grid1%ag%nx*grid1%ag%ny) - &
           & pAnyGrdParam(grid2%ag%indParam)%ptr%yC(grid2%ag%nx*grid2%ag%ny)) > &
       & fu_dy_m_to_deg(pAnyGrdParam(grid1%ag%indParam)%ptr%dy(grid1%ag%nx*grid1%ag%ny)))return

      if(abs(pAnyGrdParam(grid1%ag%indParam)%ptr%xC(grid1%ag%nx*(grid1%ag%ny-1)+1) - &
           & pAnyGrdParam(grid2%ag%indParam)%ptr%xC(grid2%ag%nx*(grid2%ag%ny-1)+1)) > &
       & fu_dx_m_to_deg(pAnyGrdParam(grid1%ag%indParam)%ptr%dx(grid1%ag%nx*(grid1%ag%ny-1)+1), &
                      & pAnyGrdParam(grid1%ag%indParam)%ptr%yC(grid1%ag%nx*(grid1%ag%ny-1)+1)))return
      if(abs(pAnyGrdParam(grid1%ag%indParam)%ptr%yC(grid1%ag%nx*(grid1%ag%ny-1)+1) - &
           & pAnyGrdParam(grid2%ag%indParam)%ptr%yC(grid2%ag%nx*(grid2%ag%ny-1)+1)) > &
       & fu_dy_m_to_deg(pAnyGrdParam(grid1%ag%indParam)%ptr%dy(grid1%ag%nx*(grid1%ag%ny-1)+1)))return
  ...
  subroutine grid_shift_indices(grid1, grid2, shift_lon, shift_lat)
    ...
    case(anygrid)
      ...
      if(fu_grids_arakawa_correspond(grid1, grid2))then
        ...
        !
        diff = (pAnyGrdParam(grid1%ag%indParam)%ptr%xC(1) -  &
              & pAnyGrdParam(grid2%ag%indParam)%ptr%xC(1)) / &
             & fu_dx_m_to_deg(pAnyGrdParam(grid1%ag%indParam)%ptr%dx(1), &
                            & pAnyGrdParam(grid2%ag%indParam)%ptr%yC(1))
        ...
        diff = (pAnyGrdParam(grid1%ag%indParam)%ptr%yC(1) -  &
              & pAnyGrdParam(grid2%ag%indParam)%ptr%yC(1)) / &
             & fu_dy_m_to_deg(pAnyGrdParam(grid1%ag%indParam)%ptr%dy(1))
...
!     CALL report(grid)
    ...
    case(anygrid)
      ...
      if(abs(pAnyGrdParam(grid%ag%indParam)%ptr%xC(1) - &
           & pAnyGrdParam(grid_sample%ag%indParam)%ptr%xC(1)) > &
       & fu_dx_m_to_deg(pAnyGrdParam(grid%ag%indParam)%ptr%dx(1), &
                      & pAnyGrdParam(grid%ag%indParam)%ptr%yC(1)))return
      if(abs(pAnyGrdParam(grid%ag%indParam)%ptr%yC(1) - &
           & pAnyGrdParam(grid_sample%ag%indParam)%ptr%yC(1)) > &
       & fu_dy_m_to_deg(pAnyGrdParam(grid%ag%indParam)%ptr%dy(1)))return

      if(abs(pAnyGrdParam(grid%ag%indParam)%ptr%xC(grid%ag%nx) - &
           & pAnyGrdParam(grid_sample%ag%indParam)%ptr%xC(grid_sample%ag%nx)) > &
       & fu_dx_m_to_deg(pAnyGrdParam(grid%ag%indParam)%ptr%dx(grid%ag%nx), &
                      & pAnyGrdParam(grid%ag%indParam)%ptr%yC(grid%ag%nx)))return
      if(abs(pAnyGrdParam(grid%ag%indParam)%ptr%yC(grid%ag%nx) - &
           & pAnyGrdParam(grid_sample%ag%indParam)%ptr%yC(grid_sample%ag%nx)) > &
       & fu_dy_m_to_deg(pAnyGrdParam(grid%ag%indParam)%ptr%dy(grid%ag%nx)))return

      if(abs(pAnyGrdParam(grid%ag%indParam)%ptr%xC(grid%ag%nx*grid%ag%ny) - &
           & pAnyGrdParam(grid_sample%ag%indParam)%ptr%xC(grid_sample%ag%nx*grid_sample%ag%ny)) > &
       & fu_dx_m_to_deg(pAnyGrdParam(grid%ag%indParam)%ptr%dx(grid%ag%nx*grid%ag%ny), &
                      & pAnyGrdParam(grid%ag%indParam)%ptr%yC(grid%ag%nx*grid%ag%ny)))return
      if(abs(pAnyGrdParam(grid%ag%indParam)%ptr%yC(grid%ag%nx*grid%ag%ny) - &
           & pAnyGrdParam(grid_sample%ag%indParam)%ptr%yC(grid_sample%ag%nx*grid_sample%ag%ny)) > &
       & fu_dy_m_to_deg(pAnyGrdParam(grid%ag%indParam)%ptr%dy(grid%ag%nx*grid%ag%ny)))return

      if(abs(pAnyGrdParam(grid%ag%indParam)%ptr%xC(grid%ag%nx*(grid%ag%ny-1)+1) - &
           & pAnyGrdParam(grid_sample%ag%indParam)%ptr%xC(grid_sample%ag%nx*(grid_sample%ag%ny-1)+1)) > &
       & fu_dx_m_to_deg(pAnyGrdParam(grid%ag%indParam)%ptr%dx(grid%ag%nx*(grid%ag%ny-1)+1), &
                      & pAnyGrdParam(grid%ag%indParam)%ptr%yC(grid%ag%nx*(grid%ag%ny-1)+1)))return
      if(abs(pAnyGrdParam(grid%ag%indParam)%ptr%yC(grid%ag%nx*(grid%ag%ny-1)+1) - &
           & pAnyGrdParam(grid_sample%ag%indParam)%ptr%yC(grid_sample%ag%nx*(grid_sample%ag%ny-1)+1)) > &
       & fu_dy_m_to_deg(pAnyGrdParam(grid%ag%indParam)%ptr%dy(grid%ag%nx*(grid%ag%ny-1)+1)))return
...
!    call msg("fu_area_coverage")
  ...
  subroutine write_anygrid_2_gradsfile(grid, fNm)
    ...
    if(associated(pAnyGrdParam(grid%ag%indParam)%ptr%xC))then
      if(size(pAnyGrdParam(grid%ag%indParam)%ptr%xC) >= nPoints)then
        nVars = nVars + 1
     endif
    endif
    if(associated(pAnyGrdParam(grid%ag%indParam)%ptr%yC))then
      if(size(pAnyGrdParam(grid%ag%indParam)%ptr%yC) >= nPoints)then
        nVars = nVars + 1
      endif
    endif
    if(nVars /= 2)then
    ...
    if(associated(pAnyGrdParam(grid%ag%indParam)%ptr%dx))then
      if(size(pAnyGrdParam(grid%ag%indParam)%ptr%dx) >= nPoints)then
        nVars = nVars + 1
        ifdx = .true.
      endif
    endif
    if(associated(pAnyGrdParam(grid%ag%indParam)%ptr%dy))then
      if(size(pAnyGrdParam(grid%ag%indParam)%ptr%dy) >= nPoints)then
        nVars = nVars + 1
        ifdy = .true.
      endif
    endif
    if(associated(pAnyGrdParam(grid%ag%indParam)%ptr%sin_map_rot))then
     if(size(pAnyGrdParam(grid%ag%indParam)%ptr%sin_map_rot) >= nPoints)then
        nVars = nVars + 1
        ifsin = .true.
      endif
    endif
    if(associated(pAnyGrdParam(grid%ag%indParam)%ptr%cos_map_rot))then
      if(size(pAnyGrdParam(grid%ag%indParam)%ptr%cos_map_rot) >= nPoints)then
        ifcos = .true.
        nVars = nVars + 1
      endif
    endif
    ...
    WRITE(u_grd_ctl,'(A5,I5,A8,2(F15.7,1x))') 'YDEF ',grid%ag%ny,' LINEAR ', &
                                                & pAnyGrdParam(grid%ag%indParam)%ptr%yc(1), &
                                                & fu_dy_m_to_deg(pAnyGrdParam(grid%ag%indParam)%ptr%dy(1))
    ...
    ! lon
    call write_grads_field(pAnyGrdParam(grid%ag%indParam)%ptr%xc, nPoints, rec, u_grd_bin)
    if(error)return
    WRITE(u_grd_ctl,'(A,A12,A,2x,A)') 'lon             ','0 99 99 0 ', 'longitude of the grid cell'
    rec = rec+1
   ! lat
    call write_grads_field(pAnyGrdParam(grid%ag%indParam)%ptr%yc, nPoints, rec, u_grd_bin)
    if(error)return
    WRITE(u_grd_ctl,'(A,A12,A,2x,A)') 'lat             ','0 99 99 0 ', 'latitude of the grid cell'
    rec = rec+1
   ! dx
    if(ifdx)then
      call write_grads_field(pAnyGrdParam(grid%ag%indParam)%ptr%dx, nPoints, rec, u_grd_bin)
      if(error)return
   ...
   ! dy
    if(ifdy)then
      call write_grads_field(pAnyGrdParam(grid%ag%indParam)%ptr%dy, nPoints, rec, u_grd_bin)
      if(error)return
   ...
   ! sin_map_rot
    if(ifsin)then
      call write_grads_field(pAnyGrdParam(grid%ag%indParam)%ptr%sin_map_rot, nPoints, rec, u_grd_bin)
      if(error)return
   ...
   ! cos_map_rot
    if(ifcos)then
      call write_grads_field(pAnyGrdParam(grid%ag%indParam)%ptr%cos_map_rot, nPoints, rec, u_grd_bin)
      if(error)return
