...
!      fMinAlert = -1; fMinForce = 0; fMaxForce = 1; fMaxAlert = 10
  ...
  subroutine check_quantity_range(quantity, grid_data,  nPoints, nx, &
    ...
    logical :: ifMinAlert, ifMinForce, ifMaxForce, ifMaxAlert
...
!         & dust_emis_0_flag, &  !This should be conservative, but causes issues
  ...
  character(len=*), intent(in) :: chQuantity
    ...
    elseif(trim(chQuantity) == "snow_depth")then
      iQ = int_missing
      call msg_warning("Depricated snow_depth.Did you mean weq_snow_depth?", 'fu_get_silam_quantity')
