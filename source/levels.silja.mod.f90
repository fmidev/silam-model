...
!    call arrange_levels_in_vertical(vertical)
  ...
  subroutine create_levels_from_namelist_v2(levels, nl)
    ...
    character(len=fnlen) :: vertical_method, level_type
    ...
    case ('CUSTOM_LAYERS')
      need_layers = .true.
    case ('SURFACE_LEVEL')
      call make_single_level(surface_level, levels)
      done = .true.
    case ('TOP_ATMOSPHERE_LEVEL')
      call make_single_level(top_atmosphere_level, levels)
      done =.true.
    case ('MEAN_SEA_LEVEL')
      call make_single_level(mean_sea_level, levels)
      done = .true.
    case ('ENTIRE_ATMOSPHERE_MEAN_LAYER')
      call make_single_level(entire_atmosphere_mean_level, levels)
      done = .true.
    case ('ENTIRE_ATMOSPHERE_LAYER')
      call make_single_level(entire_atmosphere_integr_level, levels)
      done = .true.
    case ('ENTIRE_ATMOSPHERE_INTEGR_LAYER')
      call make_single_level(entire_atmosphere_integr_level, levels)
      done = .true.
    ...
    case ('PRESSURE')
      ...
      if (need_levels) then
        ...
        call make_levels(level_values, nlevs, levels, constant_pressure)
      else ! need_layers
        if(have_layer_borders)then
          call make_layers_from_borders(pBorderVals, nlevs, levels, layer_btw_2_altitude)
        else
          if (.not. have_layers) call levels_to_layers_pr(level_values, nlevs)
          call make_layers(level_values, nlevs, levels, layer_btw_2_pressure)
        endif
      end if
    ...
    case ('HEIGHT_FROM_SURFACE')
      ...
      if (need_levels) then
        if (.not. have_levels) call layers_to_levels(level_values, nlevs, constant_height, &
                                                   & have_layer_borders, pBorderVals)
        call make_levels(level_values, nlevs, levels, constant_height)
      else
        if(have_layer_borders)then
          call make_layers_from_borders(pBorderVals, nlevs, levels, layer_btw_2_altitude)
        else
          if (.not. have_layers) call levels_to_layers_hgt_alt(level_values, nlevs)
          call make_layers(level_values, nlevs, levels, layer_btw_2_height)
        endif
      end if
    ...
    case ('ALTITUDE_FROM_SEA')
      ...
      if (need_levels) then
        ...
        call make_levels(level_values, nlevs, levels, constant_altitude)
      else
        if(have_layer_borders)then
          call make_layers_from_borders(pBorderVals, nlevs, levels, layer_btw_2_altitude)
        else
          if (.not. have_layers) call levels_to_layers_hgt_alt(level_values, nlevs)
          call make_layers(level_values, nlevs, levels, layer_btw_2_altitude)
        endif
      end if
...
!      end if
      if (need_layers) then
        call make_layers_hybrid(nl, nlevs, levels)  ! nlevs and levels are output
      else
        call make_levels_hybrid(nl, nlevs, levels)
      end if
...
!    end if
  ...
  contains
    ...
    subroutine make_levels(values, nlevs, levels, level_type_flag)
      implicit none
      ...
      do ilev = 1, nlevs
        levels(ilev) = fu_set_level_general(level_type_flag, fval1=values(ilev))
      end do
    ...
    subroutine make_layers(values, nlevs, levels, level_type_flag)
      implicit none
      real, dimension(:), intent(in) :: values
      integer, intent(in) :: nlevs, level_type_flag
      type(silja_level), dimension(:), pointer :: levels
    ...
    subroutine make_layers_from_borders(pBorders, nlevs, levels, level_type_flag)
      implicit none
      integer, intent(in) :: nlevs, level_type_flag
      type(silja_level), dimension(:), pointer :: levels
      real, dimension(:,:), pointer :: pBorders
      ...
      do ilev = 1, nlevs
        levels(ilev) = fu_set_level_general(level_type_flag, &
                                          & fval1=pBorders(1,iLev), fval2=pBorders(2,iLev))
      end do
    ...
    subroutine make_levels_hybrid(nlPtr, num_levels, levels)
      implicit none
...
!      nlptr => nl
      do ind_item = 1, num_levels
        ...
        levels(lev_num) = fu_set_hybrid_level(lev_num, a, b)
      end do
    ...
    subroutine make_layers_hybrid(nlPtr, num_layers, layers)
      implicit none
      type(Tsilam_namelist), pointer :: nlPtr
...
!      if (fu_fails(num_layers == stat + 1, 'Bad number of top levels', 'set_hybrid_layers')) return
      ...
      do ind_item = 1, num_layers
        ...
        bottoms(lev_num) = fu_set_hybrid_level(lev_num, a, b)
        if(lev_num > 1)tops(lev_num-1) = fu_set_hybrid_level(lev_num, a, b)
      end do
      ...
      tops(num_layers) = fu_set_hybrid_level(num_layers, a, b)
