MODULE source_terms_pollen
  !
  ! This module contains full description of the pollen cocktail as an atmospheric
  ! pollutant.
  ! To be exact, there must be description of the grains, removal computation - both
  ! dry and wet, and, possibly, source term dynamics. The latter, however, has 
  ! to be checked. May be, it will be anyway sent to the source_terms.
  !
  ! One type, which explains how this stuff has to be treated is Tchem_rules_pollen,
  ! which contains all necessary rules for the operation, including the types
  ! of the functions and indices used for the computations. This type is used 
  ! only here and is not seen from outside, except for the command "create".
  ! Later on, this can be 
  !
  ...
  public add_inventory_pollen_src
...
!    if(error)return
    ...
    ! Calendar day
    if(srcPollen%ifStartCDThr .or. srcPollen%ifEndCDThr)then 
  ...
  subroutine init_emission_pollen(srcPollen, dispersionMarketPtr, start_time)
    ...
    integer :: iFlds, iTmp, nQuantities
...
!    ArTmp(1:fs_dispersion) = 0.0
    ...
    if(srcPollen%ifEndCDThr)then
      ...
      if(.not. ifOK)then  ! make it from start and length
        ...
        ! Make the season length (no reason apart from filling up the stack)
        if(srcPollen%ifStartCDThr)then 
...
write(55,'(A)') "## year mon day hour, min, iDayInYear_  now_sec_since_sunrise_ mdl_timestep_sec_ dayLength_hours_  T2m_  DailyTempr_ StartCDThr_ HS_in_ HS_out_ PollenLeft_in_ PollenLeft_out_ PollenRdyToFly_in_ PollenRdyToFly_out_ fMassInjected_out_"
  ...
  subroutine add_inventory_pollen_src(pollen_src, species_list, nSpecies)
    !
  ...
  end subroutine add_inventory_pollen_src
