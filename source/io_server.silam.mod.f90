module io_server
  ...
  public global_io_init    ! Takes care of all IO initializtion
  ...
  subroutine output_input_needs(OutDef, q_st, req_st, q_dyn,  req_dyn, qd_dyn, qd_st, wdr)
    ...
    if ( allocated(OutDef%Rules%DispOutLst%ptrItem)) then
      do i=1, size(OutDef%Rules%DispOutLst%ptrItem)
        ...
        if(.not.fu_known_quantity(Qtmp))cycle
        if ( fu_pollution_cloud_quantity(Qtmp) == silja_true ) cycle 
    ...
    if (lTmp) then
     ...
     if (.not. lTmp .and. iy_start_new > 1) then !Try to extend one cell down
      ...
      if(ifVerbose)then
        call msg("Cover mask:")
        do iy = ny_gtm , 1, -1 !Top-bottom, so map looks normal
          call msg("",covermask(1:nx_gtm,iy) > 0)
        enddo
      endif
...
!    call report(meteoVarLst)
    ...
    !
    ! We have to check that the system and output verticals are compatible
...
!      call report(OutVars%MeteoTmpStack)
    ...
    IF(OutDef%Rules%ifTrajectory)THEN
      ...
      iTmp =  nint(abs(fu_period_to_compute(wdr)/model_time_step) + 2)  !! beg and end...
...
!          if(OutDef%Rules%ifNETCDF) call close_netcdf_file(iUnitNetcdf)
  ...
  subroutine collect_output(meteoMarketPtr, metBuf, ifGotNewMeteoData, &
                          & dispBuf, outBuf, &
                          & PCld, now, OutDef, wdr, model_time_step, simulation_type, ifFirstStep) !, ifLastStep) 
    !
    ...
    logical, intent(in) :: ifGotNewMeteoData, ifFirstStep !, ifLastStep
    type(silam_pollution_cloud), pointer :: PCld
...
!    type(silam_cocktail), dimension(:), pointer :: cockArPtr
    ...
    ! before_output
    ! Collect as_is stuff
    ! fill other for the first time step
    ! No needed if nothing to output
    if (ifOutputTime) then
      call collect_buffers(metBuf, dispBuf, OD, OV, now + meteo_time_shift, model_time_step, &
                         & ifFirststep, .true., fu_if_randomise(wdr))
    endif
...
!      if (ifLastStep) return
    ...
    ! after_output
    ! collect averaged and cumulative
    !
    call collect_buffers(metBuf,  dispBuf, OD, OV, now + meteo_time_shift, model_time_step, &
                       & ifFirststep, .false., fu_if_randomise(wdr))
...
!    call msg_test('Output Finished')
  ...
  subroutine collect_buffers( metBuf,  dispBuf, OD, OV, now, model_time_step, &
                           & ifFirststep, ifBeforeOut, ifRandomise)
    ! 
    ! Should be clled by collect_output twice:
    ! before and after actual output
    !
    ...
    logical, intent(in) ::  ifFirstStep, ifBeforeOut, ifRandomise
    type(silam_output_definition), pointer :: OD
    type(ToutputVariables), pointer :: OV
    type(silam_pollution_cloud), pointer :: PCld
    type(silja_time), intent(in) :: now
    ...
    ! It does not depend on splitting of sources, so all meteofields are identically
    ! must be stored to all output files despite the emission sources
    !
!call msg("collect_buffers called")
    if (ifFirststep)  call msg("First timestep")
    if (ifBeforeOut)then
      call msg("Before out")
    endif

    if (allocated(OD%Rules%MeteoOutLst%ptrItem)) then
      if ( ifBeforeOut .or.  OD%Rules%ifAverageMeteo) then
        ...
        do iQ=1,size(OD%Rules%MeteoOutLst%ptrItem)
          ...
          if (ifFirststep .and. ifBeforeOut) then
            avtype = iAsIs
          else
            avtype = OD%Rules%MeteoOutLst%ptrItem(iQ)%AvType
          endif
          if  (avtype == iAsIs .neqv. ifBeforeOut)  cycle ! Not a turn to collect
          !
...
!if(error)call unset_error('Let see it further')
      endif  ! if time for output collection
    endif ! allocated
    ...
    !call msg_test('Collect_buffers:' + fu_quantity_string(OD%Rules%DispOutLst%ptrItem(iQ)%quantity) )
            ...
            !
            slTmp = fu_pollution_cloud_quantity(OD%Rules%DispOutLst%ptrItem(iQ)%quantity) 
    ...
    select case(fu_dimension(datBuffer, indQ_arr))
      case(2)
        ...
        if(avtype == iAsIs)then
          ...
          localIdTarget = datBuffer%p2d(indQ_arr)%present%idPtr
...
!        call report(OutLst%ptrItem(i)%targetId)
  ...
  function fu_output_tmp_grid(quantity, OutDef, cloud)result(grid)
    ...
    select case(quantity)
      ...
      case default
        ...
        if(fu_pollution_cloud_quantity(quantity) == silja_true)then 
  ...
  subroutine init_mass_output(OutDef, PCld, em_source, nlStdSetup, chemRules, &
    ...
    ! Local parameter
    integer, dimension(11), parameter :: knownQauntities = &
              & (/concentration_flag, concentration_2m_flag, &
                & volume_mixing_ratio_flag, &
                & drydep_flag, wetdep_flag, &
                & advection_moment_X_flag, advection_moment_Y_flag, advection_moment_Z_flag, &
                & emission_intensity_flag, &
                & optical_density_flag, optical_column_depth_flag/)
...
!    allocate(OutDef%Rules%MassMapOutLst%ptrItem(size(OutDef%Rules%DispOutLst%ptrItem)), stat=iTmp)
    ...
    do while(iVarIni <= size(OutDef%Rules%DispOutLst%ptrItem))
      if(fu_quantity_in_quantities(OutDef%Rules%DispOutLst%ptrItem(iVarIni)%quantity, &
                                 & knownQauntities))then
  ...
  subroutine expand_dispersion_output_list(OutDef, PollutionCloud, em_source, pDispersionMarket, &
    ...
    do iVarIni = 1, size(LstTmp%ptrItem)
      ...
      if(fu_if_variable_available(PollutionCloud, pDispersionMarket, &
                                ...
                                & LstTmp%ptrItem(iVarIni)%species, nlStdSetup, dynRules))then
        ifFound = .true.
      else
        !
        ! Problem. The variable is not available but the dispersion market is already 
...
!      call msg('Checking for duplicates')
      ...
      if(fu_if_variable_available(PollutionCloud, pDispersionMarket, &
                            & LstTmp%ptrItem(iVarIni)%quantity, species, nlStdSetup, dynRules))then
        !
