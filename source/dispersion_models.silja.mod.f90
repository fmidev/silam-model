MODULE dispersion_models
  ...
  ! Description:
  ! This is the driving routine of SILAM: 
  ! It currently includes random-walk particle model of SILAM
  ! and SCD-based Eulerian SILAM version
  ! Actually, these two routines can be considered as totally 
  ! independent. They are selected by the start_silam_v3
  ! and then never cross each other.
  ! These models use significantly different sets of routines
  ! and data structures. Certain synchronization is expected but
  ! not done by v.3.7.
  !
  ...
  ! 
  ! Code owner Mikhail Sofiev, e-mail mikhail.sofiev@fmi.fi
...
!      if(error)return
      ...
      !--------------------------------------------------------------- IO structures
      !
      ! Initalise input-output structures - used by GrADS and GRIB routines
      !
      ...
      call init_netcdf_io(nlStandardSetup, nlPtr) 
      if(error)return
...
!    nlPtr => fu_namelist(nlSetupGrp, 'data_assimilation')
  ...
  SUBROUTINE set_standard_setup(nlSetup, rulesDynamics, ifOldFileFormat)
    ...
    !
    call msg("reading grib_name_table_fnm") 
    call read_GRIB_name_table(fu_expand_environment(fu_content(nlSetup,'grib_name_table_fnm')))
    ! The GRIB code table file name
    !
    call read_GRIB_code_table_v5(fu_expand_environment(fu_content(nlSetup,'grib_code_table_fnm')))
