...
!  USE globals
  ...
  public create_empty_field_in_field_3d
...
!    call msg('Number of fields in f3d: ', field3d%number_of_fields)
  ...
  subroutine create_field_in_field_3d(field_3d, id_2d, fValues)
    !
...
!    call report(id_2d)
    ! Note that the fValues are copied into the newly created field
...
!    call msg('done')
  ...
  subroutine create_empty_field_in_field_3d(field_3d, id_2d)
    !
    ! Creates a requested 2D field at a given level and sets its grid to fValue
    !
    implicit none
...
!    call msg('done')
  ...
  FUNCTION fu_field_from_3d_field(field_3d, number, ifSkipOrder) 
    ...
    ELSE
      ...
      call report(field_3d)
      call msg("####################################################################")
      CALL set_error('this field number is not in this 3d-field','fu_field_from_3d_field')
      NULLIFY(fu_field_from_3d_field)
    END IF
  ...
  INTEGER FUNCTION fu_3d_fieldsize(field_3d)
    ...
    DO i = 1, field_3d%number_of_fields
      fu_3d_fieldsize = fu_3d_fieldsize + fu_size(field_3d%fields(i)%fp)
    END DO
