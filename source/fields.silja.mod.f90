...
!print *, 'set field: 2.2: ', SIZE(field%grid_data)
    ...
    !
    call check_quantity_range(fu_quantity(id), grid_data, field%grid_data, gridpoints, nx, &
...
!print *, 'set field: 4', field%grid_data(3)
    
    field%id = id

!print *, 'set field: 5: ', fu_quantity(field%id)
...
!    print *,'set_new_field done',ASSOCIATED(field%grid_data),SIZE(field%grid_data)
  ...
  subroutine make_test_field(chFName, id, grid_data, valid_time, validity, grid)
    !
    ...
    type(silja_interval), intent(in) :: validity
...
!                                 & fu_substance_name(fld_id_past))
  ...
  subroutine update_field_data(newId, newGridData, originalField, iUpdateType)
    ...
    ! Local variables
    integer :: iCell,  n_out_of_range, n_patched, nx, ny, n_failed
    !
    ...
    if(.not.defined(newId))then
      call set_error('Undefined new field id','update_field_data')
      return
    endif
    if(.not.defined(originalField))then
      call set_error('Undefined original field','update_field_data')
      return
    endif
    if(.not.associated(newGridData))then
      call set_error('Undefined new grid data','update_field_data')
      return
    endif
    if(.not. fu_grid(originalField%id) == fu_grid(newId))then
      call report(fu_grid(originalField%id))
      call report(fu_grid(newId))
      call set_error('Grids are not equal', 'update_field_data')
      return
    endif

    select case(iUpdateType)
      case(overwrite_field)
        originalField%grid_data(1:fu_number_of_gridpoints(fu_grid(newId))) &
             & = newGridData(1:fu_number_of_gridpoints(fu_grid(newId)))
      ...
      case default
        ...
        call set_error('Strange update requested','update_field_data')
        return
    end select

    call grid_dimensions(fu_grid(newId), nx, ny)
    call check_quantity_range(fu_quantity(newId), originalField%grid_data, null(), nx*ny, nx, &
                            & .false., .false., &  ! ifRequireValidity, ifSilent
                            & n_out_of_range, n_patched, n_failed)
    !n_negative = 0
