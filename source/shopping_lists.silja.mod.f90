...
!  use grib_code_table
  ...
  PUBLIC fu_floor_level
  PUBLIC fu_ceiling_level
...
!  public fu_field_id  ! from shopping variable
  ...
  PRIVATE fu_floor_level_of_shop
  PRIVATE fu_ceiling_level_of_shop
...
!  private fu_field_id_from_shopping_var
  ...
  INTERFACE fu_floor_level
    MODULE PROCEDURE fu_floor_level_of_shop
  END INTERFACE

  INTERFACE fu_ceiling_level
    MODULE PROCEDURE fu_ceiling_level_of_shop
  END INTERFACE
...
!!!!$      ! If temperature is on the list, then also accept 2m temperature:
!!!!$      fu_quantity_in_list = fu_quantity_in_quantities(&
!!!!$          & temperature_flag, list%quantities)
!
!      CASE (u_flag,&
!          & v_flag,& !          & omega_flag,&
!          & wind_flag, &
...
!          & v_10m_flag,&
!          & wind_10m_flag)
!      ! If any of (U, V, wind) is on the list, then also accept
!      ! other wind components (U,V,W) and 10m wind.
!      fu_quantity_in_list = (&
!          & (ANY(list%quantities == u_flag)).or.&
!          & (ANY(list%quantities == v_flag)).or.&
!          & (ANY(list%quantities == wind_flag)))
!
!    CASE default
!
!      fu_quantity_in_list = fu_quantity_in_quantities(quantity, list%quantities)
...
!    END IF
  ...
  FUNCTION fu_floor_level_of_shop(shopping_list)
    ...
    !
    ! The return value of this function:
    TYPE(silja_level) :: fu_floor_level_of_shop
    
    ! Imported parameters with intent(in):
    TYPE(silja_shopping_list), INTENT(in) :: shopping_list
    
    fu_floor_level_of_shop = shopping_list%floor_level

  END FUNCTION fu_floor_level_of_shop


  ! ***************************************************************

  FUNCTION fu_ceiling_level_of_shop(shopping_list)
    ...
    !
    ! The return value of this function:
    TYPE(silja_level) :: fu_ceiling_level_of_shop
    
    ! Imported parameters with intent(in):
    TYPE(silja_shopping_list), INTENT(in) :: shopping_list
    
    fu_ceiling_level_of_shop = shopping_list%ceiling_level

  END FUNCTION fu_ceiling_level_of_shop
