...
!        call control_from_cloud(background, model, handle_negatives=.true.)
        call vector_from_model(background, model, handle_negatives=.true.)       
         ...
         call msg ("run_3dvar memusage after vector_from_model (kB)", fu_system_mem_usage())
      end if
      ...
      if (.not. error) then
        call to_physical(analysis, background, control_physical)
         call msg ("run_3dvar memusage after to_physical (kB)", fu_system_mem_usage())
        call destroy(analysis)
         call msg ("run_3dvar memusage after destroy(analysis) (kB)", fu_system_mem_usage())
        call kill_bgr_cov(bgr_cov)
         call msg ("run_3dvar memusage after kill_bgr_cov (kB)", fu_system_mem_usage())
      else
      ...
      call msg ("run_3dvar memusage after run_forecast (kB)", fu_system_mem_usage())
      if (error) return
      now = now + model_integr%rules%periodToCompute
  ...
  contains
    ...
    subroutine assimilate(background)
      ...
      if (error) then
        ...
        p_values = 0.0
      else if (simrules_3d%darules%output_level > da_out_none_flag .and. smpi_adv_rank == 0) then
...
!      stat = nf90_create(filename, NF90_CLASSIC_MODEL, ncid)
  ...
  subroutine steepest(model, model_adj, control, background, bgr_cov, report_iterates)
    ...
    call evaluate(control, background, bgr_cov, model, cost_obs, cost_bgr, cost, mdl_obs_values)
    if (error) return
    if (outputlev > da_out_none_flag) call dump_observations(model%obs_ptr, fu_species_transport(model%cloud), &
    ...
    GRADIENT_SEARCH: do iter = 1, numerics%maxIterations
      ...
      LINE_SEARCH: do iline = 1, numerics%maxLineSearchSteps
        ...
        call da_msg(log_str)
        if (outputlev > da_out_first_last_flag) call dump_observations(model%obs_ptr, fu_species_transport(model%cloud), &
  ...
  subroutine l_bfgs_b(model, model_adj, control, background, bgr_cov, report_iterates)
    ...
    do iter = 1, numerics%maxIterations
      ...
      if (task(1:2) == 'FG') then
        ...
        call report_cost(cost_obs, cost_bgr, cost, fu_mode(control), log_str)
        call da_msg(log_str)
        if (outputlev > da_out_first_last_flag .or. outputlev == da_out_first_last_flag .and. iter == 1) then
