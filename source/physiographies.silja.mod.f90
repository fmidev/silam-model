...
!    print *, 'Starting cell sizes'
    ...
    DO j=1,ny_met ! Fill-in temporary array with x-size
     DO i=1,nx_met
       arTmp(i+(j-1)*nx_met)= fu_dx_cell_m(meteo_grid, i, j)
     END DO
    END DO
    ...
    DO j=1,ny_met   ! Fill-in temporary array with y-size
      DO i=1,nx_met
        arTmp(i+(j-1)*nx_met)= fu_dy_cell_m(meteo_grid, i, j)
      END DO
    END DO
    ...
    if(fu_quantity_in_quantities(longitude_flag, Q_shop))then
      ...
      DO j=1,ny_met   ! Fill-in temporary array with y-size
        DO i=1,nx_met
          arTmp(i+(j-1)*nx_met) = fu_lon_geographical_from_grid(real(i), real(j), meteo_grid)
        END DO
      END DO
    else
    ...
    if(fu_quantity_in_quantities(latitude_flag, Q_shop))then
      ...
      DO j=1,ny_met   ! Fill-in temporary array with y-size
        DO i=1,nx_met
          arTmp(i+(j-1)*nx_met) = fu_lat_geographical_from_grid(real(i), real(j), meteo_grid)
        END DO
      END DO
    else
    ...
    do iSrc = 1, fu_NbrOfMetSrcs(wdr)
      ...
      elseif(fu_quantity_in_quantities(geopotential_sfc_flag, Qs))then
        ...
        IF (error) then 
          ...
          ptrTopo(1:fs_met) = arTmp(1:fs_met) / g
...
call msg('Physiography reads:' + fnames(i)%sp)
  ...
  subroutine make_time_zone_mapping(arSourceIdMapping, chSplitNames, meteoMarketPtr, gridMapping, groups, nGroups)
    ...
    ! local variables
    integer :: iGrp, ix, iy, indexMeteo, nx, ny, iTZ, iZone, nx_met, ny_met
    ...
    do iy = 1, ny
      do ix = 1, nx
        indexMeteo = fu_grid_index(nx_met, ix, iy, pHIS)
