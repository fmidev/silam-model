...
!  PUBLIC fu_roughness
  PUBLIC fu_albedo
  PUBLIC get_meteo_cell_size
  public get_dispersion_cell_size
  ...
  !
  TYPE(silja_field), POINTER, PUBLIC, SAVE :: topography_fld
  TYPE(silja_field), POINTER, PUBLIC, SAVE :: albedo_fld
  TYPE(silja_field), POINTER, PUBLIC, SAVE :: fraction_of_land_fld
  TYPE(silja_field), POINTER, PUBLIC, SAVE :: fraction_of_ice_fld
  TYPE(silja_field), POINTER, PUBLIC, SAVE :: fraction_of_forest_fld
  TYPE(silja_field), POINTER, PUBLIC, SAVE :: water_eq_snow_depth_fld
  TYPE(silja_field), POINTER, PUBLIC, SAVE :: ground_roughness_meteo_fld
  TYPE(silja_field), POINTER, PUBLIC, SAVE :: ground_roughness_disp_fld
!  TYPE(silja_field), POINTER, PRIVATE, SAVE :: water_roughness
  TYPE(silja_field), POINTER, PUBLIC, SAVE :: meteo_cell_x_size_fld  ! in m
  TYPE(silja_field), POINTER, PUBLIC, SAVE :: meteo_cell_y_size_fld
  TYPE(silja_field), POINTER, PUBLIC, SAVE :: meteo_longitude_fld  ! in degrees
  TYPE(silja_field), POINTER, PUBLIC, SAVE :: meteo_latitude_fld
  TYPE(silja_field), POINTER, PUBLIC, SAVE :: dispersion_cell_x_size_fld  ! in m
  TYPE(silja_field), POINTER, PUBLIC, SAVE :: dispersion_cell_y_size_fld
  TYPE(silja_field), POINTER, PUBLIC, SAVE :: leaf_area_index_fld  ! m2/m2  only used with static LAI
...
!    print *, 'Starting cell sizes'
    arTmp => fu_work_array(ny_meteo*nx_meteo)
    call set_missing(id)
    ...
    if(.not.defined(id))then ! No physiography fields at all
      ...
      CALL set_quantity(id,cell_size_x_flag)
    end if

    CALL dq_store_2d(meteoMarketPtr, id, arTmp, single_time_stack_flag, fu_if_randomise(wdr)) ! id, grid_data, permanent, iAccuracy
    IF(error)RETURN

    meteo_cell_x_size_fld => fu_sm_simple_field(meteoMarketPtr, fu_met_src(wdr,1),&
                                              & cell_size_x_flag,&
                                              & level_missing, &
                                              & single_time_stack_flag)
    ! meteo y-size  
    ...
    CALL dq_store_2d(meteoMarketPtr, id, arTmp, single_time_stack_flag, fu_if_randomise(wdr)) ! id, grid_data, permanent
    IF(error)RETURN

    meteo_cell_y_size_fld => fu_sm_simple_field(meteoMarketPtr, fu_met_src(wdr,1),&
                                              & cell_size_y_flag,&
                                              & level_missing, &
                                              & single_time_stack_flag)
    !
    ...
    if(fu_quantity_in_quantities(longitude_flag, Q_shop))then
      ...
      CALL dq_store_2d(meteoMarketPtr, id, arTmp, single_time_stack_flag, fu_if_randomise(wdr)) ! id, grid_data, permanent
      meteo_longitude_fld => fu_sm_simple_field(meteoMarketPtr, fu_met_src(wdr,1),&
                                              & longitude_flag,&
                                              & level_missing, &
                                              & single_time_stack_flag)
    else
    ...
    if(fu_quantity_in_quantities(latitude_flag, Q_shop))then
      ...
      CALL dq_store_2d(meteoMarketPtr, id, arTmp, single_time_stack_flag, fu_if_randomise(wdr)) ! id, grid_data, permanent
      meteo_latitude_fld => fu_sm_simple_field(meteoMarketPtr, fu_met_src(wdr,1),&
                                             & latitude_flag,&
                                             & level_missing, &
                                             & single_time_stack_flag)
    else
...
!!!!      end do ! Array of templates
    ...
    do iSrc = 1, fu_NbrOfMetSrcs(wdr)
      ...
      if(fu_quantity_in_quantities(fraction_of_ice_flag, Qs))then
        fraction_of_ice_fld => fu_sm_simple_field(meteoMarketPtr, fu_met_src(wdr,iSrc),&
                                                & fraction_of_ice_flag,&
                                                & level_missing, &
                                                & single_time_stack_flag)
        IF (error) then 
          ...
          id = fu_id(fraction_of_ice_fld)
        end if
      end if

      if(fu_quantity_in_quantities(fraction_of_forest_flag, Qs))then
        fraction_of_forest_fld => fu_sm_simple_field(meteoMarketPtr, fu_met_src(wdr,iSrc),&
        ...
        IF (error) then 
          ...
          id = fu_id(fraction_of_forest_fld)
        end if
      end if
      ...
      elseif(fu_quantity_in_quantities(geopotential_sfc_flag, Qs))then
        ...
        IF (error) then 
          ...
          arTmp(1:fs_meteo) = ptrTopo(1:fs_meteo) / g
          call dq_store_2d(meteoMarketPtr, id, arTmp, single_time_stack_flag, fu_if_randomise(wdr))
          if(error)return
          topography_fld => fu_sm_simple_field(meteoMarketPtr, fu_met_src(wdr,iSrc),&
                                             & relief_height_flag,&
                                             & ground_level, &
                                             & single_time_stack_flag)
          ptrTopo => fu_grid_data(topography_fld)
...
!    print *, 'END PHYSIOGRAPHY'
  ...
  REAL FUNCTION fu_topo_m(position)

    ! Description:
    ! Returns the topographic height in gpm.
    !
    ...
    IF (physiography_set) THEN
      fu_topo_m = fu_interpolate_to_position(topography_fld, position, linear)
    ELSE
      CALL set_error('physiography not set yet','fu_topo_m')
    END IF

  END FUNCTION fu_topo_m


 ! ***************************************************************


  REAL FUNCTION fu_albedo(position)

    ! Description:
    ! Returns the position's albedo.
    !
    ! Author: Mika Salonoja, FMI
    ...
    IF (physiography_set) THEN
      fu_albedo = fu_interpolate_to_position(albedo_fld, position, linear)
    ELSE
      CALL set_error('physiography not set yet','fu_albedo')
    END IF

  END FUNCTION fu_albedo


  ! ***************************************************************

  SUBROUTINE get_meteo_cell_size(pos, x_size, y_size)

    ! Returns metric size of the cell where the position pos is located
    !
    ...
    REAL, INTENT(out) :: x_size, y_size

    REAL, DIMENSION(:), POINTER :: ptr

    ptr => fu_grid_data(meteo_cell_x_size_fld)
    IF(error)then
      x_size = real_missing
      y_size = real_missing
      RETURN
    ...
    x_size = ptr(INT(fu_x(pos)+0.5) + (INT(fu_y(pos)+0.5)-1)*nx_meteo)

    ptr => fu_grid_data(meteo_cell_y_size_fld)
    IF(error)then
      y_size = real_missing
      RETURN
    else
      y_size = ptr(INT(fu_x(pos)+0.5) + (INT(fu_y(pos)+0.5)-1)*nx_meteo)
    endif

  END SUBROUTINE get_meteo_cell_size

  
  ! ***************************************************************
  
  SUBROUTINE get_dispersion_cell_size(pos, x_size, y_size)
    ...
    ptr => fu_grid_data(dispersion_cell_x_size_fld)
    IF(error)then
      x_size = real_missing
      y_size = real_missing
      RETURN
    ...
    x_size = ptr(INT(fu_x(pos)+0.5) + (INT(fu_y(pos)+0.5)-1)*nx_dispersion)
  
    ptr => fu_grid_data(dispersion_cell_y_size_fld)
    IF(error)then
      y_size = real_missing
      RETURN
    else
      y_size = ptr(INT(fu_x(pos)+0.5) + (INT(fu_y(pos)+0.5)-1)*nx_dispersion)
    endif
  
  END SUBROUTINE get_dispersion_cell_size
  ...
  subroutine write_physiography_output(meteoMarketPtr, chFNm_basic, iGrib, ifGrads, iNetCDF, now, &
    ...
    if(iNetCDF > 0)then                                            ! NetCDF
      ...
      DO i = 1, fu_number_of_2d_fields(permStack)
        ...
        nFlds = nFlds + 1
        MeteoOutLstTmp%ptrItem(nFlds)%quantity = fu_quantity(fu_id(field_2d))
        MeteoOutLstTmp%ptrItem(nFlds)%request = 2
        MeteoOutLstTmp%ptrItem(nFlds)%AvType = iAsIs
        MeteoOutLstTmp%ptrItem(nFlds)%if3D = .False.
        MeteoOutLstTmp%ptrItem(nFlds)%iVerticalTreatment = do_nothing_flag
  ...
  subroutine read_aggregate_land_use_metadata(chLandUseFileDescr, wdr, shopping_list, output_fields)
    ...
    !
    call get_id_list_from_input_file(uIn, fileFormat%iFormat, idList, nIDs)
    if(error)return
    !
